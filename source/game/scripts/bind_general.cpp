...
double getExactTime() { return devices.driver->getAccurateTime(); }

double getGameTime() { return devices.driver->getGameTime(); }

double getFrameGameTime() { return frameTime_s; }

double getFrameTime() { return devices.driver->getFrameTime(); }

double getFrameLength() { return realFrameLen; }

unsigned getBasicTime() { return unsigned(devices.driver->getTime()); }

double getGameSpeed() { return devices.driver->getGameSpeed(); }
...
static bool mpClient() { return devices.network->isClient; }

static bool mpServer() { return devices.network->isServer; }

static bool isTestScripts() { return game_state == GS_Test_Scripts; }

static bool isModdedGame() {
  if (devices.mods.activeMods.size() == 0)
    return true;
  return devices.mods.activeMods.size() > 1 ||
         devices.mods.activeMods[0] != devices.mods.getMod("base");
}
...
static bool mpIsSerializing() { return devices.network->isSerializing(); }

static void mpConnect(const std::string &address, int port,
...
static bool mpIsConnecting() {
  return devices.network->isClient && devices.network->client &&
...
static std::string localize(const std::string &str, bool requireHash,
                            bool doUnescape) {
...
static bool scriptFuncSplit(const std::string &input, std::string *name,
                            CScriptArray *arguments, bool strip) {
...
template <class T> T absceil(T a) { return a > 0 ? ceil(a) : floor(a); }

template <class T> T absfloor(T a) { return a > 0 ? floor(a) : ceil(a); }

template <class T> T scr_max(T a, T b) { return a < b ? b : a; }

template <class T, class S> T scr_mix_max(T a, S b) {
  return a < (T)b ? (T)b : a;
}

template <class T> T scr_min(T a, T b) { return b < a ? b : a; }

template <class T, class S> T scr_mix_min(T a, S b) {
  return (T)b < a ? (T)b : a;
}

template <class T> T clamp(T val, T _min, T _max) {
  if (val < _min)
    return _min;
  if (val > _max)
    return _max;
...
void QuarticRoots(double a, double b, double c, double d, double e,
                  CScriptArray &out) {
...
long long dbgAddress(void *ptr, int typeId) { return (long long)ptr; }
...
template <class T> static T wrap_toNumber(const std::string &str) {
  return toNumber<T>(str);
}

static bool wrap_toBool(const std::string &str) { return toBool(str); }
...
static void addConsComm(std::string &name, asIScriptObject *obj) {
  ...
  asIScriptFunction *conCommandFunc =
      (asIScriptFunction *)man->engine->GetUserData(EDID_consoleCommand);
...
static bool strEq_s(const std::string &str, const std::string &substr,
                    int index) {
  if (substr.size() > str.size())
    return false;
...
static bool strEq_sl(const std::string &str, const std::string &substr,
...
static bool strEqNC_s(const std::string &str, const std::string &substr,
                      int index) {
  return streq_nocase(str, substr, index);
}

static bool strEqNC_sl(const std::string &str, const std::string &substr,
                       int index, int length) {
...
std::string readFromClipboard() { return devices.driver->getClipboard(); }

static void toggleConsole() { console.toggle(); }
...
static bool isCloudActive() { return devices.cloud != nullptr; }
...
static void enterCloudQueue(const std::string &type, unsigned players,
...
#ifdef _DEBUG
static void scrAssert(void *ptr, int tid) { assert(false); }
#endif
...
void RegisterGeneralBinds(bool server, bool shadow) {
  // Debugging functions
  bind("void print(const string &)",
       asFUNCTIONPR(print, (const std::string &), void));
  bind("void info(const string &)",
       asFUNCTIONPR(info, (const std::string &), void));
  bind("void warn(const string &)",
       asFUNCTIONPR(warn, (const std::string &), void));
  bind("void error(const string &)",
       asFUNCTIONPR(error, (const std::string &), void));
  bind("void throw(const string &)", asFUNCTION(throwScriptError));

  bind("void dumpGC()", asFUNCTION(dumpGC));

  bind("string getStackTrace(bool verbose = false)",
       asFUNCTIONPR(getStackTrace, (bool), std::string));
  ...
  bind("void openFileManager(const string& folder)",
       asFUNCTION(openFileManager))
      doc("Open a folder in the user's configured file manager.",
          "Folder to open.");
  ...
  if (!server && !shadow) {
    bind("void mpConnect(const string& address, int port = 2048, const string& "
         "password = \"\")",
         asFUNCTION(mpConnect));
  ...
  bind("double get_gameTime()", asFUNCTION(getGameTime)) doc(
      "Returns the current game clock, updated periodically, in seconds.", "");
  bind("double get_frameGameTime()", asFUNCTION(getFrameGameTime)) doc(
      "Returns the current frame's game time, updated every frame, in seconds.",
      "");
  ...
  bind("double randomd(double min, double max)",
       asFUNCTIONPR(randomd, (double, double), double))
      doc("Generate a random double value.", "Minimum return.",
          "Maximum return (non-inclusive).", "Random value from [min,max).");

  bind("float randomf()", asFUNCTIONPR(randomf, (), float))
      doc("Generate a random float value.", "Random value from [0.f,1.f).");

  bind("float randomf(float min, float max)",
       asFUNCTIONPR(randomf, (float, float), float))
      doc("Generate a random float value.", "Minimum return.",
          "Maximum return (non-inclusive).", "Random value from [min,max).");

  bind("uint randomi()", asFUNCTIONPR(randomi, (), unsigned))
      doc("Generate a random uint value.",
          "Random value from 0 to UINT_MAX, inclusive.");
  bind("int randomi(int min, int max)", asFUNCTIONPR(randomi, (int, int), int))
      doc("Generate a random int value.", "Minimum return.",
          "Maximum return  (inclusive).",
          "Random value from min to max, inclusive.");

  bind("uint sysRandomi()", asFUNCTION(sysRandomi))
      doc("Attempts to generate a high-quality random number. May fall back to "
          "a weaker random generation. Slow.",
          "");

  bind("double normald(double min, double max, int steps = 3)",
       asFUNCTION(normald))
      doc("Generates an approximately normally distributed value within a "
          "range, within an average return at the mean of min and max.",
          "Minimum return", "Maximum return (non-inclusive)",
          "Number of steps to iterate (more is a tighter distribution)",
          "Random value from [min,max)");

  bind("vec3d random3d(double radius = 1.0)",
       asFUNCTIONPR(random3d, (double), vec3d))
      doc("Generates a random 3D rotation, uniformly distributed over a sphere "
          "surface.",
          "Radius of sphere.", "Random rotation vector.");

  bind("vec2d random2d(double radius = 1.0)",
       asFUNCTIONPR(random2d, (double), vec2d))
      doc("Generates a random 2D rotation, uniformly distributed over a "
          "circle's circumference.",
          "Radius of circle.", "Random rotation vector.");

  bind("vec3d random3d(double minRadius, double maxRadius)",
       asFUNCTIONPR(random3d, (double, double), vec3d))
      doc("Generates a random 3D rotation, distributed evenly on the volume "
          "between minRadius and maxRadius.",
          "Minimum radius of the vector.", "Maximum radius of the vector.",
          "Random rotation vector.");

  bind("vec2d random2d(double minRadius, double maxRadius)",
       asFUNCTIONPR(random2d, (double, double), vec2d))
      doc("Generates a random 2D rotation, distributed evenly on the area "
          "between minRadius and maxRadius.",
          "Minimum radius of the vector.", "Maximum radius of the vector.",
          "Random rotation vector.");
  ...
  bind("double clamp(double value, double minimum, double maximum)",
       asFUNCTION(clamp<double>))
  ...
  bind("void quarticRoots(double a, double b, double c, double d, double e, "
       "array<double>& roots)",
       asFUNCTION(QuarticRoots))
      doc("Calculates all real roots of a quartic polynomial.", "Coefficient A",
          "Coefficient B", "Coefficient C", "Coefficient D", "Coefficient E",
          "Array to return roots in.");

  bind("double newtonArrivalTime(double accel, const vec3d& offset, const "
       "vec3d& relativeVelocity)",
       asFUNCTION(newtonArrivalTime));
  ...
  bind("void appendRoman(int num, const string& str)",
       asFUNCTIONPR(romanNumerals, (unsigned, std::string &), void));
  ...
  bind("string localize(const string& in, bool requireHash = false, bool "
       "doUnescape = true)",
       asFUNCTION(localize));
  bind("Color toColor(const string& in)", asFUNCTION(toColor));

  bind("string toString(double num, uint precision = 5)",
       asFUNCTION(toString<double>));
  bind("string toString(float num, uint precision = 5)",
       asFUNCTION(toString<float>));
  bind("string toString(int num, uint precision = 5)",
       asFUNCTION(toString<int>));
  bind("string toString(uint num, uint precision = 5)",
       asFUNCTION(toString<unsigned>));
  bind("string toString(Color color)", asFUNCTION(toString<Color>));

  bind("string standardize(double value, bool showIntegral = false, bool "
       "roundUp = false)",
       asFUNCTION(standardize));
  bind("bool funcSplit(const string& input, string&out funcName, "
       "array<string>& arguments, bool strip = true)",
       asFUNCTION(scriptFuncSplit));

  bind("float toFloat(const string &in str)", asFUNCTION(wrap_toNumber<float>));
  bind("double toDouble(const string &in str)",
       asFUNCTION(wrap_toNumber<double>));
  bind("int toInt(const string &in str)", asFUNCTION(wrap_toNumber<int>));
  bind("uint toUInt(const string &in str)",
       asFUNCTION(wrap_toNumber<unsigned>));
  ...
  str.addExternMethod("uint toUint() const",
                      asFUNCTION(wrap_toNumber<unsigned>));
  str.addExternMethod("float toFloat() const",
                      asFUNCTION(wrap_toNumber<float>));
  str.addExternMethod("double toDouble() const",
                      asFUNCTION(wrap_toNumber<double>));
  str.addExternMethod("vec3d toVec3d() const", asFUNCTION(toVec3d));

  str.addExternMethod("string trimmed() const",
                      asFUNCTIONPR(trim, (const std::string &), std::string));
  str.addExternMethod("string& paragraphize(const string& parSep, const "
                      "string& lineSep, bool startsParagraph = false)",
                      asFUNCTION(paragraphize));
  str.addExternMethod(
      "string& replace(const string& replace, const string& with)",
      asFUNCTION(replace));
  str.addExternMethod(
      "string replaced(const string& replace, const string& with) const",
      asFUNCTION(replaced));
  str.addExternMethod("bool isWhitespace() const", asFUNCTION(strIsWhitespace));
  str.addExternMethod("bool contains(const string &in) const",
                      asFUNCTION(strContains));
  str.addExternMethod("bool contains_nocase(const string &in) const",
                      asFUNCTION(strContainsNC));
  str.addExternMethod(
      "int findFirst_nocase(const string &in, uint start = 0) const",
      asFUNCTION(strfind_nocase));
  str.addExternMethod("bool startswith(const string &in) const",
                      asFUNCTION(startsWith));
  str.addExternMethod("bool endswith(const string &in) const",
                      asFUNCTION(endsWith));
  str.addExternMethod("bool startswith_nocase(const string &in) const",
                      asFUNCTION(startsWithNC));
  str.addExternMethod("bool endswith_nocase(const string &in) const",
                      asFUNCTION(endsWithNC));
  str.addExternMethod("bool equals(const string &in, int start) const",
                      asFUNCTION(strEq_s));
  str.addExternMethod(
      "bool equals(const string &in, int start, int length) const",
      asFUNCTION(strEq_sl));
  str.addExternMethod(
      "bool equals_nocase(const string &in, int start = 0) const",
      asFUNCTION(strEqNC_s));
  str.addExternMethod(
      "bool equals_nocase(const string &in, int start, int length) const",
      asFUNCTION(strEqNC_sl));
  ...
  ccomm.addMethod("void execute(const string& args)", &pConsoleCommand)
      doc("Called whenever this command was invoked, with all extra text "
          "passed to the command.",
          "Additional text passed after the command.");

  getEngine()->SetUserData(pConsoleCommand, EDID_consoleCommand);

  bind("void addConsoleCommand(const string& name, ConsoleCommand@ comm)",
       asFUNCTION(addConsComm))
      doc("Registers a script console command.",
          "Command name to use the command.", "Instance of a command to call.");
  ...
  // Locale access
  bind("string format(const string &in fmt, const string &in)",
       asFUNCTION(format_1))
      doc("Replaces $n in fmt with the related argument.", "Format to use.",
          "$1", "Formatted string.");
  bind(
      "string format(const string &in fmt, const string &in, const string &in)",
      asFUNCTION(format_2))
      doc("Replaces $n in fmt with the related argument.", "Format to use.",
          "$1", "$2", "Formatted string.");
  bind("string format(const string &in fmt, const string &in, const string "
       "&in, const string &in)",
       asFUNCTION(format_3))
      doc("Replaces $n in fmt with the related argument.", "Format to use.",
          "$1", "$2", "$3", "Formatted string.");
  bind("string format(const string &in fmt, const string &in, const string "
       "&in, const string &in, const string &in)",
       asFUNCTION(format_4))
      doc("Replaces $n in fmt with the related argument.", "Format to use.",
          "$1", "$2", "$3", "$4", "Formatted string.");
  bind("string format(const string &in fmt, const string &in, const string "
       "&in, const string &in, const string &in, const string &in)",
       asFUNCTION(format_5))
      doc("Replaces $n in fmt with the related argument.", "Format to use.",
          "$1", "$2", "$3", "$4", "$5", "Formatted string.");
  bind("string format(const string &in fmt, const array<string>& args)",
       asFUNCTION(format_n))
      doc("Replaces $n in fmt with the related argument.", "Format to use.",
  ...
  // Cloud stuff
  {
    ...
    bind("void enterQueue(const string& type, uint players, const string& "
         "version)",
         asFUNCTION(enterCloudQueue));
    ...
    bind("bool getQueuePlayerWait(uint &out ready, uint &out cap)",
         asFUNCTION(cloudQWaiting));

    bind("void prepItem(const string& folder)", asFUNCTION(cloudCreateItem))
        doc("Prepares an item for updating or creation. Wait a few frames for "
            "cloud::itemReady to be true.");
    ...
    bind("void setItemContent(const string& folder)",
         asFUNCTION(cloudItemFolder));
    bind("void setItemImage(const string& filename)",
         asFUNCTION(cloudItemImage));
    bind("void setItemTags(array<string>& tags)", asFUNCTION(cloudItemTags));
    bind("void setItemPublic()", asFUNCTION(cloudItemVisible));
    bind("void commitItem(const string& changelog)",
         asFUNCTION(cloudItemCommit));
