...
class SaveFileWriter : public SaveFile {
  ...
  void writeIdentifier(unsigned type, int id) { *this << id; }
  ...
  void close() {
    ...
    if (result == 0) {
      ...
      if (devices.cloud)
        devices.cloud->writeCloudFile(destName, std::string("saves/") +
                                                    getBasename(destName));
  ...
  void boundary() { *this << boundaryID++; }

public:
  SaveFileWriter(const std::string &file)
      : boundaryID(100), destName(file), tempName(getTemporaryFile()) {
...
Object *SaveFile::readExistingObject() { return getObjectByID(read<int>()); }
