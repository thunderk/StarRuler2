...
static void destroyMessage(SaveMessage &msg) { msg.~SaveMessage(); }
...
template <class T> static SaveMessage &wrapRead(SaveMessage &msg, T &value) {
  try {
    msg >> value;
  } catch (net::MessageReadError) {
    scripts::throwException("Error reading from message: end of message.");
...
static void addIdentifier(SaveMessage &msg, unsigned type, int id,
                          const std::string &ident) {
  if (!msg.file.addIdentifier(type, id, ident))
    scripts::throwException(
        format("Duplicate identifier: '$1'.", ident.c_str()).c_str());
}
...
void RegisterSaveFileBinds(bool server, bool decl) {
  if (decl) {
    ClassBind msg("SaveFile", asOBJ_VALUE | asOBJ_APP_CLASS_CDA,
                  sizeof(SaveMessage));
    return;
  }
  ...
  msg.addExternMethod("void set_scriptVersion(uint version)",
                      asFUNCTION(setScriptVersion));
  msg.addExternMethod("uint get_scriptVersion()", asFUNCTION(getScriptVersion));

  msg.addExternMethod("void set_startVersion(uint version)",
                      asFUNCTION(setStartVersion));
  msg.addExternMethod("uint get_startVersion()", asFUNCTION(getStartVersion));

  msg.addExternMethod(
      "void addIdentifier(uint type, int id, const string&in ident)",
      asFUNCTION(addIdentifier));
  msg.addExternMethod("uint readIdentifier(uint type)",
                      asFUNCTION(readIdentifier));
  msg.addExternMethod("int getIdentifier(uint ident, int id)",
                      asFUNCTION(getIdentifier));
  msg.addExternMethod("void writeIdentifier(uint type, int id)",
                      asFUNCTION(writeIdentifier));
  msg.addExternMethod("uint getIdentifierCount(uint type)",
                      asFUNCTION(getIdentifierCount));
  msg.addExternMethod("uint getPrevIdentifierCount(uint type)",
                      asFUNCTION(getPrevIdentifierCount));
  ...
  msg.addExternMethod("SaveFile& opShr(bool&)", asFUNCTION(wrapRead<bool>));

#define BIND_TYPE(stype, rtype)                                                \
  msg.addMethod(                                                               \
      "SaveFile& opShl(" #stype "&)",                                          \
  ...
  msg.addExternMethod("SaveFile& opShl(ElevationMap&)",
                      asFUNCTION(writeElevation));
  msg.addExternMethod("SaveFile& opShr(ElevationMap&)",
  ...
  msg.addExternMethod("SaveFile& opShl(const Design@&)",
                      asFUNCTION(writeDesign));
  msg.addExternMethod("SaveFile& opShr(const Design@&)",
                      asFUNCTION(readDesign));
  ...
  if (type) {
    ...
    engine->SetUserData(type->GetMethodByDecl("void save(SaveFile& file)"),
                        EDID_SavableWrite);
    engine->SetUserData(type->GetMethodByDecl("void load(SaveFile& file)"),
                        EDID_SavableRead);
  } else {
