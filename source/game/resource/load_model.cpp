...
bool Library::processMeshes(int maxPriority, int amount) {
  ...
  // TODO: Probably this has a race condition (threads end while this loop is
  // running)
  queuedMeshLock.lock();
...
struct MeshLoadData {
  ...
  MeshLoadData()
      : isSphere(false), makeTangents(false), width(0), height(0), lodMesh(0),
        lodDist(1) {}
};
...
threads::threadreturn threadcall LoadMesh(void *loadData) {
  ...
  if (data.isSphere) {
    ...
    mesh = new Mesh();
    // TODO: Make work if multiple meshes happen to have the same name,
    // especially on Windows
    std::string cacheModel = devices.mods.getProfile("model_cache") + "/" +
                             getBasename(data.fileName);
    std::string sourceModel = devices.mods.resolve(data.fileName);

    if (useModelCache && fileExists(cacheModel) &&
        (getModifiedTime(cacheModel) - getModifiedTime(sourceModel)) >= 0) {
  ...
  if (mesh) {
    ...
    if (data.makeTangents) {
      ...
      for (auto i = mesh->faces.begin(), end = mesh->faces.end(); i != end;
           ++i) {
        auto &face = *i;
        ...
        mesh->tangents[face.a] += tanDir;
        if (mesh->tangents[face.a].zero())
          mesh->tangents[face.a] = tanDir;
        mesh->tangents[face.b] += tanDir;
        if (mesh->tangents[face.b].zero())
          mesh->tangents[face.b] = tanDir;
        mesh->tangents[face.c] += tanDir;
        if (mesh->tangents[face.c].zero())
          mesh->tangents[face.c] = tanDir;

        binormals[face.a] += bnDir;
        if (binormals[face.a].zero())
          binormals[face.a] = bnDir;
        binormals[face.b] += bnDir;
        if (binormals[face.b].zero())
          binormals[face.b] = bnDir;
        binormals[face.c] += bnDir;
        if (binormals[face.c].zero())
          binormals[face.c] = bnDir;
      }
    ...
    // TODO: Handle an invalid mesh being loaded (the referenced mesh must be
    // valid, but we have no data to load in)
    if (data.lodMesh)
      data.rMesh->setLOD(data.lodDist, data.lodMesh);
    queuedMeshLock.lock();
    queuedMeshes.push_back(
        std::tuple<Mesh *, render::RenderMesh *>(mesh, data.rMesh));
    queuedMeshLock.release();
  }
...
void Library::loadModels(const std::string &filename) {
  ...
  datahandler("Mesh", [&](std::string &value) { meshData->fileName = value; });
  ...
  datahandler("Sphere", [&](std::string &value) {
    if (sscanf(value.c_str(), "%d x %d", &meshData->width, &meshData->height) ==
            2 &&
        meshData->width > 0 && meshData->height > 0 &&
        meshData->width * meshData->height < 256 * 256)
