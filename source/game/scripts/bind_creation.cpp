...
struct ObjectDesc {
  ...
  ObjectDesc()
      : type(0), flags(objValid | objUninitialized), radius(1.0), owner(0),
...
void createDesc(void *mem) { new (mem) ObjectDesc(); }
...
void createLight(void *mem) { new (mem) LightDesc(); }
...
void makeNodeLight(LightDesc &desc, scene::Node *follow) {
  auto *light =
      new render::light::NodePointLight(follow); // grabbed by constructor
...
struct MeshDesc {
  ...
  MeshDesc()
      : material(0), mesh(0), iconSheet(0), iconIndex(0), memorable(false) {}
};

void createMeshDesc(void *mem) { new (mem) MeshDesc(); }
...
void bindMesh(Object *obj, MeshDesc &desc) {
  ...
  if (!desc.material || !desc.mesh) {
    throwException(
        "Mesh descriptor data incomplete (missing material/model?).");
    return;
  }
  ...
  if (!desc.iconSheet)
    ...
    mesh = new scene::MeshIconNode(desc.mesh, desc.material, desc.iconSheet,
                                   desc.iconIndex);
...
void bindGroupIcon(ObjectGroup *group, const render::SpriteSheet *sheet,
...
void RegisterObjectCreation(bool declarations) {
  if (declarations) {
    ClassBind desc("ObjectDesc", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_C,
                   sizeof(ObjectDesc));
    ClassBind mdesc("MeshDesc", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_C,
                    sizeof(MeshDesc));
    ClassBind ldesc("LightDesc", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_C,
                    sizeof(LightDesc));
  ...
  classdoc(
      lhint,
      "Scoped structure. Any objects created within the same lock hint will"
  ...
  bind("ObjectGroup@ makeObjectGroup(ObjectDesc& desc, uint count)",
       asFUNCTION(makeGroup))
      doc("Create an object group based on a description of an object.",
          "Object descriptor to use.",
          "Number of objects to create in the group.", "Created object.");
  ...
  bind("void bindMesh(Object& obj, MeshDesc& desc)", asFUNCTION(bindMesh))
      doc("Bind a graphical representation to an object in accordance to a "
          "mesh descriptor.",
          "Game object to bind to.", "Descriptor for the mesh to display.");

  bind("void bindGroupIcon(ObjectGroup& group, const SpriteSheet@ sheet, uint "
       "index)",
       asFUNCTION(bindGroupIcon))
      doc("Binds an icon to represent a ship group.", "", "", "");

  bind("Node@ bindCullingNode(Object& obj, const vec3d&in position, double "
       "radius)",
       asFUNCTION(bindCullingNode))
      doc("Bind a node that can be used for region culling to the object.",
          "Object to bind to.", "Center position for the cullable region.",
          "Radius of the cullable region.", "Handle to the created node.");

  bind("Node@ createCullingNode(const vec3d&in position, double radius)",
       asFUNCTION(createCullingNode))
      doc("Create a node that can be used for region culling.",
          "Center position for the cullable region.",
          "Radius of the cullable region.", "Handle to the created node.");

  bind("Node@ bindNode(Object& obj, const string &in type)",
       asFUNCTION(bindNode))
      doc("Bind a custom graphical node to an object.", "Object to bind to.",
          "Type of script node to create.", "Handle to the created node.");
  ...
  lhint.addExternBehaviour(asBEHAVE_RELEASE, "void f()",
                           asFUNCTION(endLockHint));
}
