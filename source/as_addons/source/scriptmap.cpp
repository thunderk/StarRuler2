...
extern bool IsHandleCompatibleWithObject(asCScriptEngine *engine, void *obj,
                                         int objTypeId, int handleTypeId);
...
int CScriptMap::GetRefCount() { return refCount.get(); }

void CScriptMap::SetGCFlag() { gcFlag = true; }

bool CScriptMap::GetGCFlag() { return gcFlag; }
...
void CScriptMap::Set(asINT64 key, void *value, int typeId) {
  ...
  if (typeId & asTYPEID_OBJHANDLE) {
    ...
    engine->AddRefScriptObject(valStruct.valueObj,
                               engine->GetTypeInfoById(typeId));
  } else if (typeId & asTYPEID_MASK_OBJECT) {
    // Create a copy of the object
    valStruct.valueObj =
        engine->CreateScriptObjectCopy(value, engine->GetTypeInfoById(typeId));
  } else {
...
bool CScriptMap::Iterator_GetValue(CScriptMap::mapType::const_iterator &it,
                                   void *value, int typeId) const {
  // Return the value
  if (typeId & asTYPEID_OBJHANDLE) {
    // A handle can be retrieved if the stored type is a handle of same or
    // compatible type or if the stored type is an object that implements the
    // interface that the handle refer to.
    if ((it->second.typeId & asTYPEID_MASK_OBJECT) &&
        IsHandleCompatibleWithObject((asCScriptEngine *)engine,
                                     it->second.valueObj, it->second.typeId,
                                     typeId)) {
      engine->AddRefScriptObject(it->second.valueObj,
                                 engine->GetTypeInfoById(it->second.typeId));
    ...
    if (isCompatible) {
      engine->AssignScriptObject(value, it->second.valueObj,
                                 engine->GetTypeInfoById(typeId));
    ...
    if (it->second.typeId == asTYPEID_INT64 && typeId == asTYPEID_DOUBLE) {
      *(double *)value = double(it->second.valueInt);
      return true;
    } else if (it->second.typeId == asTYPEID_DOUBLE &&
...
asUINT CScriptMap::GetSize() const { return asUINT(dict.size()); }
...
CScriptArray *CScriptMap::GetKeys() const {
  ...
  //                 Only problem is if multiple engines are used, as they may
  //                 not share the same type id. Alternatively it can be stored
  //                 in the user data for the map type.
...
CScriptMap::Iterator CScriptMap::GetIterator() const { return Iterator(this); }
...
CScriptMap::Iterator::Iterator() { container = 0; }
...
static void DestructIterator(CScriptMap::Iterator &it) { it.~Iterator(); }

asINT64 CScriptMap::Iterator::GetKey() {
  if (!container || modCount != container->modCount ||
      it == container->dict.end()) {
    asIScriptContext *ctx = asGetActiveContext();
    if (ctx)
...
CScriptMap::Iterator &
CScriptMap::Iterator::operator=(const CScriptMap::Iterator &other) {
  if (other.container)
...
void ScriptMapFactory_Generic(asIScriptGeneric *gen) {
  *(CScriptMap **)gen->GetAddressOfReturnLocation() =
      new CScriptMap(gen->GetEngine());
}
...
void RegisterScriptMap_Native(asIScriptEngine *engine) {
  ...
  r = engine->RegisterObjectType("map_iterator", sizeof(CScriptMap::Iterator),
                                 asOBJ_VALUE | asOBJ_APP_CLASS_CDA);
  assert(r >= 0);

  r = engine->RegisterObjectType("map", sizeof(CScriptMap),
                                 asOBJ_REF | asOBJ_GC);
  assert(r >= 0);
  // Use the generic interface to construct the object since we need the engine
  // pointer, we could also have retrieved the engine pointer from the active
  // context
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_FACTORY, "map@ f()",
                                      asFUNCTION(ScriptMapFactory_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_ADDREF, "void f()",
                                      asMETHOD(CScriptMap, AddRef),
                                      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_RELEASE, "void f()",
                                      asMETHOD(CScriptMap, Release),
                                      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod(
      "map", "map &opAssign(const map &in)",
      asMETHODPR(CScriptMap, operator=, (const CScriptMap &), CScriptMap &),
      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod(
      "map", "void set(int64, const ?&in)",
      asMETHODPR(CScriptMap, Set, (asINT64, void *, int), void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "map", "bool get(int64, ?&out) const",
      asMETHODPR(CScriptMap, Get, (asINT64, void *, int) const, bool),
      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod(
      "map", "void set(int64, int64&in)",
      asMETHODPR(CScriptMap, Set, (asINT64, asINT64 &), void), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "map", "bool get(int64, int64&out) const",
      asMETHODPR(CScriptMap, Get, (asINT64, asINT64 &) const, bool),
      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod(
      "map", "void set(int64, double&in)",
      asMETHODPR(CScriptMap, Set, (asINT64, double &), void), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "map", "bool get(int64, double&out) const",
      asMETHODPR(CScriptMap, Get, (asINT64, double &) const, bool),
      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map", "bool exists(int64) const",
                                   asMETHOD(CScriptMap, Exists),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "bool isEmpty() const",
                                   asMETHOD(CScriptMap, IsEmpty),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "uint getSize() const",
                                   asMETHOD(CScriptMap, GetSize),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "map", "void delete(int64)",
      asMETHODPR(CScriptMap, Delete, (asINT64), void), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "map", "void delete(map_iterator&)",
      asMETHODPR(CScriptMap, Delete, (CScriptMap::Iterator &), void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "void deleteAll()",
                                   asMETHOD(CScriptMap, DeleteAll),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "void deleteNulls()",
                                   asMETHOD(CScriptMap, DeleteNulls),
                                   asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map", "map_iterator iterator()",
                                   asMETHOD(CScriptMap, GetIterator),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "array<int64> @getKeys() const",
                                   asMETHOD(CScriptMap, GetKeys),
                                   asCALL_THISCALL);
  assert(r >= 0);

  // Register GC behaviours
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_GETREFCOUNT, "int f()",
                                      asMETHOD(CScriptMap, GetRefCount),
                                      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_SETGCFLAG, "void f()",
                                      asMETHOD(CScriptMap, SetGCFlag),
                                      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_GETGCFLAG, "bool f()",
                                      asMETHOD(CScriptMap, GetGCFlag),
                                      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "map", asBEHAVE_ENUMREFS, "void f(int&in)",
      asMETHOD(CScriptMap, EnumReferences), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "map", asBEHAVE_RELEASEREFS, "void f(int&in)",
      asMETHOD(CScriptMap, ReleaseAllReferences), asCALL_THISCALL);
  assert(r >= 0);

#if AS_USE_STLNAMES == 1
  // Same as isEmpty
  r = engine->RegisterObjectMethod("map", "bool empty() const",
                                   asMETHOD(CScriptMap, IsEmpty),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as getSize
  r = engine->RegisterObjectMethod("map", "uint size() const",
                                   asMETHOD(CScriptMap, GetSize),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as delete
  r = engine->RegisterObjectMethod("map", "void erase(asINT64 in)",
                                   asMETHOD(CScriptMap, Delete),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as deleteAll
  r = engine->RegisterObjectMethod(
      "map", "void clear()", asMETHOD(CScriptMap, DeleteAll), asCALL_THISCALL);
  assert(r >= 0);
#endif

  // Register iterator methods
  r = engine->RegisterObjectBehaviour(
      "map_iterator", asBEHAVE_CONSTRUCT, "void f()",
      asFUNCTION(DefaultConstructIterator), asCALL_CDECL_OBJFIRST);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "map_iterator", asBEHAVE_CONSTRUCT, "void f(const map &in)",
      asFUNCTION(ConstructIterator), asCALL_CDECL_OBJFIRST);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map_iterator", asBEHAVE_DESTRUCT,
                                      "void f()", asFUNCTION(DestructIterator),
                                      asCALL_CDECL_OBJFIRST);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map_iterator", "int64 get_key()",
                                   asMETHOD(CScriptMap::Iterator, GetKey),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "map_iterator", "bool iterate(?&out)",
      asMETHODPR(CScriptMap::Iterator, Iterate, (void *, int), bool),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "map_iterator", "bool iterate(int64 &out, ?&out)",
      asMETHODPR(CScriptMap::Iterator, Iterate, (asINT64 *, void *, int), bool),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map_iterator", "bool get_valid()",
                                   asMETHOD(CScriptMap::Iterator, IsValid),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "map_iterator", "map_iterator& opAssign(const map_iterator &in)",
      asMETHOD(CScriptMap::Iterator, operator=), asCALL_THISCALL);
  assert(r >= 0);
}
...
void RegisterScriptMap_Generic(asIScriptEngine *engine) {
  ...
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_ADDREF, "void f()",
                                      asFUNCTION(ScriptMapAddRef_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_RELEASE, "void f()",
                                      asFUNCTION(ScriptMapRelease_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map", "map &opAssign(const map &in)",
                                   asFUNCTION(ScriptMapAssign_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map", "void set(int64, ?&in)",
                                   asFUNCTION(ScriptMapSet_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "bool get(int64, ?&out) const",
                                   asFUNCTION(ScriptMapGet_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map", "void set(int64, int64&in)",
                                   asFUNCTION(ScriptMapSetInt_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "bool get(int64, int64&out) const",
                                   asFUNCTION(ScriptMapGetInt_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map", "void set(int64, double&in)",
                                   asFUNCTION(ScriptMapSetFlt_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "bool get(int64, double&out) const",
                                   asFUNCTION(ScriptMapGetFlt_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map", "bool exists(asINT64 in) const",
                                   asFUNCTION(ScriptMapExists_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "void delete(asINT64 in)",
                                   asFUNCTION(ScriptMapDelete_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("map", "void deleteAll()",
                                   asFUNCTION(ScriptMapDeleteAll_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("map", "array<int64> @getKeys() const",
                                   asFUNCTION(CScriptMapGetKeys_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);

  // Register GC behaviours
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_GETREFCOUNT, "int f()",
                                      asFUNCTION(ScriptMapGetRefCount_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_SETGCFLAG, "void f()",
                                      asFUNCTION(ScriptMapSetGCFlag_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("map", asBEHAVE_GETGCFLAG, "bool f()",
                                      asFUNCTION(ScriptMapGetGCFlag_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "map", asBEHAVE_ENUMREFS, "void f(int&in)",
      asFUNCTION(ScriptMapEnumReferences_Generic), asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "map", asBEHAVE_RELEASEREFS, "void f(int&in)",
      asFUNCTION(ScriptMapReleaseAllReferences_Generic), asCALL_GENERIC);
  assert(r >= 0);
}
