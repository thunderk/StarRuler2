...
BeamNode::BeamNode(const render::RenderState *Material, float Width,
                   const vec3d &startPoint, const vec3d &endPoint,
                   bool StaticSize)
    : material(Material), width(Width), endPosition(endPoint),
      staticSize(StaticSize), uvLength(1.f) {
...
void BeamNode::render(render::RenderDriver &driver) {
  ...
  vec3d offset =
      (endPosition - abs_position).cross(driver.cam_facing).normalized(size);
