...
ElevationMap::ElevationMap() : generated(false), grid(0) {}
...
void ElevationMap::generate(const vec2d &interval, double power) {
  ...
  foreach (p, points) {
    unsigned bigX = std::min(
        (unsigned)((p->center.x - topLeft.x) / bigInterval), bigGridSize - 1);
    unsigned bigY = std::min(
        (unsigned)((p->center.z - topLeft.y) / bigInterval), bigGridSize - 1);
  ...
  for (int iy = 0; iy < gridResolution.y; ++iy) {
    for (int ix = 0; ix < gridResolution.x; ++ix) {
      ...
      unsigned bigX = std::min(
          (unsigned)((realPos.x - gridStart.x) / bigInterval), bigGridSize - 1);
      unsigned bigY = std::min(
          (unsigned)((realPos.y - gridStart.z) / bigInterval), bigGridSize - 1);
...
double ElevationMap::get(vec2d point) { return get(point.x, point.y); }
...
bool ElevationMap::getClosestPoint(const line3dd &inLine, vec3d &closestPoint) {
  ...
  //  Always make sure the end goes to the other plane, or a line that stops
  //  before the region will never collide
  ...
  for (unsigned checks = 0; checks < 1000; ++checks) {
    ...
    BiPatch::Vector br(absx + gridInterval.x, lookup(x + 1, y + 1),
                       absy + gridInterval.y);
