...
public:
  ...
  // Server management
  void host(const std::string &gamename, int port, unsigned maxPlayers = 0,
            bool isPublic = true, bool punchthrough = true,
            const std::string &password = "");
  void connect(const std::string &hostname, int port,
               const std::string &password = "", bool tryPunchthrough = false);
  void connect(const net::Address &addr, bool tryPunchthrough, bool attemptOnly,
               const std::string &password);
  void connect(const net::Address &addr, const std::string &password = "",
               bool attemptOnly = false);
  void connect(net::Game &game, bool disablePunchthrough = false,
               const std::string &password = "");
  void disconnect();
  ...
  void sendDesignUpdate(Empire *emp, const Design *older, const Design *newer,
                        DesignClass *cls = nullptr);
  void sendEffectorUpdate(const Effector *eff);
  void sendEffectorDestruction(const Effector *eff);
  void sendEffectorTrigger(const EffectorDef *eff, const Effector *efftr,
                           Object *obj, const EffectorTarget &target,
                           double atTime);
  void sendParticleSystem(const std::string &id, const vec3d &position,
                          const vec3d &velocity, const quaterniond &rot,
                          float scale, unsigned visionMask);
  void sendParticleSystem(const std::string &id, const vec3d &position,
                          const vec3d &velocity, const quaterniond &rot,
                          float scale, Object *parent);
