...
uint8_t IPV6_MCAST_ALL_NODES[] = {0xff, 0x02, 0x00, 0x00, 0x00, 0x00,
...
Transport::Transport(AddressType Type)
    : references(1), active(true), canBroadcast(false), type(Type),
      rate(RATE_LIMIT) {
  net::prepare();
...
void Transport::grab() const { ++references; }
...
void Transport::listen(Address &address, bool rcvBroadcast) {
  ...
  if (canBroadcast) {
    if (type == AT_IPv4) {
      ...
      setsockopt(sockfd, SOL_SOCKET, IPV6_ADD_MEMBERSHIP, (const char *)&mcast,
                 sizeof(mcast));
      setsockopt(sockfd, SOL_SOCKET, IPV6_MULTICAST_IF, &zero_arg,
                 sizeof(zero_arg));
    }
  }
}
