...
void NodeSyncAnimator::animate(Node *node) {
  ...
  if (obj) {
    ...
    vis = obj->isVisibleTo(player) &&
          obj->position.distanceToSQ(devices.render->cam_pos) <
              (node->distanceCutoff * obj->radius * obj->radius);
    bool wasVisible = node->visible;

    if (!vis && node->getFlag(NF_Memorable) &&
        (node->remembered || obj->isKnownTo(player))) {
      vis = true;
    ...
    if (vis && wasVisible) {
      if (obj->lastTick > node->lastUpdate + 1.0 / 60.0) {
        interpPct = (frameTime_s - node->lastUpdate) /
                    (obj->lastTick - node->lastUpdate);
        ...
        vec3d predicted =
            obj->position +
            (obj->velocity + obj->acceleration * (tDiff * 0.5)) * tDiff;
        // node->position = node->position.interpolate(predicted, (frameTime_s -
        // node->lastUpdate) / tDiff);
...
NodeSyncAnimator *NodeSyncAnimator::getSingleton() { return &syncAnimator; }
