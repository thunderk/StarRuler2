...
Mod::Mod()
    : parent(0), isBase(false), version(1), listed(true), enabled(false),
      isNew(false), forced(false), compatibility(0), forCurrentVersion(false) {}
...
void Mod::resolve(const std::string &path,
...
void Mod::listFiles(const std::string &dir,
                    std::map<std::string, std::string> &out, const char *filter,
                    bool recurse, bool inherit) const {
  const Mod *mod = this;
  ...
  std::function<void(const std::string &, const std::string &,
                     const std::string &)>
      readDir = [&](const std::string &relpath, const std::string &abspath,
...
void Mod::resolveDirectory(const std::string &dir,
...
void Mod::getConflicts(const Mod *other,
...
void Manager::registerMod(const std::string &folder, const std::string &ident) {
  ...
  if (!mod.name.empty()) {
    ...
    if (mod.parentname.empty() && !mod.overrides.empty())
      warn("WARNING: Mod '%s' has overrides declared, but does not derive from "
           "any mod.",
           mod.name.c_str());
...
void Manager::finalize() {
  ...
  while (datafile++) {
    ...
    if (it != modNames.end()) {
      it->second->enabled =
          datafile.key == "Enabled" || datafile.key == "Forced";
  ...
  for (size_t i = 0, cnt = mods.size(); i < cnt; ++i) {
    ...
    for (size_t j = 0; j < i; ++j) {
      ...
      if (!mod->isCompatible(othermod)) {
        error("ERROR: Mod '%s' is not compatible with previously enabled mod "
              "'%s'. Disabling.",
...
bool Manager::enableMod(const std::string &name) {
  ...
  foreach (chk, activeMods) {
    ...
    if (mod->isBase && (*chk)->isBase) {
      ...
      error("Cannot enable mod: %s. A base mod is already loaded.",
            name.c_str());
      return false;
    }
    if (!mod->isCompatible(*chk)) {
      error("Cannot enable mod: %s. Incompatible with mod %s.", name.c_str(),
            (*chk)->name.c_str());
      return false;
    }
  }
...
std::string Manager::resolve(const std::string &path,
...
void Manager::resolve(const std::string &path,
                      std::vector<std::string> &check) const {
  if (path[0] == '~') {
...
void Manager::listFiles(const std::string &dir,
...
void Manager::resolveDirectory(const std::string &dir,
                               std::vector<std::string> &out) const {
...
std::string Manager::getProfile() const { return currentMod->getProfile(); }
