...
Design::Design(const Design::Descriptor &desc)
    : hull(0), stateCount(0), effectorStateCount(0), dataCount(0),
      effectorCount(0), initialized(false), id(-1), owner(0), used(false),
      obsolete(false), outdated(false), revision(0), cls(0), totalHP(0),
      forceHull(false), data(nullptr), clientData(nullptr),
      serverData(nullptr) {
  init(desc);
}
...
void Design::init(const Design::Descriptor &desc) {
  ...
  if (size < hull->minSize) {
    std::string errMsg =
        format(devices.locale.localize("ERROR_MINIMUM_SIZE").c_str(),
               hull->name, hull->minSize);
    errors.push_back(DesignError(true, errMsg));
  } else if (hull->maxSize > 0 && size > hull->maxSize) {
    std::string errMsg =
        format(devices.locale.localize("ERROR_MAXIMUM_SIZE").c_str(),
    ...
    if (type) {
      ...
      if (!type->hasTag(appliedTag)) {
        std::string errMsg = format(
            devices.locale.localize("ERROR_CANNOT_APPLY").c_str(), type->name);
        errors.push_back(DesignError(true, errMsg));
      } else {
        for (unsigned n = 0, ncnt = type->getTagValueCount(appliedTag);
             n < ncnt; ++n) {
          std::string tag = type->getTagValue(appliedTag, n);
          for (size_t j = 0; j < i; ++j) {
            if (desc.appliedSystems[j] &&
                desc.appliedSystems[j]->hasTagValue(appliedTag, tag)) {
              std::string errMsg =
                  format(devices.locale.localize("ERROR_ONE_APPLIED").c_str(),
                         devices.locale.localize(format("APPLIED_$1", tag)));
              errors.push_back(DesignError(true, errMsg));
            }
          }
        }
  ...
  for (unsigned index = 0; index < systems.size(); ++index) {
    ...
    if (sys->type->isContiguous && !sys->type->isHull &&
        !sys->type->isApplied && !sys->hexes.empty()) {
      connected.zero();
      sys->markConnected(connected, sys->hexes[0]);
      for (size_t i = 1, cnt = sys->hexes.size(); i < cnt; ++i) {
        if (!connected[sys->hexes[i]]) {
          std::string errMsg =
              format(devices.locale.localize("ERROR_NOT_CONTIGUOUS").c_str(),
                     sys->type->name);
...
void Design::buildDamageOrder() {
  ...
  std::sort(damageOrder.begin(), damageOrder.end(),
            [](Subsystem *first, Subsystem *second) -> bool {
...
const Design *Design::base() const { return original ? original.ptr : this; }
...
void Design::makeDistanceMap(Image &img, vec2i pos, vec2i size) const {
  ...
  double xrat =
      double(size.x - xspac - xspac) / double(hull->active.width) / 0.75;
  ...
  for (int y = yspac; y < size.y - yspac; ++y) {
    for (int x = xspac; x < size.x - xspac; ++x) {
      vec2d pctPos(double(x - xspac) / double(size.x - xspac - xspac),
                   double(y - yspac) / double(size.y - yspac - yspac));
...
void Design::write(net::Message &msg) const {
  ...
  for (unsigned i = 0, sysCnt = (unsigned)subsystems.size(); i < sysCnt; ++i) {
    ...
    for (unsigned short j = 0, hexCnt = (unsigned)sys.hexes.size(); j < hexCnt;
         ++j) {
...
void Design::init(net::Message &msg) {
  ...
  for (unsigned i = 0; i < sysCnt; ++i) {
    ...
    for (unsigned j = 0; j < hexCnt; ++j) {
      ...
      if (sys.type == 0 || modIndex < 0 ||
          modIndex >= (int)sys.type->modules.size())
        sys.modules[j] = 0;
      else
...
static asIScriptObject *buildDataForEngine(scripts::Manager *man, void *msg,
                                           unsigned call) {
  // Create the object
...
void Design::bindData() {
  if (data) {
    data->rewind();
    clientData = buildDataForEngine(devices.scripts.client, data,
                                    scripts::EDID_SerializableRead);
    data->rewind();
    serverData = buildDataForEngine(devices.scripts.server, data,
                                    scripts::EDID_SerializableRead);
    data->rewind();
  }
}

Design::Design(SaveFile &file)
    : initialized(true), data(nullptr), clientData(nullptr),
      serverData(nullptr) {
  ...
  file.read(hexStatusIndex.data,
            sizeof(int) * hexStatusIndex.width * hexStatusIndex.height);
  ...
  if (file >= SFV_0018) {
    if (file.read<bool>()) {
      ...
      serverData = buildDataForEngine(devices.scripts.server, &msg,
                                      scripts::EDID_SavableRead);

      data = new net::Message();
      auto *writeFunc =
          (asIScriptFunction *)devices.scripts.server->engine->GetUserData(
...
void Design::save(SaveFile &file) const {
  ...
  file.write(hexStatusIndex.data,
             sizeof(int) * hexStatusIndex.width * hexStatusIndex.height);
