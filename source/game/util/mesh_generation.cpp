...
Mesh *generateSphereMesh(unsigned int vertical, unsigned int horizontal) {
  ...
  for (unsigned int v = 1; v < vertical; ++v) {
    ...
    for (unsigned int h = 0; h <= horizontal; ++h) {
      float angle = (float)twopi * (float)(h % horizontal) /
                    hf; // Force angle on both ends to be identical
      Vertex vert;
  ...
  for (unsigned int v = 0; v < vertical; ++v) {
    unsigned int stride = horizontal + 1, line = v * stride,
                 nextline = (v + 1) * stride;
    for (unsigned int h = 0; h < horizontal; ++h) {
      if (v != 0)
      ...
      if (v != vertical - 1)
        mesh.faces.push_back(
            Mesh::Face(h + line + 1, h + nextline + 1, h + nextline));
    }
  }
