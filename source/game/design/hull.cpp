...
unsigned getHullCount() { return (unsigned)hullDefinitions.size(); }
...
HullDef::HullDef()
    : id(-1), background(0), material(0), mesh(0), iconSheet(0), iconIndex(0),
      gridSize(16, 8), backgroundScale(1.0), modelScale(1.0), active(gridSize),
      exterior(gridSize), activeCount(0), exteriorCount(0), minSize(1.0),
      maxSize(-1.0), baseHull(this), shape(nullptr), special(false) {
  active.clear(false);
  exterior.clear(false);
}

HullDef::HullDef(const HullDef &other) { *this = other; }
...
bool HullDef::calculateImpacts() {
  ...
  for (unsigned n = 0; n < 8; ++n) {
    ...
    for (unsigned i = 0; i < 64; ++i) {
      ...
      double st = sin(theta);
      vec3d start = vec3d(st * cos(phi), cos(theta), st * sin(phi));
  ...
  for (unsigned n = 0; n < 8; ++n) {
    ...
    for (unsigned i = 0; i < 64; ++i) {
      ...
      double st = sin(theta);
      vec3d point = vec3d(st * cos(phi), cos(theta), st * sin(phi));
...
vec3d HullDef::getImpact(const vec3d &offset, double radius,
                         bool constant) const {
  ...
  double theta =
      std::fmod(twopi + acos(uoffset.y / uoffset.getLength()), twopi);
...
void HullDef::calculateExterior(vec2u pos, unsigned direction) {
  ...
  do {
    if ((mask & flagExteriorFaux) &&
        !(exterior[pos] & (flagExteriorFaux | flagExteriorPass)) && active[pos])
      break;
    exterior[pos] |= mask;
    if (active[pos]) {
...
void readHullDefinitions(const std::string &filename,
  ...
  datahandler.lineHandler([&](std::string &line) {
    ...
    for (unsigned i = 0; i < cnt; ++i) {
      if (x >= def->gridSize.width) {
        error("Error: Too many characters on row for hull '%s'.",
              def->ident.c_str());
        break;
      }
      ...
      case ' ':
        break;
      default:
        error("Error: Invalid character '%c' on row for hull '%s'.", line[i],
              def->ident.c_str());
        break;
      }
    }
  ...
  datahandler("Background", [&](std::string &value) {
    ...
    if (!bg) {
      error("(Hull %s): Error: Unknown material %s.", def->ident.c_str(),
            value.c_str());
      return;
    }
  ...
  datahandler("Model", [&](std::string &value) {
    ...
    if (!mesh) {
      if (devices.render)
        error("(Hull %s): Error: Unknown model %s.", def->ident.c_str(),
              value.c_str());
      return;
    }
  ...
  datahandler("GridSize", [&](std::string &value) {
    ...
    if (args.size() != 2) {
      error("(Hull %s): Error: Invalid grid size specification.",
            def->ident.c_str());
      return;
    }
  ...
  datahandler("GridOffset", [&](std::string &value) {
    ...
    if (args.size() != 4) {
      error("(Hull %s): Error: Invalid grid offset specification.",
            def->ident.c_str());
      return;
    }
...
void writeHullDefinitions(const std::string &filename,
  ...
  foreach (it, hulls) {
    ...
    file << "\tGridSize: " << def->gridSize.x << ", " << def->gridSize.y
         << "\n";
    file << "\tGridOffset: " << def->gridOffset.topLeft.x << ","
         << def->gridOffset.topLeft.y;
    file << ", " << def->gridOffset.botRight.x << ","
         << def->gridOffset.botRight.y << "\n";
    file << "\n";
...
unsigned Shipset::getHullCount() const { return hulls.size(); }
...
void loadShipset(const std::string &filename) {
  ...
  datahandler("Hull", [&](std::string &value) {
    if (set) {
      ...
      if (hull)
        ...
        error("Could not find hull %s for shipset %s", value.c_str(),
              set->ident.c_str());
    }
  });
  ...
  datahandler("Model", [&](std::string &value) {
    if (skin && set) {
      ...
      if (!mesh) {
        if (devices.render)
          error("(%s Ship Skin %s): Error: Unknown model %s.",
                set->ident.c_str(), skin->ident.c_str(), value.c_str());
        return;
      }
  ...
  datahandler("Material", [&](std::string &value) {
    if (skin && set) {
      ...
      if (!mat) {
        if (devices.render)
          error("(%s Ship Skin %s): Error: Unknown material %s.",
                set->ident.c_str(), skin->ident.c_str(), value.c_str());
        return;
      }
...
void clearShipsets() {
  shipsets.clear();
  shipsets.shrink_to_fit();
  shipsetIndices.clear();
}

unsigned getShipsetCount() { return (unsigned)shipsets.size(); }
