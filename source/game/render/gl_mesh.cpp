...
public:
  ...
  GLMesh()
      : valid(false), lod_distance(1), lod(0), tangent_buffer(0),
        vertex_array(0), color_buffer(0), uv2_buffer(0) {}
  ...
  void resetToMesh(const Mesh &mesh) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer);
    glBufferData(GL_ARRAY_BUFFER, vertices * sizeof(Vertex), &mesh.vertices[0],
                 GL_STATIC_DRAW);

    if (!mesh.tangents.empty()) {
      glGenBuffers(1, &tangent_buffer);
      glBindBuffer(GL_ARRAY_BUFFER, tangent_buffer);
      glBufferData(GL_ARRAY_BUFFER, mesh.tangents.size() * sizeof(vec4f),
    ...
    if (!mesh.colors.empty()) {
      ...
      glBindBuffer(GL_ARRAY_BUFFER, color_buffer);
      glBufferData(GL_ARRAY_BUFFER, mesh.colors.size() * sizeof(Colorf),
    ...
    if (!mesh.uvs2.empty()) {
      ...
      glBindBuffer(GL_ARRAY_BUFFER, uv2_buffer);
      glBufferData(GL_ARRAY_BUFFER, mesh.uvs2.size() * sizeof(vec4f),
                   mesh.uvs2.data(), GL_STATIC_DRAW);
    }
  ...
  const AABBoxf &getBoundingBox() const { return bbox; }
  ...
  void render() const {
    if (lastRenderedMesh != this) {
      // We only need to check valid here, because we can't be the rendered mesh
      // if we aren't valid
      if (!valid)
        return;
...
RenderMesh *createGLMesh(const Mesh &mesh) { return new GLMesh(mesh); }
