...
bool streq_nocase(const std::string &str, const std::string &substr,
                  unsigned start, unsigned length = 0);
...
#define HANDLE_BOOL(handler, key, obj, member)                                 \
  handler(key, [&](std::string &value) {                                       \
    if (obj)                                                                   \
      obj->member = toBool(value);                                             \
  });

#define HANDLE_NUM(handler, key, obj, member)                                  \
  handler(key, [&](std::string &value) {                                       \
    if (obj)                                                                   \
      obj->member = toNumber<decltype(obj->member)>(value);                    \
  });

#define HANDLE_ENUM_W(handler, key, obj, member, Enum, ShowErrors)             \
  handler(key, [&](std::string &value) {                                       \
    if (obj) {                                                                 \
...
#define HANDLE_ENUM(handler, key, obj, member, Enum)                           \
  HANDLE_ENUM_W(handler, key, obj, member, Enum, true)

// Generic string split
void split(const std::string &input, std::vector<std::string> &out,
           char delimit, bool trim = false, bool listEmpty = false);
void split(const std::string &input, std::vector<std::string> &out,
           const char *delimit, bool trim = false, bool listEmpty = false);

// Splits a string of the format:
//  front<delim_front>inner<delim_back>back
// Back is optional
// If both delimeters are not found, returns false and nothing is done to front,
// inner, or back If both delimteres are found, returns true and front, inner,
// and back are set to the corresponding sub-strings without trimming
bool split(const std::string &input, std::string &front, char delim_front,
           std::string &inner, char delim_back, std::string *back = 0);

// Split a function call into parts
bool funcSplit(const std::string &input, std::string &name,
               std::vector<std::string> &arguments, bool strip = true);
...
std::string replaced(const std::string &str, const std::string &replace,
                     const std::string &with);
...
std::string standardize(double val, bool showIntegral = false,
                        bool roundUp = false);
std::string formatLargeNum(double val);

// Read number from string
template <class T>
T toNumber(const std::string &str, T def = 0,
...
template <class T> std::string toString(T num, unsigned precision = 0) {
...
template <> std::string toString(Color color, unsigned precision);
