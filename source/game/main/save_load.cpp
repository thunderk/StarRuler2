...
void addSubsystemIdentifiers(SaveFile &file) {
  ...
  for (unsigned i = 0; i < cnt; ++i) {
    ...
    foreach (it, def->modifierIds) {
      auto *mod = it->second;
      file.addIdentifier(SI_SubsystemModifier, mod->umodifid,
                         def->id + "::" + it->first);
    }
  }

  for (auto it = shipVariableIndices.begin(), end = shipVariableIndices.end();
       it != end; ++it)
    file.addIdentifier(SI_ShipVar, it->second, it->first);

  for (auto it = variableIndices.begin(), end = variableIndices.end();
       it != end; ++it)
    file.addIdentifier(SI_SubsystemVar, it->second, it->first);

  for (auto it = hexVariableIndices.begin(), end = hexVariableIndices.end();
       it != end; ++it)
...
bool loadGame(const std::string &filename) {
  ...
  try {
    ...
    for (unsigned i = 0; i < objectCount; ++i) {
      Object *obj = file;
      obj->load(file);
      devices.universe->children.push_back(obj);
      obj->grab();
    }
    ...
    // TODO: If we don't have a client, there could still be data to load (if we
    // load it last, we don't have to care)
    if (devices.scripts.client)
