  ...
  default:
    if (pData) {
      if (objType == engine->GetTypeIdByDecl("recti")) {
        recti *r = (recti *)pData;
        sprintf(buffer, "<%d,%d,%d,%d>", r->topLeft.x, r->topLeft.y,
...
void printContext() {
  ...
  for (unsigned d = 0; d < ctx->GetCallstackSize(); ++d) {
    ...
    func->GetName();
    error("%u: %s\n %s(%i)", d, func->GetDeclaration(),
          func->GetScriptSectionName(), ctx->GetLineNumber(d));
    for (unsigned v = 0; v < (unsigned)ctx->GetVarCount(d); ++v) {
      if (!ctx->IsVarInScope(v, d))
        continue;
      inspect(ctx, ctx->GetVarName(v, d), ctx->GetAddressOfVar(v, d),
              ctx->GetVarTypeId(v, d));
    }
  }
}

void RegisterInspectionBinds() {
  bind("void inspect(const string &in name, const ?&in)",
       asFUNCTIONPR(inspect, (const std::string &, void *, int), void));
