...
static void _event_connect(irc_session_t *session, const char *event,
                           const char *origin, const char **params,
                           unsigned int count);
static void _event_join(irc_session_t *session, const char *event,
                        const char *origin, const char **params,
                        unsigned int count);
static void _event_part(irc_session_t *session, const char *event,
                        const char *origin, const char **params,
                        unsigned int count);
static void _event_quit(irc_session_t *session, const char *event,
                        const char *origin, const char **params,
                        unsigned int count);
static void _event_nick(irc_session_t *session, const char *event,
                        const char *origin, const char **params,
                        unsigned int count);
static void _event_topic(irc_session_t *session, const char *event,
                         const char *origin, const char **params,
                         unsigned int count);
static void _event_privmsg(irc_session_t *session, const char *event,
                           const char *origin, const char **params,
                           unsigned int count);
static void _event_channel(irc_session_t *session, const char *event,
                           const char *origin, const char **params,
                           unsigned int count);
static void _event_action(irc_session_t *session, const char *event,
                          const char *origin, const char **params,
                          unsigned int count);
static void _event_mode(irc_session_t *session, const char *event,
                        const char *origin, const char **params,
                        unsigned int count);
static void _event_umode(irc_session_t *session, const char *event,
                         const char *origin, const char **params,
                         unsigned int count);
static void _event_numeric(irc_session_t *session, unsigned event,
                           const char *origin, const char **params,
                           unsigned int count);
...
public:
  ...
  struct Channel : AtomicRefCounted {
    ...
    Channel()
        : messageId(1), handledId(0), isPM(false), active(true), closed(false),
          highlight(false) {}
    ...
    unsigned getMessageCount() { return history.size(); }

    unsigned getUserCount() { return users.size(); }
  ...
  IRC()
      : running(false), connected(false), highlight(false), hasNet(false),
        session(nullptr), shouldConnect(false) {
  ...
  void kick(Channel *chan, const std::string &user,
  ...
  void _message(Channel &chnl, const char *user, const char *message,
                MessageType type = MT_Message) {
  ...
  void _message(const char *channel, const char *user, const char *message,
                MessageType type = MT_Message) {
    threads::Lock lock(mutex);
  ...
  void _messageall(const char *user, const char *message,
                   MessageType type = MT_Message) {
    threads::Lock lock(mutex);
  ...
  void _onkick(const char *channel, const char *from, const char *kicked,
               const char *message) {
    threads::Lock lock(mutex);
  ...
  unsigned getChannelCount() { return channels.size(); }
...
static threads::threadreturn threadcall IRCThread(void *ptr) {
  ...
  do {
    if (int err =
            irc_connect(irc.session, ircServer.c_str(), ircPort, 0,
                        nickname.c_str(), userDesc.c_str(), userDesc.c_str())) {
      threads::Lock lock(irc.mutex);
...
void RegisterIRCBinds() {
  ...
  chan.setReferenceFuncs(asMETHOD(IRC::Channel, grab),
                         asMETHOD(IRC::Channel, drop));
  ...
  chan.addMethod("uint get_messageCount()",
                 asMETHOD(IRC::Channel, getMessageCount));
  chan.addMethod("string get_messages(uint index)",
                 asMETHOD(IRC::Channel, getMessage));
  chan.addMethod("string get_message_senders(uint index)",
                 asMETHOD(IRC::Channel, getSender));
  chan.addMethod("uint8 get_message_sender_types(uint index)",
                 asMETHOD(IRC::Channel, getSenderType));
  chan.addMethod("uint get_message_ids(uint index)",
                 asMETHOD(IRC::Channel, getMessageId));
  chan.addMethod("IRCMessageType get_message_types(uint index)",
                 asMETHOD(IRC::Channel, getMessageType));
  chan.addMethod("void getUsers(array<string>& data)",
                 asMETHOD(IRC::Channel, getUsers));
  ...
  irc.addMethod("bool send(IRCChannel& channel, const string&in message)",
                asMETHOD(IRC, send));
  irc.addMethod("void kick(IRCChannel& channel, const string&in user, const "
                "string&in message)",
                asMETHOD(IRC, send));
  irc.addMethod("void setTopic(IRCChannel& channel, const string&in topic)",
                asMETHOD(IRC, setTopic));
  irc.addMethod("IRCChannel@ join(const string&in channel)",
                asMETHOD(IRC, join));
  irc.addMethod("IRCChannel@ privmsg(const string&in user)",
                asMETHOD(IRC, privmsg));

  irc.addMethod("string get_nickname()", asMETHOD(IRC, getNickname));
  irc.addMethod("void set_nickname(const string&in name)",
                asMETHOD(IRC, setNickname));

  irc.addMethod("uint get_channelCount()", asMETHOD(IRC, getChannelCount));
  irc.addMethod("IRCChannel@ get_channels(uint index)",
                asMETHOD(IRC, getChannel));
  irc.addMethod("IRCChannel@ getChannel(const string&in name)",
                asMETHOD(IRC, getChannelByName));
