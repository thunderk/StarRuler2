...
void shader_sprite_pos(float *output, unsigned short, void *) {
  if (render::SpriteSheet::current) {
    vec4f pos = render::SpriteSheet::current->getSourceUV(
        render::SpriteSheet::currentIndex);
    output[0] = pos.x;
    output[1] = pos.y;
    output[2] = pos.z;
    output[3] = pos.w;
...
SpriteSheet::SpriteSheet()
    : mode(SM_Horizontal), width(32), height(32), spacing(0), perLine(0),
      dirty(true) {}
...
inline vec2i getXY(const SpriteSheet &sheet, unsigned index) {
  if (sheet.dirty) {
    ...
    if (sheet.mode == SM_Horizontal) {
      ...
      if (imgWidth != 0)
        sheet.perLine =
            (imgWidth - sheet.spacing) / (sheet.width + sheet.spacing);
    } else { // SM_Vertical:
      unsigned imgHeight = texture->size.height;
      if (imgHeight != 0)
        sheet.perLine =
            (imgHeight - sheet.spacing) / (sheet.height + sheet.spacing);
    }
  }
...
vec4f SpriteSheet::getSourceUV(unsigned index) const {
  if (material.textures[0]) {
    ...
    case SM_Horizontal: {
      ...
      float x = float((index % perRow) * (width + spacing) + spacing) /
                float(imgSize.width);
      float y = float((index / perRow) * (height + spacing) + spacing) /
                float(imgSize.height);

      return vec4f(x, y, x + float(width) / float(imgSize.width),
                   y + float(height) / float(imgSize.height));
    } break;
    case SM_Vertical: {
      unsigned perCol = (imgSize.height - spacing) / (height + spacing);

      float x = float((index / perCol) * (width + spacing) + spacing) /
                float(imgSize.width);
      float y = float((index % perCol) * (height + spacing) + spacing) /
                float(imgSize.height);
...
void SpriteSheet::render(RenderDriver &driver, unsigned index,
                         const vec3d *vertices, const Color *color) const {
  current = this;
  currentIndex = index;
...
void SpriteSheet::render(RenderDriver &driver, unsigned index, recti rectangle,
                         const Color *color, const recti *clip,
                         const Shader *shader, double rotation) const {
  current = this;
  currentIndex = index;
