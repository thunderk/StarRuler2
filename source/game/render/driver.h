...
public:
  ...
  virtual void getBillboardVecs(vec3d &upLeft, vec3d &upRight,
                                double rotation = 0) const = 0;
  virtual void getBillboardVecs(const vec3d &from, vec3d &upLeft,
                                vec3d &upRight, double rotation = 0) const = 0;
  ...
  virtual ShaderProgram *createShaderProgram(const char *vertex_shader,
                                             const char *fragment_shader) = 0;
  virtual Texture *createTexture(Image &image, bool mipmap = true,
                                 bool cachePixels = false) = 0;
  ...
  virtual void drawQuad(const vec2<float> *vertices,
                        const vec2<float> *textureCoords,
                        const Color *colors) = 0;

  virtual void drawQuad(const vec3d *vertices, const vec2<float> *textureCoords,
                        const Color *colors) = 0;

  virtual void drawRectangle(const recti &rectangle, const RenderState *mat,
                             Color color, const recti *clip = 0) = 0;

  virtual void drawRectangle(recti rectangle, const RenderState *mat = 0,
                             const recti *sourceRect = 0,
                             const Color *color = 0, const recti *clip = 0) = 0;

  virtual void drawRectangle(recti rectangle, const RenderState *mat,
                             const recti *sourceRect, const Color *color,
                             const recti *clip, double rotation) = 0;

  virtual void drawBillboard(vec3d center, double width, const RenderState &mat,
                             double rotation, Color *color = 0) = 0;

  virtual void drawBillboard(vec3d center, double width, const RenderState &mat,
                             const recti &source, Color *color = 0) = 0;
