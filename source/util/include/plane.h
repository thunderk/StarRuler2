...
template <class T> struct plane {
  ...
  plane(const vec3<T> &direction, T distance)
      : dir(direction), dist(distance) {}

  plane(const vec3<T> &point, const vec3<T> &direction)
      : dir(direction), dist((T)point.dot(direction)) {}

  // Creates a plane from a triangle specified in clockwise order (looking at
  // the plane)
