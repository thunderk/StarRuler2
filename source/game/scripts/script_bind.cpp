...
void setEngine(asIScriptEngine *_engine) { engine = _engine; }
...
_DOC bind(const char *declaration, asSFuncPtr func, void *obj) {
  check_doc(engine->RegisterGlobalFunction(declaration, func,
                                           asCALL_THISCALL_ASGLOBAL, obj));
}
...
_DOC ClassBind::addFactory(const char *declaration, asSFuncPtr func) {
  check_doc(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_FACTORY,
                                            declaration, func, asCALL_CDECL));
}

_DOC ClassBind::addGenericFactory(const char *declaration, asSFuncPtr func,
                                  void *userdata) {
  int fid = engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_FACTORY,
                                            declaration, func, asCALL_GENERIC);
#ifdef _DEBUG
  check(fid);
#endif
...
_DOC ClassBind::addConstructor(const char *declaration, asSFuncPtr func) {
  check_doc(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_CONSTRUCT,
                                            declaration, func,
                                            asCALL_CDECL_OBJFIRST));
}

_DOC ClassBind::addDestructor(const char *declaration, asSFuncPtr func) {
  check_doc(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_DESTRUCT,
                                            declaration, func,
                                            asCALL_CDECL_OBJFIRST));
}

_DOC ClassBind::addMethod(const char *declaration, asSFuncPtr func) {
  check_doc(engine->RegisterObjectMethod(name.c_str(), declaration, func,
                                         asCALL_THISCALL));
}

_DOC ClassBind::addBehaviour(asEBehaviours behav, const char *declaration,
                             asSFuncPtr func) {
  check_doc(engine->RegisterObjectBehaviour(name.c_str(), behav, declaration,
                                            func, asCALL_THISCALL));
}

_DOC ClassBind::addExternBehaviour(asEBehaviours behav, const char *declaration,
                                   asSFuncPtr func, bool ptrFirst) {
  check_doc(engine->RegisterObjectBehaviour(
      name.c_str(), behav, declaration, func,
      ptrFirst ? asCALL_CDECL_OBJFIRST : asCALL_CDECL_OBJLAST));
}

_DOC ClassBind::addGenericBehaviour(asEBehaviours behav,
                                    const char *declaration, asSFuncPtr func,
                                    void *userdata) {
  int fid = engine->RegisterObjectBehaviour(name.c_str(), behav, declaration,
                                            func, asCALL_GENERIC);
#ifdef _DEBUG
  check(fid);
#endif
...
_DOC ClassBind::addLooseBehaviour(asEBehaviours behav, const char *declaration,
                                  asSFuncPtr func) {
  check_doc(engine->RegisterObjectBehaviour(name.c_str(), behav, declaration,
                                            func, asCALL_CDECL));
}

void ClassBind::addGarbageCollection(asSFuncPtr setflag, asSFuncPtr getflag,
                                     asSFuncPtr getrefcount,
                                     asSFuncPtr enumrefs,
                                     asSFuncPtr releaserefs) {
  check(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_SETGCFLAG,
                                        "void f()", setflag, asCALL_THISCALL));
  check(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_GETGCFLAG,
                                        "bool f()", getflag, asCALL_THISCALL));
  check(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_GETREFCOUNT,
                                        "int f()", getrefcount,
                                        asCALL_THISCALL));
  check(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_ENUMREFS,
                                        "void f(int&in)", enumrefs,
                                        asCALL_THISCALL));
  check(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_RELEASEREFS,
                                        "void f(int&in)", releaserefs,
                                        asCALL_THISCALL));
}

_DOC ClassBind::addExternMethod(const char *declaration, asSFuncPtr func,
                                bool ptrFirst) {
  check_doc(engine->RegisterObjectMethod(name.c_str(), declaration, func,
                                         ptrFirst ? asCALL_CDECL_OBJFIRST
                                                  : asCALL_CDECL_OBJLAST));
}

_DOC ClassBind::addGenericMethod(const char *declaration, asSFuncPtr func,
                                 void *userdata) {
  int fid = engine->RegisterObjectMethod(name.c_str(), declaration, func,
                                         asCALL_GENERIC);
#ifdef _DEBUG
  check(fid);
#endif
...
void ClassBind::setReferenceFuncs(asSFuncPtr grab, asSFuncPtr drop) {
  check(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_ADDREF,
                                        "void f()", grab, asCALL_THISCALL));
  check(engine->RegisterObjectBehaviour(name.c_str(), asBEHAVE_RELEASE,
                                        "void f()", drop, asCALL_THISCALL));
}
...
_DOC InterfaceBind::addMethod(const char *declaration,
...
EnumBind::setter::setter(const char *E, const char *I)
    : enumName(E), itemName(I) {}
...
static std::map<std::pair<asIScriptEngine *, std::string>, std::string>
    class_documentation;
static std::map<std::pair<asIScriptEngine *, std::string>, GlobalDoc *>
    global_documentation;
static std::map<std::tuple<asIScriptEngine *, std::string, std::string>,
                MemberDoc *>
...
MemberDoc *getMemberDocumentation(asIScriptEngine *eng, std::string clsname,
                                  std::string varname) {
  std::tuple<asIScriptEngine *, std::string, std::string> key(eng, clsname,
                                                              varname);
...
void GlobalDoc::operator()(const char *_doc) {
  doc = _doc;
  threads::Lock lock(docMtx);
  global_documentation[std::pair<asIScriptEngine *, std::string>(
      engine, varname)] = this;
}

MemberDoc::MemberDoc(std::string clsname, std::string decl) : clsname(clsname) {
  std::vector<std::string> parts;
  split(decl, parts, ' ', true);
  varname = parts[parts.size() - 1];
}
...
void MemberDoc::operator()(const char *_doc) {
  doc = _doc;
  threads::Lock lock(docMtx);
  member_documentation[std::tuple<asIScriptEngine *, std::string, std::string>(
      engine, clsname, varname)] = this;
}

void ClassBind::document(std::string str) {
  threads::Lock lock(docMtx);
  class_documentation[std::pair<asIScriptEngine *, std::string>(engine, name)] =
      str;
}
