...
int threadCount = 0;
bool stopProcessing = false, pauseProcessing = false,
...
bool isRunning() { return !threadsActive.check(0); }
...
void resumeMessageHandling() { pauseMessageHandlers = false; }

#ifdef PROFILE_PROCESSING
threads::atomic_int printProcProfile;

void printProcessingProfile() { printProcProfile = threadCount + 1; }
    ...
    if (printProcProfile > 0) {
      ...
      for (unsigned i = 0, cnt = threadProc->objTime.size(); i < cnt; ++i) {
        ...
        print("    %s: %d objects in %.4gms (~%.3gms per)",
              getScriptObjectType(i)->name.c_str(), threadProc->objCount[i],
      ...
      if (threadProc->globalCount != 0) {
        ...
        if (threadProc->switchedCount != 0)
          print("        Switched objects: %d objects in %.4gms (~%.3gms per)",
                ...
                (threadProc->switchedTime / threadProc->switchedCount) *
                    1000.0);
        if (threadProc->targetUpdates != 0)
          print("        Target Update: %d updates in %.4gms (~%.3gms per)",
                ...
                threadProc->targUpdateTime * 1000.0 /
                    threadProc->targetUpdates);
      }
