...
struct SArrayCache {
  ...
  int cmpFuncReturnCode; // To allow better error message in case of multiple
                         // matches
...
static void CleanupObjectTypeArrayCache(asITypeInfo *type) {
  SArrayCache *cache =
      reinterpret_cast<SArrayCache *>(type->GetUserData(ARRAY_CACHE));
  if (cache)
    delete cache;
}
...
static CScriptArray *ScriptArrayFactoryDefVal(asITypeInfo *ot, asUINT length,
                                              void *defVal) {
...
// This optional callback is called when the template type is first used by the
// compiler. It allows the application to validate if the template can be
// instanciated for the requested subtype at compile time, instead of at
// runtime. The output argument dontGarbageCollect allow the callback to tell
// the engine if the template instance type shouldn't be garbage collected, i.e.
// no asOBJ_GC flag.
static bool ScriptArrayTemplateCallback(asITypeInfo *ot,
                                        bool &dontGarbageCollect) {
  // Make sure the subtype can be instanciated with a default
  // factory/constructor, otherwise we won't be able to instanciate the
  // elements.
  ...
  if ((typeId & asTYPEID_MASK_OBJECT)) {
    ...
    // If the object type is not garbage collected then the array also doesn't
    // need to be
    if (!(flags & asOBJ_GC))
...
static void RegisterScriptArray_Native(asIScriptEngine *engine) {
  ...
  engine->SetTypeInfoUserDataCleanupCallback(CleanupObjectTypeArrayCache,
                                             ARRAY_CACHE);

  // Register the array type as a template
  r = engine->RegisterObjectType("array<class T>", 0,
                                 asOBJ_REF | asOBJ_GC | asOBJ_TEMPLATE);
  assert(r >= 0);

  // Register a callback for validating the subtype before it is used
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_TEMPLATE_CALLBACK, "bool f(int&in, bool&out)",
      asFUNCTION(ScriptArrayTemplateCallback), asCALL_CDECL);
  assert(r >= 0);

  // Templates receive the object type as the first parameter. To the script
  // writer this is hidden
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in)",
      asFUNCTIONPR(ScriptArrayFactory, (asITypeInfo *), CScriptArray *),
      asCALL_CDECL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint)",
      asFUNCTIONPR(ScriptArrayFactory2, (asITypeInfo *, asUINT),
                   CScriptArray *),
      asCALL_CDECL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint, const T &in)",
      asFUNCTIONPR(ScriptArrayFactoryDefVal, (asITypeInfo *, asUINT, void *),
                   CScriptArray *),
      asCALL_CDECL);
  assert(r >= 0);
  ...
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_LIST_FACTORY,
      "array<T>@ f(int&in type, int&in list) {repeat T}",
      asFUNCTIONPR(ScriptArrayListFactory, (asITypeInfo *, void *),
                   CScriptArray *),
      asCALL_CDECL);
  assert(r >= 0);
  ...
  r = engine->RegisterObjectMethod(
      ...
      asMETHODPR(CScriptArray, At, (asUINT) const, const void *),
      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("array<T>", "T &get_last()",
                                   asMETHODPR(CScriptArray, Last, (), void *),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "array<T>", "const T &get_last() const",
      asMETHODPR(CScriptArray, Last, () const, const void *), asCALL_THISCALL);
  assert(r >= 0);
  ...
  // Other methods
  r = engine->RegisterObjectMethod(
      "array<T>", "void insertAt(uint, const T&in)",
      asMETHOD(CScriptArray, InsertAt), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void removeAt(uint)",
                                   asMETHOD(CScriptArray, RemoveAt),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void insertLast(const T&in)",
                                   asMETHOD(CScriptArray, InsertLast),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void removeLast()",
                                   asMETHOD(CScriptArray, RemoveLast),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void remove(const T&in)",
                                   asMETHOD(CScriptArray, Remove),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void removeAll(const T&in)",
                                   asMETHOD(CScriptArray, RemoveAll),
                                   asCALL_THISCALL);
  assert(r >= 0);

  // TODO: Should length() and resize() be deprecated as the property accessors
  // do the same thing?
  r = engine->RegisterObjectMethod("array<T>", "uint length() const",
                                   asMETHOD(CScriptArray, GetSize),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void reserve(uint)",
                                   asMETHOD(CScriptArray, Reserve),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
  ...
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void sortAsc()",
                                   asMETHODPR(CScriptArray, SortAsc, (), void),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "array<T>", "void sortAsc(uint, uint)",
      asMETHODPR(CScriptArray, SortAsc, (asUINT, asUINT), void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void sortDesc()",
                                   asMETHODPR(CScriptArray, SortDesc, (), void),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "array<T>", "void sortDesc(uint, uint)",
      asMETHODPR(CScriptArray, SortDesc, (asUINT, asUINT), void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void reverse()",
                                   asMETHOD(CScriptArray, Reverse),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "array<T>", "int find(const T&in) const",
      asMETHODPR(CScriptArray, Find, (void *) const, int), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
  ...
  r = engine->RegisterObjectMethod(
      ...
      asMETHOD(CScriptArray, operator==), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "bool isEmpty() const",
                                   asMETHOD(CScriptArray, IsEmpty),
                                   asCALL_THISCALL);
  assert(r >= 0);

  // Register virtual properties
  r = engine->RegisterObjectMethod("array<T>", "uint get_length() const",
                                   asMETHOD(CScriptArray, GetSize),
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "array<T>", "void set_length(uint)",
      asMETHODPR(CScriptArray, Resize, (asUINT), void), asCALL_THISCALL);
  assert(r >= 0);

  // Register GC behaviours in case the array needs to be garbage collected
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_GETREFCOUNT, "int f()",
      asMETHOD(CScriptArray, GetRefCount), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_SETGCFLAG, "void f()",
      asMETHOD(CScriptArray, SetFlag), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_GETGCFLAG, "bool f()",
      asMETHOD(CScriptArray, GetFlag), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_ENUMREFS, "void f(int&in)",
      asMETHOD(CScriptArray, EnumReferences), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_RELEASEREFS, "void f(int&in)",
      asMETHOD(CScriptArray, ReleaseAllHandles), asCALL_THISCALL);
  assert(r >= 0);

#if AS_USE_STLNAMES == 1
  // Same as length
  r = engine->RegisterObjectMethod("array<T>", "uint size() const",
                                   asMETHOD(CScriptArray, GetSize),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as isEmpty
  r = engine->RegisterObjectMethod("array<T>", "bool empty() const",
                                   asMETHOD(CScriptArray, IsEmpty),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as insertLast
  r = engine->RegisterObjectMethod("array<T>", "void push_back(const T&in)",
                                   asMETHOD(CScriptArray, InsertLast),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as removeLast
  r = engine->RegisterObjectMethod("array<T>", "void pop_back()",
                                   asMETHOD(CScriptArray, RemoveLast),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as insertAt
  r = engine->RegisterObjectMethod("array<T>", "void insert(uint, const T&in)",
                                   asMETHOD(CScriptArray, InsertAt),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as removeAt
  r = engine->RegisterObjectMethod("array<T>", "void erase(uint)",
                                   asMETHOD(CScriptArray, RemoveAt),
                                   asCALL_THISCALL);
  assert(r >= 0);
#endif
...
CScriptArray::CScriptArray(asITypeInfo *ot, void *buf) {
  ...
  if ((ot->GetSubTypeId() & asTYPEID_MASK_OBJECT) == 0) {
    ...
    // TODO: list: runtime optimize: Instead of increasing the references, clear
    // the
    //             handles in the incoming buffer so AngelScript won't release
    //             them
    ...
    //       constructing with the default constructor and then assigning the
    //       value
    CreateBuffer(&buffer, length);
...
void CScriptArray::SetValue(asUINT index, void *value) {
  ...
  if ((subTypeId & ~asTYPEID_MASK_SEQNBR) && !(subTypeId & asTYPEID_OBJHANDLE))
    ...
    void *tmp = *(void **)ptr;
    *(void **)ptr = *(void **)value;
    objType->GetEngine()->AddRefScriptObject(*(void **)value,
                                             objType->GetSubType());
    if (tmp)
      objType->GetEngine()->ReleaseScriptObject(tmp, objType->GetSubType());
  } else if (subTypeId == asTYPEID_BOOL || subTypeId == asTYPEID_INT8 ||
    ...
    *(short *)ptr = *(short *)value;
           ...
           subTypeId >
               asTYPEID_DOUBLE) // enums have a type id larger than doubles
    *(int *)ptr = *(int *)value;
  else if (subTypeId == asTYPEID_INT64 || subTypeId == asTYPEID_UINT64 ||
...
asUINT CScriptArray::GetSize() const { return numElements; }

bool CScriptArray::IsEmpty() const { return numElements == 0; }
...
void CScriptArray::Reserve(asUINT maxElements) {
  ...
  newBuffer = (SArrayBuffer *)new asBYTE[sizeof(SArrayBuffer) - 1 +
                                         elementSize * maxElements];
#else
  newBuffer = (SArrayBuffer *)new (nothrow)
      asBYTE[sizeof(SArrayBuffer) - 1 + elementSize * maxElements];
#endif
  if (newBuffer) {
...
void CScriptArray::Resize(int delta, asUINT at) {
  ...
  if (buffer->maxElements < buffer->numElements + delta) {
...
#if defined(__S3E__) // Marmalade doesn't understand (nothrow)
    newBuffer =
        (SArrayBuffer *)new asBYTE[sizeof(SArrayBuffer) - 1 +
                                   elementSize * (buffer->numElements + delta)];
#else
    newBuffer = (SArrayBuffer *)new (nothrow)
        asBYTE[sizeof(SArrayBuffer) - 1 +
               elementSize * (buffer->numElements + delta)];
#endif
    if (newBuffer) {
    ...
    // TODO: memcpy assumes the objects in the array doesn't hold pointers to
    // themselves
    //       This should really be using the objects copy constructor to copy
    //       each object to the new location. It would most likely be a hit on
    //       the performance though.
    memcpy(newBuffer->data, buffer->data, at * elementSize);
    if (at < buffer->numElements)
      memcpy(newBuffer->data + (at + delta) * elementSize,
    ...
    // TODO: memmove assumes the objects in the array doesn't hold pointers to
    // themselves
    //       This should really be using the objects copy constructor to copy
    //       each object to the new location. It would most likely be a hit on
    //       the performance though.
    memmove(buffer->data + at * elementSize,
    ...
    //       each object to the new location. It would most likely be a hit on
    //       the performance though.
    memmove(buffer->data + (at + delta) * elementSize,
            buffer->data + at * elementSize,
            (buffer->numElements - at) * elementSize);
...
asITypeInfo *CScriptArray::GetArrayObjectType() const { return objType; }
...
void CScriptArray::RemoveLast() { RemoveAt(buffer->numElements - 1); }

// Return a pointer to the array element. Returns 0 if the index is out of
// bounds
...
void CScriptArray::CreateBuffer(SArrayBuffer **buf, asUINT numElements) {
  if (subTypeId & asTYPEID_MASK_OBJECT) {
#if defined(__S3E__) // Marmalade doesn't understand (nothrow)
    *buf = (SArrayBuffer *)new asBYTE[sizeof(SArrayBuffer) - 1 +
                                      sizeof(void *) * numElements];
#else
    *buf = (SArrayBuffer *)new (nothrow)
        asBYTE[sizeof(SArrayBuffer) - 1 + sizeof(void *) * numElements];
#endif
  } else {
#if defined(__S3E__)
    *buf = (SArrayBuffer *)new asBYTE[sizeof(SArrayBuffer) - 1 +
...
#else
    *buf = (SArrayBuffer *)new (nothrow)
        asBYTE[sizeof(SArrayBuffer) - 1 + elementSize * numElements];
#endif
  }
...
bool CScriptArray::operator==(const CScriptArray &other) const {
  ...
  SArrayCache *cache =
      reinterpret_cast<SArrayCache *>(objType->GetUserData(ARRAY_CACHE));
  if ((subTypeId & ~asTYPEID_MASK_SEQNBR) && (cache && cache->eqFunc)) {
    ...
    if (cmpContext == 0) {
      // TODO: Ideally this context would be retrieved from a pool, so we don't
      // have to
      //       create a new one everytime. We could keep a context with the
      //       array object but that would consume a lot of resources as each
      //       context is quite heavy.
...
int CScriptArray::Find(void *value) const { return Find(0, value); }
...
void CScriptArray::Copy(void *dst, void *src) { memcpy(dst, src, elementSize); }
...
void CScriptArray::Sort(asUINT index, asUINT count, bool asc) {
      ...
      if (ctx) {
        ...
        if (cache && cache->cmpFuncReturnCode == asMULTIPLE_FUNCTIONS)
          ...
          sprintf_s(tmp, 512, "Type '%s' has multiple matching opCmp methods",
                    subType->GetName());
#else
          sprintf(tmp, "Type '%s' has multiple matching opCmp methods",
                  subType->GetName());
#endif
        else
#if defined(_MSC_VER) && _MSC_VER >= 1500 && !defined(__S3E__)
          sprintf_s(tmp, 512, "Type '%s' does not have a matching opCmp method",
                    subType->GetName());
#else
          sprintf(tmp, "Type '%s' does not have a matching opCmp method",
                  subType->GetName());
#endif
  ...
  for (int i = start + 1; i < end; i++) {
    ...
    while (j >= start &&
           Less(GetDataPointer(tmp), At(j), asc, cmpContext, cache)) {
...
void CScriptArray::CopyBuffer(SArrayBuffer *dst, SArrayBuffer *src) {
  ...
  if (subTypeId & asTYPEID_OBJHANDLE) {
    ...
    if (dst->numElements > 0 && src->numElements > 0) {
      int count = dst->numElements > src->numElements ? src->numElements
                                                      : dst->numElements;
      ...
      for (; d < max; d++, s++) {
        ...
        *d = *s;
        ...
        // Release the old ref after incrementing the new to avoid problem
        // incase it is the same ref
        if (tmp)
...
void CScriptArray::Precache() {
  ...
  // Check if it is an array of objects. Only for these do we need to cache
  // anything Type ids for primitives and enums only has the sequence number
  // part
  if (!(subTypeId & ~asTYPEID_MASK_SEQNBR))
    return;
    ...
    for (asUINT i = 0; i < subType->GetMethodCount(); i++) {
      ...
      if (func->GetParamCount() == 1 && (!mustBeConst || func->IsReadOnly())) {
        ...
        if (returnTypeId == asTYPEID_INT32 &&
            strcmp(func->GetName(), "opCmp") == 0)
          isCmp = true;
        if (returnTypeId == asTYPEID_BOOL &&
            strcmp(func->GetName(), "opEquals") == 0)
          isEq = true;

        if (!isCmp && !isEq)
          continue;

        // The parameter must either be a reference to the subtype or a handle
        // to the subtype
        int paramTypeId = 0;
  ...
  // Set the user data only at the end so others that retrieve it will know it
  // is complete
...
// GC behaviour
int CScriptArray::GetRefCount() { return refCount.get(); }
...
static void ScriptArrayFactory2_Generic(asIScriptGeneric *gen) {
  ...
  *(CScriptArray **)gen->GetAddressOfReturnLocation() =
      ScriptArrayFactory2(ot, length);
}
...
static void ScriptArrayFactoryDefVal_Generic(asIScriptGeneric *gen) {
  ...
  *(CScriptArray **)gen->GetAddressOfReturnLocation() =
      ScriptArrayFactoryDefVal(ot, length, defVal);
}
...
static void ScriptArrayTemplateCallback_Generic(asIScriptGeneric *gen) {
  ...
  *(bool *)gen->GetAddressOfReturnLocation() =
      ScriptArrayTemplateCallback(ot, *dontGarbageCollect);
}
...
static void RegisterScriptArray_Generic(asIScriptEngine *engine) {
  ...
  r = engine->RegisterObjectBehaviour(
      "array<T>", asBEHAVE_LIST_FACTORY,
      "array<T>@ f(int&in, int&in) {repeat T}",
      asFUNCTION(ScriptArrayListFactory_Generic), asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_ADDREF, "void f()",
                                      asFUNCTION(ScriptArrayAddRef_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_RELEASE, "void f()",
                                      asFUNCTION(ScriptArrayRelease_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "T &opIndex(uint)",
                                   asFUNCTION(ScriptArrayAt_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "const T &opIndex(uint) const",
                                   asFUNCTION(ScriptArrayAt_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
  ...
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void removeAt(uint)",
                                   asFUNCTION(ScriptArrayRemoveAt_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void insertLast(const T&in)",
                                   asFUNCTION(ScriptArrayInsertLast_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void removeLast()",
                                   asFUNCTION(ScriptArrayRemoveLast_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "uint length() const",
                                   asFUNCTION(ScriptArrayLength_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void reserve(uint)",
                                   asFUNCTION(ScriptArrayReserve_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void resize(uint)",
                                   asFUNCTION(ScriptArrayResize_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void sortAsc()",
                                   asFUNCTION(ScriptArraySortAsc_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void sortAsc(uint, uint)",
                                   asFUNCTION(ScriptArraySortAsc2_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void sortDesc()",
                                   asFUNCTION(ScriptArraySortDesc_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void sortDesc(uint, uint)",
                                   asFUNCTION(ScriptArraySortDesc2_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void reverse()",
                                   asFUNCTION(ScriptArrayReverse_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "int find(const T&in) const",
                                   asFUNCTION(ScriptArrayFind_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "array<T>", "int find(uint, const T&in) const",
  ...
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "bool isEmpty() const",
                                   asFUNCTION(ScriptArrayIsEmpty_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "uint get_length() const",
                                   asFUNCTION(ScriptArrayLength_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("array<T>", "void set_length(uint)",
                                   asFUNCTION(ScriptArrayResize_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
