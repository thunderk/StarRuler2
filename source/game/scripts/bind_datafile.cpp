...
bool isAccessible(const std::string &filename) {
  ...
  for (auto path = validPaths.begin(), end = validPaths.end(); path != end;
       ++path)
...
std::string getModProfile() { return devices.mods.getProfile(); }
...
public:
  ReadFile(const std::string &filename, bool AllowLines)
      : DataReader(filename, AllowLines), references(1) {
...
static bool readNext(ReadFile &rf) { return rf++; }
...
public:
  ...
  WriteFile(const char *filename)
      : file(filename, std::ios_base::out | std::ios_base::binary),
        curIndent(0), allowMultiline(true), endl("\r\n") {
  ...
  void indent(int amount = 1) { curIndent += amount; }

  void deindent(int amount = 1) { curIndent -= amount; }

  void writeLine(const std::string &line) { file << line << endl; }
...
public:
  ...
  void navigate(const std::string &dirname, const std::string &filter,
                bool recurse, bool resolve) {
    files.clear();
  ...
  void listDir(const std::string &dirname, const std::string &filter,
               bool recurse = true) {
  ...
  unsigned size() { return (unsigned)files.size(); }
...
FileList *makeFileList(const std::string &dirname, const std::string &filter,
                       bool recurse, bool resolve) {
...
FileList *makeFileList_e() { return new FileList(); }
...
void RegisterDatafiles() {
  ...
  bind("string get_modProfile(const string&in folder)",
       asFUNCTION(getModProfileChild))
      doc("Get the path to a child directory in the mod's profile.",
          "Child directory", "Path to the folder.");
  bind("string get_baseProfile(const string&in folder)",
       asFUNCTION(getBaseProfileChild))
  ...
  bind("void deleteFile(const string&in filename)", asFUNCTION(deleteFile)) doc(
  ...
  bind("bool isDirectory(const string&in path)", asFUNCTION(isDir))
      doc("Check whether a particular path is a directory.",
  ...
  bind("bool fileExists(const string&in path)", asFUNCTION(fileEx))
      doc("Check whether a particular file exists.",
          "Path of the file to check.", "Whether a file exists at that path.");
  bind("string path_join(const string&in first, const string&in second)",
       asFUNCTION(path_join)) doc("Concatenate two file paths.", "First path.",
                                  "Second path.", "Concatenated path.");
  bind("string path_up(const string&in path)", asFUNCTION(path_up))
      doc("Go up one directory from a path.", "Path to go up from.",
          "Parent path.");
  bind("bool path_inside(const string&in dirname, const string&in fname)",
       asFUNCTION(path_inside))
      doc("Check whether a filename is inside a directory.", "Folder name.",
          "Filename"., "Whether the file is inside the folder.");
  bind("string getBasename(const string&in path, bool includeExtension = true)",
       asFUNCTION(getBasename))
      doc("Get the basename of a path.", "Path to get basename from.",
          "Whether to include the extension in the basename.",
          "Path basename.");
  bind("int64 getSystemTime()", asFUNCTION(systemTime));
  bind("string strftime(const string& format, int64 time)",
       asFUNCTION(strftime_wrap));

  ClassBind read("ReadFile", asOBJ_REF);
  read.addFactory(
      "ReadFile@ f(const string &in filename, bool allowLines = false)",
      asFUNCTION(makeReadFile)) doc("Opens a text datafile for reading.",
                                    "Path to the file to open. Must be within "
                                    "the profile or game folders.",
                                    "", "");
  read.setReferenceFuncs(asMETHOD(ReadFile, grab), asMETHOD(ReadFile, drop));

  read.addMember("int indent", offsetof(ReadFile, indent))
      doc("Number of indents for the current line. Each space or tab counts as "
          "one indent.");
  ...
  read.addMethod("string position()", asMETHOD(ReadFile, position))
      doc("Returns a formatted representation of the position in the file.",
          "A string like 'data/states.txt | Line 80'");
  read.addExternMethod("bool opPostInc()", asFUNCTION(readNext))
      doc("Advances to the next non-empty line.",
  ...
  write.addMember("bool allowMultiline", offsetof(WriteFile, allowMultiline))
      doc("Whether multi-line values should be written in multiline format or "
  ...
  write.addMethod("void deindent(int amount = 1)",
                  asMETHOD(WriteFile, deindent));
  write.addMethod("void writeLine(const string &in)",
                  asMETHOD(WriteFile, writeLine));
  write.addMethod("void writeKeyValue(const string &in, const string &in)",
                  asMETHOD(WriteFile, writeKeyValue));
  write.addMethod("void writeEmptyLines(uint8 count = 1)",
                  asMETHOD(WriteFile, writeEmptyLines));

  ClassBind flist("FileList", asOBJ_REF);
  flist.addFactory("FileList@ f(const string &in dirname, const string &in "
                   "filter, bool recurse = false, bool resolve = true)",
                   asFUNCTION(makeFileList));
  ...
  flist.addMethod("void navigate(const string&in dirname, const string&in "
                  "filter, bool recurse = false, bool resolve = true)",
                  asMETHOD(FileList, navigate));
  flist.addMethod("uint get_length()", asMETHOD(FileList, size));
  flist.addMethod("const string& get_relativePath(uint num)",
                  asMETHOD(FileList, relative));
  flist.addMethod("string get_basename(uint num)",
                  asMETHOD(FileList, basename));
  flist.addMethod("string get_extension(uint num)",
                  asMETHOD(FileList, extension));
  flist.addMethod("const string& get_path(uint num)",
                  asMETHOD(FileList, absolute));
  flist.addMethod("bool get_isDirectory(uint num)",
                  asMETHOD(FileList, isDirectory));
}
