...
void loadXNormals(std::ifstream &file, std::vector<unsigned> &normIndices,
                  std::vector<vec3f> &normals, Matrix &matrix) {
  std::string line;
  ...
  for (unsigned i = 0; i < faces; ++i) {
    ...
    auto args = sscanf(line.c_str(), " %u;%u,%u,%u,%u", &type, &indices[0],
...
void loadXMesh(std::ifstream &file, Mesh &mesh, Matrix &matrix) {
  ...
  for (unsigned int i = 0;
       i + 2 < posIndices.size() && i + 2 < normIndices.size(); i += 3) {
    Vertex verts[3];
