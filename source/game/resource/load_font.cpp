...
void Library::loadFonts(const std::string &filename) {
  ...
  auto makeFont = [&]() {
    ...
    if (font_locale.empty() || font_locale == game_locale) {
      ...
      if (!p_ext || strcmp_nocase(p_ext, ".fnt") == 0)
        ...
        font = render::loadFontFT2(*devices.render, font_file.c_str(), pages,
                                   size);
  ...
  datahandler("Locale", [&](std::string &value) { font_locale = value; });

  datahandler("Replace",
              [&](std::string &value) { replaces.push_back(value); });

  datahandler("Size", [&](std::string &value) { size = toNumber<int>(value); });
  ...
  datahandler.defaultHandler([&](std::string &key, std::string &value) {
    if (key.compare(0, 4, "Page") == 0) {
      ...
      if (numbers.size() == 2)
        pages.push_back(
            std::pair<int, int>(toNumber<int>(numbers[0], 0, std::hex),
