...
void parseElementIdentifier(const std::string &str, int &style, unsigned &flags,
                            bool addIfMissing) {
...
#define HANDLE_RELCOORD(handler, key, obj, member, pos)                        \
...
#define HANDLE_GCOLOR(handler, key, obj, member, pos)                          \
  handler(key, [&](std::string &value) {                                       \
    if (!obj || obj->member.empty())                                           \
      return;                                                                  \
...
void Library::loadSkin(const std::string &filename, gui::skin::Skin *skin) {
  ...
  skinhandler("Margin", [&](std::string &value) {
    ...
    if (numbers.size() == 4) {
      ele->margin = recti(toNumber<int>(numbers[0]), toNumber<int>(numbers[1]),
                          toNumber<int>(numbers[2]), toNumber<int>(numbers[3]));
