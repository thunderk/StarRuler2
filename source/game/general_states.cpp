...
template <class T> void copySimple(void *dest, void *src) {
  if (src)
    *(T *)dest = *(T *)src;
  else
    new (dest) T();
...
template <class T> void *parseNumber(const std::string &str) {
  return (void *)new T(toNumber<T>(str));
}

template <class T> void defaultConstruct(void *dest, void *src) {
  new (dest) T();
}

template <class T> void destruct(void *mem) {
  if (mem)
    ((T *)mem)->~T();
}

template <class T> void defaultWrite(net::Message &msg, void *mem) {
  msg << *(T *)mem;
}

template <class T> void defaultRead(net::Message &msg, void *mem) {
  msg >> *(T *)mem;
}
...
void resetStateValueTypes() {
  ...
  stateValueTypes["int"].setup(
      sizeof(int), "int", copySimple<int>, parseNumber<int>, nullptr,
      defaultWrite<int>, defaultRead<int>, [](asIScriptGeneric *f, void *mem) {
        if (mem)
  ...
  stateValueTypes["uint"].setup(sizeof(unsigned), "uint", copySimple<unsigned>,
                                parseNumber<unsigned>, nullptr,
                                defaultWrite<unsigned>, defaultRead<unsigned>,
  ...
  stateValueTypes["double"].setup(sizeof(double), "double", copySimple<double>,
                                  parseNumber<double>, nullptr,
                                  defaultWrite<double>, defaultRead<double>,
                                  [](asIScriptGeneric *f, void *mem) {
                                    if (mem)
  ...
  stateValueTypes["float"].setup(sizeof(double), "float", copySimple<float>,
                                 parseNumber<float>, nullptr,
                                 defaultWrite<float>, defaultRead<float>,
                                 [](asIScriptGeneric *f, void *mem) {
                                   if (mem)
  ...
  stateValueTypes["bool"].setup(
      sizeof(bool), "bool", copySimple<bool>,
      [](const std::string &str) -> void * { return new bool(toBool(str)); },
  ...
  stateValueTypes["locked_int"].setup(
      sizeof(LocklessInt), "locked_int",
      [](void *dest, void *src) {
        if (src)
      ...
      [](void *memory) { ((LocklessInt *)memory)->~LocklessInt(); },
      [](net::Message &msg, void *mem) { msg << ((LocklessInt *)mem)->value; },
      [](net::Message &msg, void *mem) { msg >> ((LocklessInt *)mem)->value; },
      [](asIScriptGeneric *f, void *mem) { f->SetReturnObject(mem); });

  stateValueTypes["locked_double"].setup(
      sizeof(LocklessDouble), "locked_double",
      [](void *dest, void *src) {
        if (src)
      ...
      [](void *memory) { ((LocklessDouble *)memory)->~LocklessDouble(); },
  ...
  stateValueTypes["vec3d"].setup(sizeof(vec3d), "vec3d", copySimple<vec3d>,
                                 nullptr);

  stateValueTypes["quaterniond"].setup(sizeof(quaterniond), "quaterniond",
                                       copySimple<quaterniond>, nullptr);

  stateValueTypes["string"].setup(
      ...
      [](const std::string &str) -> void * { return new std::string(str); },
      [](void *mem) { ((std::string *)mem)->~basic_string(); });

  stateValueTypes["Empire"].setup(
      sizeof(Empire *), "Empire@", copySimple<Empire *>, nullptr, nullptr,
      [](net::Message &msg, void *mem) {
  ...
  stateValueTypes["Object"].setup(
      sizeof(LockedHandle<Object>), "Object@",
      [](void *dest, void *src) {
        if (src)
      ...
      nullptr, // Object@ cannot have a default value
      [](void *mem) { ((LockedHandle<Object> *)mem)->~LockedHandle(); },
      [](net::Message &msg, void *mem) {
      ...
      [](void *mem) { ((LockedHandle<Object> *)mem)->set(nullptr); },
      [](asIScriptGeneric *f, void *mem) {
  ...
  stateValueTypes["Blueprint"].setup(
      sizeof(Blueprint), "Blueprint", defaultConstruct<Blueprint>, nullptr,
      destruct<Blueprint>, nullptr, nullptr,
      [](asIScriptGeneric *f, void *mem) { f->SetReturnAddress(mem); },
      [](void *mem) {
...
void StateValueDefinition::setup(
    unsigned Size, std::string Type, decltype(init) Init,
    decltype(parser) Parse, decltype(clear) Clear, decltype(writeSync) Write,
    decltype(readSync) Read, decltype(returnFunc) Return,
...
bool StateValueDefinition::returnable() const { return returnFunc != 0; }
...
void StateValueDefinition::setReturn(asIScriptGeneric *gen,
                                     void *memory) const {
  if (returnFunc)
    returnFunc(gen, memory);
}

bool StateValueDefinition::isParam() const { return (bool)paramSetFunc; }

void StateValueDefinition::setFromParam(asIScriptGeneric *gen,
                                        void *memory) const {
  if (paramSetFunc)
...
void loadStateDefinitions(const std::string &filename,
  ...
  auto finishDefinition = [&def, &used_names, &flags]() {
    ...
    def->asVar.setup(
        def->totalDataSize, def->name.c_str(),
        ...
        nullptr, [type](void *mem) { type->unprepare(mem); });
  ...
  while (true) {
    ...
    if (!inType) {
      ...
      int args = sscanf(parse.c_str(), " %80s %c %80s %c", name, &bracket,
                        value, &bracket2);
      if ((args != 2 && args != 4) || (args == 2 && bracket != '{') ||
...
void finalizeStateDefinitions() {
  for (auto i = stateDefinitions.begin(), end = stateDefinitions.end();
       i != end; ++i) {
    auto &def = **i;
    ...
    for (auto t = def.types.begin(), tend = def.types.end(); t != tend;) {
      ...
      const StateValueDefinition *valueType =
          getStateValueType(member.typeName);
      if (valueType == 0 || (member.synced && !valueType->syncable())) {
        if (valueType)
          error("%s error: Type '%s' for member '%s' cannot be synced",
                def.name.c_str(), member.typeName.c_str(), member.name.c_str());
        else
          error("%s error: Unknown type '%s' for member '%s'", def.name.c_str(),
    ...
    for (auto m = def.methods.begin(), mend = def.methods.end(); m != mend;
         ++m, ++methodIndex) {
      auto &method = *m;
...
StateDefinition::StateDefinition(const StateDefinition &other)
    : types(other.types), totalDataSize(other.totalDataSize), base(&other),
      methods(other.methods) {
  // TODO: default args are not deleted, so we'll just copy the pointers blindly
  // for now
}
...
unsigned StateList::count() const { return (unsigned)def->types.size(); }
