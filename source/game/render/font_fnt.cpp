...
namespace resource {
extern render::Texture *queueImage(const std::string &abs_file, int priority,
...
public:
  ...
  unsigned getBaseline() const { return glyphHeight; }
  ...
  void draw(render::RenderDriver *driver, const char *text, int X, int Y,
    ...
    while (fontChar c = it++) {
      if (c == L'\n') {
        ...
        if (lastC) {
          auto k = std::lower_bound(kerning.begin(), kerning.end(),
                                    KernEntry(lastC, c));
        ...
        if (glyph.draw) {
          rectf pos = rectf::area(
              vec2f((float)(x + glyph.wOff), (float)(y + glyph.hOff)),
  ...
  vec2i drawChar(render::RenderDriver *driver, int c, int lastC, int X, int Y,
    ...
    if (lastC) {
      auto k =
          std::lower_bound(kerning.begin(), kerning.end(), KernEntry(lastC, c));
      if (k != kerning.end() && k->left == lastC && k->right == c)
        x += k->offset;
    }
    ...
    if (glyph.draw) {
      rectf pos =
          rectf::area(vec2f((float)(x + glyph.wOff), (float)(y + glyph.hOff)),
                      vec2f(glyph.w, glyph.h));
      rectf source = rectf::area(
          vec2f((float)glyph.x * xFactor, (float)glyph.y * yFactor),
          vec2f((float)glyph.w * xFactor, (float)glyph.h * yFactor));
  ...
  FontFNT()
      : lowChars(new Glyph[256]), glyphHeight(0), textureHeight(1),
        textureWidth(1) {
...
Font *loadFontFNT(render::RenderDriver *driver, const char *filename) {
  ...
  if (fscanf(file,
             " common lineHeight=%i base=%*i scaleW=%i scaleH=%i pages=%i "
  ...
  if (fscanf(file, "kernings count=%i ", &kerns) == 1) {
    while (kerns--) {
      ...
      if (fscanf(file, "kerning first=%i second=%i amount=%i ", &left, &right,
                 &offset) == 3) {
