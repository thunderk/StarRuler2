...
unsigned getNumberOfProcessors() { return sysconf(_SC_NPROCESSORS_ONLN); }
...
int atomic_int::operator++() { return __sync_add_and_fetch(&value, 1); }

int atomic_int::operator++(int) { return __sync_fetch_and_add(&value, 1); }
...
void atomic_int::operator=(int _value) { value = _value; }
...
int atomic_int::get_basic() { return value; }

void atomic_int::set_basic(int val) { value = val; }

_threadlocalPointer::_threadlocalPointer() { pthread_key_create(&key, 0); }

void _threadlocalPointer::set(void *ptr) { pthread_setspecific(key, ptr); }

void *_threadlocalPointer::get() { return pthread_getspecific(key); }

_threadlocalPointer::~_threadlocalPointer() { pthread_key_delete(key); }

int swap(int *ptr, int newval) { return __sync_lock_test_and_set(ptr, newval); }
