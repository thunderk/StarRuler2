  ...
  double get(int globalIndex) {
    ...
    if (index == -1) {
      if (globalIndex > 0 && globalIndex < (int)nsVariableNames.size()) {
        // Formula variable exists, just hasn't been set in this particular
        // namespace, which means we should consider it a 0 so we don't fuck
        // with execution order

        // error("Formula variable '%s' does not exist.",
        // nsVariableNames[globalIndex].c_str()); scripts::logException();
        return 0.0;
      } else {
...
static FormulaNamespace *makeNamespace() { return new FormulaNamespace(); }
...
static double varName(void *user, const std::string *name) { return 0.0; }
...
public:
  ...
  ~ScriptFormula() { delete formula; }
};

static ScriptFormula *makeFormula_e() { return new ScriptFormula(); }
...
void RegisterFormulaBinds(bool server) {
  ...
  classdoc(ns, "A namespace of variables that can be accessed from formulas "
               "attached to it.");

  ns.addFactory("Namespace@ f()", asFUNCTION(makeNamespace));
  ns.setReferenceFuncs(asMETHOD(FormulaNamespace, grab),
                       asMETHOD(FormulaNamespace, drop));
  ...
  ns.addMethod("int lookup(const string&in name, bool create = true)",
      ...
      doc("Lookup the index of a variable in the namespace.",
          ...
          "Index of the variable. -1 if it does not exist and was not "
          "created.");
  ...
  ns.addMethod("double get(const string&in name)",
      ...
      doc("Get or calculate the value of a variable by name.",
          "Name of the variable.", "Value of that variable.");
  ...
  ns.addMethod("double get(int index)",
      ...
      doc("Get or calculate the value of a variable by index.",
          "Index of the variable.", "Value of that variable.");
  ...
  ns.addMethod("void setConstant(const string&in name, double value)",
               asMETHODPR(FormulaNamespace, setConstant,
                          (const std::string &, double), void))
      doc("Set a constant value for a variable.",
          "Name of the variable to set.", "Value to set the variable to.");

  ns.addMethod("void setConstant(int index, double value)",
               asMETHODPR(FormulaNamespace, setConstant, (int, double), void))
      doc("Set a constant value for a variable.",
          "Index of the variable to set.", "Value to set the variable to.");
  ...
  ns.addMethod("void setFormula(const string&in name, const string&in formula)",
               asMETHODPR(FormulaNamespace, setFormula,
                          (const std::string &, const std::string &), void))
      doc("Set a formula to evaluate for a variable. Formula is evaluated in "
          "this namespace.",
          "Name of the variable to set.", "Formula to set the variable to.");

  ns.addMethod("void setFormula(int index, const string&in formula)",
               asMETHODPR(FormulaNamespace, setFormula,
                          (int, const std::string &), void))
      doc("Set a formula to evaluate for a variable. Formula is evaluated in "
          "this namespace.",
          "Index of the variable to set.", "Formula to set the variable to.");
  ...
  f.addFactory("Formula@ f(const string&in formula)", asFUNCTION(makeFormula))
      doc("Construct a new formula.", "Formula expression to use.",
          "Constructed formula.");
  f.setReferenceFuncs(asMETHOD(ScriptFormula, grab),
                      asMETHOD(ScriptFormula, drop));

  f.addMethod("void parse(const string&in formula)",
              asMETHOD(ScriptFormula, parse))
      doc("Parse an expression into this formula.",
          "Formula expression to use.");

  f.addMethod("double evaluate(Namespace@ ns = null)",
              asMETHOD(ScriptFormula, evaluate))
      doc("Evaluate the formula expression with the current state of "
          "the namespace it was constructed with.",
          "Namespace to retrieve variables from.", "Value of the formula.");
}
