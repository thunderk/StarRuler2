...
// Maximum distance away from the camera something can render (according to its
// scale)
...
MeshIconNode::MeshIconNode(const render::RenderMesh *Mesh,
  ...
  if (Material->baseMat != render::MAT_Solid ||
      (Sheet && Sheet->material.baseMat != render::MAT_Solid))
...
bool MeshIconNode::preRender(render::RenderDriver &driver) {
  auto fromCamera =
      abs_position + (driver.cam_facing * abs_scale) - driver.cam_pos;
  sortDistance = fromCamera.getLength();

  if (sortDistance > MaxRenderDistFactor * abs_scale ||
      !driver.getViewFrustum().overlaps(abs_position, abs_scale))
    return false;
  else
    return true;
}
  ...
  if (render3DIcons) {
    ...
    if (iconSheet) {
      if (sortDistance > BothRenderDistFactor * scaleMod) {
        ...
        // Calculate best-fit facing of the 2D sprite (Assuming the sprite is
        // facing +x)
        ...
        // If it's facing along the camera vector, we can't get an accurate
        // angle
        ...
        color.a =
            std::min(1.0, (sortDistance / scaleMod - BothRenderDistFactor) /
                              (IconRenderDistFactor - BothRenderDistFactor));

        Color c = color;

        driver.drawBillboard(abs_position, iconScale, iconSheet->material,
                             iconSheet->getSource(iconIndex), rot, c);
