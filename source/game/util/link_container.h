 ...
 * Reads can occur at the same time as writes, but writes must not happen
 * simultaneously!
 *
 * Intended for use with small data structures that need to be read from
 * multiple threads, and can change but do so infrequently enough that the extra
...
protected:
  ...
  inline PoolHeader &getHeader(void *mem) const { return *(PoolHeader *)mem; }
  ...
  inline PoolElem &getElem(void *at) const { return *(PoolElem *)at; }
  ...
  void getIndex(unsigned int index, void *&outPool,
                unsigned char &outElem) const {
    void *pool = start;
    while (pool) {
      auto &header = getHeader(pool);
...
public:
  LinkContainer() : start(nullptr), count(0) {}
  ...
  void add(const T &data) {
    ...
    {
      // We can only insert into the last pool we have, otherwise we need to
      // create a new one
      auto *pool = getLast();
      auto &header = getHeader(pool);
      if (header.filledElements < PoolSize) {
  ...
  template <typename CB> void iterateAll(CB cb) const {
    void *pool = start;
    while (pool) {
      for (unsigned char i = 0; i < PoolSize; ++i) {
        auto &elem = getElem(pool, i);
          ...
          if (elem.data.key == key) {
            if (!elem.data.dirty && dirtyResolution >= 0) {
              if (dirtyResolution == 0) {
                ...
                if (fabs(elem.data.doubleValue - elem.data.prevDoubleValue) >=
                    dirtyResolution) {
                  elem.data.dirty = true;
                  dirtyCount++;
                }
                ...
                elem.data.value = value;
                if (llabs((int64_t)elem.data.value -
                          (int64_t)elem.data.prevValue) >= dirtyResolution) {
                  elem.data.dirty = true;
                  dirtyCount++;
                }
              }
...
public:
  ...
  LinkMapBase(uint64_t defaultValue)
      : defaultInt(defaultValue), dirtyCount(0) {}

  LinkMapBase(double defaultValue)
      : defaultDouble(defaultValue), dirtyCount(0) {}

  void setDefaultValue(uint64_t newValue) { defaultInt = newValue; }

  uint64_t getDefaultValue() const { return defaultInt; }

  void setDefaultDouble(double newValue) { defaultDouble = newValue; }

  double getDefaultDouble() const { return defaultDouble; }
  ...
  template <typename CB> void iterateDirty(CB cb) const {
    void *pool = start;
    while (pool) {
      for (unsigned char i = 0; i < PoolSize; ++i) {
        auto &elem = getElem(pool, i);
  ...
  template <typename CB> void handleDirty(CB cb) {
    void *pool = start;
    while (pool) {
      for (unsigned char i = 0; i < PoolSize; ++i) {
        auto &elem = getElem(pool, i);
  ...
  bool getDirtyCount() const { return dirtyCount; }

  bool hasDirty() const { return dirtyCount != 0; }
  ...
  bool contains(uint64_t key) const { return getMapElem(key) != nullptr; }
  ...
  void setDouble(uint64_t key, double value, double dirtyResolution = 0.0) {
    setTyped<double, true>(key, reinterpret_cast<uint64_t &>(value),
                           dirtyResolution);
  }
};
