...
void GLTexture::bind() { glBindTexture(GL_TEXTURE_2D, glName); }

unsigned GLTexture::getID() const { return glName; }

GLTexture::GLTexture() : glName(0), pixelDepth(1) {
  type = TT_2D;
  loaded = false;
}
...
void createTexture(Image &image, bool mipmap) {
  ...
  if (GLEW_ARB_texture_storage) {
    ...
    for (unsigned i = 0; i < levels; ++i) {
      glTexImage2D(GL_TEXTURE_2D, i, format, w, h, 0, format, GL_UNSIGNED_BYTE,
                   nullptr);
      w /= 2;
      h /= 2;
    }
...
void loadTexture(Image &image, const recti &pixels, unsigned lod = 0) {
  ...
  if (pixels.getSize() == vec2i(image.width, image.height)) {
    ...
    switch (image.format) {
    case FMT_Grey:
      ptrPixels = (void *)image.grey;
      break;
    case FMT_Alpha:
      ptrPixels = (void *)image.grey;
      break;
    case FMT_RGB:
      ptrPixels = (void *)image.rgb;
      break;
    case FMT_RGBA:
      ptrPixels = (void *)image.rgba;
      break;
      NO_DEFAULT
    }

    glTexSubImage2D(GL_TEXTURE_2D, lod, pixels.topLeft.x, pixels.topLeft.y,
                    pixels.getWidth(), pixels.getHeight(), format,
                    GL_UNSIGNED_BYTE, ptrPixels);
    ...
    switch (image.format) {
    case FMT_Grey:
      ptrPixels = (void *)&image.get_grey(0, pixels.topLeft.y);
      break;
    case FMT_Alpha:
      ptrPixels = (void *)&image.get_alpha(0, pixels.topLeft.y);
      break;
    case FMT_RGB:
      ptrPixels = (void *)&image.get_rgb(0, pixels.topLeft.y);
      break;
    case FMT_RGBA:
      ptrPixels = (void *)&image.get_rgba(0, pixels.topLeft.y);
      break;
      NO_DEFAULT
    }
    ...
    for (unsigned y = pixels.topLeft.y, endY = pixels.botRight.y; y < endY;
         ++y) {
      void *ptrPixels;
      switch (image.format) {
      case FMT_Grey:
        ptrPixels = (void *)&image.get_grey(pixels.topLeft.x, y);
        break;
      case FMT_Alpha:
        ptrPixels = (void *)&image.get_alpha(pixels.topLeft.x, y);
        break;
      case FMT_RGB:
        ptrPixels = (void *)&image.get_rgb(pixels.topLeft.x, y);
        break;
      case FMT_RGBA:
        ptrPixels = (void *)&image.get_rgba(pixels.topLeft.x, y);
        break;
        NO_DEFAULT
      }

      glTexSubImage2D(GL_TEXTURE_2D, lod, pixels.topLeft.x, y,
                      pixels.getWidth(), 1, format, GL_UNSIGNED_BYTE,
                      ptrPixels);
    }
  }
...
void GLTexture::loadStart(Image &image, bool mipmap, bool cachePixels,
                          unsigned lod) {
  devices.render->reportErrors();
  loaded = true;
...
void GLTexture::loadPartial(Image &image, const recti &pixels, bool cachePixels,
                            unsigned lod) {
  glBindTexture(GL_TEXTURE_2D, glName);
...
GLTexture::~GLTexture() { glDeleteTextures(1, &glName); }

bool GLCubeMap::isPixelActive(vec2i px) const { return true; }
...
void GLCubeMap::load(Image &image, bool mipmap, bool cachePixels) {
  ...
  glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, format, tile.width,
               tile.height, 0, format, GL_UNSIGNED_BYTE, img->grey);
  delete img;

  img = getTile(2, 1);
  glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, format, tile.width,
               tile.height, 0, format, GL_UNSIGNED_BYTE, img->grey);
  delete img;

  img = getTile(1, 2);
  glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, format, tile.width,
               tile.height, 0, format, GL_UNSIGNED_BYTE, img->grey);
  delete img;

  img = getTile(3, 1);
  glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, format, tile.width,
               tile.height, 0, format, GL_UNSIGNED_BYTE, img->grey);
  delete img;

  img = getTile(0, 1);
  glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, format, tile.width,
               tile.height, 0, format, GL_UNSIGNED_BYTE, img->grey);
  delete img;

  img = getTile(1, 0);
  glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, format, tile.width,
               tile.height, 0, format, GL_UNSIGNED_BYTE, img->grey);
  delete img;
...
void GLCubeMap::bind() { glBindTexture(GL_TEXTURE_CUBE_MAP, glName); }

unsigned GLCubeMap::getID() const { return glName; }
...
GLCubeMap::~GLCubeMap() { glDeleteTextures(1, &glName); }

void GLCubeMap::loadStart(Image &image, bool mipmap, bool cachePixels,
                          unsigned lod) {
  load(image, mipmap);
}

void GLCubeMap::loadPartial(Image &image, const recti &pixels, bool cachePixels,
                            unsigned lod) {}
