...
// This one is not static because it needs to be friend with the CScriptHandle
// class
...
CScriptHandle &CScriptHandle::Assign(void *ref, int typeId) {
  ...
  // If the argument is another CScriptHandle, we should copy the content
  // instead
...
void CScriptHandle::Cast(void **outRef, int typeId) {
  ...
  *outRef = 0;
  ...
  // RefCastObject will increment the refCount of the returned object if
  // successful
...
#if AS_CAN_USE_CPP11
  // With C++11 it is possible to use asGetTypeTraits to automatically determine
  // the flags that represent the C++ class
  r = engine->RegisterObjectType("ref", sizeof(CScriptHandle),
                                 asOBJ_VALUE | asOBJ_ASHANDLE |
                                     asGetTypeTraits<CScriptHandle>());
  assert(r >= 0);
#else
  r = engine->RegisterObjectType("ref", sizeof(CScriptHandle),
                                 asOBJ_VALUE | asOBJ_ASHANDLE |
...
void CScriptHandle_ConstructCopy_Generic(asIScriptGeneric *gen) {
  CScriptHandle *other =
      reinterpret_cast<CScriptHandle *>(gen->GetArgAddress(0));
  CScriptHandle *self = reinterpret_cast<CScriptHandle *>(gen->GetObject());
...
void RegisterScriptHandle_Generic(asIScriptEngine *engine) {
  ...
  assert(r >= 0);
  r = engine->RegisterObjectMethod("ref", "void opCast(?&out)",
                                   asFUNCTION(CScriptHandle_Cast_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("ref", "ref &opHndlAssign(const ref &in)",
                                   asFUNCTION(CScriptHandle_Assign_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("ref", "ref &opHndlAssign(const ?&in)",
                                   asFUNCTION(CScriptHandle_AssignVar_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("ref", "bool opEquals(const ref &in) const",
                                   asFUNCTION(CScriptHandle_Equals_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("ref", "bool opEquals(const ?&in) const",
                                   asFUNCTION(CScriptHandle_EqualsVar_Generic),
                                   asCALL_GENERIC);
  assert(r >= 0);
}
