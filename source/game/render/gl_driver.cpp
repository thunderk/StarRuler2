...
Texture *RenderDriver::createTexture() { return new GLTexture(); }

Texture *RenderDriver::createCubemap() { return new GLCubeMap(); }
...
public:
  ...
  frustum viewFrustum;
  vec3d cam_right, bb_up_p_right, bb_up_m_right;
  ...
  GLDriver()
      : textureStage(GL_TEXTURE0), pLastRenderState(0), skybox(0),
        skyboxMesh(0), cleared(false), intermediateBlend(false), fov(50.0),
        zNear(1.0), zFar(240000.0) {
  ...
  void setCameraData(Camera &camera) {
    ...
    auto yaw = quaterniond::fromAxisAngle(vec3d::up(-1.0),
                                          atan2(cam_facing.z, cam_facing.x));
    auto pitch =
        quaterniond::fromAxisAngle(vec3d::right(), asin(-cam_facing.y));

    BBFacingRot = yaw * pitch;

    viewFrustum = frustum(camera.screenToRay(0, 0), camera.screenToRay(1, 0),
                          camera.screenToRay(0, 1), camera.screenToRay(1, 1));
  }
  ...
  void setBBTransform(vec3d pos, double width, double rot) override {
    ...
    (quaterniond::fromAxisAngle(cam_facing, -rot) * BBFacingRot)
        .toTransform(m, pos - cam_pos, vec3d(width * 0.5));
    glMultMatrixd(m.m);
  }

  void resetTransformation() override { glPopMatrix(); }
  ...
  void set2DRenderState() { switchToRenderState(state_2d); }
  ...
  void switchToRenderState(const RenderState &state) {
      ...
      if (glDirectStateAccess) {
        if (tex == 0) {
          ...
          if (state.filterMin != tex->prevRenderState.filterMin) {
            ...
            case TF_Nearest:
              mode = tex->hasMipMaps ? GL_NEAREST_MIPMAP_NEAREST : GL_NEAREST;
              break;
            case TF_Linear:
              mode = tex->hasMipMaps ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR;
              break;
              NO_DEFAULT
            }
            glMultiTexParameteriEXT(texunit, glType, GL_TEXTURE_MIN_FILTER,
                                    mode);
            changed = true;
          }
          ...
          if (state.filterMag != tex->prevRenderState.filterMag) {
            ...
            glMultiTexParameteriEXT(texunit, glType, GL_TEXTURE_MAG_FILTER,
                                    mode);
            changed = true;
          }
    ...
    if (!state.shader && (state.lighting != activeRenderState.lighting ||
                          activeRenderState.shader)) {
      if (state.lighting)
        glEnable(GL_LIGHTING);
      else
  ...
  void getBillboardVecs(const vec3d &from, vec3d &upLeft, vec3d &upRight,
  ...
  void drawBillboard(vec3d center, double width, const RenderState &mat,
  ...
  void drawBillboard(vec3d center, double width, const RenderState &mat,
                     const recti &source, Color *color) {
    rectf texCoords;
    ...
    auto UpMinRight = bb_up_m_right * width,
         UpPlusRight = bb_up_p_right * width;
  ...
  void drawBillboard(vec3d center, double width, const RenderState &mat,
                     const recti &source, double rotation,
  ...
  virtual void drawQuad(const RenderState *mat, const vec2<float> *vertices,
  ...
  void drawQuad(const vec3d *vertices, const vec2<float> *textureCoords,
                const Color *colors) {
    auto *buffer = VertexBufferTCV::fetch(pLastRenderState);
    auto *verts = buffer->request(1, PT_Quads);
  ...
  void drawQuad(const vec2<float> *vertices, const vec2<float> *textureCoords,
                const Color *colors) {
    auto *buffer = VertexBufferTCV::fetch(pLastRenderState);
    auto *verts = buffer->request(1, PT_Quads);
  ...
  void drawRectangle(const recti &rectangle, const RenderState *mat,
                     Color color, const recti *clip = 0) {
    ...
    if (clip && !clip->isRectInside(rect)) {
      ...
      uv.topLeft.x = (float)(rect.topLeft.x - rectangle.topLeft.x) /
                     (float)rectangle.getWidth();
      uv.topLeft.y = (float)(rect.topLeft.y - rectangle.topLeft.y) /
                     (float)rectangle.getHeight();
      uv.botRight.x = 1.f - (float)(rectangle.botRight.x - rect.botRight.x) /
                                (float)rectangle.getWidth();
      uv.botRight.y = 1.f - (float)(rectangle.botRight.y - rect.botRight.y) /
                                (float)rectangle.getHeight();
    }
  ...
  RenderMesh *createMesh(const Mesh &mesh) { return createGLMesh(mesh); }

  Shader *createShader() { return createGLShader(); }

  ShaderProgram *createShaderProgram(const char *vertex_shader,
                                     const char *fragment_shader) {
    return createGLShaderProgram(vertex_shader, fragment_shader);
  }

  Texture *createTexture(Image &image, bool mipmap = true,
  ...
  void clear(unsigned flags) {
    // If we need to clear the depth buffer, we need to enable the depth buffer
    // for writing
  ...
  bool init() {
    ...
    isIntelCard =
        vendorString != nullptr && std::string(vendorString) == "Intel";
  ...
  void clearRenderPrepared() { isPrepared = false; }

  bool isRenderPrepared() { return isPrepared; }
  ...
  void prepareRender3D(Camera &camera, const recti *clip) {
    ...
    camera.setRenderConstraints(zNear, zFar, fov, aspect, (double)screenSize.x,
                                (double)screenSize.y);
    glMatrixMode(GL_MODELVIEW);
    {
      ...
      { // Setup lights in the scene
        ...
        while (lightIndex < lightCount) {
          ...
          screenLight[lightIndex] =
              vec2f((float)onScreen.x / (float)screenSize.x,
                    1.f - (float)onScreen.y / (float)screenSize.y);
      ...
      if (skybox) {
        ...
        glPushMatrix();
        glScaled((zNear + zFar) * 0.5, (zNear + zFar) * 0.5,
                 (zNear + zFar) * 0.5);
        skyboxMesh->render();
        glPopMatrix();
      } else {
  ...
  void drawFPSGraph(const recti &location) {
    ...
    float x = (float)location.topLeft.width, yOff = (float)location.topLeft.y,
          height = (float)location.getHeight();
    ...
    float yFrameLine =
        floor(yOff + height * (float)(1.0 - (0.0166667 / max_time)));
    ...
    if (frames.size() > 1) {
      ...
      for (auto frame = frames.begin(), end = frames.end(); frame != end;
           ++frame, ++v) {
        v->col = Colorf((float)(*frame / 0.033333),
                        1.f - (float)(*frame / 0.0333333), 0);
        v->pos =
            vec3f(x, yOff + height * (1.f - (float)(*frame / max_time)), 0);
  ...
  void setSkybox(const RenderState *mat) { skybox = mat; }

  void setSkyboxMesh(const RenderMesh *mesh) { skyboxMesh = mesh; }

  void setScissor(const recti &clip) const {
    double x = (double)clip.topLeft.x / (double)screenSize.width;
    double y = (double)(screenSize.height - clip.botRight.y) /
               (double)screenSize.height;
    double w = clip.getWidth() / (double)screenSize.width;
    double h = clip.getHeight() / (double)screenSize.height;

    glScissor((int)(x * frameSize.x), (int)(y * frameSize.y),
              (int)(w * frameSize.x), (int)(h * frameSize.y));
  }
...
RenderDriver *createGLDriver() { return new GLDriver(); }
