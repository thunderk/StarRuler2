...
void createLog() {
  logFile.open(path_join(getProfileRoot(), "log.txt"),
               std::ofstream::trunc | std::ofstream::out);
  errorLog.open(path_join(getProfileRoot(), "errors.log.txt"),
...
void storeLog(const std::string &filename) {
  // NOTE: This only happens during a crash situation, so we disregard the
  // various possible side effects if something goes wrong
  logFile.flush();
  logFile.close();
...
const char *sectionNames[NS_COUNT] = {"Unknown", "Network",        "Processing",
                                      "Loading", "Script Tick",    "Render",
                                      "Startup", "Message Thread", "Animation"};
