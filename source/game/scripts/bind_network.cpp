...
Object *readObject(net::Message &msg, bool create, int knownType) {
  unsigned typeID = knownType >= 0 ? (unsigned)knownType
                                   : msg.readLimited(ObjectTypeCount - 1);
...
static void receiveArray(CScriptArray *arr, YieldedMessage **ctx) {
  ...
  if (!subType || !subType->Implements(srtype)) {
    scripts::throwException(
        "Cannot sync into array of non-serializable objects.");
    return;
  }
...
static void createMessage(void *mem) { new (mem) net::Message(); }

static void copyMessage(void *mem, net::Message &other) {
  new (mem) net::Message(other);
}

static void destroyMessage(net::Message &msg) { msg.~Message(); }
...
static bool emptyMessage(net::Message &msg) { return msg.size() == 0; }
...
static net::msize_t reserveInt(net::Message &msg) {
  try {
    ...
    scripts::throwException(
        "Message needs to be aligned before reserving space.");
    return 0;
  }
}
...
template <class T> static net::Message &wrapRead(net::Message &msg, T &value) {
  try {
    msg >> value;
  } catch (net::MessageReadError) {
    scripts::throwException("Error reading from message: end of message.");
...
template <class T> static T wrapExplicitRead(net::Message &msg) {
  T value = T();
...
static double readFixed(net::Message &msg, double minimum, double maximum,
                        unsigned bits) {
  try {
...
static void writeFixed(net::Message &msg, double value, double minimum,
                       double maximum, unsigned bits) {
...
void RegisterEarlyNetworkBinds(asIScriptEngine *engine) {
  ...
  engine->RegisterObjectMethod("array<T>", "void syncFrom(DataList@& list)",
                               asFUNCTION(receiveArray), asCALL_CDECL_OBJFIRST);
}
...
void RegisterNetworkBinds(bool server) {
  ...
  ClassBind msg("Message", asOBJ_VALUE | asOBJ_APP_CLASS_CDA,
                sizeof(net::Message));
  ...
  msg.addExternMethod("Message& opAssign(const Message&in)",
                      asFUNCTION(assignMessage));
  ...
  msg.addExternMethod("void writeSignedSmall(int value)",
                      asFUNCTION(writeSignedSmall));
  msg.addExternMethod("int readSignedSmall()", asFUNCTION(readSignedSmall));

  msg.addExternMethod("void writeLimited(uint value, uint limit)",
                      asFUNCTION(writeLimited));
  msg.addExternMethod("uint readLimited(uint limit)", asFUNCTION(readLimited));

  msg.addExternMethod("void writeBitValue(uint value, uint bits)",
                      asFUNCTION(writeBitValue));
  msg.addExternMethod("uint readBitValue(uint bits)", asFUNCTION(readBitValue));

  msg.addExternMethod("void writeFixed(double value, double minimum = 0.0, "
                      "double maximum = 1.0, uint bits = 16)",
                      asFUNCTION(writeFixed))
      doc("Writes a double that is guaranteed to fall into the rage "
          "[minimum,maximum) using a specified number of bits.",
          "", "", "", "", "");
  msg.addExternMethod("double readFixed(double minimum = 0.0, double maximum = "
                      "1.0, uint bits = 16)",
                      asFUNCTION(readFixed));

  msg.addExternMethod("void writeSmallVec3(const vec3d& v)",
                      asFUNCTION(writeSmallVec3))
      doc("Writes a compressed vec3d into a very small space, sacrificing "
          "minimal quality.",
          "");
  msg.addExternMethod("vec3d readSmallVec3()", asFUNCTION(readSmallVec3));

  msg.addExternMethod("void writeMedVec3(const vec3d& v)",
                      asFUNCTION(writeVec3))
      doc("Writes a compressed vec3d into a smaller space, sacrificing very "
          "little quality.",
          "");
  msg.addExternMethod("vec3d readMedVec3()", asFUNCTION(readVec3));

  msg.addExternMethod("void writeDirection(const vec3d& v)",
                      asFUNCTION(writeDir))
      doc("Writes a unit vector in a much smaller size.", "");
  msg.addExternMethod("vec3d readDirection()", asFUNCTION(readDir));

  msg.addExternMethod("void writeRotation(const quaterniond& v)",
                      asFUNCTION(writeRot))
  ...
  msg.addExternMethod("Message& opShr(bool&)", asFUNCTION(wrapRead<bool>));
...
#define BIND_TYPE(stype, rtype)                                                \
  ...
  msg.addExternMethod("Message& opShl(ElevationMap&)",
                      asFUNCTION(writeElevation));
  msg.addExternMethod("Message& opShr(ElevationMap&)",
                      asFUNCTION(readElevation));
  ...
  msg.addExternMethod("Message& opShl(const Design@&)",
                      asFUNCTION(writeDesign));
  ...
  bind("bool receive(DataList@& list, Object@& obj)",
       asFUNCTION(receiveObject));
  ...
  if (type) {
    ...
    engine->SetUserData(type->GetMethodByDecl("void write(Message& msg)"),
                        EDID_SerializableWrite);
    engine->SetUserData(type->GetMethodByDecl("void read(Message& msg)"),
...

  {
    ...
    linkMap.addExternMethod("void writeAll(Message& msg)",
                            asFUNCTION(writeLinkMapAll));
    linkMap.addExternMethod("void writeDirty(Message& msg)",
                            asFUNCTION(writeLinkMapDelta));
    linkMap.addExternMethod("void readAll(Message& msg)",
                            asFUNCTION(readLinkMap));
    linkMap.addExternMethod("void readDirty(Message& msg)",
                            asFUNCTION(readLinkMap));
  }
}
