...
namespace scene {
scriptNodeType::scriptNodeType(const std::string &Name,
                               const std::string &ident)
    : id(nextScriptNodeID++), name(Name), identifier(ident), factory(0),
      preRender(0), render(0) {}
...
void scriptNodeType::bind() {
  ...
  if (type) {
    factory = type->GetFactoryByDecl(
        format("$1@ $1(Node&)", type->GetName()).c_str());
    ...
    for (unsigned i = 0; i < methods.size(); ++i) {
      ...
      method->func =
          type->GetMethodByDecl(descContaining.declaration().c_str(), false);
      if (method->func) {
        method->desc = descContaining;
        method->passContaining = true;
      } else {
        method->func =
            type->GetMethodByDecl(method->desc.declaration().c_str(), false);
      }
    }
  } else {
...
ScriptedNode::ScriptedNode(scriptNodeType *nodeType)
    : scriptObject(0), errors(0), type(*nodeType) {
  ...
  while (true) {
    ...
    if (!inType) {
      ...
      if (desc.returnType.type) {
        error("Error: '%s': node functions may not return a value.",
              parse.c_str());
        continue;
      }
