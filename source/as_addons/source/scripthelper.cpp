...
int CompareRelation(asIScriptEngine *engine, void *lobj, void *robj, int typeId,
                    int &result) {
  // TODO: If a lot of script objects are going to be compared, e.g. when
  // sorting an array,
  ...
  if (ti) {
    ...
    for (asUINT n = 0; n < ti->GetMethodCount(); n++) {
      ...
      if (strcmp(f->GetName(), "opCmp") == 0 &&
          f->GetReturnTypeId(&flags) == asTYPEID_INT32 && flags == asTM_NONE &&
          f->GetParamCount() == 1) {
        int paramTypeId;
        ...
        if (!(flags & asTM_INREF) || typeId != paramTypeId ||
            ((flags & asTM_OUTREF) && !(flags & asTM_CONST)))
          break;
...
int CompareEquality(asIScriptEngine *engine, void *lobj, void *robj, int typeId,
                    bool &result) {
  // TODO: If a lot of script objects are going to be compared, e.g. when
  // searching for an
  //       entry in a set, then the method and context should be cached between
  //       calls.
  ...
  if (ti) {
    ...
    for (asUINT n = 0; n < ti->GetMethodCount(); n++) {
      ...
      if (strcmp(f->GetName(), "opEquals") == 0 &&
          f->GetReturnTypeId(&flags) == asTYPEID_BOOL && flags == asTM_NONE &&
          f->GetParamCount() == 1) {
        int paramTypeId;
        f->GetParam(0, &paramTypeId, &flags);
...
int ExecuteString(asIScriptEngine *engine, const char *code, void *ref,
                  int refTypeId, asIScriptModule *mod, asIScriptContext *ctx) {
  ...
  // GetModule will free unused types, so to be on the safe side we'll hold on
  // to a reference to the type
  asITypeInfo *type = 0;
  ...
  asIScriptFunction *func = 0;
  int r =
      execMod->CompileFunction("ExecuteString", funcCode.c_str(), -1, 0, &func);
  if (r < 0)
    return r;
  ...
  if (ref != 0 && refTypeId != asTYPEID_VOID) {
    if (refTypeId & asTYPEID_OBJHANDLE) {
      ...
      *reinterpret_cast<void **>(ref) =
          *reinterpret_cast<void **>(execCtx->GetAddressOfReturnValue());
      ...
      assert(*reinterpret_cast<void **>(ref) != 0);
      engine->AssignScriptObject(ref, execCtx->GetAddressOfReturnValue(),
                                 engine->GetTypeInfoById(refTypeId));
    } else {
...
int WriteConfigToStream(asIScriptEngine *engine, ostream &strm) {
  ...
  // Keep a list of the template types, as the methods for these need to be
  // exported first
  ...
  for (n = 0; n < c; n++) {
    ...
    if (type->GetFlags() & asOBJ_SCRIPT_OBJECT) {
      ...
      // Only the type flags are necessary. The application flags are
      // application specific and doesn't matter to the offline compiler. The
      // object size is also unnecessary for the offline compiler
      strm << "objtype \"" << engine->GetTypeDeclaration(type->GetTypeId())
           << "\" " << (unsigned int)(type->GetFlags() & asOBJ_MASK_VALID_FLAGS)
           << "\n";

      // Store the template types (but not template instances)
      if ((type->GetFlags() & asOBJ_TEMPLATE) && type->GetSubType() &&
          (type->GetSubType()->GetFlags() & asOBJ_TEMPLATE_SUBTYPE))
  ...
  for (n = 0; n < c; n++) {
    ...
    strm << "typedef " << ti->GetName() << " \""
         << engine->GetTypeDeclaration(ti->GetTypedefTypeId()) << "\"\n";
  }
  ...
  for (n = 0; n < c; n++) {
    ...
    // Child funcdefs do not have any namespace, as they belong to the parent
    // object
  ...
  struct TypeWriter {
    static void Write(asIScriptEngine *engine, ostream &strm, asITypeInfo *type,
                      string &currNamespace, asDWORD &currAccessMask) {
      const char *nameSpace = type->GetNamespace();
      if (nameSpace != currNamespace) {
        strm << "namespace \"" << nameSpace << "\"\n";
        currNamespace = nameSpace;
      ...
      if (type->GetFlags() & asOBJ_SCRIPT_OBJECT) {
        for (asUINT m = 0; m < type->GetMethodCount(); m++) {
          ...
          strm << "intfmthd " << typeDecl.c_str() << " \""
               << Escape::Quotes(func->GetDeclaration(false)).c_str() << "\"\n";
        }
      } else {
        asUINT m;
        for (m = 0; m < type->GetFactoryCount(); m++) {
          ...
          strm << "objbeh \"" << typeDecl.c_str() << "\" " << asBEHAVE_FACTORY
               << " \"" << Escape::Quotes(func->GetDeclaration(false)).c_str()
               << "\"\n";
        }
        for (m = 0; m < type->GetBehaviourCount(); m++) {
          ...
          if (beh == asBEHAVE_CONSTRUCT)
            // Prefix 'void'
            strm << "objbeh \"" << typeDecl.c_str() << "\" " << beh
                 << " \"void "
                 << Escape::Quotes(func->GetDeclaration(false)).c_str()
                 << "\"\n";
            ...
            strm << "objbeh \"" << typeDecl.c_str() << "\" " << beh
                 << " \"void "
                 << Escape::Quotes(func->GetDeclaration(false)).c_str() + 1
                 << "\"\n";
          else
            strm << "objbeh \"" << typeDecl.c_str() << "\" " << beh << " \""
                 << Escape::Quotes(func->GetDeclaration(false)).c_str()
        ...
        for (m = 0; m < type->GetPropertyCount(); m++) {
          ...
          strm << "objprop \"" << typeDecl.c_str() << "\" \""
               << type->GetPropertyDeclaration(m) << "\"\n";
        }
      }
    }
  };

  // Write the members of the template types, so they can be fully registered
  // before any other type uses them
  // TODO: Order the template types based on dependency to avoid failure if one
  // type uses instances of another
  strm << "\n// Template type members\n";
  for (set<asITypeInfo *>::iterator it = templateTypes.begin();
       it != templateTypes.end(); ++it) {
    asITypeInfo *type = *it;
  ...
  for (n = 0; n < c; n++) {
    ...
    engine->GetGlobalPropertyByIndex(n, &name, &nameSpace, &typeId, &isConst, 0,
                                     0, &accessMask);
    if (accessMask != currAccessMask) {
      strm << "access " << hex << (unsigned int)(accessMask) << dec << "\n";
      currAccessMask = accessMask;
  ...
  if (typeId > 0)
    strm << "strfactory \"" << ((flags & asTM_CONST) ? "const " : "")
...
int ConfigEngineFromStream(asIScriptEngine *engine, istream &strm,
  ...
  struct in {
    static asETokenClass GetToken(asIScriptEngine *engine, string &token,
                                  const string &text, asUINT &pos) {
      asUINT len = 0;
      asETokenClass t =
          engine->ParseToken(&text[pos], text.length() - pos, &len);
  ...
  // Since we are only going to compile the script and never actually execute
  // it, we turn off the initialization of global variables, so that the
  // compiler can just register dummy types and functions for the application
  // interface.
  ...
  while (pos < config.length()) {
    string token;
    // TODO: The position where the initial token is found should be stored for
    // error messages
    ...
    if (token == "ep") {
      ...
      if (ep != asEP_COPY_SCRIPT_SECTIONS && ep != asEP_MAX_STACK_SIZE &&
      ...
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR, "Failed to set namespace");
        return -1;
      }
      ...
      r = engine->RegisterObjectType(
          name.c_str(), (atol(flags.c_str()) & asOBJ_VALUE) ? 1 : 0,
          atol(flags.c_str()));
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR, "Failed to register object type");
        return -1;
      }
      ...
      asEBehaviours behave =
          static_cast<asEBehaviours>(atol(behaviour.c_str()));
      if (behave == asBEHAVE_TEMPLATE_CALLBACK) {
        // TODO: How can we let the compiler register this? Maybe through a
        // plug-in system? Or maybe by implementing the callback as a script
        // itself
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_WARNING,
                             "Cannot register template callback without the "
                             "actual implementation");
      } else {
        r = engine->RegisterObjectBehaviour(name.c_str(), behave, decl.c_str(),
                                            asFUNCTION(0), asCALL_GENERIC);
        if (r < 0) {
          engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                               asMSGTYPE_ERROR, "Failed to register behaviour");
          return -1;
        }
      }
      ...
      r = engine->RegisterObjectMethod(name.c_str(), decl.c_str(),
                                       asFUNCTION(0), asCALL_GENERIC);
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR,
                             "Failed to register object method");
        return -1;
      }
      ...
      asITypeInfo *type =
          engine->GetTypeInfoById(engine->GetTypeIdByDecl(name.c_str()));
      if (type == 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR,
                             "Type doesn't exist for property registration");
        return -1;
      }
      ...
      r = engine->RegisterObjectProperty(name.c_str(), decl.c_str(),
                                         type->GetPropertyCount());
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR,
                             "Failed to register object property");
        return -1;
      }
    } else if (token == "intf") {
      ...
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR, "Failed to register interface");
        return -1;
      }
      ...
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR,
                             "Failed to register interface method");
        return -1;
      }
    } else if (token == "func") {
      string decl;
      ...
      r = engine->RegisterGlobalFunction(decl.c_str(), asFUNCTION(0),
                                         asCALL_GENERIC);
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR,
                             "Failed to register global function");
        return -1;
      }
    } else if (token == "prop") {
      string decl;
      ...
      r = engine->RegisterGlobalProperty(
          decl.c_str(), reinterpret_cast<void *>(
                            asPWORD(engine->GetGlobalPropertyCount() + 1)));
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR,
      ...
      r = engine->RegisterStringFactory(type.c_str(), asFUNCTION(0),
                                        asCALL_GENERIC);
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR,
                             "Failed to register string factory");
        return -1;
      }
      ...
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR,
                             "Failed to register the default array type");
        return -1;
      }
    } else if (token == "enum") {
      string type;
      ...
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR, "Failed to register enum type");
        return -1;
      }
      ...
      r = engine->RegisterEnumValue(type.c_str(), name.c_str(),
                                    atol(value.c_str()));
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR, "Failed to register enum value");
        return -1;
      }
      ...
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR, "Failed to register typedef");
        return -1;
      }
      ...
      if (r < 0) {
        engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0,
                             asMSGTYPE_ERROR, "Failed to register funcdef");
        return -1;
      }
    }
  }
    ...
    for (asUINT n = 1; n < ctx->GetCallstackSize(); n++) {
      ...
      if (function) {
        if (function->GetFuncType() == asFUNC_SCRIPT) {
          text << function->GetScriptSectionName() << " ("
