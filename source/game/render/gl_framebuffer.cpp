...
public:
  ...
  virtual void loadStart(Image &image, bool mipmap = true,
                         bool cachePixels = false, unsigned lod = 0) {}
  virtual void loadPartial(Image &image, const recti &pixels,
                           bool cachePixels = false, unsigned lod = 0) {}
  virtual void loadFinish(bool mipmap, unsigned lod = 0) {}

  virtual void load(Image &image, bool mipmap = true,
...
glFrameBuffer::glFrameBuffer(const vec2i &Size) {
  ...
  // glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, Size.width,
  // Size.height); glBindRenderbuffer(GL_RENDERBUFFER, 0);
...
void glFrameBuffer::bind() { glBindTexture(GL_TEXTURE_2D, textures[0]); }

unsigned glFrameBuffer::getID() const { return textures[0]; }
...
void glFrameBuffer::loadStart(Image &image, bool mipmap, bool cachePixels,
                              unsigned lod) {
  throw "Cannot load framebuffer from image.";
}

void glFrameBuffer::loadPartial(Image &image, const recti &pixels,
                                bool cachePixels, unsigned lod) {
  throw "Cannot load framebuffer from image.";
}
