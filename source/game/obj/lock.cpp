...
void performDelayedObjectReleases() {
  threads::Lock lock(delayedReleaseLock);
  for (auto it = delayedReleaseObjects.begin(),
            end = delayedReleaseObjects.end();
       it != end; ++it)
    (*it)->drop();
...
void ObjectMessage::operator delete(void *p) { objMessagePool.dealloc(p); }

unsigned getLockCount() { return (unsigned)lockGroups.size(); }

LockGroup *getLock(unsigned index) { return lockGroups[index]; }

LockGroup *getActiveLockGroup() { return activeLockGroup; }

Object *getActiveObject() { return activeObject; }

void setActiveObject(Object *obj) { activeObject = obj; }
...
void handleObjectClears() {
  ...
  threads::Lock lock(clearLock);
  for (auto i = queuedClearObjects.begin(), end = queuedClearObjects.end();
       i != end; ++i) {
    Object *obj = *i;
    obj->clearScripts();
    obj->drop();
...
void printLockStats() { printNextLockStats = true; }

LockGroup::LockGroup() : tickIndex(0) {
#ifdef PROFILE_LOCKS
...
bool LockGroup::hasLock() { return mutex.hasLock(); }
...
bool updateLockGlobals(double time) {
  ...
  if (statTime < time) {
    ...
    for (size_t i = 0, cnt = lockGroups.size(); i < cnt; ++i) {
      ...
      if (printNextLockStats) {
        ...
        print("  Bad Locks: %d / %d (%.0g%)", grp->badLocks,
              (grp->goodLocks + grp->badLocks),
              (double)grp->badLocks / (double)(grp->goodLocks + grp->badLocks) *
                  100.0);
        print("  Gained %d objects, Lost %d objects.", grp->gainedObjects,
              grp->lostObjects);
      }
  ...
  if (nextTargetUpdateTime < time) {
    ...
    for (unsigned i = 0, cnt = (childCount + 255) / 256; i < cnt; ++i)
      devices.universe->setRandomTarget(
          devices.universe->children[randomi(0, childCount - 1)]
              ->targets[randomi(0, OBJ_TARGETS - 1)]);
  ...
  // We need to process deletion of objects' script classes while the server is
  // paused, or component calls could randomly crash
  handleObjectClears();
  ...
  if (doGC) {
    ...
    if (gcEnd >= gcStart + 0.01)
      print("Server GC took %dms (mode %d)", (int)((gcEnd - gcStart) * 1000.0),
            mode);
#endif
...
bool LockGroup::process(double time, int limit) {
  ...
  while (tickIndex < objects.size()) {
    ...
    if (obj->lockGroup != this) {
      ...
      // Only tick if the object wants to be ticked
      if ((time > obj->nextTick || obj->getFlag(objWakeUp)) &&
          !obj->getFlag(objStopTicking)) {
#ifdef PROFILE_PROCESSING
        if (obj->type->id != procType) {
          double curTime = devices.driver->getAccurateTime();
          if (procType != -1) {
            processing::threadProc->measureType(procType, curTime - procStart,
                                                procCount);
          }
          procStart = curTime;
        ...
        // We need to sleep periodically to guarantee progress at high cpu
        // utilization
        if (i % 1024 == 0) {
          unlockGroup(this);
          threads::sleep(1);
...
void unlockObject(LockGroup *lock) { _unlockGroup(lock); }

void unlockGroup(LockGroup *lock) { _unlockGroup(lock); }

bool hasQueuedChildren() { return queuedChildren != 0; }

bool hasRemainingMessages() { return remainingMessages != 0; }
