...
inline double Vector::length() const { return sqrt(length2()); }
...
inline Vector Vector::operator-() const { return Vector(-d[0], -d[1], -d[2]); }
...
inline Vector Vector::cross(const Vector &v) const {
  return Vector(d[1] * v.d[2] - d[2] * v.d[1], d[2] * v.d[0] - d[0] * v.d[2],
                d[0] * v.d[1] - d[1] * v.d[0]);
}
...
inline double Vector::x() const { return d[0]; }

inline double Vector::y() const { return d[1]; }

inline double Vector::z() const { return d[2]; }
