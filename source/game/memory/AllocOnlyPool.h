...
template <class T, class L> class AllocOnlyPool {
  L lock;
  T *pStart, *pNext, *pEnd;
  unsigned allocated;
...
public:
  ...
  ~AllocOnlyPool() { delete[] (unsigned char *)pStart; }
...
// Allocated memory is only cleared when all objects in the pool are deleted
// When all memory is used up, falls back on global operator new/delete
template <class Lock> class AllocOnlyRegion {
...
public:
  ...
  ~AllocOnlyRegion() { delete[] (unsigned char *)pStart; }
