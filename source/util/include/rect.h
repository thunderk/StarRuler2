...
template <class T> T _clip(T v, T s, T x1, T x2, T w) {
  // -_-
  return (T)(w > 0 ? (double)v +
                         (((double)x2 - (double)x1) / (double)w) * (double)s
                   : (double)v);
}
...
template <class T> struct rect {
  ...
  rect() {}
  rect(vec2<T> a, vec2<T> b) : topLeft(a), botRight(b) {}
  rect(T x1, T y1, T x2, T y2) : topLeft(x1, y1), botRight(x2, y2) {}

  template <class Q> rect(const rect<Q> &other) {
  ...
  static rect<T> centered(const vec2<T> around, const vec2<T> size) {
    return area(around.x - size.x / 2, around.y - size.y / 2, size.width,
                size.height);
  }

  static rect<T> centered(const rect<T> &within, const vec2<T> size) {
    return centered(vec2<T>((within.topLeft.x + within.botRight.x) / 2,
                            (within.topLeft.y + within.botRight.y) / 2),
                    size);
  }
  ...
  vec2<T> getSize() const { return botRight - topLeft; }

  T getWidth() const { return botRight.x - topLeft.x; }

  T getHeight() const { return botRight.y - topLeft.y; }

  vec2<T> getBotLeft() const { return vec2<T>(topLeft.x, botRight.y); }

  vec2<T> getTopRight() const { return vec2<T>(botRight.x, topLeft.y); }
  ...
  vec2<T> getCenter() const {
    return vec2<T>(topLeft.x + (botRight.x - topLeft.x) / 2,
  ...
  bool isWithin(const vec2<T> &pos) const {
    return pos.x >= topLeft.x && pos.y >= topLeft.y && pos.x < botRight.x &&
           pos.y < botRight.y;
  }
  ...
  rect<T> padded(T horiz, T vert) const {
    return rect<T>(topLeft.x + horiz, topLeft.y + vert, botRight.x - horiz,
                   botRight.y - vert);
  }

  rect<T> padded(T x1, T y1, T x2, T y2) const {
    return rect<T>(topLeft.x + x1, topLeft.y + y1, botRight.x - x2,
                   botRight.y - y2);
  }

  rect<T> resized(T w = 0, T h = 0, double horizAlign = 0.0,
                  double vertAlign = 0.0) const {
    rect<T> result = *this;
    if (w != 0) {
  ...
  rect<T> aspectAligned(double aspect, double horizAlign = 0.5,
  ...
  rect<T> clipAgainst(const rect<T> &other) const {
    return rect<T>(std::max(topLeft.x, other.topLeft.x),
  ...
  rect<T> clipProportional(const rect<T> &from, const rect<T> &to) const {
    ...
    return rect<T>(_clip(topLeft.x, size.width, from.topLeft.x, to.topLeft.x,
                         otherSize.width),
                   _clip(topLeft.y, size.height, from.topLeft.y, to.topLeft.y,
                         otherSize.height),
                   _clip(botRight.x, size.width, from.botRight.x, to.botRight.x,
                         otherSize.width),
                   _clip(botRight.y, size.height, from.botRight.y,
                         to.botRight.y, otherSize.height));
  }
};
...
template <class T> struct relpos {
  ...
  void setOffset(T value) { pos = value; }

  void setPercentage(double value) { percent = value; }
...
template <class T> struct relrect {
  relpos<T> left, top;
