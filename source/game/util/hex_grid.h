...
template <class T = bool> struct HexGrid {
  T *data;
  ...
  HexGrid(vec2u size)
      : data(new T[size.width * size.height]), width(size.width),
        height(size.height) {}

  HexGrid(unsigned Width, unsigned Height)
      : data(new T[Width * Height]), width(Width), height(Height) {}

  HexGrid(const HexGrid<T> &other) : data(0), width(0), height(0) {
    *this = other;
  }
  ...
  vec2u size() const { return vec2u(width, height); }
  ...
  void zero() { memset(data, 0, sizeof(T) * width * height); }
  ...
  bool valid(const vec2u &pos) const { return pos.x < width && pos.y < height; }
  ...
  T &get(unsigned x, unsigned y) { return data[x * height + y]; }

  const T &get(unsigned x, unsigned y) const { return data[x * height + y]; }
  ...
  T &get(vec2u pos) { return data[pos.x * height + pos.y]; }

  const T &get(vec2u pos) const { return data[pos.x * height + pos.y]; }
  ...
  const T &operator[](vec2u pos) const { return data[pos.x * height + pos.y]; }

  T &operator[](vec2u pos) { return data[pos.x * height + pos.y]; }
  ...
  static bool advancePosition(vec2u &pos, const vec2u &size,
                              HexGridAdjacency direction, unsigned amount = 1) {
  ...
  // If only one dimension is a valid destination, it will move in that
  // dimension
  bool advance(unsigned &x, unsigned &y, HexGridAdjacency direction,
               unsigned amount = 1) const {
    vec2u pos(x, y);
  ...
  bool advance(vec2u &pos, HexGridAdjacency direction,
               unsigned amount = 1) const {
