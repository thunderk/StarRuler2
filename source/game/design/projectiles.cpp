...
struct ProjEffect : public ObjectMessage {
  ...
  ProjEffect(const Effector *fctr, Object *target, Object *src,
             vec3d relImpactPt, float eff, float part, float del)
      : ObjectMessage(target), source(src), effector(fctr),
        relImpact(relImpactPt), effectiveness(eff), partiality(part),
        delay(del) {
    source->grab();
    effector->grab();
  }
  ...
  void process() override {
    effector->triggerEffect(source, object, relImpact, effectiveness,
                            partiality, delay);
  }
};
...
struct ProjImpactEffect : public scene::NodeEvent {
  ...
  ProjImpactEffect(scene::Node *parentNode,
                   const scene::ParticleSystemDesc *sys, const vec3d &From,
                   const vec3d &Vel, float Scale, float Delay)
      : NodeEvent(parentNode), system(sys), from(From), vel(Vel), scale(Scale),
        delay(Delay) {}
  ...
  void process() override {
    ...
    auto *pSys =
        scene::playParticleSystem(system, node, pos, rot, vel, scale, delay);
    if (pSys)
      pSys->drop();
  }
...
Object *findTarget(Empire *owner, const vec3d &position, double radius,
                   const void *ofType = nullptr) {
  if (!owner)
    return nullptr;
  ...
  devices.physics->findInBox(
      ...
      [&obj, owner, &position, radius, ofType](const PhysicsItem &item) {
        if (obj)
          return;
        if (item.type == PIT_Object) {
          ...
          if (ofType && target->type != ofType)
            return;
          if (position.distanceToSQ(target->position) >
...
bool Projectile::tick(double time) {
  ...
  if (type == PT_Missile) {
    if (target && target->isValid()) {
      if (recover <= 0.f) {
        ...
        if (target->type->blueprintOffset != 0) {
          auto *blueprint =
              (Blueprint *)(((size_t)target) + target->type->blueprintOffset);
          ...
          if (dsg != nullptr) {
            ...
            projImpact =
                target->rotation *
                    dsg->hull->getImpact(impactOffset, target->radius, true) +
                target->position;
          }
        }
        ...
        if (angle < rot) {
          ...
          quaterniond dirq =
              quaterniond::fromImpliedTransform(vec3d::front(), dir);
          quaterniond vdirq =
              quaterniond::fromImpliedTransform(vec3d::front(), vdir);
          ...
          //    -- It really is not :( stupid missiles can't even turn 180
          //    degrees
      ...
      if (target) {
        ...
        target =
            findTarget(source->owner, target->position, 50.0, target->type);
        prevTarget->drop();
      } else {
    ...
    if (target) {
      if (target->isValid()) {
        if (tracking > 0) {
          vec3d targDir =
              (target->position - (source->position + position)).normalize();
  ...
  // If our line still collides with our last impacted object, we can check for
  // only intervening objects
  ...
  if (mode != PM_OnlyHitsTarget) {
    if (mode == PM_PassthroughInvalid) {
      ...
      devices.physics->findInBox(
          AABBoxd(line, scale),
          [&other, &line, &prevDistSQ, &impactPt, projScale, eff,
           src](const PhysicsItem &item) {
            // if(item.type != PIT_Object)
            //	return;
            ...
            if (distSQ <= width * width && (!eff || eff->canTarget(src, obj))) {
              ...
              if (!other || objDistSQ < prevDistSQ) {
                ...
                impactPt = closePt -
                           line.getDirection() * sqrt(width * width - distSQ);
              }
            }
          },
      ...
      devices.physics->findInBox(
          AABBoxd(line, scale),
          [&other, &line, &prevDistSQ, &impactPt,
           projScale](const PhysicsItem &item) {
            // if(item.type != PIT_Object)
            //	return;
            Object *obj = item.object;
            vec3d closePt = line.getClosestPoint(obj->position, false);
  ...
  if (other != nullptr && other->type->blueprintOffset != 0) {
    auto *blueprint =
        (Blueprint *)(((size_t)other) + other->type->blueprintOffset);
    const Design *dsg = blueprint->design;
    ...
    if (dsg != nullptr) {
      ...
      impactPt =
          other->rotation * dsg->hull->getImpact(impactOffset, other->radius,
                                                 type == PT_Beam) +
          other->position;
    }
  }
  ...
  if (other) {
    ...
    if (effector) {
      if (player && other->isVisibleTo(player)) {
        if (auto *sfx =
                effector->type.skins[effector->skinIndex].impact_sound) {
          if (sfx->loaded && !audio::disableSFX) {
            auto *sound = devices.sound->play3D(
                sfx->source, snd_vec(other->position), false, true);
            if (sound) {
              if (source)
    ...
    if (effector && lockGroup) {
      ...
      if (effector->type.skins[effector->skinIndex].impact &&
          (!player || other->isVisibleTo(player)))
        scene::queueNodeEvent(new ProjImpactEffect(
            other->node, effector->type.skins[effector->skinIndex].impact,
            impactPt, other->velocity,
            (float)sqrt(effector->relativeSize *
                        (source ? source->radius : 1.0)),
            delay));
      if (!devices.network->isClient)
        lockGroup->addMessage(new ProjEffect(
            effector, other, source, impactPt - other->position, efficiency,
            type == PT_Beam ? (float)time : 1.f, delay));
      else
        other->drop();
    } else {
      other->drop();
...
void *Projectile::operator new(size_t size) { return projPool.alloc(); }

void Projectile::operator delete(void *p) {
  return projPool.dealloc((Projectile *)p);
}

Projectile::Projectile(ProjType Type)
    : lastTick(devices.driver->getGameTime()), source(0), target(0),
      lastImpact(0), graphics(0), type(Type), impact(0), efficiency(1.f),
      endNotice(0), mode(PM_Normal), recover(0.f) {}
...
Projectile *Projectile::load(SaveFile &file) {
  ...
  if ((bool)file) {
    ...
    if (dsg == nullptr || subsysIndex >= dsg->subsystems.size() ||
      ...
      efftr =
          &dsg->subsystems[subsysIndex]
               .effectors[effectorIndex]; // No refcounting on design effectors
    }
  } else {
    effId = file;
  ...
  // Create graphics
  /*proj->graphics = proj->effector->type.createGraphics(proj->effector,
  proj->scale); if(!proj->graphics) { delete proj; throw "Invalid projectile
  effector graphics.";
  }
...
std::vector<Projectile *> *source = new std::vector<Projectile *>,
                          *dest = new std::vector<Projectile *>;
...
public:
  ...
  ~ProcessProjectiles() {
    // This happens when the game is ending. We may not complete an entire cycle
    // at the right time.
    if (host)
  ...
  bool run() {
    ...
    while (tickMax--) {
      ...
      // Take the next projectile, tick it, and put it on the output stack
      // (unless it needs deleted)
...
void resumeProjectiles() { PauseProjectiles = false; }
