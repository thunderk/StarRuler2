...
std::string path_join(const std::string &one, const std::string &two) {
  ...
  return one.substr(0, endOfPath) + "/" +
         two.substr(startOfPath, std::string::npos);
}
...
bool path_inside(const std::string &folder, const std::string &subpath) {
  ...
  std::string prefix = getAbsolutePath(folder),
              toPath = getAbsolutePath(subpath);
...
bool listDirectory(const std::string &dir, std::vector<std::string> &out,
                   const char *filter) {
  auto full_dir = dir + "\\*";
...
std::string getBasename(const std::string &filename, bool includeExtension) {
  ...
  std::string name =
      (dir_end == filename.npos) ? filename : filename.substr(dir_end + 1);
...
threads::threadreturn threadcall WatchDir(void *arg) {
  ...
  HANDLE dir =
      CreateFile(d->path.c_str(), FILE_LIST_DIRECTORY,
  ...
  while (true) {
    ReadDirectoryChangesW(dir, &buffer, sizeof(buffer), false,
                          FILE_NOTIFY_CHANGE_LAST_WRITE, &r, NULL, NULL);

    threads::Lock lock(watch_mtx);
    size_t ind = 0;
    while (ind < r) {
      FILE_NOTIFY_INFORMATION *pevt = (FILE_NOTIFY_INFORMATION *)&buffer[ind];
      std::wstring filename_w((wchar_t *)pevt->FileName,
                              pevt->FileNameLength / 2);

      std::string filename;
      filename.reserve(filename_w.size());
