...
struct XInputLibrary {
  ...
  bool load() {
    // There are three current variants of the library, we try to get the most
    // recent one XInput1_4.dll from Window 8 XInput1_3.dll from DirectX
    ...
    xInputGetState =
        (decltype(xInputGetState))GetProcAddress(dll, "XInputGetState");
    xInputSetState =
        (decltype(xInputSetState))GetProcAddress(dll, "XInputSetState");
...
public:
  ...
  bool poll() {
    if (isXInput) {
      ...
      if (xinput.getState(index, state)) {
        ...
        buttonUpdate(9, state.Gamepad.bLeftTrigger >
                            XINPUT_GAMEPAD_TRIGGER_THRESHOLD);
        buttonUpdate(10, state.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_THUMB);
        buttonUpdate(11,
                     state.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER);
        buttonUpdate(12, state.Gamepad.bRightTrigger >
                             XINPUT_GAMEPAD_TRIGGER_THRESHOLD);
        ...
        axisCount = 6;
        axes[0] = deadZone(state.Gamepad.sThumbLX,
                           XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, 32768);
        axes[1] = deadZone(state.Gamepad.sThumbLY,
                           XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, 32768);
        axes[2] = deadZone(state.Gamepad.sThumbRX,
                           XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE, 32768);
        axes[3] = deadZone(state.Gamepad.sThumbRY,
                           XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE, 32768);
        axes[4] = deadZone(state.Gamepad.bLeftTrigger,
                           XINPUT_GAMEPAD_TRIGGER_THRESHOLD, 255);
        axes[5] = deadZone(state.Gamepad.bRightTrigger,
                           XINPUT_GAMEPAD_TRIGGER_THRESHOLD, 255);
      ...
      // Statically figured out button/axes numbers from the xboxdrv linux
      // driver.
      int btnCnt;
      ...
      auto buttonFromAxis = [&](unsigned btn, unsigned index, float thresMin,
                                float thresMax) {
        if ((int)index >= axCnt) {
          buttons[btn] = JBS_Off;
          return;
        ...
        setButton(btn,
                  glfwAxes[index] >= thresMin && glfwAxes[index] <= thresMax);
      };
  ...
  bool connected() const { return glfwJoystickPresent(index) == GL_TRUE; }
...
void RegisterJoystickBinds() {
  ClassBind joystick("Joystick", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_C,
                     sizeof(Joystick));
  ...
  joystick.addMethod("float get_axis(uint index) const",
                     asMETHOD(Joystick, getAxis));
  joystick.addMethod("uint8 get_button(uint index) const",
                     asMETHOD(Joystick, getButton));
  joystick.addMethod("bool get_pressed(uint index) const",
                     asMETHOD(Joystick, getPressed));
  joystick.addMethod("void setVibration(float low, float high) const",
                     asMETHOD(Joystick, setVibration));
