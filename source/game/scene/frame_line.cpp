...
namespace scene {
FrameLineNode::FrameLineNode(const render::RenderState &material,
                             float duration)
    : mat(material), interpDuration(duration), age(0), life(1000.f),
      fadeAge(1000.f) {
...
void FrameLineNode::render(render::RenderDriver &driver) {
  ...
  auto *verts =
      render::VertexBufferTCV::fetch(&mat)->request(1, render::PT_Lines);

  float alpha = age > fadeAge
                    ? 1.f - std::min((age - fadeAge) / (life - fadeAge), 1.f)
                    : 1.f;
