...
template <class T> struct AABBox {
  ...
  AABBox(const vec3<T> &minvec, const vec3<T> &maxvec)
      : minimum(minvec), maximum(maxvec) {}
  AABBox(T minX, T minY, T minZ, T maxX, T maxY, T maxZ)
      : minimum(minX, minY, minZ), maximum(maxX, maxY, maxZ) {}
  AABBox(const line3d<T> &line) : minimum(line.start), maximum(line.end) {
    fix();
  }

  AABBox(const line3d<T> &line, double width)
      : minimum(line.start), maximum(line.end) {
    fix();
    minimum -= vec3d(width);
    maximum += vec3d(width);
  }
  ...
  bool operator!=(const AABBox &other) const { return !(*this == other); }

  vec3<T> getSize() const { return maximum - minimum; }

  vec3<T> getCenter() const { return (minimum + maximum) * 0.5; }
  ...
  bool isWithin(const AABBox &other) const {
    return minimum.x >= other.minimum.x && minimum.y >= other.minimum.y &&
  ...
  bool overlaps(const AABBox &other) const {
    if (minimum.x > other.maximum.x || minimum.z > other.maximum.z ||
        maximum.x < other.minimum.x || maximum.z < other.minimum.z ||
...
#define BBOX_POINTDIM(dim)                                                     \
  if (point.dim < minimum.dim)                                                 \
    minimum.dim = point.dim;                                                   \
  else if (point.dim > maximum.dim)                                            \
    maximum.dim = point.dim;
...
#define BBOX_BOXDIM(dim)                                                       \
  if (box.minimum.dim < minimum.dim)                                           \
    minimum.dim = box.minimum.dim;                                             \
  if (box.maximum.dim > maximum.dim)                                           \
    maximum.dim = box.maximum.dim;
