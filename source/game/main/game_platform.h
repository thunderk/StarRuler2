...
public:
  ...
  virtual void logException(unsigned code, void *exception, unsigned version,
                            const char *comment) = 0;

  // Servers
  virtual void announceServer(unsigned ip4, unsigned short port,
                              const std::string &password) = 0;
  virtual void announceDisconnect() = 0;
  virtual void inviteFriend() = 0;
  virtual std::string getLobbyConnectAddress(const std::string &lobbyID,
                                             std::string *pwd = nullptr) = 0;
  virtual uint64_t getLobby() = 0;
  virtual void joinLobby(uint64_t id) = 0;

  virtual void enterQueue(const std::string &type, unsigned players,
  ...
  // Cloud Files
  virtual void addCloudFolder(const std::string &home,
                              const std::string &folder) = 0;
  virtual void writeCloudFile(const std::string &filename,
                              const std::string &cloudname) = 0;
