  ...
  if (source) {
    ...
    try {
      ...
      if (source) {
        ...
        audio::ISound *sound =
            devices.sound->play2D(source, loop, true, priority);
        if (sound) {
          sound->grab();
          if (!pause)
            sound->resume();
...
audio::ISound *Sound::play3D(const vec3d &pos, bool loop, bool pause,
                             bool priority) const {
  if (!loaded)
    return nullptr;
  ...
  if (source) {
    auto *ptr =
        devices.sound->play3D(source, snd_vec(pos), loop, true, priority);
    if (ptr) {
      ptr->grab();
      if (!pause)
        ptr->resume();
...
bool Library::hasQueuedSounds() { return !queuedSounds.empty(); }
...
void Library::loadSounds(const std::string &filename) {
    ...
    if (load_resources && use_sound) {
      if (streamed) {
        ...
        if (fileExists(snd->streamSource))
          ...
          error("Could not locate streaming sound source: '%s'",
                sound_file.c_str());
      } else {
        resource::QueuedSound queued = {
            priority, snd, devices.mods.resolve(sound_file), volume};
  ...
  datahandler("File", [&](std::string &value) { sound_file = value; });

  datahandler("Volume",
              [&](std::string &value) { volume = toNumber<float>(value); });
