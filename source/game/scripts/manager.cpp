...
static bool loadCached(Module &mod, const File &fl,
                       const std::string &cache_file, unsigned cache_version);
static void saveCached(Module &mod, const std::string &cache_file,
...
asIScriptFunction *Manager::getFunction(const std::string &def, const char *fin,
                                        const char *ret) {
...
Manager::Manager(asIJITCompiler *Jit)
    : prevGCSize(5000), pauseScripts(false), clearScripts(false) {
  // Create the engine
...
void Manager::addIncludePath(std::string path) { includePaths.push_back(path); }
...
public:
  ...
  CachedScript(const std::string &filename)
      : loaded(false), path(filename), size(0) {}
...
const std::vector<std::string> &loadScript(const std::string &filename,
                                           size_t &size) {
...
void parseFile(Manager *man, File &fl, const std::string &filename,
  ...
  reg_compile(pre_import_from,
              "^[ \t]*from[ \t]+([A-Za-z0-9._-]+)[ \t]+import[ \t]+([^;]+);");
  ...
  for (auto iLine = lines.begin(), end = lines.end(); iLine != end; ++iLine) {
    ...
    if (mayBeDirective && reg_match(line, match, pre_priority)) {
      ...
      std::string includeFile =
          getAbsolutePath(devices.mods.resolve(filePart, getDirname(filename)));
      ...
      if (!fileExists(includeFile)) {
        ...
        foreach (it, man->includePaths) {
          includeFile = getAbsolutePath(devices.mods.resolve(filePart, *it));
...
void importSymbol(Module &mod, Module &otherMod, const std::string &symbol) {
  ...
  glob = other->importedGlobals.GetFirstIndex(self->defaultNamespace,
                                              symbol.c_str());
  if (glob != -1) {
    mod.globalExports.push_back(other->scriptGlobals.GetSize() +
                                (unsigned)glob);
    return;
  }
  ...
  {
    const asCArray<unsigned int> &idxs = other->globalFunctions.GetIndexes(
        other->defaultNamespace, symbol.c_str());
  ...
  {
    const asCArray<unsigned int> &idxs =
        other->importedGlobalFunctions.GetIndexes(other->defaultNamespace,
                                                  symbol.c_str());
    for (unsigned i = 0; i < idxs.GetLength(); ++i) {
  ...
  if (!foundFunction) {
    ...
    auto *getAcc = (asCScriptFunction *)other->GetFunctionByName(
        (std::string("get_") + symbol).c_str());
    auto *setAcc = (asCScriptFunction *)other->GetFunctionByName(
        (std::string("set_") + symbol).c_str());
...
void buildModule(Manager *man, File &fl, Module &mod) {
  ...
  mod.module->AddScriptSection(fl.path.c_str(), fl.contents.c_str(),
                               fl.contents.size());
  ...
  foreach (inc, fl.includes) {
    mod.module->AddScriptSection(inc->second->path.c_str(),
                                 inc->second->contents.c_str(),
                                 inc->second->contents.size());
    ...
    fl.priority_render =
        std::max(fl.priority_render, inc->second->priority_render);
...
bool compileModule(Manager *man, Module &mod) {
  ...
  foreach (it, mod.imports) {
    ...
    if (f == man->modules.end()) {
      error("ERROR: %s: Cannot find module '%s' to import from.",
    ...
    if (impMod.compiling) {
      error("ERROR: %s: Cannot import from module '%s', circular import.",
            mod.name.c_str(), it->first.c_str());
    ...
    if (it->second == "*") {
      ...
      if (!impMod.exports.empty()) {
        ...
        for (unsigned i = 0, cnt = other->importedClassTypes.GetLength();
             i < cnt; ++i)
          self->ImportType(other->importedClassTypes[i]);

        for (unsigned i = 0, cnt = other->importedEnumTypes.GetLength();
             i < cnt; ++i)
          self->ImportType(other->importedEnumTypes[i]);

        for (unsigned i = 0, cnt = other->importedTypeDefs.GetLength(); i < cnt;
             ++i)
          self->ImportType(other->importedTypeDefs[i]);

        for (unsigned i = 0, cnt = other->importedFuncDefs.GetLength(); i < cnt;
             ++i)
        ...
        for (unsigned i = 0, cnt = other->importedGlobalFunctions.GetSize();
             i < cnt; ++i) {
          auto *func =
              (asCScriptFunction *)other->importedGlobalFunctions.Get(i);
          self->ImportGlobalFunction(func);
        }
        ...
        for (unsigned i = 0, cnt = other->importedGlobals.GetSize(); i < cnt;
             ++i)
  ...
  if (mod.module) {
    ...
    foreach (exp, mod.exports) {
      ...
      glob = self->importedGlobals.GetFirstIndex(self->defaultNamespace,
                                                 symbol.c_str());
      if (glob != -1) {
        mod.globalExports.push_back(self->scriptGlobals.GetSize() +
                                    (unsigned)glob);
        continue;
      }
      ...
      {
        const asCArray<unsigned int> &idxs = self->globalFunctions.GetIndexes(
            self->defaultNamespace, symbol.c_str());
        for (unsigned i = 0; i < idxs.GetLength(); ++i) {
      ...
      if (!foundExports) {
        // Check if it's a global accessor
        auto *getAcc = (asCScriptFunction *)self->GetFunctionByName(
            (std::string("get_") + symbol).c_str());
        auto *setAcc = (asCScriptFunction *)self->GetFunctionByName(
            (std::string("set_") + symbol).c_str());
        ...
        if (getAcc || setAcc) {
          ...
          error("ERROR: %s: Cannot find '%s' to export.", mod.name.c_str(),
                symbol.c_str());
        }
      }
    }
...
void bindImports(Manager *man, Module &mod) {
  ...
  for (unsigned i = 0; i < fcnt; ++i) {
    ...
    if (it == man->modules.end()) {
      error("ERROR: '%s': Cannot import function '%s': module '%s' does not "
            "exist.",
    ...
    if (func == 0) {
      error("ERROR: '%s': Cannot import function '%s' from module '%s': "
...
void sortPriorities(Manager *man) {
  ...
  for (auto it = man->modules.begin(); it != man->modules.end(); ++it) {
    ...
    if (mod.callbacks[SC_init] || mod.callbacks[SC_preInit] ||
        mod.callbacks[SC_postInit])
      man->priority_init.insert(
          std::pair<int, Module *>(-fl.priority_init, &mod));

    if (mod.callbacks[SC_render])
      man->priority_render.insert(
          std::pair<int, Module *>(-fl.priority_render, &mod));

    if (mod.callbacks[SC_draw])
      man->priority_draw.insert(
          std::pair<int, Module *>(-fl.priority_draw, &mod));

    if (mod.callbacks[SC_tick])
      man->priority_tick.insert(
          std::pair<int, Module *>(-fl.priority_tick, &mod));

    if (mod.callbacks[SC_sync_initial])
      man->priority_sync.insert(
          std::pair<int, Module *>(-fl.priority_sync, &mod));
  }
}
  ...
  for (auto it = files.begin(); it != files.end(); ++it) {
    ...
    if (i != 2) {
      warn("WARNING: File '%s' has duplicate module name '%s'."
           " Using temporary module name '%s' instead.",
           fl.path.c_str(), fl.module.c_str(), modname.c_str());
    }
...
void renderRebuildSet(Manager *man, Module &curMod,
...
void visitRebuildSet(Manager *man, Module &mod,
                     std::unordered_set<Module *> &totalSet,
  ...
  foreach (it, modules) {
    ...
    if (mod.tickTime > 0 || mod.drawTime > 0 || mod.renderTime > 0) {
      print(" %s -- tick: %.3gms -- draw: %.3gms -- render: %.3gms",
            mod.name.c_str(), mod.tickTime * 1000.0, mod.drawTime * 1000.0,
            mod.renderTime * 1000.0);
    }
  }
}
#endif
...
int Manager::garbageCollect(bool full) {
  ...
  if (full /* || size > prevGCSize + (prevGCSize / 2)*/) {
    ...
    asUINT runs =
        (asUINT)(log((double)(newObjects + detected + 2)) / log(2.0)) * 10;
    ...
    asUINT runs =
        (asUINT)(log((double)(newObjects + detected + 2)) / log(2.0)) * 50;
...
void Manager::scriptThreadCreate() { ++scriptThreadsExistent; }

void Manager::scriptThreadDestroy() { --scriptThreadsExistent; }
...
void Manager::resumeScriptThreads() { pauseScripts = false; }
...
void Manager::scriptThreadEnd() { --scriptThreadsActive; }

Call::Call() : manager(0), ctx(0), arg(0), nested(false) {}
...
struct ReadBytecode : public asIBinaryStream {
  ...
  ReadBytecode(std::ifstream &is) : stream(is) {}
...
struct WriteBytecode : public asIBinaryStream {
  ...
  WriteBytecode(std::ofstream &os) : stream(os) {}
