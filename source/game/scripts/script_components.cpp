...
extern void writeObject(net::Message &msg, Object *obj,
...
void logAsyncLoad() {
  int counts[] = {callCount[0].get_basic(), callCount[1].get_basic(),
                  callCount[2].get_basic(), callCount[3].get_basic()};
  float total = float(counts[0] + counts[1] + counts[2] + counts[3]) / 100.f;

  print("  0us- 10us: %d (%.0f%%)\n 10us-100us: %d (%.0f%%)\n100us-  1ms: %d "
        "(%.0f%%)\n  1ms-  inf: %d (%.0f%%)",
        counts[0], float(counts[0]) / total, counts[1],
        float(counts[1]) / total, counts[2], float(counts[2]) / total,
        counts[3], float(counts[3]) / total);
}
#else
void logAsyncLoad() { print("Async profiling disabled."); }
#endif
...
std::unordered_map<const StateValueDefinition *, std::pair<Component *, bool>>
    componentDefs;

Threaded(bool) RELOCK_SAFE_METHODS = false;
Threaded(bool) PREVENT_RELOCKING = false;

unsigned getComponentCount() { return (unsigned)components.size(); }
  ...
  while (true) {
    ...
    if (!inType) {
      ...
      if (!curFlags.local && desc.returnType.type) {
        error("Error: '%s': non-local function cannot return a value.",
              parse.c_str());
        continue;
      } else if (desc.returnType == GT_Custom_Handle) {
        error("Error: '%s': script types cannot be return values.",
              parse.c_str());
        continue;
      }
...
public:
  ...
  AsyncScriptCall(WrappedMethod *Method, GenericCallData &arguments,
                  size_t offset, bool FromGUI, GenericValue *pReturn = 0
#ifdef LOG_SLOW_ASYNC_CALLS
                  ,
                  double *RetTime = 0
#endif
                  )
      : ObjectMessage((Object *)arguments.object), method(Method),
        args(arguments), compOffset(offset), fromGUI(FromGUI),
        retValue(pReturn) {
    object->grab();
#ifdef LOG_SLOW_ASYNC_CALLS
    retTime = RetTime;
  ...
  void process() {
    ...
    if (!script ||
        (fromGUI && method->restricted &&
         !devices.network->getCurrentPlayer().controls(object->owner))) {
      if (retValue) {
        retValue->asyncReturned = true;

#ifdef LOG_SLOW_ASYNC_CALLS
        if (retTime)
          *retTime = devices.driver->getAccurateTime();
#endif
      }
      return;
    }
  ...
  if (method->isComponentMethod) {
    ...
    if (offset == 0) {
      throwException(format("Object of type '$1' does not have component '$2'.",
                            obj->type->name, getComponent(method->compId)->name)
                         .c_str());
      return retVal;
    }
  ...
  if (method->func) {
    ...
    if (PREVENT_RELOCKING) {
      if (method->relocking && (!method->async || hasReturn)) {
        throwException(
            "Cannot call relocking method from non-relocking outer function.");
        return retVal;
      }
      if (hasReturn && (!method->safe || RELOCK_SAFE_METHODS) && activeGroup &&
          obj != getActiveObject()) {
        throwException(
            "Cannot call methods with return data on different objects unless "
            "the outer function is declared relocking.");
        return retVal;
      }
    }
    ...
    if ((obj->isLocked() || (method->safe && !RELOCK_SAFE_METHODS)) &&
        !method->async && initialized) {
      Call cl = devices.scripts.server->call(method->func);
      cl.setObject(script);
      if (method->passPlayer) {
        if (isGUI)
      ...
      if (method->restricted && isGUI) {
        if (!devices.network->getCurrentPlayer().controls(obj->owner)) {
          info(format("Cannot call method '$1' on unowned objects.",
                      method->wrapped.name)
                   .c_str());
          return retVal;
        }
      ...
      if (hasReturn) {
        if (initialized) {
          ...
          obj->lockGroup->addMessage(new AsyncScriptCall(method, args, offset,
                                                         isGUI, &retVal
#ifdef LOG_SLOW_ASYNC_CALLS
                                                         ,
                                                         &retTime
          ...
          // Attempt to process the message immediately where possible
          // (typically the GUI, which needs the fast response)
          ...
          if (!activeGroup) {
            ...
            // Fast path: Often, the ret value will have been produced by the
            // end of the loop
            unsigned spins = 0;
            while (!retVal.asyncReturned) {
              ++spins;
            ...
            // Slow path: Perhaps it can't be resolved, or will take a lot
            // longer than we'd like
            //  This can involve a lot of other messages taking a very long time
            //  to return
            while (!retVal.asyncReturned) {
#ifdef LOG_SLOW_ASYNC_CALLS
              wasFast = false;
...
#ifdef LOG_SLOW_ASYNC_CALLS
          if (dur > 1.0e-3) {
            error("Long wait (%.1fms, wasted %.1fms%s) for %s",
                  (end - start) * 1.0e3, (end - retTime) * 1.0e3,
                  (activeGroup && wasFast) ? " fast" : "",
                  method->func->GetName());
        ...
        obj->lockGroup->addMessage(
            new AsyncScriptCall(method, args, offset, isGUI));
      } else {
        obj->queueDeferredMessage(
            new AsyncScriptCall(method, args, offset, isGUI));
      }
    }
...
void objectWait(Object *obj) {
  // Check relocking requirement
  if (PREVENT_RELOCKING) {
    throwException("Cannot call relocking Object.wait() method from "
                   "non-relocking outer function.");
    return;
  }
...
bool getSafeCallWait() { return RELOCK_SAFE_METHODS; }

void setSafeCallWait(bool wait) { RELOCK_SAFE_METHODS = wait; }
...
void RegisterObjectComponentWrappers(ClassBind &obj, bool server,
  ...
  foreach (it, components) {
...
#ifdef DOCUMENT_API
        int fid = bindGeneric(obj, method->wrapped, wrapperFunc, (void *)method,
                              true);
        auto *doc = new Documentor(fid, method->fullDeclaration);
        doc->documented = !method->doc_desc.empty() ||
                          !method->doc_return.empty() ||
                          !method->doc_args.empty();
    ...
    if (type) {
      ...
      if (type->optionalComponents[comp.id]) {
        obj.addGenericMethod(
            format("bool get_has$1() const", comp.name).c_str(),
            asFUNCTION(hasComponent), &comp)
            doc("Returns true if the object has the associated component.", "");

        obj.addGenericMethod(format("void activate$1()", comp.name).c_str(),
                             asFUNCTION(activateComponent), &comp)
            doc("Activates an optional component if not already activated.");

        obj.addGenericMethod(format("void deactivate$1()", comp.name).c_str(),
                             asFUNCTION(deactivateComponent), &comp)
      ...
      obj.addGenericMethod(
          ...
          asFUNCTION(getKnownComponentPtr),
          (void *)(size_t)type->componentOffsets[comp.id])
          doc("Returns the component handle for this type of component.", "");
    } else {
      // Check whether a component exists
      obj.addGenericMethod(format("bool get_has$1() const", comp.name).c_str(),
                           asFUNCTION(hasComponent), &comp)
          doc("Returns true if the object has the associated component.", "");
      ...
      obj.addGenericMethod(
          format("Component_$1@ get_$1()", comp.name, comp.name).c_str(),
          asFUNCTION(getComponentPtr), &comp)
          doc("Returns the component handle for this type of component.",
              "Can be null if no component of this type exists for this "
              "object.");
    }
  }
}
...
public:
  AsyncEmpireScriptCall(WrappedMethod *Method, GenericCallData &arguments,
                        size_t offset, bool FromGUI, GenericValue *pReturn = 0)
      : method(Method), args(arguments), compOffset(offset), fromGUI(FromGUI),
        retValue(pReturn) {}

  void process(Empire *emp) override {
    asIScriptObject *script = *(asIScriptObject **)((size_t)emp + compOffset);

    if (!script || (fromGUI && method->restricted &&
                    !devices.network->getCurrentPlayer().controls(emp))) {
      if (script) // Attempted to call restricted method
        info(format("Cannot call method '$1' on other empires.",
                    method->wrapped.name)
                 .c_str());
      if (retValue)
        retValue->asyncReturned = true;
...
static GenericValue emp_wrapperFunc(void *arg, GenericCallData &args) {
  ...
  if (offset == 0) {
    throwException(format("Empire does not have component '$2'.",
                          getComponent(method->compId)->name)
                       .c_str());
    return retVal;
  }
    ...
    if (!method->async || !activeGroup) {
      ...
      if (PREVENT_RELOCKING) {
        if (hasReturn || method->relocking) {
          throwException("Cannot call relocking method from non-relocking "
                         "outer function.");
          return retVal;
        }
      }

      emp->queueMessage(new AsyncEmpireScriptCall(
          method, args, offset, isGUI, hasReturn ? &retVal : nullptr));
...
void BindEmpireComponentOffsets() {
  ...
  for (auto var = def.types.begin(), endvar = def.types.end(); var != endvar;
       ++var) {
    auto &type = *var->def;
    Component *comp = getComponent(&type);
    if (comp) {
      if (empComponentOffsets[comp->id] != 0)
        error("ERROR: Empire has two components of type '%s'.",
              comp->name.c_str());
      else
...
void RegisterEmpireComponentWrappers(ClassBind &emp, bool server) {
  ...
  foreach (it, components) {
    ...
    foreach (m, comp.methods) {
      ...
      if (server || (!method->server && !method->shadow)) {
#ifdef DOCUMENT_API
        int fid = bindGeneric(emp, method->wrapped, emp_wrapperFunc,
                              (void *)method, true);
        auto *doc = new Documentor(fid, method->fullDeclaration);
        doc->documented = !method->doc_desc.empty() ||
                          !method->doc_return.empty() ||
        ...
        doc->argDoc = method->doc_args;
#else
        bindGeneric(emp, method->wrapped, emp_wrapperFunc, (void *)method,
                    true);
#endif
      }
    }
    ...
    // Check whether a component exists
    int fid = getEngine()->RegisterObjectMethod(
        emp.name.c_str(), format("bool get_has$1() const", comp.name).c_str(),
...
public:
  AsyncNodeCall(scene::ScriptedNode *node, WrappedMethod *Method,
                GenericCallData &Args)
      : NodeEvent(node), method(Method), args(Args) {}
...
void RegisterNodeMethodWrappers(ClassBind &cls, bool server,
                                scene::scriptNodeType *type) {
...
void bindComponentClasses() {
  ...
  foreach (it, components) {
    ...
    comp.type =
        devices.scripts.server->getClass(args[0].c_str(), args[1].c_str());
    if (!comp.type) {
    ...
    foreach (m, comp.methods) {
      ...
      if (method->wrapped.constFunction) {
        ...
        desc.prepend(
            ArgumentDesc(comp.containingType, method->wrapped.constFunction));
      ...
      if (!devices.network->isClient || method->shadow ||
          (method->local && !method->server))
        error("Method not found: %s -> %s", comp.typeDecl.c_str(),
