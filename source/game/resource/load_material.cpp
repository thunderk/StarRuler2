...
struct QueuedTexture {
  ...
  QueuedTexture(render::Texture *dest, int Priority, Image *source, bool Mipmap,
                bool CachePixels)
      : priority(Priority), tex(dest), mipmap(Mipmap), cachePixels(CachePixels),
        row(0), lod(0) {
    images.push_back(source);
  }

  QueuedTexture(render::Texture *dest, int Priority, const std::string &source,
                bool Mipmap, bool CachePixels)
      : priority(Priority), tex(dest), filename(source), mipmap(Mipmap),
        cachePixels(CachePixels), row(0), lod(0) {}
...
bool Library::hasQueuedImages() { return !queuedImages.empty(); }
...
bool Library::hasQueuedTextures() { return !queuedTextures.empty(); }
...
bool QueuedTexture::load(unsigned maxBytes, unsigned &loadedBytes) {
  ...
  if(lod == 0/* && loadRows == img->height && img->width * img->height <= DRIVER_MIPMAP_SIZE*/) {
    ...
    tex->loadPartial(*img, recti(0, row, img->width, row + loadRows),
                     cachePixels, lod);
    row += loadRows;
...
bool Library::processTextures(int maxPriority, bool singleFrame) {
  ...
  while (!queuedTextures.empty()) {
    ...
    // Track filenames even if we can't load the resource at the moment, in case
    // the file is created while we're running
...
render::Texture *queueImage(const std::string &abs_file, int priority = -20,
  ...
  if (queue) {
    queuedImagesLock.lock();
    resource::QueuedTexture queued =
        resource::QueuedTexture(tex, priority, abs_file, mipmap, cachePixels);
...
render::Texture *queueImage(Image *img, int priority, bool mipmap,
                            bool cachePixels) {
  render::Texture *tex = render::RenderDriver::createTexture();

  queuedTexturesLock.lock();
  resource::QueuedTexture queued =
      resource::QueuedTexture(tex, priority, img, mipmap, cachePixels);
...
void queueTextureUpdate(render::Texture *tex, Image *img, int priority,
                        bool mipmap, bool cachePixels) {
  queuedTexturesLock.lock();
  resource::QueuedTexture queued =
      resource::QueuedTexture(tex, priority, img, mipmap, cachePixels);
...
void Library::loadMaterials(const std::string &filename) {
  ...
  datahandler("Template", [&](std::string &value) {
    ...
    if (it != materials.end())
      ...
      error("Could not find template material '%s' for '%s'", value.c_str(),
            matName.c_str());
  });
    ...
    devices.mods.listFiles(
        ...
        [&](const std::string &filename) {
          ...
          *mat = g.base;
          ...
          if (load_resources) {
            mat->textures[0] = queueImage(filename, texPriority, mat->mipmap,
                                          mat->cachePixels);
            if (watch_resources)
  ...
  datahandler("Inherit", [&](std::string &value) {
    ...
    if (it != materials.end())
      *state = *it->second;
      ...
      error("Could not find material '%s' to inherit for '%s'", value.c_str(),
            matName.c_str());
  });
  ...
  HANDLE_ENUM(datahandler, "WrapHorizontal", state, wrapHorizontal,
              textureWraps);
  ...
  datahandler("Diffuse", [&](std::string &value) {
    ...
    if (int args = sscanf(value.c_str(), "#%2x%2x%2x%2x", &r, &g, &b, &a)) {
      ...
      sscanf(value.c_str(), "%f,%f,%f,%f", &state->diffuse.r, &state->diffuse.g,
             &state->diffuse.b, &state->diffuse.a);
    }
  });
  ...
  datahandler("Specular", [&](std::string &value) {
    ...
    if (int args = sscanf(value.c_str(), "#%2x%2x%2x%2x", &r, &g, &b, &a)) {
      ...
      sscanf(value.c_str(), "%f,%f,%f,%f", &state->specular.r,
             &state->specular.g, &state->specular.b, &state->specular.a);
    }
  });
  ...
  datahandler.defaultHandler([&](std::string &key, std::string &value) {
    if (key.size() >= 7 && (key.compare(0, 7, "Texture") == 0 ||
                            key.compare(0, 7, "Cubemap") == 0)) {
      ...
      if (load_resources) {
        state->textures[texNum] = queueImage(value, priority, state->mipmap,
                                             state->cachePixels, key[0] == 'C');
        if (watch_resources)
          watchTexture(value);
      }
    }
