...
#define libraryDir(dir, type)                                                  \
  devices.mods.listFiles(dir, "*.txt", loadLibraryFile<type>);
...
asCJITCompiler *makeJITCompiler() {
  ...
  return new asCJITCompiler(flags | JIT_NO_SUSPEND | JIT_SYSCALL_FPU_NORESET |
                            JIT_ALLOC_SIMPLE | JIT_FAST_REFCOUNT);
#else
  return new asCJITCompiler(flags | JIT_SYSCALL_FPU_NORESET | JIT_ALLOC_SIMPLE |
                            JIT_FAST_REFCOUNT);
#endif
#else
  return 0;
...
void onScripts(
    ...
    std::function<void(const std::pair<std::string, std::string> &)> f) {
...
bool initGlobal(bool loadGraphics, bool createWindow) {
  ...
  try {
    if (use_sound) {
      ...
      if (audioDevice)
        devices.sound =
            audio::createAudioDevice(audioDevice->toString().c_str());
      else
...
void loadServerScripts() {
...
#ifdef PROFILE_LOCKS
  devices.scripts.cache_server->threadedCallMutex.name =
      "Server Threaded Call Lock";
  ...
  auto load = [](const std::pair<std::string, std::string> &file) {
    ...
    devices.scripts.cache_server->load(makeModuleName(file.first), file.second);
  };
  ...
  devices.scripts.cache_server->compile(
      devices.mods.getProfile("as_cache/server"), SERVER_SCRIPT_BUILD);
...
void loadShadowScripts() {
...
#ifdef PROFILE_LOCKS
  devices.scripts.cache_shadow->threadedCallMutex.name =
      "Shadow Threaded Call Lock";
  ...
  auto load = [](const std::pair<std::string, std::string> &file) {
    ...
    devices.scripts.cache_shadow->load(makeModuleName(file.first), file.second);
  };
  ...
  devices.scripts.cache_shadow->compile(
      devices.mods.getProfile("as_cache/shadow"), SERVER_SCRIPT_BUILD);
...
void loadClientScripts() {
  ...
  auto load = [](const std::pair<std::string, std::string> &file) {
    ...
    devices.scripts.client->load(makeModuleName(file.first), file.second);
  };
  ...
  double load_end = devices.driver->getAccurateTime();
  devices.scripts.client->compile(devices.mods.getProfile("as_cache/client"),
                                  CLIENT_SCRIPT_BUILD);
...
void startPreload() {
  ...
  Loading::addTask("ServerScripts", 0, [] { loadServerScripts(); });

  Loading::addTask("ShadowScripts", 0, [] { loadShadowScripts(); });

  Loading::addTask("ClientScripts", 0, [] { loadClientScripts(); });

  Loading::addTask("CleanupScripts",
                   "ServerScripts,ShadowScripts,ClientScripts", [] {
                     clearScriptList();
...
bool isPreloading() { return preloading; }
...
void cancelLoad() {
  ...
  while (idleSoundsActive) {
    devices.driver->handleEvents(1);
  }
  while (idleHullsActive) {
    devices.driver->handleEvents(1);
  }
...
void initMods(const std::vector<std::string> &mods) {
  ...
  Loading::addTask("CloudMods", "CloudInit", [] {
    ...
    if (devices.cloud) {
      CloudDownload dl;
      for (unsigned i = 0, cnt = devices.cloud->getDownloadedItemCount();
           i < cnt; ++i) {
        if (devices.cloud->getDownloadedItem(i, dl)) {
          ...
          if (folderNameStarts != std::string::npos)
            devices.mods.registerMod(
                dl.path, dl.path.substr(folderNameStarts + 1,
                                        folderNameEnds - folderNameStarts));
        }
      }
  ...
  Loading::addTask("ModInit", "CloudMods", [&] {
    ...
    std::string bindFile =
        path_join(devices.mods.getProfile("settings"), "keybinds.txt");
    ...
    std::string stFile =
        path_join(devices.mods.getProfile("settings"), "settings.txt");
      ...
      [] {
        if (create_window) {
          ...
          if (fullscreen) {
            ...
            auto *ovr =
                devices.settings.engine.getSetting("bOverrideResolution");
            if (ovr)
            ...
            auto *screenWidth =
                devices.settings.engine.getSetting("iResolutionX");
            windat.width = screenWidth ? *screenWidth : 1280;

            auto *screenHeight =
                devices.settings.engine.getSetting("iResolutionY");
            windat.height = screenHeight ? *screenHeight : 720;

            auto *fsCapture =
                devices.settings.engine.getSetting("bCursorCapture");
            cursorCapture = fsCapture ? *fsCapture : true;
          }
  ...
  if (load_resources) {
    Loading::addTask(
        "Errors", "Window", [] { devices.library.generateErrorResources(); },
        threads::getThreadID());
  }
  ...
  Loading::addTask("Components", "Generics", [] {
    devices.mods.listFiles(
        "data/components", "*.txt",
        [](const std::string &file) { scripts::loadComponents(file); });
  });
  ...
  Loading::addTask("Effects", "ModInit", [] {
    devices.mods.listFiles("data/effects", "*.txt", loadEffectDefinitions,
                           true);
  });
  Loading::addTask("Effectors", "Effects", [] {
    devices.mods.listFiles("data/effectors", "*.txt", loadEffectorDefinitions,
                           true);
  });
  Loading::addTask("SubSystems", "Effects,Effectors,GameConfig", [] {
    devices.mods.listFiles("data/subsystems", "*.txt", loadSubsystemDefinitions,
                           true);
  ...
  Loading::addTask("Materials", "Shaders", [] {
    ...
    devices.mods.listFiles("data/shipsets", "materials.txt",
                           loadLibraryFile<resource::RT_Material>, true);
  });

  if (load_resources) {
    Loading::addTask(
        "CompileShaders", "Shaders,Window",
        [] { devices.library.compileShaders(); }, threads::getThreadID());
  }
  ...
  Loading::addTask("Particles", "Materials,Sounds", [] {
    devices.mods.listFiles("data/particles", "*.ps",
                           loadLibraryFile<resource::RT_ParticleSystem>, true);
  });
  ...
  Loading::addTask("Models", "ModInit", [] {
    ...
    devices.mods.listFiles("data/shipsets", "models.txt",
                           loadLibraryFile<resource::RT_Mesh>, true);
  });

  Loading::addTask("Shipsets", "Models,Materials,SubSystems", [] {
    devices.mods.listFiles("data/shipsets", "hulls.txt", loadHullDefinitions,
                           true);
  ...
  if (load_resources) {
    ...
    Loading::addTask(
        "ProcessTextures", "ProcessImages,Window",
        [] { devices.library.processTextures(10); }, threads::getThreadID());

    Loading::addTask(
        "ProcessModels", "Models,Window,Shipsets",
        [] { devices.library.processMeshes(); }, threads::getThreadID());
  }
  ...
  Loading::addTask(
      "MenuScripts",
      "LocateScripts,SkinStyles,Materials,Models,Sounds,SubSystems", [] {
        std::map<std::string, std::string> scriptfiles;
        ...
        auto loadScript = [](const std::pair<std::string, std::string> &file) {
          ...
          devices.scripts.menu->load(makeModuleName(file.first), file.second);
        };
        ...
        devices.scripts.menu->compile(devices.mods.getProfile("as_cache/menu"),
                                      MENU_SCRIPT_BUILD);
...
void destroyMod() {
  // Make sure we finish loading, or we could de-initialize while adding
  // resources
  cancelLoad();
...
void setGameSettings(net::Message &msg) { game_settings = msg; }

void clearGameSettings() { game_settings.clear(); }
...
void loadEngineSettings() {
...

  {
    ...
    cat->settings.push_back(new NamedGeneric(
        "iDoubleClickMS", (int)devices.driver->getDoubleClickTime()));
    devices.settings.engine.addCategory(cat);
  }
  ...
  // Generate new tokens
  {
    ...
    if (token &&
        (token->toString().empty() || token->toString() == "JGeaaKcaaa")) {
      std::string pass;
      do {
        pass.clear();
