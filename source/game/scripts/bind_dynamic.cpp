...
static void nodeCast(asIScriptGeneric *f) {
  scene::ScriptedNode *node =
      dynamic_cast<scene::ScriptedNode *>((scene::Node *)f->GetObject());
...
static void restrictedState(asIScriptGeneric *f) {
  Object *obj = (Object *)f->GetObject();
  auto *member =
      (StateDefinition::stateDefMember *)f->GetFunction()->GetUserData();
...
void buildEmpAttribIndices() {
  ...
  for (auto var = def.types.begin(), endvar = def.types.end(); var != endvar;
    ...
    // Doubles are accessible generically in order to facilitate empire
    // attributes
...
void RegisterDynamicTypes(bool server) {
  // Bind global object states
  {
    ...
    if (&def != &errorStateDefinition) {
      ...
      for (auto var = def.types.begin(), endvar = def.types.end();
           var != endvar; ++var) {
        // Bind member
        auto &mtype = *var->def;
        ...
        if (mtype.isParam()) {
          if (var->def->returnable()) {
            if (var->access != SR_Visible) {
              object.addGenericMethod(
                  format("$1 get_$2() const",
                         mtype.returnType.empty() ? mtype.type
                                                  : mtype.returnType,
                         var->name)
                      .c_str(),
          ...
          if (server) {
            object.addGenericMethod(
                format("void set_$2($1 value)", mtype.type, var->name).c_str(),
                asFUNCTION(paramStateSet), (void *)&*var)
                doc("Member created from datafiles.", "");
          }
        } else if (server || var->access == SR_Visible) {
          object.addMember(format("$1 $2", mtype.type, var->name).c_str(),
                           base + var->offset)
              doc("Member created from datafiles.");
  ...
  for (auto i = objTypeList.begin(), end = objTypeList.end(); i != end; ++i) {
    ...
    classdoc(object, "Derived Object created based on datafiles. Can be cast "
                     "to and from an Object@.");
    bindObject(object, type, server);
    RegisterObjectComponentWrappers(object, server, type);

    object.addExternMethod("Object@ opImplCast() const", asFUNCTION(baseCast));
    genObj.addGenericMethod(format("$1@ opCast() const", object.name).c_str(),
                            asFUNCTION(castObj), type);
    genObj.addGenericMethod(
        format("bool get_is$1() const", object.name).c_str(),
  ...
  // Bind game config
  {
    ...
    for (size_t i = 0; i < gameConfig.count; ++i)
      bindGlobal(format("double $1", gameConfig.names[i]).c_str(),
                 (void *)&gameConfig.values[i]);
  ...
  if (empStates) {
    ...
    bind("uint getEmpireAttribute(const string& name)",
         asFUNCTION(getEmpAttribIndex));
    bind("string getEmpireAttributeName(uint id)",
         asFUNCTION(getEmpAttribName));
    emp.addExternMethod("double get_attributes(uint index) const",
                        asFUNCTION(getEmpAttrib));
    if (server)
      emp.addExternMethod("void set_attributes(uint index, double value)",
                          asFUNCTION(setEmpAttrib));

    emp.addExternMethod("void writeSyncedStates(Message& msg)",
                        asFUNCTION(writeEmpStates));
    emp.addExternMethod("void readSyncedStates(Message& msg)",
                        asFUNCTION(readEmpStates));
    ...
    for (auto var = def.types.begin(), endvar = def.types.end(); var != endvar;
      ...
      emp.addMember(format("$1 $2", mtype.type, var->name).c_str(),
                    base + var->offset) doc("Member created from datafiles.");
  ...
  for (auto i = nodeTypes.begin(), end = nodeTypes.end(); i != end; ++i) {
    ...
    classdoc(node, "Derived Node created based on datafiles. Can be cast to "
                   "and from a Node@.");

    node.addGenericFactory(format("$1@ f()", type->name).c_str(),
                           asFUNCTION(nodeFactory), type);

    node.addExternMethod("Node@ opImplCast() const", asFUNCTION(nodeBaseCast));
