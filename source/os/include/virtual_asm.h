...
// Implementation in virtual_asm_<operating system>.cpp (e.g.
...
struct CodePage {
  ...
  // Call finalize when done writing to the code page to guarantee that it can
  // be executed No more writing may be done to the allocated pages
  void finalize();

  // Returns the pointer to the first currently unused chunk of the page
  template <class T> T getFunctionPointer() {
    return reinterpret_cast<T>((byte *)page + used);
  }

  byte *getActivePage() const { return (byte *)page + used; }

  // Marks bytes as used;
  // future calls to getFunctionPointer() will not reference the location that
  // is being marked as used
  void markBytesUsed(unsigned int count) { used += count; }
  ...
  unsigned int getFreeSize() const { return size - used; }

  // Returns the smallest page (in bytes) that can be allocated by a code page
  // (Sizes other than multiples of this size allocate an extra page)
...
// Stores the code pointer and provides access to various processor-level
// operations
//  To work with the processor, create a set of 'Register' instances, each
//  taking the RegCode of the associated register (e.g. Register eax(cpu, EAX))
// Implementation in virtual_asm_<processor instruction set>.cpp (e.g.
// virtual_asm_x86.cpp)
struct Processor {
  ...
  //  e.g. 32 bits for x86, indicating that operations should treat addresses as
  ...
  // Optionally takes a bitMode override (defaults to the same bitMode as the
  // exe)
  Processor(CodePage &codePage, unsigned defaultBitMode = sizeof(void *) * 8);

  // Creates a jump to the new code page, and marks the current address as used
  // on the old code page Updates output pointer to the new code page's active
  // page
  ...
  void resetBitMode() { bitMode = lastBitMode; }
  ...
  template <class T> Processor &operator<<(T b) {
    *(T *)op = b;
    op += sizeof(T);
    return *this;
  }
  ...
  template <class T> Processor &operator<<(MemAddress addr);

  // Pushes bytes representing a prefix
  template <class T> Processor &operator<<(AddrPrefix pr);

  template <class T> Processor &operator<<(RegPrefix pr);

  // Calls the function, passing the arguments specified by 'args'
  // args is a string like "rrcmrm" which specifies arguments as sourced by a
  // Register*, MemAddres*, or a constant EBP is invalid during the call
  ...
  // Prepares for a call to manual call to a cdecl function (Do not use with
  // call_cdecl)
  ...
  // Pushes a constant value onto the stack (Pushes are always pushSize() large,
  // values beyond this size are an error)
  ...
  // Pushes the memory at <address> onto the stack (Pushes are always pushSize()
  // large, pushing larger values invokes multiple pushes)
  void push(MemAddress address);
  // Pops the value on the stack to the memory at <address> (Pops are always
  // pushSize() large, popping larger values invokes multiple pops)
  ...
  // Decrements ecx and jumps if it becomes 0; Optionally conditionally jumps
  // based on a Zero/NotZero test
  void loop(volatile byte *dest, JumpType type = Jump);

  // Copies from *esi to *edi, and adjusts them both by the data size according
...
struct FloatingPointUnit {
  ...
  // Compares FPU_0 to floatReg, setting the CPU's flags according to the
  // values' relation
  //  Optionally pops the fpu stack
  void compare_toCPU(FloatReg floatReg, bool pop = true);

  // Pushes the specified data type stored at <address> onto the FPU stack
  // (becomes FPU_0)
...
// Temporary struct that represents an addition to a memory address, with
// optional scaling
struct ScaledIndex {
  RegCode reg;
  unsigned char scaleFactor;

  ScaledIndex(RegCode Reg, unsigned char Scale)
      : reg(Reg), scaleFactor(Scale) {}
};
...
// Converts a MemAddress from the default unsigned <cpu bit mode> to match the
// passed type
template <class T> MemAddress as(MemAddress addr) {
...
//   Also provides the means to generate MemAddresses relative to a register via
//   dereference (e.g. *eax+8)
// Implementation in virtual_asm_<processor instruction set>.cpp (e.g.
// virtual_asm_x86.cpp)
struct Register {
  Processor &cpu;
...
// Converts a MemAddress from the default unsigned <cpu bit mode> to match the
// passed type
template <class T> Register as(Register reg) {
