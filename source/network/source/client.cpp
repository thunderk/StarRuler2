...
Client::Client(const std::string &hostname, int port, bool makeConnection,
               AddressType type)
    : trans(new Transport(type)), conn(0), hostname(hostname),
      established(false), resolved(false), hasConnection(makeConnection) {
  address.port = port;
}
...
Connection *Client::getConnection() { return conn; }
...
BroadcastClient::~BroadcastClient() { stop(); }

void BroadcastClient::handle(uint8_t type,
                             BroadcastClient::bcMessageHandler func) {
  threads::Lock lock(handlerMutex);
  handlers[type] = func;
}
...
void BroadcastClient::handleMessage(Transport *transport, Address addr,
                                    Message *msg) {
  uint8_t type = msg->getType();
...
void BroadcastClient::broadcast(Message &msg) { trans.broadcast(msg, port); }
