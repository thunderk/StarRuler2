...
struct WrappedMethod {
  ...
  WrappedMethod()
      : compId(0), index(0), func(0), local(false), server(false),
        shadow(false), onlyPrimitive(true), restricted(false), async(false),
        relocking(false), safe(false), passPlayer(false),
        passContaining(false) {}
};
...
void RegisterObjectComponentWrappers(ClassBind &cls, bool server,
                                     ScriptObjectType *type = 0);
