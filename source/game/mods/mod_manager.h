...
struct Mod {
  ...
  void listFiles(const std::string &dir,
                 ...
                 const char *filter = "*", bool recurse = false,
                 bool inherit = true) const;
  void resolveDirectory(const std::string &dir,
                        std::vector<std::string> &out) const;

  void loadFileList();
  bool isCompatible(const Mod *other) const;
  void getConflicts(const Mod *other,
...
public:
  ...
  // Finds all directories that match the passed relative dir and outputs their
  // absolute paths
  void resolveDirectory(const std::string &dir,
                        std::vector<std::string> &out) const;
