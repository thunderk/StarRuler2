...
unsigned char sib(unsigned char ScaledIndex, unsigned char Mode,
...
unsigned Processor::pushSize() { return sizeof(void *); }

void Processor::call_thiscall_prep(unsigned argBytes) {
#ifndef _MSC_VER
  // THISCALL on GCC passes the this pointer on the stack, and is mostly
  // identical to cdecl
...
template <> Processor &Processor::operator<<(MemAddress addr) {
  if (addr.absolute_address != 0) {
    if (addr.scaleFactor == 0)
      ...
      return *this << mod_rm(addr.other, ADR, SIB)
                   << sib(addr.scaleReg, scale, EBP) << addr.absolute_address;
    }
  }
  ...
  if (addr.scaleFactor != 0) {
    ...
    if (addr.offset == 0 && addr.code != EBP)
      return *this << mod_rm(addr.other, ADR, SIB)
                   << sib(addr.scaleReg, scale, addr.code);
    else if (addr.offset >= CHAR_MIN && addr.offset <= CHAR_MAX)
      return *this << mod_rm(addr.other, ADR8, SIB)
                   << sib(addr.scaleReg, scale, addr.code) << (char)addr.offset;
    else
      return *this << mod_rm(addr.other, ADR32, SIB)
                   << sib(addr.scaleReg, scale, addr.code) << addr.offset;
  }

  if (addr.offset == 0 &&
      addr.code !=
          EBP) { //[EBP] means absolute address, so we use [EBP+0] instead
...
void Processor::push(Register &reg) { *this << byte(0x50u + reg.code); }

void Processor::pop(Register &reg) { *this << byte(0x58u + reg.code); }
...
void Processor::push(MemAddress address) {
  ...
  case 32:
    *this << '\xFF' << address;
    break;
  case 64:
    MemAddress upper(address);
    moveToUpperDWORD(upper);
    *this << '\xFF' << upper;
    *this << '\xFF' << address;
    break;
  }
...
void Processor::jump(JumpType type, volatile byte *dest) {
  ...
  if (offset >= CHAR_MIN && offset <= CHAR_MAX)
    ...
    *this << longJumpCodes[Jump]
          << offset - 3; // Long jump is 3 bytes larger, jump is from the end of
                         // the full opcode
  else
    *this << '\x0F' << longJumpCodes[type]
          << offset - 4; // Conditional long jump is 4 bytes larger, jump is
                         // from the end of the full opcode
}
...
void Processor::string_copy(unsigned size) {
  if (size == 1)
    ...
    *this << '\xA5'
          << '\xA5'; // Just copy twice for 8 bytes (for 64 bit compatibility)
  else
...
void Processor::ret() { *this << '\xC3'; }

void Processor::debug_interrupt() { *this << '\xCC'; }

MemAddress::MemAddress(Processor &CPU, void *address)
    : cpu(CPU), code(ESP), absolute_address(address), other(NONE), offset(0),
      bitMode(cpu.bitMode), Signed(false), scaleFactor(0) {}

MemAddress::MemAddress(Processor &CPU, RegCode Code)
    : cpu(CPU), code(Code), absolute_address(0), other(NONE), offset(0),
      bitMode(cpu.bitMode), Signed(false), scaleFactor(0) {}

MemAddress::MemAddress(Processor &CPU, RegCode Code, int Offset)
    : cpu(CPU), code(Code), absolute_address(0), other(NONE), offset(Offset),
      bitMode(cpu.bitMode), Signed(false), scaleFactor(0) {}
...
void MemAddress::operator=(unsigned int value) {
  switch (bitMode) {
  case 8:
    cpu << '\xC6' << *this << (byte)value;
    break;
  case 16:
    cpu << '\x66' << '\xC7' << *this << (unsigned short)value;
    break;
  case 32:
    cpu << '\xC7' << *this << value;
    break;
...
void MemAddress::operator|=(unsigned int value) {
  ...
  switch (bitMode) {
  case 8:
    cpu << '\x80' << *this << (byte)value;
    break;
  case 16:
    cpu << '\x66' << '\x81' << *this << (unsigned short)value;
    break;
  case 32:
    cpu << '\x81' << *this << value;
    break;
...
void MemAddress::operator=(void *value) { cpu << '\xC7' << *this << value; }
...
Register::Register(Processor &CPU, RegCode Code)
    : cpu(CPU), code(Code), bitMode(0) {}
...
MemAddress Register::operator*() const { return MemAddress(cpu, code); }
...
void Register::operator-() { cpu << '\xF7' << mod_rm(EX_3, REG, code); }

void Register::operator--() {
  switch (getBitMode()) {
  case 8:
    cpu << '\xFE' << mod_rm(EX_1, REG, code);
    break;
  case 16:
    cpu << '\x66' << byte('\x48' + code);
    break;
  case 32:
    cpu << byte('\x48' + code);
    break;
...
void Register::operator++() {
  switch (getBitMode()) {
  case 8:
    cpu << '\xFE' << mod_rm(EX_0, REG, code);
    break;
  case 16:
    cpu << '\x66' << byte('\x40' + code);
    break;
  case 32:
    cpu << byte('\x40' + code);
    break;
...
void Register::operator&=(unsigned long long mask) {
  switch (getBitMode()) {
  case 8:
    cpu << '\x80' << mod_rm(EX_4, REG, code) << (byte)mask;
    break;
  case 16:
    cpu << '\x66' << '\x81' << mod_rm(EX_4, REG, code) << (unsigned short)mask;
    break;
  case 32:
  case 64:
    if (code == EAX)
...
void Register::operator&=(Register other) {
  switch (getBitMode()) {
  case 8:
    cpu << '\x20' << mod_rm(other.code, REG, code);
    break;
  case 16:
    cpu << '\x66' << '\x21' << mod_rm(other.code, REG, code);
    break;
  case 32:
  case 64:
    cpu << '\x21' << mod_rm(other.code, REG, code);
    break;
  }
}
...
void Register::operator|=(unsigned long long mask) {
  switch (getBitMode()) {
  case 8:
    cpu << '\x80' << mod_rm(EX_1, REG, code) << (byte)mask;
    break;
  case 16:
    cpu << '\x66' << '\x81' << mod_rm(EX_1, REG, code) << (unsigned short)mask;
    break;
  case 32:
  case 64:
    if (code == EAX)
...
void Register::operator=(unsigned long long value) {
  switch (getBitMode()) {
  case 8:
    cpu << (byte)('\xB0' + code) << (byte)value;
    break;
  case 16:
    cpu << '\x66' << (byte)('\xB8' + code) << (unsigned short)value;
    break;
  case 32:
    cpu << (byte)('\xB8' + code) << (unsigned int)value;
    break;
  case 64:
    throw 0;
  }
...
void Register::copy_zeroing(Register &other) {
  ...
  case 16:
    throw 0;
  case 32:
    cpu << '\x0F' << '\xB6' << mod_rm(code, REG, other.code);
    break;
  case 64:
    throw 0;
  }
...
void Register::operator==(Register other) {
  switch (getBitMode()) {
  case 8:
    cpu << '\x3A' << mod_rm(code, REG, other.code);
    break;
  case 16:
    cpu << '\x66' << '\x3B' << mod_rm(code, REG, other.code);
    break;
  case 32:
    cpu << '\x3B' << mod_rm(code, REG, other.code);
    break;
  case 64:
    throw 0;
  }
...
void Register::operator==(unsigned int test) {
  switch (getBitMode()) {
  case 8:
    cpu << '\x80' << mod_rm(EX_7, REG, code) << (byte)test;
    break;
  case 16:
    cpu << '\x66' << '\x80' << mod_rm(EX_7, REG, code) << (unsigned short)test;
    break;
  case 32:
    cpu << '\x81' << mod_rm(EX_7, REG, code) << test;
    break;
  case 64:
    throw 0;
  }
...
void Register::divide() { cpu << '\xF7' << mod_rm(EX_6, REG, code); }

void Register::divide_signed() { cpu << '\xF7' << mod_rm(EX_7, REG, code); }
...
void FloatingPointUnit::init() { cpu << '\xDB' << '\xE3'; }

void FloatingPointUnit::negate() { cpu << '\xD9' << '\xE0'; }

void FloatingPointUnit::exchange(FloatReg reg) {
  cpu << '\xD9' << mod_rm(EX_1, REG, reg);
}

void FloatingPointUnit::load_const_0() { cpu << '\xD9' << '\xEE'; }

void FloatingPointUnit::load_const_1() { cpu << '\xD9' << '\xE8'; }
