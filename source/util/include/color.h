...
struct Color {
  ...
  Color getInterpolated(const Color &other, float pct) const {
    if (pct <= 0)
      ...
      return Color((unsigned char)(float(r) + float(other.r - r) * pct),
  ...
  Color operator*(const Color &other) const {
    return Color((unsigned char)(float(r) / 255.f * float(other.r)),
...
struct Colorf {
  ...
  explicit Colorf(const Color &c)
      : r(float(c.r) / 255.f), g(float(c.g) / 255.f), b(float(c.b) / 255.f),
        a(float(c.a) / 255.f) {}
