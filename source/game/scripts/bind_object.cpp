...
ObjArray *ObjArray::create() { return new ObjArray(); }

ObjArray *ObjArray::create_n(unsigned count) { return new ObjArray(count); }
...
bool ObjArray::empty() const { return objs.empty(); }

unsigned ObjArray::size() const { return objs.size(); }

void ObjArray::reserve(unsigned size) { objs.reserve(size); }
...
void ObjArray::push_back(Object *obj) { objs.push_back(obj); }
...
void ObjArray::sortAsc() { sortAsc_bound(0, objs.size()); }
...
void ObjArray::sortDesc_bound(unsigned lower, unsigned upper) {
  ...
  std::sort(objs.begin() + lower, objs.begin() + upper,
            [](const Object *a, const Object *b) { return b < a; });
}

void ObjArray::sortDesc() { sortDesc_bound(0, objs.size()); }
...
static Object *objByID(int id) { return getObjectByID(id); }
...
template <ObjectFlag flag> static void setObjFlag(Object &obj, bool value) {
  obj.setFlag(flag, value);
}

template <ObjectFlag flag> static bool getObjFlag(const Object &obj) {
...
bool objKnown(Object *obj) { return obj->isKnownTo(Empire::getPlayerEmpire()); }

Empire *objGetOwner(Object *obj) { return obj->owner; }

int objType(Object *obj) { return obj->type->id; }
...
bool objIsPhysical(Object *obj) { return !obj->getFlag(objNoPhysics); }
...
public:
  ...
  void process() { object->setOwner(newOwner); }

  ~AsyncTakeover() { object->drop(); }
};
...
public:
  ...
  void process() { object->addTimedEffect(effect); }

  ~AsyncEffect() { object->drop(); }
};
...
static void unlockObjectLock(ObjectLock *lock) { lock->~ObjectLock(); }
...
static void destroyWaitSafe(bool *mem) { setSafeCallWait(*mem); }
...
static Blueprint::HexStatus *bpHexStatus(Blueprint &bp, unsigned x,
                                         unsigned y) {
  if (!bp.design)
    return 0;
...
static Blueprint::SysStatus *bpHexSysStatus(Blueprint &bp, unsigned x,
                                            unsigned y) {
  if (!bp.design)
    return 0;
...
public:
  SendObjDamage(Object *obj, const DamageEvent &evt, double position,
                const vec2d &direction)
      : ObjectMessage(obj), event(evt), position(position),
        direction(direction) {
    object->grab();
  }

  void process() { object->damage(event, position, direction); }
};

static void sendObjDamage(Object *obj, DamageEvent &evt, double position,
                          const vec2d &direction) {
  if (obj == getActiveObject())
    obj->damage(evt, position, direction);
  else
    obj->lockGroup->addMessage(
        new SendObjDamage(obj, evt, position, direction));
}

static ObjArray *findInBox(const vec3d &minBound, const vec3d &maxBound,
                           unsigned mask) {
  ObjArray *results = new ObjArray();
  results->reserve(100);

  AABBoxd box(minBound, maxBound);
  box.fix();

  devices.physics->findInBox(
      box,
      [results](const PhysicsItem &item) {
...
static void createLockedHandle_c(LockedHandle<Object> *mem,
                                 LockedHandle<Object> &other) {
...
static void runHexLine(Blueprint *bp, Object *obj, asIScriptObject *cb,
                       const vec2d &direction) {
...
public:
  ...
  bool has(Object *obj) const { return contains(obj); }
...
uint64_t objGetStatInt(Object &obj, uint64_t id) { return obj.stats.get(id); }
...
unsigned int objGetStatCount(Object &obj) { return obj.stats.size(); }
...
public:
  ...
  ModObjStat(Object *obj) : ObjectMessage(obj) { object->grab(); }

  static void doModInt(Object *obj, uint64_t id, ObjectStatMode mode,
                       int64_t value, int64_t dirtyRes) {
    switch (mode) {
    case OSM_Set:
  ...
  static void doModDouble(Object *obj, uint64_t id, ObjectStatMode mode,
                          double value, double dirtyRes) {
    switch (mode) {
    case OSM_Set:
  ...
  ~ModObjStat() { object->drop(); }
};

void objModStatDouble(Object *obj, uint64_t id, ObjectStatMode mode,
                      double value, double dirtyResolution) {
  if (obj->lockGroup == nullptr || !obj->isValid())
    return;
...
void objModStatInt(Object *obj, uint64_t id, ObjectStatMode mode, int64_t value,
                   int64_t dirtyResolution) {
  if (obj->lockGroup == nullptr || !obj->isValid())
    return;
  if (!obj->isLocked() || Object::GALAXY_CREATION) {
...
void bindObject(ClassBind &object, ScriptObjectType *type, bool server) {
  ...
  if (!server) {
    object.addMember("const bool alwaysVisible",
                     offsetof(Object, alwaysVisible))
    ...
    object.addExternMethod("void set_name(const string &in Name)",
                           asFUNCTION(objSetName))
        doc("", "New name for the object.");

    object.addExternMethod("Empire@ get_owner() const", asFUNCTION(objGetOwner))
        doc("", "The empire that owns this object.");

    object.addExternMethod("void set_owner(Empire@ emp)",
                           asFUNCTION(objSetOwner))
        doc("Set a new owner for the object.",
            "Empire that should own the object.");

    object.addExternMethod("void finalizeCreation()",
                           asFUNCTION(finalizeObject))
  ...
  object.addExternMethod("bool get_engaged() const",
                         asFUNCTION(getObjFlag<objEngaged>))
      doc("", "Whether the object is engaged.");

  object.addExternMethod("bool get_named() const",
                         asFUNCTION(getObjFlag<objNamed>))
      doc("", "Whether the object has been named..");

  object.addExternMethod("bool get_inCombat() const",
                         asFUNCTION(getObjFlag<objCombat>))
      doc("", "Whether the object is considered in combat.");

  object.addMethod("bool isFocus() const", asMETHOD(Object, isFocus)) doc(
      "", "Whether the object is a focus object. "
  ...
  object.addMethod("bool get_initialized() const",
                   asMETHOD(Object, isInitialized))
  ...
  object.addExternMethod("bool get_selected() const",
                         asFUNCTION(getObjFlag<objSelected>))
      doc("", "Whether the object is currently selected.");

  object.addExternMethod("void set_selected(bool value)",
                         asFUNCTION(setObjFlag<objSelected>))
      doc("", "Whether the object is currently selected.");

  object.addExternMethod("bool get_isPhysical() const",
                         asFUNCTION(objIsPhysical))
      doc("", "Whether the object is a physical object.");

  object.addExternMethod("bool get_notDamageable() const",
                         asFUNCTION(getObjFlag<objNoDamage>))
      doc("", "Whether the object cannot be damaged at all by projectiles.");

  if (server) {
    object.addMethod("void wake()", asMETHOD(Object, wake))
        doc("Tells the processing threads to tick the object next time "
            "(ignoring requested timeout)");
    ...
    object.addExternMethod("bool get_memorable() const",
                           asFUNCTION(getObjFlag<objMemorable>))
        doc("", "Whether the object should have past vision state tracked.");

    object.addExternMethod("void set_noCollide(bool value)",
                           asFUNCTION(setObjFlag<objNoCollide>))
        doc("Set whether the object should not be collided with (enforced by "
            "scripts).",
            "");

    object.addExternMethod("void set_notDamageable(bool value)",
                           asFUNCTION(setObjFlag<objNoDamage>))
        doc("Set whether the object cannot be damaged at all by projectiles.",
            "");

    object.addExternMethod("bool get_noCollide() const",
                           asFUNCTION(getObjFlag<objNoCollide>))
        doc("", "Whether the object should not be collided with.");

    object.addExternMethod("void set_engaged(bool value)",
                           asFUNCTION(setObjFlag<objEngaged>))
        doc("Set whether the object is engaged.", "");

    object.addExternMethod("void set_named(bool value)",
                           asFUNCTION(setObjFlag<objNamed>))
        doc("Set whether the object is named.", "");

    object.addExternMethod("void set_inCombat(bool value)",
                           asFUNCTION(setObjFlag<objCombat>))
        doc("Set whether the object is considered in combat.", "");

    object.addExternMethod("void wait()", asFUNCTION(objectWait))
        doc("Waits for all asynchronous messages to finish on this object. "
            "This is considered a relocking operation.");

    object.addMethod("void destroy()", asMETHOD(Object, flagDestroy))
        doc("Flag the object for destruction and eventual disposal.");

    object.addExternMethod("bool get_destroying()",
                           asFUNCTION(getObjFlag<objQueueDestroy>))
        doc("Whether the object is waiting to be destroyed.", "");

    object.addExternMethod("void damage(DamageEvent& evt, double position, "
                           "const vec2d&in direction)",
                           asFUNCTION(sendObjDamage))
    ...
    object.addExternMethod("void addTimedEffect(const TimedEffect &in eff)",
                           asFUNCTION(sendTimedEffect))
        doc("Add an effect to the object that executes then finishes after a "
            "particular amount of time.",
            "The effect to add.");

    object.addMethod("bool isVisibleTo(Empire& emp) const",
                     asMETHOD(Object, isVisibleTo))
        doc("Returns true if the specified empire can see the object.",
            "Empire to check vision for.", "");

    object.addMethod("bool isKnownTo(Empire& emp) const",
                     asMETHOD(Object, isKnownTo))
        doc("Returns true if the specified empire has ever seen the object.",
            "Empire to check vision for.", "");

    object.addExternMethod("Object@ get_targets(uint index) const",
                           asFUNCTION(objTarget))
        doc("Retrieve one of the targeting system targets the object is "
            "holding.",
  ...
  object.addExternMethod("const ObjectGroup@ get_group() const",
                         asFUNCTION(objGroup))
      doc("", "The object group associated with the object.");
  ...
  object.addExternMethod("uint getStatCount() const",
                         asFUNCTION(objGetStatCount));
  object.addExternMethod("int64 getStatValueByIndex(uint index) const",
                         asFUNCTION(objGetStatInt_index));
  object.addExternMethod("uint64 getStatIdByIndex(uint index) const",
                         asFUNCTION(objGetStatId_index));

  object.addExternMethod("double getStatDouble(uint64 id) const",
                         asFUNCTION(objGetStatDouble));
  object.addExternMethod("int64 getStatInt(uint64 id) const",
                         asFUNCTION(objGetStatInt));

  object.addExternMethod("void modStatDouble(uint64 id, ObjectStatMode mode, "
                         "double value, double dirtyResolution = 0.0) const",
                         asFUNCTION(objModStatDouble));
  object.addExternMethod("void modStatInt(uint64 id, ObjectStatMode mode, "
                         "int64 value, int64 dirtyResolution = 0) const",
                         asFUNCTION(objModStatInt));

  if (!server) {
    object.addExternMethod("vec3d get_node_position()", asFUNCTION(objNodePos))
        doc("", "The absolute physical position of the object's graphics node "
                "this frame."
                " Can be different from the object position due to graphics "
                "interpolation.");

    object.addExternMethod("quaterniond get_node_rotation()",
                           asFUNCTION(objNodeRotation))
        doc("", "The absolute physical rotation of the object's graphics node "
                "this frame."
                " Can be different from the object rotation due to graphics "
                "interpolation.");
  }
  ...
  ClassBind lh(handle_name.c_str(),
               asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CDK,
               sizeof(LockedHandle<Object>));
  lh.addConstructor("void f()", asFUNCTION(createLockedHandle));
  lh.addConstructor(format("void f($1@ obj)", obj_name).c_str(),
                    asFUNCTION(createLockedHandle_v));
  lh.addConstructor(format("void f(const $1& other)", handle_name).c_str(),
                    asFUNCTION(createLockedHandle_c));
  lh.addDestructor("void f()", asFUNCTION(destroyLockedHandle));

  lh.addMethod(format("$1@ get() const", obj_name).c_str(),
               asMETHOD(LockedHandle<Object>, get));
  lh.addMethod(format("$1@ get_safe() const", obj_name).c_str(),
               asMETHOD(LockedHandle<Object>, get_safe));
  lh.addMethod(format("void set($1@ value)", obj_name).c_str(),
               asMETHOD(LockedHandle<Object>, set_withref));
  ...
  // Bucket class
  {
    ...
    ob.setReferenceFuncs(asMETHOD(ObjectBucket, grab),
                         asMETHOD(ObjectBucket, drop));
    ob.addGenericFactory(format("$1@ f()", bucket_name.c_str()).c_str(),
                         asFUNCTION(ob_factory), scriptType);

    ob.addMethod("uint get_length() const", asMETHOD(ObjectBucket, size));
    ob.addMethod(
        format("$1@ get_opIndex(uint index) const", obj_name.c_str()).c_str(),
        asMETHOD(ObjectBucket, opInd));

    ob.addMethod("void clear()", asMETHOD(ObjectBucket, eraseAll));
    ob.addMethod(format("void add($1& obj)", obj_name).c_str(),
                 asMETHOD(ObjectBucket, insert));
    ob.addMethod(format("void remove($1& obj)", obj_name).c_str(),
                 asMETHOD(ObjectBucket, erase));
    ob.addMethod(format("bool contains($1& obj) const", obj_name).c_str(),
                 asMETHOD(ObjectBucket, has));
  }

  // Reading and writing to messages
  if (type) {
    ClassBind savefile("SaveFile");
    savefile.addExternMethod(format("SaveFile& opShl($1@+)", obj_name).c_str(),
                             asFUNCTION(saveObject));
    savefile.addGenericMethod(format("SaveFile& opShr($1@&)", obj_name).c_str(),
                              asFUNCTION(loadObjectType), type);

    ClassBind message("Message");
    message.addExternMethod(format("Message& opShl($1@+)", obj_name).c_str(),
                            asFUNCTION(writeObjectScr));
    message.addGenericMethod(format("Message& opShr($1@&)", obj_name).c_str(),
                             asFUNCTION(readObjectType), type);
  }
}

void RegisterObjectArray(const std::string &objName) {
  std::string arrName = "array<";
  arrName += objName + "@>";
  ClassBind arr(arrName.c_str(), asOBJ_REF);

  arr.setReferenceFuncs(asMETHOD(ObjArray, grab), asMETHOD(ObjArray, drop));

  arr.addFactory(format("$1@ f()", arrName).c_str(),
                 asFUNCTION(ObjArray::create));
  arr.addFactory(format("$1@ f(uint count)", arrName).c_str(),
                 asFUNCTION(ObjArray::create_n));

  arr.addMethod(format("$1& opAssign(const $1&)", arrName).c_str(),
                asMETHOD(ObjArray, operator=));

  arr.addMethod(format("$1@& opIndex(uint)", objName).c_str(),
                asMETHOD(ObjArray, operator[]));
  arr.addMethod(format("$1@ opIndex(uint) const", objName).c_str(),
                asMETHOD(ObjArray, index_value));
  arr.addMethod(format("$1@& get_last()", objName).c_str(),
                asMETHOD(ObjArray, last));
  arr.addMethod(format("$1@ get_last() const", objName).c_str(),
                asMETHOD(ObjArray, last_value));

  arr.addMethod(format("void insertAt(uint, $1@)", objName).c_str(),
                asMETHOD(ObjArray, insert));
  arr.addMethod(format("void insertLast($1@)", objName).c_str(),
                asMETHOD(ObjArray, push_back));
  arr.addMethod(format("void remove(const $1@)", objName).c_str(),
                asMETHOD(ObjArray, remove));
  arr.addMethod("void removeLast()", asMETHOD(ObjArray, pop_back));
  arr.addMethod("void removeAt(uint)", asMETHOD(ObjArray, erase));
  arr.addMethod(format("int find(const $1@) const", objName).c_str(),
                asMETHOD(ObjArray, find));
  ...
  arr.addMethod("void sortDesc(uint, uint)",
                asMETHOD(ObjArray, sortDesc_bound));
  arr.addMethod(format("int findSorted(const $1@) const", objName).c_str(),
                asMETHOD(ObjArray, findSorted));
  arr.addMethod(format("void removeSorted(const $1@)", objName).c_str(),
                asMETHOD(ObjArray, removeSorted));
}
...
void RegisterObjectBinds(bool declarations, bool server, bool shadow) {
  ...
  if (void *objArray = getEngine()->GetTypeInfoById(
          getEngine()->GetTypeIdByDecl("array<Object@>")))
  ...
  objectGroup.addMember("quaterniond formationFacing",
                        offsetof(ObjectGroup, formationFacing));
  objectGroup.setReferenceFuncs(asMETHOD(ObjectGroup, grab),
                                asMETHOD(ObjectGroup, drop));
  objectGroup.addExternMethod("Object@ get_objects(uint index)",
                              asFUNCTION(objGroup_object));
  objectGroup.addExternMethod("const Object@ get_objects(uint index) const",
                              asFUNCTION(objGroup_object));
  objectGroup.addExternMethod("Object@ get_owner()",
                              asFUNCTION(objGroup_owner));
  objectGroup.addExternMethod("const Object@ get_owner() const",
                              asFUNCTION(objGroup_owner));
  objectGroup.addMethod("uint get_objectCount() const",
                        asMETHOD(ObjectGroup, getObjectCount));
  objectGroup.addMethod("vec3d get_center() const",
                        asMETHOD(ObjectGroup, getCenter));
  objectGroup.addMethod("uint get_maxObjectCount() const",
                        asMETHOD(ObjectGroup, getMaxObjectCount))
      doc("Returns the maximum number of objects ever in this group (Usually "
          "the original number the group was created with).",
          "");

  ClassBind object("Object");
  classdoc(
      object,
      "Abstract class for all game objects, implemented "
  ...
  bind("Object@ getObjectByID(int id)", asFUNCTION(objByID))
      doc("Returns the Object associated with the ID.",
          "Unique identifier of the object to retrieve.",
          "Null if the object doesn't exist.");

  if (server)
    bind("Object@ getOrCreateObject(int id)", asFUNCTION(createObjByID)) doc(
        "Get the object belonging to the identifier, or create a dummy object"
        " with that id for later initialization. Should only be used for"
        " synchronization of objects in multiplayer or during save game "
        "loading.",
        "Unique identifier of the object.", "Object with that identifier.");

  // Blueprint data
  EnumBind hf("HexFlags");
  classdoc(hf,
           "Bitmask flags used in individual hexagon status on blueprints.");
  ...
  classdoc(hs,
           "Indication of the status of an individual hexagon on a blueprint.");
  ...
  ss.addMember("uint16 workingHexes",
               offsetof(Blueprint::SysStatus, workingHexes))
      doc("Amount of hexes of the subsystem that are still active and not "
          "destroyed.");
  ...
  classdoc(
      bp, "A physical instance of a design. A blueprint belongs to one ship and"
          " holds all the information about the internals of that particular "
          "ship.");
  ...
  bp.addMember("double shipEffectiveness",
               offsetof(Blueprint, shipEffectiveness))
  ...
  if (server) {
    ...
    bp.addMethod("float tick(Object&, double)", asMETHOD(Blueprint, think))
        doc("Tick function to handle the blueprint, should be called from the"
            " object script handler's tick.",
            "Object this blueprint is for.", "Time passed since last tick.",
            "Suggestion for the amount of"
    ...
    bp.addMethod("void ownerChange(Object&, Empire@, Empire@)",
                 asMETHOD(Blueprint, ownerChange))
        doc("Call when the blueprint changes owners.",
            "Object this blueprint is for.", "Previous empire.", "New Empire.");

    bp.addExternMethod("HexStatus@ getHexStatus(uint x, uint y)",
                       asFUNCTION(bpHexStatus))
        doc("", "X coordinate of the hex.", "Y coordinate of the hex.",
            "Current status of the hex on this blueprint.");

    bp.addExternMethod("SysStatus@ getSysStatus(uint index)",
                       asFUNCTION(bpSysStatus))
        doc("", "Index of the subsystem", "Current status of the subsystem.");

    bp.addExternMethod("SysStatus@ getSysStatus(uint x, uint y)",
                       asFUNCTION(bpHexSysStatus))
        doc("", "X coordinate of the hex.", "Y coordinate of the hex.",
            "Current status of the subsystem the hex belongs to.");
  }

  bp.addMethod("bool hasTagActive(SubsystemTag tag)",
               asMETHOD(Blueprint, hasTagActive))
      doc("", "Tag to check for.",
          "Whether any subsystems with this tag are present and active.");

  bp.addMethod(
      "double getTagEfficiency(SubsystemTag tag, bool ignoreInactive = true)",
      asMETHOD(Blueprint, getTagEfficiency))
      doc("Calculate the total efficiency of subsystems with a particular tag.",
          "Tag to check for.",
          "Whether to ignore remaining efficiency on subsystems that are "
          "disabled.",
          "Total efficiency.");

  bp.addMethod("double getEfficiencySum(SubsystemVariable var, SubsystemTag "
               "tag = ST_NULL, bool ignoreInactive = true)",
               asMETHOD(Blueprint, getEfficiencySum))
      doc("Calculate the total value of a subsystem variable as affected by "
          "subsystem efficiency.",
          "Variable to total.", "Tag to filter subsystems by.",
          "Whether to ignore remaining efficiency on subsystems that are "
          "disabled.",
          "Total value.");

  bp.addMethod("double getEfficiencyFactor(SubsystemVariable var, SubsystemTag "
               "tag = ST_NULL, bool ignoreInactive = true)",
               asMETHOD(Blueprint, getEfficiencyFactor))
      doc("Calculate the total percentage efficiency with respect to a "
          "variable.",
          "Variable to use.", "Tag to filter subsystems by.",
          "Whether to ignore remaining efficiency on subsystems that are "
          "disabled.",
          "Efficiency factor..");

  bp.addMethod("vec3d getOptimalFacing(SubsystemVariable var, SubsystemTag tag "
               "= ST_NULL, bool ignoreInactive = true)",
               asMETHOD(Blueprint, getOptimalFacing))
      doc("Calculate the optimal facing so that effectors in this blueprint "
          "can fire with optimal value for the passed variable.",
          "Variable to use.", "Tag to filter subsystems by.",
          "Whether to ignore remaining efficiency on subsystems that are "
          "disabled.",
          "Optimal facing.");

  bp.addMethod("Object@ getCombatTarget()",
               asMETHOD(Blueprint, getCombatTarget))
      doc("Find a target this ship is firing on.", "");

  bp.addExternMethod("const HexStatus@ getHexStatus(uint index) const",
                     asFUNCTION(bpHexStatusInd))
      doc("", "Hex index.", "Current status of the hex on this blueprint.");

  bp.addExternMethod("const HexStatus@ getHexStatus(uint x, uint y) const",
                     asFUNCTION(bpHexStatus))
      doc("", "X coordinate of the hex.", "Y coordinate of the hex.",
          "Current status of the hex on this blueprint.");

  bp.addExternMethod("const SysStatus@ getSysStatus(uint index) const",
                     asFUNCTION(bpSysStatus))
      doc("", "X coordinate of the hex.", "Y coordinate of the hex.",
          "Current status of the subsystem the hex belongs to.");

  bp.addExternMethod("const SysStatus@ getSysStatus(uint x, uint y) const",
                     asFUNCTION(bpHexSysStatus))
      doc("", "X coordinate of the hex.", "Y coordinate of the hex.",
          "Current status of the subsystem the hex belongs to.");
  ...
  bp.addMethod("bool doesAutoTarget(Object& obj, Object& target)",
      ...
      doc("Check whether the effectors in this blueprint can auto-target an "
          "object.",
          "Object this blueprint is for.",
          "Object to check auto-targeting for.",
          "Whether the object can be auto-targeted.");

  if (server) {
    bp.addMethod("void save(Object& obj, SaveFile& file)",
                 asMETHOD(Blueprint, save));
    bp.addExternMethod("void load(Object& obj, SaveFile& file)",
                       asFUNCTION(loadBlueprint));

    bp.addMethod(
        "void target(Object& obj, Object& target, uint flags = 0)",
        asMETHODPR(Blueprint, target, (Object *, Object *, TargetFlags), void))
        doc("Set all effectors in this blueprint to target a particular "
            "object.",
            "Object this blueprint is for.",
            "Object to target with all effectors.",
            "Flags for the effector target.");

    bp.addMethod("void target(Object& obj, uint efftrIndex, Object& target, "
                 "uint flags = 0)",
                 asMETHODPR(Blueprint, target,
                            (Object *, unsigned, Object *, TargetFlags), void))
        doc("Set an effector in this blueprint to target a particular object.",
            "Object this blueprint is for.",
            "Index of the effector to target for.",
            "Object to target with the effector.",
            "Flags for the effector target.");

    bp.addMethod(
        "void target(Object& obj, const Subsystem@ sys, Object& target, uint "
        "flags = 0)",
        asMETHODPR(Blueprint, target,
                   (Object *, const Subsystem *, Object *, TargetFlags), void))
    ...
    bp.addMethod("void clearTracking(Object& obj)",
        ...
        doc("Clear the turret tracking for all effectors, letting them retrack "
            "fast.",
            "Object this blueprint is for.");

    bp.addMethod("void sendDetails(Object& obj, Message& msg) const",
                 asMETHOD(Blueprint, sendDetails))
        doc("For networking: sends the details of this blueprint through the "
            "message.",
            "Object this blueprint is for.", "Message to add details to.");

    bp.addMethod("void recvDetails(Object& obj, Message& msg)",
                 asMETHOD(Blueprint, recvDetails))
        doc("For networking: read the details of this blueprint from a "
            "message.",
            "Object this blueprint is for.", "Message to read details from.");

    bp.addMethod("bool sendDelta(Object& obj, Message& msg) const",
                 asMETHOD(Blueprint, sendDelta))
        doc("For networking: sends a delta of this blueprint through the "
            "message.",
            "Object this blueprint is for.", "Message to add delta to.",
            "Whether a delta was needed/written.");

    bp.addMethod("void recvDelta(Object& obj, Message& msg)",
                 asMETHOD(Blueprint, recvDelta))
    ...
    bp.addMethod("void create(Object& obj, const Design@ design)",
                 asMETHOD(Blueprint, create))
        doc("Initialize this blueprint.", "Object this blueprint is for.",
            "Design to initialize the blueprint with.");

    bp.addMethod("void start(Object& obj, bool fromRetrofit = false)",
                 asMETHOD(Blueprint, start))
        doc("Start the blueprint's effects.", "Object this blueprint is for.",
            "Whether this was started from a retrofit event.");

    bp.addMethod("void retrofit(Object& obj, const Design@ toDesign)",
                 asMETHOD(Blueprint, retrofit))
        doc("Retrofit the blueprint to a new design.",
            "Object this blueprint is for.", "Design to retrofit to.");

    bp.addMethod("any@+ getHookData(uint index)",
                 asMETHOD(Blueprint, getHookData))
        doc("Get data storage for a hook index.", "Hook index",
            "Data for that hook.");

    bp.addExternMethod("int& integer(const Subsystem@ sys, uint num)",
                       asFUNCTION(bpInt))
        doc("", "Subsystem to retrieve the state for.",
            "Index of the state to get.",
            "Integer state value for the subsystem.");

    bp.addExternMethod("double& decimal(const Subsystem@ sys, uint num)",
                       asFUNCTION(bpDouble))
        doc("", "Subsystem to retrieve the state for.",
            "Index of the state to get.",
            "Decimal state value for the subsystem.");

    bp.addExternMethod("bool& boolean(const Subsystem@ sys, uint num)",
                       asFUNCTION(bpBool))
        doc("", "Subsystem to retrieve the state for.",
            "Index of the state to get.",
    ...
    bp.addMethod(
        "void damage(Object& obj, DamageEvent& evt, const vec2d&in direction)",
        asMETHODPR(Blueprint, damage, (Object *, DamageEvent &, const vec2d &),
                   void))
    ...
    bp.addMethod("void damage(Object& obj, DamageEvent& evt, double position, "
                 "const vec2d&in direction)",
                 asMETHODPR(Blueprint, damage,
                            (Object *, DamageEvent &, double, const vec2d &),
                            void))
        doc("Handle damage to the blueprint from a particular direction.",
            "Object the blueprint is for.",
            "Event relating information about the damage dealt.",
    ...
    bp.addMethod(
        "void damage(Object& obj, DamageEvent& evt, const vec2u&in position)",
        asMETHODPR(Blueprint, damage, (Object *, DamageEvent &, const vec2u &),
                   void))
    ...
    bp.addMethod("void damage(Object& obj, DamageEvent& evt, const vec2u&in "
                 "position, HexGridAdjacency direction, bool runGlobal = true)",
                 asMETHODPR(Blueprint, damage,
                            (Object *, DamageEvent &, const vec2u &,
                             HexGridAdjacency, bool),
                            void))
        doc("Create a line of damage beginning at a particular hex and "
            "continuing in a straight line.",
            "Object the blueprint is for.",
            "Event relating information about the damage dealt.",
            "Hexagon position to start the damage line at.",
            "Direction to continue the damage line in.",
            "Whether to allow global damage reactors to respond to the "
            "damage.");
    ...
    bp.addMethod("void damageHex(Object& obj, DamageEvent& evt, const vec2u&in "
                 "position, bool runGlobal = true)",
                 asMETHODPR(Blueprint, damage,
                            (Object *, DamageEvent &, const vec2u &, bool),
                            void))
    ...
    bp.addMethod("double repair(Object& obj, double amount)",
        ...
        doc("Add generic repair to the blueprint.",
            "Object the blueprint is for.", "Amount of hp to repair.",
            "Amount of repair that was left over after this call.");

    bp.addMethod(
        "double repair(Object& obj, const vec2u&in position, double amount)",
        asMETHODPR(Blueprint, repair, (Object *, const vec2u &, double),
                   double))
        doc("Add repair to a particular hex on the blueprint.",
            "Object the blueprint is for.",
            "Position on the blueprint to repair.", "Amount of hp to repair.",
            "Amount of repair that was left over after this call.");

    if (!shadow) {
      InterfaceBind line("BlueprintHexLine");
      line.addMethod("bool process(const vec2u& pos)", &hexLineFunc);
      bp.addExternMethod("void runHexLine(Object& obj, BlueprintHexLine& line, "
                         "const vec2d& direction)",
                         asFUNCTION(runHexLine));
    }
  }

  // Objects can be locked easily with raii
  ClassBind lock("ObjectLock", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CD,
                 sizeof(ObjectLock));
  lock.addConstructor("void f(const Object@ obj, bool priority = false)",
                      asFUNCTION(createObjectLock));
  lock.addExternBehaviour(asBEHAVE_DESTRUCT, "void f()",
                          asFUNCTION(unlockObjectLock));
  lock.addMethod("void release()", asMETHOD(ObjectLock, release));

  // Structure for setting whether to wait for safe calls to finish
  ClassBind waitsafe("WaitForSafeCalls", asOBJ_VALUE | asOBJ_APP_CLASS_CD,
                     sizeof(bool));
  ...
  waitsafe.addExternBehaviour(asBEHAVE_DESTRUCT, "void f()",
                              asFUNCTION(destroyWaitSafe));
  ...
  // Physics searches
  bind("array<Object@>@ findInBox(const vec3d &in min, const vec3d &in max, "
       "uint ownerFilter = 0)",
       asFUNCTION(findInBox))
      doc("Returns an array of objects within the specified region.",
          "Minimum bound of the region.", "Maximum bound of the region.",
          "Optional owner filter mask. Bit 1 filters for objects not owned by "
          "a player.",
          "");

  bind("Object@ trace(const line3dd &in ray, uint ownerFilter = 0)",
       asFUNCTION(trace)) doc("Returns the first object located along a line.",
                              "Line to trace along.",
                              "Optional owner filter mask. Bit 1 filters for "
                              "objects not owned by a player.",
