...
unsigned Processor::maxIntArgs64() { return 4; }

unsigned Processor::maxFloatArgs64() { return 4; }
...
CodePage::CodePage(unsigned int Size, void *requestedStart)
    : used(0), final(false), references(1) {
  SYSTEM_INFO info;
  GetSystemInfo(&info);
  ...
  // Search for progressively more distant possible page locations, then just
  // get any available one
  for (int i = 1; i < 256; ++i) {
    void *request = (char *)requestedStart + i * pageStep;
    page = VirtualAlloc(request, size, MEM_COMMIT | MEM_RESERVE,
...
CodePage::~CodePage() { VirtualFree(page, 0, MEM_RELEASE); }
