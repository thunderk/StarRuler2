...
template <class T> void copyPtrType(GenericValue &to, GenericValue &from) {
...
void initGenericTypes() {
  ...
  auto callObj = [](Call &cl, GenericValue &val) { cl.callObjRet(val.ptr); };

  auto pushObj = [](Call &cl, ArgumentDesc &desc, GenericValue &val,
                    customHandle custom) { cl.pushObj(val.ptr); };
  ...
  genericTypes["vec3d"] =
      (new GenericType())
          ->setup<vec3d *>(
              ...
              pushObj,
              // Write to Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) {
                if (val.v3)
                  msg << *val.v3;
                else
  ...
  genericTypes["ref"] =
      (new GenericType())
          ->setup<void *>(
              ...
              // Write to Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) { throw "Cannot write a ref to message"; },
              // Read from Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) -> bool {
  ...
  genericTypes["any"] =
      (new GenericType())
          ->setup<void *>(
              ...
              // Write to Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) { throw "Cannot write any to message"; },
              // Read from Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) -> bool {
  ...
  auto callPtr = [](Call &cl, GenericValue &val) { cl.call(val.ptr); };

  auto pushPtr = [](Call &cl, ArgumentDesc &desc, GenericValue &val,
                    customHandle custom) { cl.push(val.ptr); };
  ...
  genericTypes["Node&"] = (new GenericType())
                              ->setup<void *>(
                                  "Node&",
                                  // Get from argument
                                  getPtr,
  ...
  genericTypes["Object&"] =
      (new GenericType())
          ->setup<void *>(
              ...
              // Write to Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) { writeObject(msg, val.obj); },
              // Read from Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) -> bool {
  ...
  genericTypes["Object@"] =
      (new GenericType())
          ->setup<void *>(
              ...
              callPtr,
              // Push to argument
              [](Call &cl, ArgumentDesc &desc, GenericValue &val,
                 customHandle custom) {
                if (val.obj)
                  val.obj->grab();
                cl.push(val.obj);
  ...
  genericTypes["Player&"] =
      (new GenericType())
          ->setup<void *>(
              ...
              // Write to Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) { msg << val.player->id; },
              // Read from Message
              [](net::Message &msg, ArgumentDesc &desc, GenericValue &val,
                 customRW custom) -> bool {
                int id;
...
GenericCallDesc::GenericCallDesc()
    : returnType(nullptr), argCount(0), constFunction(false),
      returnsArray(false) {}

GenericCallDesc::GenericCallDesc(std::string decl, bool hasReturnType,
                                 bool customRefs)
    : returnType(nullptr), argCount(0), constFunction(false),
      returnsArray(false) {
...
void GenericCallDesc::parse(std::string decl, bool hasReturnType,
                            bool customRefs) {
...
void GenericCallDesc::append(std::vector<std::string> &argstr,
                             bool customRefs) {
...
std::string GenericCallDesc::declaration(bool ret, bool forceConst,
                                         unsigned start) {
...
GenericCallData::GenericCallData(GenericCallData &data)
    : desc(data.desc), object(data.object) {
  data.object = 0;
...
GenericValue GenericCallData::call(Call &cl) { return desc.call(cl); }

void GenericCallData::write(net::Message &msg, unsigned startAt,
                            customRW custom) {
  for (unsigned i = startAt; i < desc.argCount; ++i) {
...
bool GenericCallData::read(net::Message &msg, unsigned startAt,
                           customRW custom) {
  for (unsigned i = startAt; i < desc.argCount; ++i) {
    GenericValue &val = values[i];
...
asIScriptFunction *getFunction(Manager *manager, const char *module,
                               GenericCallDesc &desc, bool acceptConst) {
  return manager->getFunction(module,
                              desc.declaration(true, acceptConst).c_str());
}
...
void bindGeneric(int fid, GenericCallDesc &desc, GenericHandler handler,
                 void *arg) {
...
int bindGeneric(GenericCallDesc &desc, GenericHandler handler, void *arg,
                bool acceptConst) {
  // Register the function to the engine
  auto eng = getEngine();
  int fid =
      eng->RegisterGlobalFunction(desc.declaration(true, acceptConst).c_str(),
...
int bindGeneric(ClassBind &cls, GenericCallDesc &desc, GenericHandler handler,
                void *arg, bool acceptConst) {
  // Register the function to the engine
  auto eng = getEngine();
  int fid = eng->RegisterObjectMethod(
      cls.name.c_str(), desc.declaration(true, acceptConst).c_str(),
      asFUNCTION(handleGeneric), asCALL_GENERIC);
  assert(fid >= 0);
...
int bindGeneric(ClassBind &cls, const std::string &decl, GenericCallDesc &desc,
                GenericHandler handler, void *arg) {
  auto eng = getEngine();
  int fid =
      eng->RegisterObjectMethod(cls.name.c_str(), decl.c_str(),
                                asFUNCTION(handleGeneric), asCALL_GENERIC);
  assert(fid >= 0);
