...
const char *messageNames[MT_COUNT - MT_START] = {"MT_Event_Call",
...
const char *DisconnectReasonText[] = {
    "timed out",        "error", "closed", "kicked", "version mismatch",
...
struct AsyncObjectDetailRequest : public ObjectMessage {
  ...
  AsyncObjectDetailRequest(Object *obj, int toID)
      : ObjectMessage(obj), plyID(toID) {
    obj->grab();
  }

  void process() { devices.network->sendObjectDetails(object, plyID); }

  ~AsyncObjectDetailRequest() { object->drop(); }
};
  ...
  while (!stopSync) {
    ...
    if (devices.network->hasSyncedClients && devices.network->serverReady) {
      ...
      // Send any remaining deltas and time syncs
      {
        ...
        for (auto i = devices.network->players.begin(),
                  end = devices.network->players.end();
             i != end; ++i) {
          ...
          tmsg.reset();
          tmsg << devices.driver->getGameTime()
               << devices.driver->getGameSpeed() << devices.driver->getTime();
          devices.network->send(&player, tmsg);

          auto &batch = devices.network->batches[player.id];

          auto sendBatch = [&player](bool &condition, net::Message &msg,
                                     threads::Mutex &lock) {
            if (condition) {
              lock.lock();
...
void NetworkManager::host(const std::string &gamename, int port,
                          unsigned maxPlayers, bool isPublic, bool punchthrough,
  ...
  heartbeat =
      new net::LobbyHeartbeat(net::Address(MS_SERVER, MS_PORT), BROADCAST_PORT);
  if (punchthrough)
  ...
  server->threadExit = [](bool main) { cleanupThread(); };

  server->connHandle(
      net::MT_Connect,
      [this](net::Server &srv, net::Connection &conn, net::Message &mess) {
  ...
  server->connHandle(
      ...
      [this](net::Server &srv, net::Connection &conn, net::Message &mess) {
        ...
        print("Disconnection from %s - %s (%s)", name.c_str(),
              conn.address.toString().c_str(), DisconnectReasonText[reason]);
  ...
  server->connHandle(
      ...
      [this](net::Server &srv, net::Connection &conn, net::Message &mess) {
        ...
        if (serverReady) {
          ...
          for (unsigned i = 0; i < cnt; ++i) {
            ...
            if (other->player == nullptr &&
                other->lastPlayer.ipEquals(pl->address)) {
              pl->emp = other;
              other->player = pl;
        ...
        print("%s is now known as %s", conn.address.toString().c_str(),
              nick.c_str());
  ...
  server->connHandle(
      MT_Event_Call,
      [this](net::Server &srv, net::Connection &conn, net::Message &msg) {
        Player *pl = getPlayer(conn);
        if (pl) {
  ...
  server->connHandle(
      ...
      [this](net::Server &srv, net::Connection &conn, net::Message &msg) {
        ...
        if (obj) {
          AsyncObjectDetailRequest *request =
              new AsyncObjectDetailRequest(obj, pl->id);
...
public:
  ...
  AsyncNetworkParticles(const scene::ParticleSystemDesc *System)
      : NodeEvent(nullptr), sys(System), nodeObject(nullptr) {}
  ...
  void process() override {
    ...
    scene::playParticleSystem(
        sys, node, position, rot,
        nodeObject ? velocity + nodeObject->velocity : velocity, scale);
  }
};
...
struct AsyncObjectDelta : public ObjectMessage {
  ...
  AsyncObjectDelta(Object *obj, double fromTime)
      : ObjectMessage(obj), time(fromTime), msg(net::MT_Invalid) {
    obj->grab();
  }

  void process() { object->recvDelta(msg, time); }

  ~AsyncObjectDelta() { object->drop(); }
};
...
struct AsyncObjectDetails : public ObjectMessage {
  ...
  AsyncObjectDetails(Object *obj, double fromTime)
      : ObjectMessage(obj), time(fromTime), msg(net::MT_Invalid) {
    obj->grab();
  }

  void process() { object->recvDetailed(msg, time); }

  ~AsyncObjectDetails() { object->drop(); }
};
...
struct AsyncObjectDetailsSight : public ObjectMessage {
  ...
  AsyncObjectDetailsSight(Object *obj, double fromTime)
      : ObjectMessage(obj), time(fromTime), msg(net::MT_Invalid) {
    obj->grab();
  }
  ...
  ~AsyncObjectDetailsSight() { object->drop(); }
};

NetworkManager::NetworkManager()
    : currentPlayer(1), nextPlayerId(2), isServer(false), isClient(false),
      server(0), client(0), defaultSequence(0), heartbeat(nullptr),
      punch(nullptr), query(nullptr), serverReady(false), clientReady(false),
      hasGalaxy(false), hasSyncedClients(false), connected(false),
      monitorBandwidth(false), lastMonitorTick(0), menuTimer(0.0),
      galaxyProgress(0.f), empireProgress(0.f), objectProgress(0.f),
      galaxiesInFlight(0) {
...
void NetworkManager::connect(const std::string &hostname, int port,
                             const std::string &password,
                             bool tryPunchthrough) {
  net::prepare();
  net::Address addr(hostname, port);
  connect(addr, password);
  if (tryPunchthrough)
    punch =
        new net::LobbyPunchthrough(net::Address(MS_SERVER, MS_PORT), client);
  net::clear();
}

void NetworkManager::connect(const net::Address &addr, bool tryPunchthrough,
                             bool attemptOnly, const std::string &password) {
  net::prepare();
...
void NetworkManager::connect(net::Game &game, bool disablePunchthrough,
                             const std::string &password) {
...
void NetworkManager::connect(const net::Address &addr,
                             const std::string &password, bool attemptOnly) {
  net::prepare();
  disconnect();
  resetNetState();
  ...
  client->threadExit = [](bool main) { cleanupThread(); };
  ...
  client->handle(net::MT_Disconnect, [this](net::Client &cl,
                                            net::Message &msg) {
    if (devices.cloud)
  ...
  client->handle(MT_Player_ID, [this](net::Client &cl, net::Message &msg) {
    ...
    uint64_t lobby = 0;
    msg >> lobby;
    if (devices.cloud && lobby != 0 &&
        (!devices.cloud->inQueue() && devices.cloud->getLobby() == 0))
  ...
  client->handle(MT_Object_Data, [this](net::Client &cl, net::Message &msg) {
    ...
    //  Technically the header is ~3 bits, but all of these currently use at
    //  least a char in addition
    while (msg.canRead<char>()) {
      ...
      case OE_Create: {
        ...
        msg.copyTo(tmp, msg.getReadPosition().bytes,
                   msg.getReadPosition().bytes + size);
      ...
      case OE_VisionDetails: {
        ...
        if (obj && !obj->getFlag(objUninitialized)) {
          ...
          msg.readRotation(ad->rot.xyz.x, ad->rot.xyz.y, ad->rot.xyz.z,
                           ad->rot.w);
          unsigned size = msg.readSmall();
  ...
  client->handle(MT_Script_Sync_Initial, [this](net::Client &cl,
                                                net::Message &msg) {
    std::string modname;
    msg >> modname;
  ...
  client->handle(MT_Script_Sync_Periodic,
                 [this](net::Client &cl, net::Message &msg) {
  ...
  client->handle(MT_Effector_Update,
                 [this](net::Client &srv, net::Message &msg) {
                   if (monitorBandwidth)
                     monitorIn(msg);
  ...
  client->handle(
      MT_Effector_Trigger, [this](net::Client &srv, net::Message &msg) {
        if (monitorBandwidth)
          monitorIn(msg);
        ...
        while (msg.canRead<char>()) {
          ...
          if (msg.readBit()) {
            ...
            // NOTE: If we don't know about the design, we can't get through the
            // message safely
            ...
            const Subsystem &sys =
                design->subsystems[msg.readLimited(design->subsystems.size())];
          ...
          if (!obj || !target) {
            ...
            if (eff->tracking > 0) {
              if (msg.readBit())
                msg.readDirection(targ.tracking.x, targ.tracking.y,
                                  targ.tracking.z, 8);
              else
            ...
            if (!obj->getFlag(objUninitialized) &&
                !target->getFlag(objUninitialized))
              eff->type.triggerGraphics(
                  obj, targ, eff, nullptr, nullptr, 1.f,
                  std::min(t - devices.driver->getGameTime(), 0.0));
          }
        }
      });

  client->handle(MT_Particle_System,
                 [this](net::Client &srv, net::Message &msg) {
                   std::string pSys;
                   msg >> pSys;
                   ...
                   if (msg.readBit()) {
                     ...
                     msg.readSmallVec3(effect->position.x, effect->position.y,
                                       effect->position.z);
                   } else {
                     msg.readMedVec3(effect->position.x, effect->position.y,
                                     effect->position.z);
                   }
                   msg.readSmallVec3(effect->velocity.x, effect->velocity.y,
                                     effect->velocity.z);
                   msg.readRotation(effect->rot.xyz.x, effect->rot.xyz.y,
                                    effect->rot.xyz.z, effect->rot.w);
                   msg >> effect->scale;
  ...
  client->handle(MT_Time_Sync, [this](net::Client &srv, net::Message &msg) {
    ...
    double goalTime = srvGameTime + double(srvTime - devices.driver->getTime() -
                                           serverTimeOffset) /
                                        1000.0 * srvGameSpeed;
...
void NetworkManager::queryServers() {
  if (query == nullptr) {
    // Prepare the network for Windows, the lobby query will keep the network
    // alive
    net::prepare();
...
void NetworkManager::tick(double time) {
  ...
  if (server) {
    ...
    if (!server->active) {
      disconnect();
      return;
    } else if (devices.cloud && heartbeat->identified &&
               heartbeat->externalIP) {
      unsigned port = heartbeat->address.port;
      if (heartbeat->externalPort > 0)
        port = heartbeat->externalPort;
      devices.cloud->announceServer(heartbeat->externalIP, (unsigned short)port,
                                    password);
    }
  }
...
void NetworkManager::dumpIncomingMonitor() {
  ...
  for (unsigned i = 0; i < MT_COUNT - MT_START; ++i) {
    if (lastIncomingPackets[i] == 0)
      continue;
    print("  %s: %s/s in %d packets (~%s per)", messageNames[i],
          toSize(lastIncomingData[i]).c_str(), lastIncomingPackets[i],
...
void NetworkManager::dumpOutgoingMonitor() {
  ...
  for (unsigned i = 0; i < MT_COUNT - MT_START; ++i) {
    if (lastOutgoingPackets[i] == 0)
      continue;
    print("  %s: %s/s in %d packets (~%s per)", messageNames[i],
          toSize(lastOutgoingData[i]).c_str(), lastOutgoingPackets[i],
...
void NetworkManager::dumpIncomingTotals() {
  ...
  for (unsigned i = 0; i < MT_COUNT - MT_START; ++i) {
    if (totalIncomingPackets[i] == 0)
      continue;
    print("  %s: %s in %d packets (~%s/packet, ~%s/s)", messageNames[i],
          toSize(totalIncomingData[i]).c_str(), totalIncomingPackets[i],
...
void NetworkManager::dumpOutgoingTotals() {
  ...
  for (unsigned i = 0; i < MT_COUNT - MT_START; ++i) {
    if (totalOutgoingPackets[i] == 0)
      continue;
    print("  %s: %s in %d packets (~%s/packet, ~%s/s)", messageNames[i],
          toSize(totalOutgoingData[i]).c_str(), totalOutgoingPackets[i],
...
void NetworkManager::sendMasked(net::Message &msg, unsigned mask) {
  ...
  foreach (it, players) {
    ...
    if (!pl.wantsDeltas)
      continue;
    // Only filter out players with an empire chosen (spectators still get the
    // message, should be done differently?)
...
void NetworkManager::sendOther(Player *notTo, net::Message &msg,
                               bool requireGalaxy) {
  assert(server);
  threads::ReadLock lock(playerLock);
  foreach (it, players) {
...
void NetworkManager::sendParticleSystem(const std::string &id,
                                        const vec3d &position,
                                        const vec3d &velocity,
                                        const quaterniond &rot, float scale,
                                        unsigned visionMask) {
...
void NetworkManager::sendGalaxy(Player *player) {
  ...
  for (unsigned i = 0; i < empCnt; ++i) {
    ...
    info("Sending empire '%s' (%d).", emp->name.c_str(), emp->id);
  ...
  for (unsigned i = 0; i < 256; ++i) {
    ...
    info(" %s: %d bytes, %.1f per", type->name.c_str(), objBytes[i].bytes,
         (double)objBytes[i].bytes / (double)objBytes[i].count);
  }
  bytes = 0;
...
void NetworkManager::sendObjectDelta(Object *obj) {
  ...
  for (auto i = players.begin(), end = players.end(); i != end; ++i) {
    ...
    if (!ply.conn || !ply.conn->active || !ply.wantsDeltas)
      continue;
    if (ply.emp && !obj->isVisibleTo(ply.emp) &&
        !(obj->getFlag(objMemorable) && obj->isKnownTo(ply.emp)))
      continue;
...
void NetworkManager::sendObjectDetails(Object *obj, int toPlayerID) {
  ...
  for (auto i = players.begin(), end = players.end(); i != end; ++i) {
    auto &ply = *i->second;
    if (!ply.conn || !ply.conn->active || !ply.wantsDeltas ||
        (toPlayerID != -1 && ply.id != toPlayerID))
      continue;
    if (ply.emp && !obj->isVisibleTo(ply.emp) &&
        !(obj->getFlag(objMemorable) && obj->isKnownTo(ply.emp)))
      continue;
...
void NetworkManager::sendObjectVisionDelta(Object *obj, unsigned prevMask,
                                           unsigned newMask) {
  assert(server);
  if (!hasSyncedClients)
    return;
  ...
  for (auto i = players.begin(), end = players.end(); i != end; ++i) {
    ...
    if (!ply.conn || !ply.conn->active || !ply.wantsDeltas)
      continue;
    auto &batch = batches[ply.id];
    // We check by mask rather than vision mask; the callers are responsible for
    // checking visionMask
    ...
    if (mask != 0 && (mask & newMask) == 0 && (mask & prevMask) != 0) {
      ...
      msg.writeSmallVec3(obj->acceleration.x, obj->acceleration.y,
                         obj->acceleration.z);
      msg.writeRotation(obj->rotation.xyz.x, obj->rotation.xyz.y,
                        obj->rotation.xyz.z, obj->rotation.w);
...
void NetworkManager::sendDesign(Empire *emp, DesignClass *cls,
                                const Design *dsg) {
  if (server) {
    if (!hasSyncedClients)
      return;
...
void NetworkManager::sendDesignUpdate(Empire *emp, const Design *older,
                                      const Design *newer, DesignClass *cls) {
  if (server) {
    if (!hasSyncedClients)
      return;
...
void NetworkManager::sendEffectorTrigger(const EffectorDef *eff,
                                         const Effector *efftr, Object *obj,
                                         const EffectorTarget &target,
                                         double atTime) {
  if (!hasSyncedClients)
    return;
  ...
  if (efftr->effectorId == 0) {
    ...
    msg.writeLimited(
        efftr->effectorIndex,
        dsg->subsystems[efftr->subsysIndex].type->effectors.size());
  } else {
    msg.write0();
  ...
  if (efftr->tracking > 0) {
    ...
    bool straightAt =
        (targ->position - obj->position).normalize().dot(globalDir) >
        0.99992470183; // Less than the error present in the direction
  ...
  for (auto i = players.begin(), end = players.end(); i != end; ++i) {
    auto &ply = *i->second;
    // NOTE: Normally we only require that a player requests the galaxy to send
    // events, but bullets are ephemeral and high-bandwidth
    if (!ply.conn || !ply.conn->active || !ply.hasGalaxy ||
        (ply.emp && ((ply.emp->visionMask & sendMask) == 0)))
      continue;
    auto &batch = batches[ply.id];
...
bool NetworkManager::isSerializing() { return serializationInProgress; }
