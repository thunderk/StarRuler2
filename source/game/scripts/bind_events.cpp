...
void ReadEvents(const std::string &filename) {
  ...
  while (datafile++) {
    ...
    if (!local && sendToServer.returnType.type) {
      error(datafile.position() +
            " - ERROR: Non-local events cannot have return values.");
      continue;
    } else if (sendToServer.returnType == GT_Custom_Handle) {
      error(datafile.position() +
            " - ERROR: Return type cannot be a custom class.");
      continue;
    }
    ...
    if (local && !recvServer && !recvShadow) {
      error(datafile.position() +
            " - ERROR: Send-to-client events cannot be marked as local.");
      continue;
    }
    ...
    if (scopes.size() != 3) {
      error(
          (datafile.position() + " - ERROR: Invalid event handler format '%s'.")
              .c_str(),
          handler.c_str());
      continue;
    }
    ...
    if (!desc.recvServer || desc.recvLocal || !devices.network->isClient) {
      ...
      if (!desc.recvFunc) {
        if (desc.recvServer || desc.recvShadow) {
          ...
          desc.recvFunc = man->getFunction(
              desc.recvModule.c_str(), desc.recvCall.declaration(true).c_str());
        }
        if (!desc.recvFunc) {
    ...
    for (unsigned i = 0; i < aCnt; ++i) {
      ...
      if (arg.type == GT_Custom_Handle) {
        ...
        if (desc.recvMenu) {
          ...
          rcvType = man->engine->GetTypeInfoByName(in_name.c_str());
          srType = devices.scripts.server->engine->GetTypeInfoByName(
              in_name.c_str());
          clType = devices.scripts.client->engine->GetTypeInfoByName(
              in_name.c_str());
        }
        ...
        if (!srType || !clType || !rcvType) {
          ...
          arg.customRead =
              (void *)rcvType->GetMethodByDecl("void read(Message&)");
          arg.customWrite =
              (void *)rcvType->GetMethodByDecl("void write(Message&)");

          unsigned r = i;
          if (desc.recvServer && desc.passPlayer)
            --r;

          desc.sendToClient.arguments[r + 1].customWrite =
              (void *)srType->GetMethodByDecl("void write(Message&)");

          desc.sendToServer.arguments[r].customType = arg.customType;
          desc.sendToServer.arguments[r].customRead = arg.customRead;
          desc.sendToServer.arguments[r].customWrite =
              (void *)clType->GetMethodByDecl("void write(Message&)");
        }

        if (!arg.customType) {
          error("Error: Could not find class '%s' in module '%s' for event "
                "description.",
...
GenericValue handleSendToServer(void *arg, GenericCallData &args) {
  ...
  if (desc.recvLocal || !devices.network->isClient) {
    if (desc.recvFunc && desc.recvManager) {
      ...
      if (desc.onlyPrimitive) {
        ...
        for (unsigned i = 0; i < args.desc.argCount; ++i) {
          ...
          if (!arg.customWrite || !arg.customRead || !arg.customType ||
              !desc.recvManager)
            continue;
          ...
          {
            scripts::Call cl =
                getActiveManager()->call((asIScriptFunction *)arg.customWrite);
          ...
          asIScriptObject *ret =
              (asIScriptObject *)desc.recvManager->engine->CreateScriptObject(
                  type);

          // Call read function
          {
            scripts::Call cl =
                desc.recvManager->call((asIScriptFunction *)arg.customRead);
            cl.setObject(ret);
            cl.push(&msg);
            cl.call();
          }
        ...
        auto transferCustom = [&](ArgumentDesc &arg,
                                  asIScriptObject *ptr) -> asIScriptObject * {
    ...
    auto writeCustom = [](net::Message &msg, ArgumentDesc &adesc,
                          GenericValue &val) {
      ...
      if (adesc.customWrite) {
        // Call write function
        scripts::Call cl =
            getActiveManager()->call((asIScriptFunction *)adesc.customWrite);
...
GenericValue handleSendToClient_local(void *arg, GenericCallData &args) {
  ...
  auto writeCustom = [&](net::Message &msg, ArgumentDesc &adesc,
    ...
    if (adesc.customWrite) {
      // Call write function
      scripts::Call cl =
          devices.scripts.server->call((asIScriptFunction *)adesc.customWrite);
      cl.setObject(val.script);
      cl.push(&msg);
      cl.call();
    }
...
GenericValue handleSendToClient(void *arg, GenericCallData &args) {
  ...
  if (!devices.network->isServer || pl == nullptr ||
      pl == &devices.network->currentPlayer) {
    msg.finalize();
    msg.rewind();
    handleEventMessage(&devices.network->currentPlayer, msg);
...
scripts::Manager *handleEventMessage(Player *from, net::Message &msg,
                                     bool interceptMenu) {
  int eventId;
  msg >> eventId;
  ...
  if (!edesc.recvMenu && !game_running) {
    ...
    queuedClientEvents.push(
        std::tuple<Player, net::Message *>(*from, new net::Message(msg)));
    return nullptr;
  }
  ...
  auto readCustom = [edesc](net::Message &msg, ArgumentDesc &desc,
                            GenericValue &val) {
    if (desc.customType && desc.customRead && edesc.recvManager) {
      // Create object
      asITypeInfo *type = (asITypeInfo *)desc.customType;
      val.script =
          (asIScriptObject *)edesc.recvManager->engine->CreateScriptObject(
              type);

      // Call read function
      scripts::Call cl =
          edesc.recvManager->call((asIScriptFunction *)desc.customRead);
      cl.setObject(val.script);
      cl.push(&msg);
      cl.call();
    }
...
static Player *getPlayer(int id) { return devices.network->getPlayer(id); }

static std::string playerName(Player &player) {
  return std::string(player.nickname);
}

static void ctor_addr(void *mem) { new (mem) net::Address(); }
...
static void dtor_addr(net::Address &addr) { addr.~Address(); }

static void ctor_game(void *mem) { new (mem) net::Game(); }
...
static void dtor_game(net::Game &addr) { addr.~Game(); }

static void queryServers() { devices.network->queryServers(); }
...
static void mpKick(int playerId) { devices.network->kick(playerId); }

static void mpConnect(net::Game &game, bool disablePunchthrough,
                      const std::string &pwd) {
  devices.network->connect(game, disablePunchthrough, pwd);
}

static void hostGame(const std::string &gamename, int port, unsigned maxPlayers,
                     bool isPublic, bool punchthrough,
                     const std::string &password) {
  devices.network->host(gamename, port, maxPlayers, isPublic, punchthrough,
                        password);
}

static unsigned dcReason() { return devices.network->disconnection; }

static float glxProgress() { return devices.network->galaxyProgress; }
...
void RegisterEventBinds(bool server, bool shadow, bool menu) {
  ...
  player.addExternMethod("bool opEquals(const Player& other) const",
                         asFUNCTION(playerEquals));
  if (server)
    player.addExternMethod("void linkEmpire(Empire@ empire)",
                           asFUNCTION(linkEmpire));
  ...
  ClassBind ga("GameAddress", asOBJ_VALUE | asOBJ_APP_CLASS_CDA,
               sizeof(net::Address));
  ga.addConstructor("void f()", asFUNCTION(ctor_addr));
  ga.addConstructor("void f(const string& hostname, int port = 2048)",
                    asFUNCTION(ctor_addrv));
  ga.addDestructor("void f()", asFUNCTION(dtor_addr));
  ga.addExternMethod("GameAddress& opAssign(const GameAddress& other)",
                     asFUNCTION(cpy_addr));
  ga.addExternMethod("bool opEquals(const GameAddress& other) const",
                     asFUNCTION(eq_addr));
  ga.addMember("int port", offsetof(net::Address, port));
  ga.addMethod("string toString(bool showPort = true) const",
               asMETHOD(net::Address, toString));

  ClassBind gs("GameServer", asOBJ_VALUE | asOBJ_APP_CLASS_CDA,
               sizeof(net::Game));
  gs.addConstructor("void f()", asFUNCTION(ctor_game));
  gs.addDestructor("void f()", asFUNCTION(dtor_game));
  gs.addExternMethod("GameServer& opAssign(const GameServer& other)",
                     asFUNCTION(cpy_game));
  ...
  bind("void mpGetServers(array<GameServer>& list)",
       asFUNCTION(getQueriedServers));
  bind("void mpKick(int playerId)", asFUNCTION(mpKick));
  bind("void mpHost(const string& gamename = \"\", uint port = 2048, uint "
       "maxPlayers = 0, bool isPublic = true, bool punchthrough = true, const "
       "string& password = \"\")",
       asFUNCTION(hostGame));
  bind("void mpConnect(const GameServer& game, bool disablePunchthrough = "
       "false, const string& password = \"\")",
       asFUNCTION(mpConnect));

  if (void *plArray = getEngine()->GetTypeInfoById(
          getEngine()->GetTypeIdByDecl("array<Player@>")))
