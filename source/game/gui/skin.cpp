...
bool elementLower(Element *x, Element *y) { return x->flags < y->flags; }

int getDynamicIndex(std::map<std::string, unsigned> &dyn,
                    const std::string &name, bool addIfMissing) {
...
unsigned getStyleCount() { return (unsigned)dynStyleIndices.size(); }
...
void Style::addElement(Element *ele) {
  auto at =
      std::lower_bound(elements.begin(), elements.end(), ele, elementLower);
  if (at == elements.end())
...
void Layer::draw(render::RenderDriver &driver, recti pos, const recti *clip,
                 Color color) const {
...
void Element::draw(render::RenderDriver &driver, recti pos, const recti *clip,
                   Color color) const {
...
bool Element::isPixelActive(recti box, vec2i px) const {
  ...
  case DM_Tiled:
    if (px.x < dmargin.topLeft.x) {
      tx = (int)((float)px.x * (float)margin.topLeft.x /
                 (float)dmargin.topLeft.x) +
           area.topLeft.x;
    } else if (px.x >= box.getWidth() - dmargin.botRight.x) {
      tx = area.botRight.x -
           (box.getWidth() - (int)((float)px.x * (float)margin.botRight.x /
                                   (float)dmargin.botRight.x));
    } else {
      tx = area.topLeft.x + margin.topLeft.x;
      if (horizMode == DM_Scaled) {
        tx += (int)(((double)(px.x - dmargin.topLeft.x) /
                     (double)(box.getWidth() - dmargin.topLeft.x -
                              dmargin.botRight.x)) *
                    (double)(area.getWidth() - margin.topLeft.x -
                             margin.botRight.x));
      } else {
        tx += (px.x - dmargin.topLeft.x) %
              (area.getWidth() - margin.topLeft.x - margin.botRight.x);
      }
    }
    break;
  }
  ...
  case DM_Tiled:
    if (px.y < dmargin.topLeft.y) {
      ty = (int)((float)px.y * (float)margin.topLeft.y /
                 (float)dmargin.topLeft.y) +
           area.topLeft.y;
    } else if (px.y >= box.getWidth() - dmargin.botRight.y) {
      ty = area.botRight.y -
           (box.getHeight() - (int)((float)px.y * (float)margin.botRight.y /
                                    (float)dmargin.botRight.y));
    } else {
      ty = area.topLeft.y + margin.topLeft.y;
      if (horizMode == DM_Scaled) {
        ty += (int)(((double)(px.y - dmargin.topLeft.y) /
                     (double)(box.getHeight() - dmargin.topLeft.y -
                              dmargin.botRight.y)) *
                    (double)(area.getHeight() - margin.topLeft.y -
                             margin.botRight.y));
      } else {
        ty += (px.y - dmargin.topLeft.y) %
              (area.getHeight() - margin.topLeft.y - margin.botRight.y);
      }
    }
    break;
  }
