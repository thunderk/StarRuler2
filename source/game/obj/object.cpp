...
const char *obj_callback_decl[SOC_COUNT] = {
    ...
    "void damage($1& obj, DamageEvent& evt, double position, const vec2d& "
    "direction)",
...
unsigned getScriptObjectTypeCount() { return (unsigned)objTypeList.size(); }
    ...
    if (states == &errorStateDefinition) {
      ...
      if (states->base) {
        if (states->base != objData) {
          states = objData;
          error("ERROR: %s does not inherit from Object.", type.name.c_str());
        }
...
void Object::grab() const { ++references; }
...
void Object::queueDeferredMessage(ObjectMessage *msg) {
  ...
  defer->next = prev;
  while (threads::compare_and_swap((void **)&deferredMessages, (void *)prev,
                                   (void *)defer) != prev) {
...
double Object::think(double seconds) {
      ...
      if (owner && owner->valid() && sightRange > 0.0) {
        ...
        if (sightDelay <= sightCheckSteps) {
          ...
          devices.physics->findInBox(
              AABBoxd::fromCircle(position, sightRange),
              [mask, from, dist](const PhysicsItem &item) {
                ...
                if (distance <
                    (dist + other->radius) * (dist + other->radius)) {
                  if (distance < (other->seeableRange + other->radius) *
                                     (other->seeableRange + other->radius))
                    other->donatedVision |= mask;
                }
              },
              ~owner->mask);

          // Delay psuedo-randomly by 1.5-2.8 seconds (vision persists for 3
          // seconds)
  ...
  if (physItem) {
    if (!group) {
      // Update physics positon (this can move across a grid boundary, which
      // needs access to the world)
      if (position != prevPos)
        devices.physics->updateItem(*physItem,
                                    AABBoxd::fromCircle(position, radius));
    } else {
      // Update our position without informing the group
      if (position != prevPos)
        physItem->bound = AABBoxd::fromCircle(position, radius);
      // If we are the group's owner, update the group to match the members'
      // locations (touches world)
  ...
  if (devices.network->isServer && devices.network->hasSyncedClients) {
    ...
    if (newEmpireVisibleMask != prevVisibleMask) {
      devices.network->sendObjectVisionDelta(this, prevVisibleMask,
                                             newEmpireVisibleMask);
...
void Object::damage(DamageEvent &evt, double position, const vec2d &direction) {
  if (type && script) {
    scripts::Call cl =
        devices.scripts.server->call(type->functions[SOC_damage]);
    if (cl.valid()) {
      cl.setObject(script);
      cl.push((void *)this);
      cl.push((void *)&evt);
...
void Object::repair(double amount) {
  if (type && script) {
    scripts::Call cl =
        devices.scripts.server->call(type->functions[SOC_repair]);
    if (cl.valid()) {
      cl.setObject(script);
      cl.push((void *)this);
      cl.push(amount);
...
Object::Object(ScriptObjectType *Type, LockGroup *group, int ID)
    : references(1), id(0), group(0),
      lockGroup(group ? group : getRandomLock()), originalLock(0), lockHint(0),
      deferredMessages(0), owner(Empire::getDefaultEmpire()),
      alwaysVisible(false), sightRange(1500.f), seeableRange(FLT_MAX),
      visionTimes(), visibleMask(0), donatedVision(0), prevVisibleMask(0),
      sightedMask(0), sightDelay(0), flags(objValid | objUninitialized),
      physItem(0), radius(0), lastTick(devices.driver->getGameTime()), node(0),
      script(0), type(Type) {
  if (ID != 0) {
    id = ID;
  } else {
...
bool Object::isLocked() const {
  return getActiveLockGroup() == lockGroup || !processing::isRunning() ||
         getFlag(objUninitialized);
}
...
void Object::postInit() {
  if (type) {
    scripts::Call cl =
        devices.scripts.server->call(type->functions[SOC_postInit]);
    if (cl.valid() && script) {
      cl.setObject(script);
      cl.push((void *)this);
      cl.call();
...
void Object::setOwner(Empire *newOwner) {
  ...
  if (type) {
    scripts::Call cl =
        devices.scripts.server->call(type->functions[SOC_ownerChange]);
    if (cl.valid() && script) {
      cl.setObject(script);
      cl.push((void *)this);
...
void Object::destroy(bool fromUniverse) {
  if (getFlag(objValid)) {
    ...
    if (type && !fromUniverse && script) {
      scripts::Call cl =
          devices.scripts.server->call(type->functions[SOC_destroy]);
      if (cl.valid()) {
        cl.setObject((asIScriptObject *)script);
        cl.push((void *)this);
        cl.call();
    ...
    if (!group) {
      ...
      if (group->getOwner() == this) {
        if (group->removeOwner()) {
          scripts::Call cl =
              devices.scripts.server->call(type->functions[SOC_groupDestroyed]);
          if (cl.valid() && script) {
            cl.setObject((asIScriptObject *)script);
            cl.push((void *)this);
            cl.call();
...
bool Object::getFlag(ObjectFlag flag) const { return (flags & flag) != 0; }
...
bool Object::setFlagSecure(ObjectFlag flag, bool flagOn) {
  ...
  do {
    ...
    // See what the new value would be, and keep trying until we are sure of the
    // result
...
bool Object::isValid() const { return getFlag(objValid); }

bool Object::isFocus() const { return getFlag(objFocus); }
...
bool Object::isInitialized() const { return !getFlag(objUninitialized); }

void Object::wake() { setFlag(objWakeUp, true); }

void Object::sleep(double seconds) { nextTick += seconds; }

void Object::updateTargets() {
  // Non-physical objects don't track targets.
  // However, we may load an old save where they did, so try to collect our
  // pointers.
  ...
  // If we have a better object to swap with, do so. Otherwise, occasionally
  // swap to a random target to prevent stagnation
  if (best)
...
static inline void sendDetails(net::Message &msg, Object &obj,
                               bool sendStates = true,
...
static inline void recvDetails(net::Message &msg, Object &obj,
                               bool recvStates = true,
                               bool recvPosition = true) {
  unsigned char empID;
  msg >> empID;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    obj.stats.set(uint64_t(type) << 56 | uint64_t(left) << 32 | uint64_t(right),
                  value);
  }
...
Object *recvObjectInitial(net::Message &msg, double fromTime) {
  ...
  if (msg.readBit())
    msg.readSmallVec3(obj->acceleration.x, obj->acceleration.y,
                      obj->acceleration.z);

  obj->lastTick = fromTime;
  obj->nextTick = obj->lastTick;

  msg.readRotation(obj->rotation.xyz.x, obj->rotation.xyz.y,
  ...
  if (msg.readBit()) {
    ...
    obj->physItem = devices.physics->registerItem(
        AABBoxd::fromCircle(obj->position, obj->radius), obj);
  }
...
ObjectLock::ObjectLock(Object *Obj, bool priority) : released(false) {
...
ScriptObjectType::ScriptObjectType()
    : nextID(1), states(0), scriptType(0), blueprintOffset(0) {
...
void ScriptObjectType::bind(const char *module, const char *decl) {
  ...
  for (int i = 0; i < SOC_COUNT; ++i)
    functions[i] =
        scriptType->GetMethodByDecl(format(obj_callback_decl[i], name).c_str());
  ...
  if (states) {
    for (size_t i = 0; i < states->methods.size(); ++i) {
      ...
      if (method->wrapped.constFunction) {
        ...
        desc.prepend(scripts::ArgumentDesc(scripts::GT_Object_Ref,
                                           method->wrapped.constFunction));
      ...
      // Prepend the containing object to see if we need it
      desc.prepend(
          scripts::ArgumentDesc(refType, method->wrapped.constFunction));
      ...
      if (!devices.network->isClient || method->shadow ||
        ...
        error("Could not find %s method '%s'", name.c_str(),
