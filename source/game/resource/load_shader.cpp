...
ShaderGlobal::ShaderGlobal()
    : type(render::Shader::VT_invalid), size(0), ptr(0), arraySize(1) {}
...
void loadToInts(int *ints, std::vector<std::string>::iterator from,
                std::vector<std::string>::iterator to) {
  for (; from != to; ++from, ++ints)
    *ints = atoi(from->c_str());
}

void loadToInt2s(int *ints, std::vector<std::string>::iterator from,
                 std::vector<std::string>::iterator to) {
  for (; from != to; ++from, ints += 2)
    sscanf(from->c_str(), "<%d,%d>", ints, ints + 1);
}

void loadToInt3s(int *ints, std::vector<std::string>::iterator from,
                 std::vector<std::string>::iterator to) {
  for (; from != to; ++from, ints += 3)
    sscanf(from->c_str(), "<%d,%d,%d>", ints, ints + 1, ints + 2);
}

void loadToInt4s(int *ints, std::vector<std::string>::iterator from,
                 std::vector<std::string>::iterator to) {
  for (; from != to; ++from, ints += 4)
    sscanf(from->c_str(), "<%d,%d,%d,%d>", ints, ints + 1, ints + 2, ints + 3);
}

void loadToFloats(float *floats, std::vector<std::string>::iterator from,
                  std::vector<std::string>::iterator to) {
  for (; from != to; ++from, ++floats)
    *floats = (float)atof(from->c_str());
}

void loadToFloat2s(float *floats, std::vector<std::string>::iterator from,
                   std::vector<std::string>::iterator to) {
  for (; from != to; ++from, floats += 2)
    sscanf(from->c_str(), "<%f,%f>", floats, floats + 1);
}

void loadToFloat3s(float *floats, std::vector<std::string>::iterator from,
                   std::vector<std::string>::iterator to) {
  for (; from != to; ++from, floats += 3)
    sscanf(from->c_str(), "<%f,%f,%f>", floats, floats + 1, floats + 2);
}

void loadToFloat4s(float *floats, std::vector<std::string>::iterator from,
                   std::vector<std::string>::iterator to) {
  for (; from != to; ++from, floats += 4)
    sscanf(from->c_str(), "<%f,%f,%f,%f>", floats, floats + 1, floats + 2,
           floats + 3);
}
...
void Library::iterateShaderGlobals(
    std::function<void(std::string &, resource::ShaderGlobal *)> func) {
  foreach (it, shaderGlobals)
...
void Library::loadShaders(const std::string &filename) {
  ...
  auto BuildShader = [&]() {
    ...
    if (load_resources)
      program = devices.render->createShaderProgram(vertex_file.c_str(),
                                                    fragment_file.c_str());
          ...
          if (control == "#if" || control == "#elif") {
            if (anyBlockActive) {
              ...
              if (condition == "fallback") {
                auto *fallback =
                    devices.settings.engine.getSetting("bShaderFallback");
                if (fallback)
                  enterBlock = *fallback;
  ...
  datahandler("Variable", [&](std::string &value) {
        ...
        case render::Shader::VT_float:
          if (call == "time") {
            ...
            loadToFloats(pArgs, args.begin() + 1,
                         (int)args.size() <= var.count + 1
                             ? args.end()
                             : args.begin() + (1 + var.count));
    ...
    if (var._intcall == 0) {
      auto from = args.begin(), to = args.size() <= var.count
                                         ? args.end()
                                         : args.begin() + var.count;
      switch (var.type) {
      case render::Shader::VT_int:
        loadToInts(var._ints, from, to);
        break;
      case render::Shader::VT_int2:
        loadToInt2s(var._ints, from, to);
        break;
      case render::Shader::VT_int3:
        loadToInt3s(var._ints, from, to);
        break;
      case render::Shader::VT_int4:
        loadToInt4s(var._ints, from, to);
        break;

      case render::Shader::VT_float:
        loadToFloats(var._floats, from, to);
        break;
      case render::Shader::VT_float2:
        loadToFloat2s(var._floats, from, to);
        break;
      case render::Shader::VT_float3:
        loadToFloat3s(var._floats, from, to);
        break;
      case render::Shader::VT_float4:
        loadToFloat4s(var._floats, from, to);
        break;
      }
    }
