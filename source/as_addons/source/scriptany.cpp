...
void RegisterScriptAny_Native(asIScriptEngine *engine) {
  ...
  r = engine->RegisterObjectBehaviour("any", asBEHAVE_FACTORY, "any@ f()",
                                      ...
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("any", asBEHAVE_FACTORY, "any@ f(?&in)",
                                      asFUNCTION(ScriptAnyFactory2_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "any", asBEHAVE_FACTORY, "any@ f(const int64&in)",
      asFUNCTION(ScriptAnyFactory2_Generic), asCALL_GENERIC);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "any", asBEHAVE_FACTORY, "any@ f(const double&in)",
      asFUNCTION(ScriptAnyFactory2_Generic), asCALL_GENERIC);
  assert(r >= 0);
  ...
  // asMETHOD(CScriptAny,GetRefCount), asCALL_THISCALL); assert( r >= 0 ); r =
  // engine->RegisterObjectBehaviour("any", asBEHAVE_SETGCFLAG, "void f()",
  // asMETHOD(CScriptAny,SetFlag), asCALL_THISCALL); assert( r >= 0 ); r =
  // engine->RegisterObjectBehaviour("any", asBEHAVE_GETGCFLAG, "bool f()",
  // asMETHOD(CScriptAny,GetFlag), asCALL_THISCALL); assert( r >= 0 ); r =
  // engine->RegisterObjectBehaviour("any", asBEHAVE_ENUMREFS, "void f(int&in)",
  // asMETHOD(CScriptAny,EnumReferences), asCALL_THISCALL); assert( r >= 0 ); r
  // = engine->RegisterObjectBehaviour("any", asBEHAVE_RELEASEREFS, "void
  // f(int&in)", asMETHOD(CScriptAny,ReleaseAllHandles), asCALL_THISCALL);
  // assert( r >= 0 );
}
...
void RegisterScriptAny_Generic(asIScriptEngine *engine) {
  ...
  // Register GC behaviours
  r = engine->RegisterObjectBehaviour("any", asBEHAVE_GETREFCOUNT, "int f()",
                                      asFUNCTION(ScriptAny_GetRefCount_Generic),
                                      asCALL_GENERIC);
  assert(r >= 0);
...
bool CScriptAny::Retrieve(void *ref, int refTypeId) const {
  ...
  if (refTypeId & asTYPEID_OBJHANDLE) {
    ...
    if ((value.typeId & asTYPEID_MASK_OBJECT)) {
      // Don't allow the retrieval if the stored handle is to a const object but
      // not the wanted handle
      if ((value.typeId & asTYPEID_HANDLETOCONST) &&
          !(refTypeId & asTYPEID_HANDLETOCONST))
        return false;

      // RefCastObject will increment the refCount of the returned pointer if
      // successful
      engine->RefCastObject(
          value.valueObj, engine->GetTypeInfoById(value.typeId),
          engine->GetTypeInfoById(refTypeId), reinterpret_cast<void **>(ref));
      if (*(asPWORD *)ref == 0)
        return false;
      return true;
    }
