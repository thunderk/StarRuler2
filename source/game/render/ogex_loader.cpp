...
struct StringView {
  ...
  void substr(std::string &out, std::string::size_type first,
              std::string::size_type count) {
    out.assign(str.begin() + (start + first),
               str.begin() + (start + first + count));
  }

  void advance(std::string::size_type off) { start += off; }
};
...
void loadMeshOGEX(const char *filename, Mesh &mesh) {
  ...
  while (file.good()) {
    ...
    if (line == "{") {
      ...
      innerParse = nullptr;
    } else if (line == "float[16]" || line == "float[3]" ||
               line == "float[2]") {
      // Simplify parsing
    } else if (innerParse) {
      if (innerParse(line))
      ...
      if (line[0] != '{' && splitKeyValue(line, head, tail, " ") &&
          !tail.empty()) {
        ...
        if (tail[0] == '{') {
          ...
          if (head == "Mesh") {
            ...
            if (section.attrib == "position") {
              innerParse = [&](std::string &line) -> bool {
                ...
                while (!segment.empty()) {
                  ...
                  if (values.size() == 3)
                    part->positions.push_back(vec3f(atof(values[0].c_str()),
                                                    atof(values[1].c_str()),
                                                    atof(values[2].c_str())));
                  values.clear();
              ...
              innerParse = [&](std::string &line) -> bool {
                ...
                while (!segment.empty()) {
                  split(segment, values, ",", true);
                  if (values.size() == 3)
                    part->normals.push_back(vec3f(atof(values[0].c_str()),
                                                  atof(values[1].c_str()),
                                                  atof(values[2].c_str())));
                  values.clear();
            ...
            } else if (section.attrib == "texcoord" ||
                       section.attrib == "texcoord[1]" ||
                       section.attrib == "texcoord[2]") {
              int index = 0;
              ...
              innerParse = [&, index](std::string &line) -> bool {
                ...
                while (!segment.empty()) {
                  ...
                  if (values.size() == 2) {
                    vec2f uv =
                        vec2f(atof(values[0].c_str()), atof(values[1].c_str()));
              ...
              innerParse = [&](std::string &line) -> bool {
                ...
                while (!segment.empty()) {
                  split(segment, values, ",", true);
                  if (values.size() == 3)
                    part->colors.push_back(Colorf(atof(values[0].c_str()),
                                                  atof(values[1].c_str()),
                                                  atof(values[2].c_str())));
                  values.clear();
      ...
      readIndexArray:
        innerParse = [&](std::string &line) -> bool {
          ...
          while (!segment.empty()) {
            split(segment, values, ",", true);
            if (values.size() == 3)
              part->faces.push_back(Mesh::Face(atoi(values[0].c_str()),
                                               atoi(values[1].c_str()),
                                               atoi(values[2].c_str())));
            values.clear();
  ...
  auto process = [&](GEXNode &n, Matrix transform) {
    ...
    if (p != parts.end()) {
      ...
      for (size_t i = 0; i < part.faces.size(); ++i) {
        ...
        if (face.a < mesh.vertices.size() && face.b < mesh.vertices.size() &&
