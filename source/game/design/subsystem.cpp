...
int getSubsystemDefCount() { return (int)subsystems.size(); }
...
unsigned getShipVariableCount() { return (unsigned)shipVariableIndices.size(); }
...
static int formulaVarIndex(const std::string *nameptr) {
  ...
  if (name.find('.') != std::string::npos) {
    ...
    if (formulaSubsystem) {
      for (unsigned i = 0, cnt = (unsigned)formulaSubsystem->modules.size();
           i < cnt; ++i) {
...
Formula *parseFormula(const std::string &str, const SubsystemDef *def,
                      const SubsystemDef::ModifyStage *modifier) {
...
bool conditionMatches(const SubsystemDef *cur,
                      std::vector<std::pair<int, std::string>> &conditions) {
  bool passesAll = true;
...
void parseConditions(const std::string &value,
                     std::vector<std::pair<int, std::string>> &conditions) {
  ...
  foreach (it, conds) {
    ...
    conditions.push_back(std::pair<int, std::string>(cond, value));
  }
}

void parseShipModifier(SubsystemDef::ShipModifier &mod,
  ...
  if (funcSplit(func, name, args)) {
    mod.modifyName = name;
    for (size_t i = 0, cnt = args.size(); i < cnt && i < MODIFY_STAGE_MAXARGS;
         ++i)
...
void loadSubsystemDefinitions(const std::string &filename) {
...

  {
    ...
    templateBlock.lineHandler(
        [&](std::string &line) { temp->lines.push_back(line); });
  }
  {
    ...
    subsysBlock("BaseColor",
                [&](std::string &value) { def->baseColor = toColor(value); });

    subsysBlock("TypeColor",
                [&](std::string &value) { def->typeColor = toColor(value); });
    ...
    subsysBlock("Picture", [&](std::string &value) { def->picMat = value; });
    ...
    subsysBlock("Tags", [&](std::string &value) {
      ...
      def->alwaysTakeDamage =
          def->tags.find("AlwaysTakeDamage") != def->tags.end();
    ...
    subsysBlock("OnCheckErrors",
                [&](std::string &value) { def->def_onCheckErrors = value; });
    ...
    subsysBlock("Hook",
                [&](std::string &value) { def->hooks.push_back(value); });
...

    {
      ...
      modifyBlock.closeBlock([&]() { stage = 0; });
      ...
      modifyBlock.defaultHandler([&](std::string &key, std::string &value) {
        ...
        if (!value.empty() && value[0] == '=') {
          ...
          case SVT_SubsystemVariable: {
            ...
            stage->str_variables.push_back(
                std::pair<int, std::string>(var.index, value));
          } break;
          case SVT_HexVariable: {
            ...
            stage->str_hexVariables.push_back(
                std::pair<int, std::string>(var.index, value));
          } break;
          case SVT_ShipVariable: {
            ...
            stage->str_shipVariables.push_back(
                std::pair<int, std::string>(var.index, value));
          } break;
          }
        } else {
...

    {
      ...
      moduleBlock("Color",
                  [&](std::string &value) { mod->color = toColor(value); });

      moduleBlock("Required",
                  [&](std::string &value) { mod->required = toBool(value); });

      moduleBlock("Unique",
                  [&](std::string &value) { mod->unique = toBool(value); });

      moduleBlock("Vital",
                  [&](std::string &value) { mod->vital = toBool(value); });
      ...
      moduleBlock("Sprite",
                  [&](std::string &value) { mod->spriteMat = value; });
      ...
      moduleBlock("Hook",
                  [&](std::string &value) { mod->hooks.push_back(value); });

      moduleBlock("OnEnable",
                  [&](std::string &value) { mod->def_onEnable = value; });

      moduleBlock("OnDisable",
                  [&](std::string &value) { mod->def_onDisable = value; });
      ...
      moduleBlock("OnCheckErrors",
                  [&](std::string &value) { mod->def_onCheckErrors = value; });
...

      {
        ...
        assertBlock("Unique",
                    [&](std::string &value) { ass->unique = toBool(value); });

        assertBlock("Fatal",
                    [&](std::string &value) { ass->fatal = toBool(value); });

        assertBlock("Message", [&](std::string &value) {
          ass->message = devices.locale.localize(value);
        });

        assertBlock.closeBlock([&]() { ass = 0; });
      }
      {
        auto &requiresBlock = moduleBlock.block("Requires");
        requiresBlock.openBlock(
            [&](std::string &value) -> bool { return true; });
        ...
        requiresBlock.lineHandler([&](std::string &line) {
          ...
          std::string vformula =
              format("Ship.REQUIRES_$1 + $2", args[0], args[1]);
          ...
          ass.message =
              devices.locale.localize(format("ERROR_INSUFFICIENT_$1", args[0]));
    ...
    }
    {
      ...
      effectorBlock("Skin", [&](std::string &value) {
        ...
        if (s != efftr->type->skinNames.end()) {
          ...
          error("Effector '%s' has no skin '%s'", efftr->type->name.c_str(),
                value.c_str());
      });

      effectorBlock.closeBlock([&]() { efftr = 0; });
    ...
    }
    {
      ...
      requiresBlock.openBlock([&](std::string &value) -> bool { return true; });
      ...
      requiresBlock.lineHandler([&](std::string &line) {
        ...
        var.str_formula = format("Ship.REQUIRES_$1 + $2", args[0], args[1]);
    ...
    }
    {
      ...
      providesBlock.openBlock([&](std::string &value) -> bool { return true; });
...
void bindSubsystemHooks() {
  foreach (it, subsystems) {
    ...
    if (!def->def_onCheckErrors.empty())
      def->scr_onCheckErrors = devices.scripts.server->getFunction(
          def->def_onCheckErrors, "(Design& design, Subsystem& sys)", "bool");

    foreach (it, def->modules) {
      auto *m = *it;
      if (!m->def_onCheckErrors.empty())
        m->scr_onCheckErrors = devices.scripts.server->getFunction(
            m->def_onCheckErrors,
    ...
    if (!devices.network->isClient) {
      foreach (it, def->modules) {
        ...
        if (!m->def_onEnable.empty()) {
          m->scr_onEnable = devices.scripts.server->getFunction(
              m->def_onEnable, "(Event& evt, const vec2u& position)", "void");
        } else {
          m->scr_onEnable = 0;
        }

        if (!m->def_onDisable.empty()) {
          m->scr_onDisable = devices.scripts.server->getFunction(
              m->def_onDisable, "(Event& evt, const vec2u& position)", "void");
        } else {
          m->scr_onDisable = 0;
        }
      }
...
SubsystemDef::SubsystemDef()
    : index(-1), ordering(0), damageOrder(0), elevation(0), defaultModule(0),
      coreModule(0), typeColor((unsigned)0x00000000), scr_onCheckErrors(0),
...
void SubsystemDef::finalize() {
  ...
  auto loadFormula = [this](const std::string &str) -> Formula * {
    try {
      ...
      error("Error in Subsystem '%s' formula '%s': %s", id.c_str(), str.c_str(),
            err.msg.c_str());
  ...
  auto addModifier = [this](const std::string &name, const std::string &argname,
                            int prior) -> ModifyStage * {
    // Check for duplicates
  ...
  auto addVariableModifiers = [this, addModifier](SubsystemDef::Variable &var) {
    ...
    if (stage) {
      ...
      case SVT_SubsystemVariable:
        stage->str_variables.push_back(
            std::pair<int, std::string>(var.index, var.name + " * factor"));
        break;
      case SVT_HexVariable:
        stage->str_hexVariables.push_back(std::pair<int, std::string>(
            var.index, "Hex." + var.name + " * factor"));
        break;
      }
    }
    ...
    if (stage) {
      switch (var.type) {
      case SVT_SubsystemVariable:
        stage->str_variables.push_back(std::pair<int, std::string>(
            var.index, var.name + " + Base::" + var.name + " * factor"));
        break;
      case SVT_HexVariable:
        stage->str_hexVariables.push_back(std::pair<int, std::string>(
            var.index,
            "Hex." + var.name + " + Base::Hex." + var.name + " * factor"));
        break;
      }
    }
    ...
    if (stage) {
      ...
      case SVT_SubsystemVariable:
        stage->str_variables.push_back(
            std::pair<int, std::string>(var.index, var.name + " + amount"));
        break;
      case SVT_HexVariable:
        stage->str_hexVariables.push_back(std::pair<int, std::string>(
            var.index, "Hex." + var.name + " + amount"));
        break;
      }
    }
  };
  ...
  for (auto i = effects.begin(), end = effects.end(); i != end; ++i) {
    for (unsigned n = 0; n < i->type->valueCount; ++n) {
      ...
      if (i->values[n] == 0 && !desc.defaultValue) {
        error("Error: Subsystem '%s' effect '%s' is missing values", id.c_str(),
              i->type->name.c_str());
        break;
      }
    }
  ...
  for (auto i = effectors.begin(), end = effectors.end(); i != end; ++i) {
    for (unsigned n = 0; n < i->type->valueCount; ++n) {
      auto &desc = i->type->values[n];
      if (i->values[n] == 0 && !desc.defaultValue) {
        error("Error: Subsystem '%s' effector '%s' is missing values",
              id.c_str(), i->type->name.c_str());
        break;
      }
    }
  }
  ...
  auto makeAppliedStage = [&](std::string &line,
                              std::vector<SubsystemDef::AppliedStage> &list) {
    std::string name;
    std::vector<std::string> args;
    ...
    if (it == modifierIds.end()) {
      if (!isOptional) {
        error("Error: could not add modifier '%s.%s'.", id.c_str(),
              name.c_str());
      }
      return;
    }
    ...
    if (stage->argumentNames.size() != args.size()) {
      error("Error: Modifier '%s.%s' requires %d argument(s), %d given.",
            id.c_str(), name.c_str(), stage->argumentNames.size(), args.size());
      return;
    }
  ...
  foreach (m, modules) {
    ...
    // Check that we have values for all effect variables
    for (auto i = (*m)->effects.begin(), end = (*m)->effects.end(); i != end;
         ++i) {
      for (unsigned n = 0; n < i->type->valueCount; ++n) {
        auto &desc = i->type->values[n];
        if (i->values[n] == 0 && !desc.defaultValue) {
          error("Error: Subsystem '%s', module '%s', effect '%s' is missing "
                "values",
                id.c_str(), (*m)->name.c_str(), i->type->name.c_str());
          break;
        }
      }
    }
...
bool SubsystemDef::ModuleDesc::onCheckErrors(Design *design, Subsystem *sys,
                                             const vec2u &hex) const {
  if (!scr_onCheckErrors)
    return false;
...
void SubsystemDef::ModuleDesc::onEnable(EffectEvent &evt,
...
void SubsystemDef::ModuleDesc::onDisable(EffectEvent &evt,
                                         const vec2u &position) const {
  if (!scr_onDisable)
    return;
...
const std::string &SubsystemDef::ModuleDesc::getTagValue(int index,
                                                         unsigned num) const {
  auto it = tagValues.find(index);
  if (it != tagValues.end()) {
    if (num >= it->second.size())
...
bool SubsystemDef::ModuleDesc::hasTagValue(int index,
...
static double formulaIndex(void *user, int index) {
  ...
  case VT_ConstantVariable:
    ...
    case CV_ShipEmptyHexes:
      return lookupData.design->hull->activeCount -
             lookupData.design->usedHexCount;
    case CV_IsCore:
      if (lookupData.hexIndex < 0 ||
          (unsigned)lookupData.hexIndex >= lookupData.system->hexes.size())
        return 0.0;
      return (lookupData.system->modules[lookupData.hexIndex] ==
              lookupData.system->type->coreModule)
                 ? 1.0
                 : 0.0;
    case CV_HexExterior:
      if (lookupData.hexIndex < 0)
        return 0.0;
      return lookupData.design->hull->isExterior(
                 lookupData.system->hexes[lookupData.hexIndex])
                 ? 1.0
                 : 0.0;
  ...
  case VT_HexVariable:
    ...
    if (index < 0) {
      ...
      unsigned baseIndex =
          lookupData.hexIndex * lookupData.system->type->hexVariables.size();
      if (isBase)
  ...
  case VT_SumVariable: {
    if (lookupData.design == nullptr)
      return 0.0;
    double value = 0.0;
    for (size_t i = 0, cnt = lookupData.design->subsystems.size(); i < cnt;
         ++i) {
...
Subsystem::Subsystem()
    : type(nullptr), exteriorHexes(0), hasErrors(false), variables(0),
      hexVariables(0), inDesign(nullptr), index(0), defaults(nullptr),
      effectors(nullptr) {}
...
void SubsystemDef::ModifyStage::applyVariables(Subsystem *sys) const {
  foreach (v, variables) {
    ...
    if (formula)
      sys->variables[localIndex] = (float)formula->evaluate(
          &formulaVariable, &lookupData, &formulaIndex);
  }
}

void SubsystemDef::ModifyStage::applyHexVariables(Subsystem *sys,
                                                  int hexIndex) const {
  foreach (v, hexVariables) {
    ...
    if (formula)
      sys->hexVariables[localIndex] = (float)formula->evaluate(
          &formulaVariable, &lookupData, &formulaIndex);
  }
}

void SubsystemDef::ModifyStage::applyShipVariables(Design *dsg,
                                                   Subsystem *sys) const {
  foreach (v, shipVariables) {
    ...
    if (formula)
      dsg->shipVariables[index] = (float)formula->evaluate(
          &formulaVariable, &lookupData, &formulaIndex);
  }
}
...
struct RunStage {
  ...
  RunStage(int a, const SubsystemDef::AppliedStage &as)
    ...
    for (unsigned i = 0; i < MODIFY_STAGE_MAXARGS; ++i) {
      if (as.formulas[i])
        args[i] = (float)as.formulas[i]->evaluate(&formulaVariable, &lookupData,
                                                  &formulaIndex);
      else
...
void applyShipModifier(const SubsystemDef *type,
                       SubsystemDef::ShipModifier &mod,
...
void Subsystem::initVariables(Design *design) {
  ...
  if (design->owner) {
    ...
    if (!ssdata || !ssdata->unlocked) {
      std::string errMsg = format(
          devices.locale.localize("ERROR_NOT_UNLOCKED").c_str(), type->name);
  ...
  for (unsigned i = 0; i < type->variables.size(); ++i) {
    ...
    if (formula)
      variables[i] = (float)formula->evaluate(&formulaVariable, &lookupData,
                                              &formulaIndex);
    else
      variables[i] = 0;
      ...
      foreach (v, rs.stage->variables) {
        ...
        if (localIndex == i && formula) {
          ...
          for (int repeats = 0; repeats < rs.amount; ++repeats)
            variables[localIndex] = (float)formula->evaluate(
                &formulaVariable, &lookupData, &formulaIndex);
        }
      }
    }
  ...
  for (unsigned i = 0; i < type->shipVariables.size(); ++i) {
    ...
    if (formula)
      design->shipVariables[index] = (float)formula->evaluate(
          &formulaVariable, &lookupData, &formulaIndex);
  }
  ...
  for (unsigned i = 0; i < modCnt; ++i) {
    if (moduleCounts[i] > 0) {
      ...
      if (ssdata) {
        if (ssdata->modulesUnlocked.size() <= (unsigned)mod.index ||
            !ssdata->modulesUnlocked[mod.index]) {
          std::string errMsg = format(
              devices.locale.localize("ERROR_MODULE_NOT_UNLOCKED").c_str(),
              mod.name);
  ...
  for (unsigned i = 0; i < hexCnt; ++i) {
    ...
    for (unsigned j = 0; j < varCnt; ++j) {
      ...
      if (formula)
        hexVariables[base + j] = (float)formula->evaluate(
            &formulaVariable, &lookupData, &formulaIndex);
      else
    ...
    foreach (it, mod->hexAppliedStages) {
      if (!it->stage->hexVariables.empty()) {
        ...
        memcpy(&lookupData.args, &rs.args,
               sizeof(float) * MODIFY_STAGE_MAXARGS);
    ...
    foreach (it, mod->adjacentModifiers) {
      ...
      for (unsigned j = 0; j < MODIFY_STAGE_MAXARGS; ++j) {
        if (it->stage.formulas[j])
          mod.stage.arguments[j] = it->stage.formulas[j]->evaluate(
              &formulaVariable, &lookupData, &formulaIndex);
      }
  ...
  foreach (it, type->shipModifiers) {
    ...
    for (unsigned i = 0; i < MODIFY_STAGE_MAXARGS; ++i) {
      if (it->stage.formulas[i])
        mod.stage.arguments[i] = it->stage.formulas[i]->evaluate(
            &formulaVariable, &lookupData, &formulaIndex);
    }
  ...
  if (type->hasCore && !foundCore) {
    std::string errMsg =
        format(devices.locale.localize("ERROR_NO_CORE").c_str(), type->name);
  ...
  if (type->hasCore && foundCore && type->exteriorCore &&
      !design->hull->isExterior(core)) {
    std::string errMsg =
        format(devices.locale.localize("ERROR_NEEDS_EXTERIOR_CORE").c_str(),
               type->name);
  ...
  for (unsigned i = 0, modCnt = type->modules.size(); i < modCnt; ++i) {
    ...
    if (mod->required && moduleCounts[i] == 0 && mod != type->coreModule) {
      std::string errMsg =
          format(devices.locale.localize("ERROR_REQUIRED_MODULE").c_str(),
                 type->name, mod->name);
    ...
    if (mod->unique && moduleCounts[i] > 1) {
      std::string errMsg =
          format(devices.locale.localize("ERROR_UNIQUE_MODULE").c_str(),
                 type->name, mod->name);
  ...
  if (!type->canUseOn(design->hull)) {
    std::string errMsg = format(
        devices.locale.localize("ERROR_INVALID_HULL").c_str(), type->name);
    design->errors.push_back(DesignError(true, errMsg, this));
    hasErrors = true;
  }
}
...
void Subsystem::applyAdjacencies(Design *design) {
  ...
  for (size_t n = 0, hexCnt = hexes.size(); n < hexCnt; ++n) {
    ...
    for (unsigned i = 0; i < 6; ++i) {
      ...
      for (size_t j = 0, jcnt = otherSys.adjacentModifiers.size(); j < jcnt;
           ++j)
        applyShipModifier(this->type, otherSys.adjacentModifiers[j], modStages);

      for (size_t j = 0, jcnt = otherSys.hexAdjacentModifiers[hexId].size();
           j < jcnt; ++j)
        applyShipModifier(this->type, otherSys.hexAdjacentModifiers[hexId][j],
                          modStages);
    }
  ...
  for (unsigned i = 0; i < type->effects.size(); ++i) {
    for (unsigned j = 0; j < type->effects[i].values.size(); ++j) {
      if (auto *formula = type->effects[i].values[j])
        effects[i].values[j] =
            formula->evaluate(&formulaVariable, &lookupData, &formulaIndex);
      else if (effects[i].type->values[j].defaultValue)
        effects[i].values[j] =
            effects[i].type->values[j].defaultValue->evaluate();
    }
  }
  ...
  for (unsigned i = 0; i < type->effectors.size(); ++i) {
    ...
    for (unsigned j = 0; j < type->effectors[i].values.size(); ++j) {
      if (auto *formula = type->effectors[i].values[j])
        effectors[i].values[j] =
            formula->evaluate(&formulaVariable, &lookupData, &formulaIndex);
      else if (effectors[i].type.values[j].defaultValue)
        effectors[i].values[j] =
            effectors[i].type.values[j].defaultValue->evaluate(effVariable,
                                                               &effectors[i]);
    }
    ...
    if (type->hexLimitArc && design->hull->active.valid(core) &&
        !design->hasTag(overrideHexArcLimitTag)) {
      ...
      if (!design->hull->isExteriorInDirection(core, dir)) {
        double diff =
            rad - HexGrid<>::RadiansFromAdjacency(HexGridAdjacency(dir));
        ...
        if (!design->hull->isExteriorInDirection(core, dir)) {
          std::string errMsg =
              format(devices.locale.localize("ERROR_HEX_LIMIT_ARC").c_str(),
                     type->name);
      ...
      // Move the turret angle to the center position so we scrunch the firing
      // arc
      if (eff.fireArc > 0) {
    ...
    effectors[i].relativeSize =
        3.0 * sqrt((double)hexes.size() / (double)design->hull->activeCount);
  }
...
void Subsystem::addHook(Design *design, const SubsystemDef::HookDesc &hook) {
  ...
  if (parts.size() == 1)
    cls =
        devices.scripts.server->getClass("subsystem_effects", parts[0].c_str());
  ...
  asIScriptObject *obj =
      (asIScriptObject *)devices.scripts.server->engine->CreateScriptObject(
          cls);
  if (obj == nullptr)
    return;
  ...
  for (unsigned i = 0, cnt = hook.formulas.size(); i < cnt; ++i) {
    if (hook.formulas[i])
      hook.argValues[i] = hook.formulas[i]->evaluate(
          &formulaVariable, &lookupData, &formulaIndex);
    else
...
void Subsystem::evaluateAsserts(Design *design) {
  ...
  auto checkAssert = [](Subsystem *sys, Design *design,
                        const SubsystemDef::Assert &ass) {
    if (!ass.formula)
      return;

    double val =
        ass.formula->evaluate(&formulaVariable, &lookupData, &formulaIndex);
    if (val <= 0.0) {
      // Check uniqueness
      ...
      if (ass.unique) {
        ...
        design->errors.push_back(DesignError(ass.fatal, msg, 0, 0));
        sys->hasErrors = true;
      } else {
        design->errors.push_back(DesignError(ass.fatal, msg, sys, 0));
        sys->hasErrors = true;
      }
    }
  };
...
void Subsystem::ownerChange(EffectEvent &event, Empire *prevEmpire,
  ...
  if (!hookClasses.empty()) {
    ...
    if (evt.obj != nullptr && evt.obj->type->blueprintOffset != 0)
      evt.blueprint =
          (Blueprint *)(((size_t)evt.obj) + evt.obj->type->blueprintOffset);
    ...
    for (size_t i = 0, cnt = hookClasses.size(); i < cnt; ++i) {
      ...
      auto cl =
          devices.scripts.server->call(ScriptHookFunctions[EH_Owner_Change]);
...
DamageEventStatus Subsystem::damage(DamageEvent &event,
                                    const vec2u &position) const {
  for (unsigned i = 0; i < type->effects.size(); ++i) {
...
DamageEventStatus Subsystem::globalDamage(DamageEvent &event, vec2u &position,
  ...
  if (!hookClasses.empty()) {
    ...
    for (size_t i = 0, cnt = hookClasses.size(); i < cnt; ++i) {
      ...
      auto cl =
          devices.scripts.server->call(ScriptHookFunctions[EH_GlobalDamage]);
      unsigned retVal = 0;
      cl.setObject(hookClasses[i]);
      cl.push(&evt);
      cl.push(&event);
...
float *Subsystem::variable(int index) {
  if (index < 0)
    return 0;
  if ((size_t)index >= type->variableIndices.size() ||
      type->variableIndices[index] < 0)
    return 0;
...
float *Subsystem::hexVariable(int index, int hexIndex) {
  if (index < 0 || hexIndex < 0 || (unsigned)hexIndex >= hexes.size())
    return 0;
  if ((size_t)index >= type->hexVariableIndices.size() ||
      type->hexVariableIndices[index] < 0)
    return 0;
...
void Subsystem::init(SaveFile &file) {
  ...
  if (file >= SFV_0010) {
    ...
    for (unsigned i = 0; i < type->effects.size(); ++i) {
      ...
      file.read(effects[i].values,
                sizeof(double) * effects[i].type->valueCount);
    }
  }
  ...
  for (unsigned i = 0; i < cnt; ++i) {
    ...
    int index = -1;
    if ((size_t)globIndex < type->variableIndices.size() &&
        type->variableIndices[globIndex] >= 0)
  ...
  for (unsigned i = 0; i < varCnt; ++i) {
    ...
    int index = -1;
    if ((size_t)globIndex < type->hexVariableIndices.size() &&
        type->hexVariableIndices[globIndex] >= 0)
...
Subsystem::Subsystem(net::Message &msg)
    : hasErrors(false), inDesign(nullptr), index(0) {
