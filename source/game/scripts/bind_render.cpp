...
template <class From, class To> To *dynamicCast(From *f) {
  return dynamic_cast<To *>(f);
}

template <class From, class To> To *grabbedCast(From *f) {
...
void takeScreenshot(const std::string &fname, bool increment = true) {
  Image *screen = devices.render->getScreen(0, 0, devices.driver->win_width,
                                            devices.driver->win_height);

  std::string filename =
      devices.mods.getGlobalProfile("screenshots") + "/" + fname;
  if (filename.empty())
    filename = "screenshot";
  if (filename.size() > 4 &&
  ...
  if (!isAccessible(filename)) {
    scripts::throwException(
        "Cannot access file outside game or profile directories.");
    return;
  }
...
asITypeInfo *getNodeArrayType() {
  return (asITypeInfo *)asGetActiveContext()->GetEngine()->GetUserData(
      EDID_nodeArray);
}
...
static const render::SpriteSheet &getSheet_index(unsigned index) {
  ...
  return devices.library.getSpriteSheet(
      devices.library.spritesheet_names[index]);
}
...
static const render::RenderState *getGroupMat(render::MaterialGroup &group,
                                              unsigned index) {
...
static void draw_rect_m(const recti &pos, const render::RenderState *mat,
                        const Color &color) {
  Color colors[4] = {color, color, color, color};
  devices.render->drawRectangle(pos, mat, 0, colors, getClip());
}

static void draw_rect_g(const recti &pos, const Color &topleft,
                        const Color &topright, const Color &botright,
                        const Color &botleft) {
...
static void draw_mat_c(const render::RenderState *mat, const recti &pos,
                       const Color &color) {
  devices.render->drawRectangle(pos, mat, color, getClip());
}

static void draw_mat_csh(const render::RenderState *mat, const recti &pos,
                         const Color &color, const render::Shader *shader) {
...
static void draw_mat_s(const render::RenderState *mat, const recti &pos,
                       const recti *src) {
  devices.render->drawRectangle(pos, mat, src, 0, getClip());
}

static void draw_mat_sc(const render::RenderState *mat, const recti &pos,
                        const recti *src, const Color &color) {
  Color colors[4] = {color, color, color, color};
  devices.render->drawRectangle(pos, mat, src, colors, getClip());
}

static void draw_mat_scr(const render::RenderState *mat, const recti &pos,
                         const recti *src, const Color &color,
                         double rotation) {
  Color colors[4] = {color, color, color, color};
...
static void draw_mat_scc(const render::RenderState *mat, const recti &pos,
                         const recti *src, const Color &topleft,
                         const Color &topright, const Color &botright,
                         const Color &botleft) {
  Color colors[4] = {topleft, topright, botright, botleft};
  devices.render->drawRectangle(pos, mat, src, colors, getClip());
...
static void draw_fps(const recti &pos) { devices.render->drawFPSGraph(pos); }

static vec2i sprite_size(const render::SpriteSheet &sheet) {
  return vec2i(sheet.width, sheet.height);
}

static render::Sprite sprite_offset(const render::SpriteSheet &sheet,
                                    unsigned number) {
  return render::Sprite(&sheet, number);
}

static void sprite_sourceUV(const render::SpriteSheet &sheet, unsigned index,
                            vec4f &output) {
  output = sheet.getSourceUV(index);
}

static void draw_sprite(const render::SpriteSheet &sheet, unsigned index,
                        const recti &pos) {
  sheet.render(*devices.render, index, pos, 0, getClip());
}

static void draw_sprite_s(const render::SpriteSheet &sheet, unsigned index,
                          const recti &pos, const render::Shader *shader) {
  sheet.render(*devices.render, index, pos, 0, getClip(), shader);
}

static void draw_sprite_c(const render::SpriteSheet &sheet, unsigned index,
                          const recti &pos, const Color &color) {
  Color colors[4] = {color, color, color, color};
  sheet.render(*devices.render, index, pos, colors, getClip());
}

static void draw_sprite_cr(const render::SpriteSheet &sheet, unsigned index,
                           const recti &pos, const Color &color,
                           double rotation) {
  Color colors[4] = {color, color, color, color};
...
static void draw_sprite_cc(const render::SpriteSheet &sheet, unsigned index,
                           const recti &pos, const Color &topleft,
                           const Color &topright, const Color &botright,
                           const Color &botleft) {
  Color colors[4] = {topleft, topright, botright, botleft};
  sheet.render(*devices.render, index, pos, colors, getClip());
...
static void draw_sprt_c(const render::Sprite *sprt, const recti &pos,
                        const Color &color) {
  if (sprt->sheet)
...
static void draw_sprt_csh(const render::Sprite *sprt, const recti &pos,
                          const Color &color, const render::Shader *shader) {
  Color blended = sprt->color * color;
  if (sprt->sheet) {
    Color colors[4] = {blended, blended, blended, blended};
    sprt->sheet->render(*devices.render, sprt->index, pos, colors, getClip(),
                        shader);
  } else if (sprt->mat)
    draw_mat_csh(sprt->mat, pos, blended, shader);
}

static void draw_sprt_cr(const render::Sprite *sprt, const recti &pos,
                         const Color &color, double rotation) {
  Color blended = sprt->color * color;
  if (sprt->sheet) {
    Color colors[4] = {blended, blended, blended, blended};
    sprt->sheet->render(*devices.render, sprt->index, pos, colors, getClip(), 0,
                        rotation);
  } else if (sprt->mat)
...
static void setVsync(int frames) { devices.driver->setVerticalSync(frames); }
...
static void poly_start_c(unsigned polyCount, const render::RenderState *mat,
                         const Color &col) {
  if (polyCount == 0)
    return;
...
static void poly_start_t(render::PrimitiveType type, unsigned polyCount,
  ...
  case render::PT_Lines:
    allocatedVerts = 2 * polyCount;
    break;
  case render::PT_LineStrip:
    allocatedVerts = 1 + polyCount;
    break;
  case render::PT_Triangles:
    allocatedVerts = 3 * polyCount;
    break;
  case render::PT_Quads:
    allocatedVerts = 4 * polyCount;
    break;
  }
...
static void poly_coord(const vec2i &pos) {
  if (auto *vert = getNextVertex()) {
    ...
    scripts::throwException(
        "Attempted to write to more vertices than were allocated.");
  }
}
...
static void poly_coord_uvc(const vec2i &pos, const vec2f &uv,
                           const Color &col) {
  if (auto *vert = getNextVertex()) {
    vertCol = col;
    vert->col = col;
...
static void poly3_coord_uvc(const vec3d &pos, const vec2f &uv,
                            const Color &col) {
  if (auto *vert = getNextVertex()) {
    vertCol = col;
    vert->col = col;
...
static void draw_line_c(const vec2i &from, const vec2i &to, const Color &color,
                        int size, const render::RenderState *mat) {
  double angle = (to - from).radians();
  vec2d off(cos(angle - 1.5708) * double(size) * 0.5,
            sin(angle - 1.5708) * double(size) * 0.5);
...
static void draw_line(const vec2i &from, const vec2i &to, int size,
                      const render::RenderState *mat) {
  draw_line_c(from, to, Color(0xffffffff), size, mat);
}

static void draw_model(render::RenderMesh *mesh, render::RenderState *mat,
                       recti &pos, quaterniond rotation, double Scale) {
  render::renderVertexBuffers();

  vec2i center = pos.getCenter();
  double scale =
      (double)std::min(pos.getWidth(), pos.getHeight()) * 0.5 * Scale;

  complex_clip_start();
  Matrix transform;

  rotation.toTransform(transform, vec3d(center.x, center.y, 0),
...
static void drawBuffers() { render::renderVertexBuffers(); }

static void meshRenderLOD(render::RenderMesh *mesh, double lodDist) {
  mesh->selectLOD(lodDist)->render();
}

static render::Camera *cameraFactory() { return new render::Camera(); }
...
static void renderWorld() { devices.render->renderWorld(); }

static Object *objectFromPixel(render::Camera &cam, const vec2i &px) {
  if (devices.universe) {
    Object *obj = devices.universe->getClosestOnLine(cam.screenToRay(
...
static double cam_dist(const vec3d &pos, double scale) {
  auto fromCamera =
      pos + (devices.render->cam_facing * scale) - devices.render->cam_pos;
  return fromCamera.getLength();
}

static vec3d cam_pos() { return devices.render->cam_pos; }

static vec3d cam_up() { return devices.render->cam_up; }

static vec3d cam_facing() { return devices.render->cam_facing; }
...
static ObjArray *boxSelect(render::Camera &cam, const recti &box) {
  ...
  vec2i screenSize =
      vec2i(devices.driver->win_width, devices.driver->win_height);
  ...
  frustum ViewFrustum =
      frustum(cam.screenToRay(boxd.topLeft.x, boxd.topLeft.y),
...
static void nodeConeSelect(const line3dd &line, double slope,
  ...
  devices.nodePhysics->findInBox(bound, [&](const PhysicsItem &item) {
...
static void renderBillboard(const render::RenderState &mat, const vec3d &pos,
                            double width, double rotation) {
  devices.render->drawBillboard(pos, width, mat, rotation);
}

static void renderBillboard_sheet(const render::SpriteSheet &sheet,
                                  unsigned index, const vec3d &pos,
                                  double width, double rotation) {
  devices.render->drawBillboard(pos, width, sheet.material,
                                sheet.getSource(index), rotation);
}

static void renderBillboard_sprite(const render::Sprite &sprite,
                                   const vec3d &pos, double width,
                                   double rotation) {
  if (sprite.mat)
...
static void renderBillboard_c(const render::RenderState &mat, const vec3d &pos,
                              double width, double rotation, Color &color) {
  devices.render->drawBillboard(pos, width, mat, rotation, &color);
}

static void renderBillboard_sheet_c(const render::SpriteSheet &sheet,
                                    unsigned index, const vec3d &pos,
                                    double width, double rotation,
                                    Color &color) {
  devices.render->drawBillboard(pos, width, sheet.material,
                                sheet.getSource(index), rotation, color);
}

static void renderBillboard_sprite_c(const render::Sprite &sprite,
                                     const vec3d &pos, double width,
                                     double rotation, Color &color) {
  if (sprite.mat)
    renderBillboard_c(*sprite.mat, pos, width, rotation, color);
  else if (sprite.sheet)
    renderBillboard_sheet_c(*sprite.sheet, sprite.index, pos, width, rotation,
                            color);
}

static void renderPlane(const render::RenderState &mat, const vec3d &pos,
                        double width, Color &color, double angle) {
  vec3d tr;
  vec3d tl;
  if (angle != 0.0) {
  ...
  vec3d verts[] = {pos - tr, pos - tl, pos + tr, pos + tl};
...
#define decl_access(member, type)                                              \
  void set_##member(type v) { member = v; }                                    \
  type get_##member() { return member; }

#define bind_access(member, type)                                              \
  mat.addMethod(#type " get_" #member "() const",                              \
                asMETHOD(ScriptMaterial, get_##member));                       \
  mat.addMethod("void set_" #member "(" #type " value)",                       \
                asMETHOD(ScriptMaterial, set_##member));
...
ScriptMaterial *createMat() { return new ScriptMaterial(); }

ScriptMaterial *copyMat(const ScriptMaterial &other) {
  return new ScriptMaterial(other);
}

static void createSprite_e(void *memory) { new (memory) render::Sprite(); }

static void createSprite(void *memory, const render::SpriteSheet *sheet,
                         unsigned index) {
  new (memory) render::Sprite(sheet, index);
}

static void createSprite_mat(render::Sprite *memory,
                             const render::RenderState *material) {
  new (memory) render::Sprite(material);
}

static void createSprite_c(render::Sprite *memory,
...
static void createSprite_mat_c(render::Sprite *memory,
                               const render::RenderState *material,
                               const Color &color) {
  new (memory) render::Sprite(material);
  memory->color = color;
...
static void delSprite(render::Sprite *sprite) { sprite->~Sprite(); }
...
static render::Sprite &copySprite(render::Sprite &into,
                                  const render::Sprite &other) {
  into = other;
  return into;
}
...
static render::Sprite spriteColorized(render::Sprite &sprt, const Color &color,
                                      float blend = 1.f) {
...
public:
  SetNodeObject(scene::Node *node, Object *obj)
      : NodeEvent(node), object(obj) {}
  ...
  void process() override { node->setObject(object); }
};
...
template <scene::NodeFlag flag> bool getNodeFlag(scene::Node &node) {
...
void renderNode(scene::Node &node) { node.render(*devices.render); }
...
bool isNodeInView(const scene::Node &node) {
  return devices.render->getViewFrustum().overlaps(node.abs_position,
                                                   node.abs_scale);
}

void applyTransform(const vec3d &pos, const vec3d &scale,
...
void applyAbsTransform(const vec3d &pos, const vec3d &scale,
                       const quaterniond &rot) {
  Matrix transform;
  rot.toTransform(transform, pos, scale);
  devices.render->setTransformationAbs(transform);
...
void popTransform() { devices.render->resetTransformation(); }

void getBBVecs(vec3d &upLeft, vec3d &upRight, double rot) {
  devices.render->getBillboardVecs(upLeft, upRight, rot);
}

void getBBVecsFacing(const vec3d &pos, vec3d &upLeft, vec3d &upRight,
                     double rot) {
...
scene::BeamNode *makeBeam(const render::RenderState *mat, float width,
                          const vec3d &startPoint, const vec3d &endPoint,
                          bool staticSize) {
  auto *node =
      new scene::BeamNode(mat, width, startPoint, endPoint, staticSize);
...
scene::MeshNode *makeMesh(const render::RenderMesh *mesh,
                          const render::RenderState *mat) {
...
template <class T> void setShaderValue(asIScriptGeneric *f) {
...
scene::ParticleSystem *playParticleSys(const std::string &name,
                                       const vec3d &position, float scale,
                                       scene::Node *parent) {
  auto *ps =
      scene::playParticleSystem(devices.library.getParticleSystem(name), parent,
                                position, quaterniond(), vec3d(), scale);
  if (parent)
    parent->drop();
  return ps;
}

scene::ParticleSystem *playParticleSys_ps(scene::ParticleSystemDesc *desc,
                                          const vec3d &position, float scale,
                                          scene::Node *parent) {
  auto *ps = scene::playParticleSystem(desc, parent, position, quaterniond(),
                                       vec3d(), scale);
  if (parent)
    parent->drop();
  return ps;
...
struct PlayParticles : public scene::NodeEvent {
  ...
  PlayParticles(const scene::ParticleSystemDesc *sys, const vec3d &pos,
                const quaterniond &rot, float Scale, Object *Parent = nullptr)
      : scene::NodeEvent(nullptr), desc(sys), position(pos), rotation(rot),
  ...
  void process() override {
    ...
    auto *ps =
        scene::playParticleSystem(desc, pnode, position, rotation, vel, scale);
    if (ps)
      ps->drop();
  }
};

void playParticleSys_server(const std::string &name, const vec3d &position,
                            const quaterniond &rot, float scale, unsigned mask,
                            bool transmit) {
  if (auto *player = Empire::getPlayerEmpire())
    if (player->visionMask & mask)
      scene::queueNodeEvent(new PlayParticles(
          devices.library.getParticleSystem(name), position, rot, scale));
  if (transmit && devices.network->isServer)
    devices.network->sendParticleSystem(name, position, vec3d(), rot, scale,
                                        mask);
}

void playParticleSys_server_o(const std::string &name, const vec3d &position,
                              const quaterniond &rot, float scale, Object *obj,
                              bool transmit) {
  if (auto *player = Empire::getPlayerEmpire())
    if (obj->isVisibleTo(player))
      scene::queueNodeEvent(new PlayParticles(
          devices.library.getParticleSystem(name), position, rot, scale, obj));
  if (transmit && devices.network->isServer)
    devices.network->sendParticleSystem(name, position, vec3d(), rot, scale,
                                        obj);
}
...
public:
  ...
  bool isValid() { return rt != nullptr; }

  void set() { devices.render->setRenderTarget(rt); }

  void reset() { devices.render->setRenderTarget(nullptr); }

  void animate(double time) { nextFrame += time; }
  ...
  void draw(const recti &rect) {
    ...
    camera->setRenderConstraints(
        1.0, 1000.0, 70.0, (double)rect.getWidth() / (double)rect.getHeight(),
...
public:
  ...
  vec2i getSize(unsigned index = 0) {
    if (index >= RENDER_MAX_TEXTURES || !images[index].loaded ||
        !images[index].img)
      return vec2i();
  ...
  void load(const std::string &filename, unsigned index = 0) {
    ...
    if (!state->textures[index])
      state->textures[index] =
          (render::Texture *)devices.library.getErrorTexture();
...
DynamicTexture *makeDynamicTexture() { return new DynamicTexture(); }
...
template <class T> void bindNode(ClassBind &bind) {
  ...
  bind.addMethod("void markForDeletion()", asMETHOD(T, markForDeletion))
      doc("Flags the node for destruction, removing it from the node tree and "
          "destroying it some time in the future.");
  ...
  bind.addExternMethod("void set_transparent(bool)",
                       asFUNCTION(setNodeFlag<scene::NF_Transparent>))
      doc("Sets whether the node should be treated as transparent.", "");
  bind.addExternMethod("void set_memorable(bool)",
                       asFUNCTION(setNodeFlag<scene::NF_Memorable>))
      doc("Sets whether the node should be visible when the associated object "
          "is only remembered.",
          "");
  bind.addExternMethod("bool get_memorable()",
                       asFUNCTION(getNodeFlag<scene::NF_Memorable>));
  bind.addExternMethod("void set_animInvis(bool)",
                       asFUNCTION(setNodeFlagInv<scene::NF_AnimOnlyVisible>))
      doc("Sets whether the node should call preRender when invisible.", "");
  bind.addExternMethod("void set_autoCull(bool)",
                       asFUNCTION(setNodeFlagInv<scene::NF_NoCulling>))
      doc("Sets whether the node should be automatically culled based on the "
          "camera view frustum.",
          "");
  bind.addExternMethod("void set_fixedSize(bool)",
                       asFUNCTION(setNodeFlag<scene::NF_FixedSize>))
      doc("Sets whether the node is a fixed apparent size. Disables cone "
          "behavior in node searches.",
          "");
  bind.addExternMethod("void set_customColor(bool)",
                       asFUNCTION(setNodeFlag<scene::NF_CustomColor>))
      doc("Sets whether this node should not receive its object's empire's "
          "color.",
          "");
  bind.addExternMethod("void set_needsTransform(bool)",
                       asFUNCTION(setNodeFlagInv<scene::NF_NoMatrix>))
      doc("Sets whether the node should build its matrix when the transform is "
          "updated (e.g. for applyTransform).",
          "");
  bind.addExternMethod("void applyTransform()", asFUNCTION(pushNodeTransform))
      doc("Applies the node's position, scale, and rotation to the model "
          "transform.");
  ...
  bind.addExternMethod("void animate(double seconds)", asFUNCTION(animateNode))
      doc("Animates the node and its children.",
          "Amount of time to animate by.");
  bind.addExternMethod("bool isInView() const", asFUNCTION(isNodeInView))
      doc("Returns true if the node's bounding sphere is within the camera's "
          "view frustum.",
          "");
  bind.addMethod(
      "void hintParentObject(Object@ obj, bool checkDistance = true)",
      asMETHOD(T, hintParentObject))
      doc("Hint that this node should be parented to an object's node.",
          "Whether to check whether the node is fully contained or assume it "
          "is.",
          "Object to hint for.");
  ...
  bind.addMember("double distanceCutoff", offsetof(T, distanceCutoff))
      doc("Squared distance at which nodes synced to objects will not be "
  ...
  bind.addMember("bool remembered", offsetof(T, remembered))
      doc("For memorable nodes, tracks whether the associated objects is "
          "remembered.");
  bind.addMember("Node@ parent", offsetof(T, parent))
      doc("Parent node in the node tree.");
}

void bindScriptNode(ClassBind &bind) { bindNode<scene::ScriptedNode>(bind); }
...
void RegisterRenderBinds(bool decl, bool isMenu, bool server) {
  if (decl) {
    ...
    ClassBind sprt("Sprite", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CDA,
                   sizeof(render::Sprite));
    return;
  }
  ...
  mat.addBehaviour(asBEHAVE_RELEASE, "void f()",
                   asMETHOD(ScriptMaterial, drop));
  mat.addFactory("Material@ f()", asFUNCTION(createMat));
  mat.addFactory("Material@ f(const Material&)", asFUNCTION(copyMat));

  mat.addMethod("Material& opAssign(const Material&)",
                asMETHOD(ScriptMaterial, assign));
  ...
  for (unsigned i = 0; i < RENDER_MAX_TEXTURES; ++i)
    mat.addMember(format("const Texture@ texture$1", toString(i)).c_str(),
                  offsetof(render::RenderState, textures) +
                      (i * sizeof(render::Texture *)));

  light.addMember("vec3f position",
                  offsetof(render::light::PointLight, position));
  light.addMember("float att_constant",
                  offsetof(render::light::PointLight, att_constant));
  light.addMember("float att_linear",
                  offsetof(render::light::PointLight, att_linear));
  light.addMember("float radius", offsetof(render::light::PointLight, radius));
  light.addMember("float att_quadratic",
                  offsetof(render::light::PointLight, att_quadratic));
  light.addMember("Colorf diffuse",
                  offsetof(render::light::PointLight, diffuse));
  light.addMember("Colorf specular",
                  offsetof(render::light::PointLight, specular));
  ...
  bind("const MatGroup& getMatGroup(const string& id)",
       asFUNCTION(getMatGroup));
  group.addExternMethod("string getMaterialName(uint index) const",
                        asFUNCTION(getGroupMatID));
  group.addExternMethod("const Material@ getMaterial(uint index) const",
                        asFUNCTION(getGroupMat));
  group.addExternMethod("uint get_materialCount() const",
                        asFUNCTION(getGroupMatCount));
  ...
  bind("const SpriteSheet@ getSpriteSheet(const string &in)",
       asFUNCTION(getSheet));
  ...
  sprt.addConstructor("void f(const SpriteSheet@ sheet, uint index)",
                      asFUNCTION(createSprite));
  sprt.addConstructor("void f(const Material@ mat)",
                      asFUNCTION(createSprite_mat));
  sprt.addConstructor(
      "void f(const SpriteSheet@ sheet, uint index, const Color& color)",
      asFUNCTION(createSprite_c));
  sprt.addConstructor("void f(const Material@ mat, const Color& color)",
                      asFUNCTION(createSprite_mat_c));
  ...
  sprt.addExternMethod("Sprite& opAssign(const Sprite&in other)",
                       asFUNCTION(copySprite));
  sprt.addExternMethod("bool get_valid() const", asFUNCTION(spriteValid));
  sprt.addExternMethod(
      "Sprite colorized(const Color& color, float blend = 1.f) const",
      asFUNCTION(spriteColorized));
  sprt.addExternMethod("Sprite opMul(const Color& color) const",
                       asFUNCTION(sprt_color));
  ...
  bind("void takeScreenshot(const string& filename, bool increment = true)",
       asFUNCTION(takeScreenshot));
  ...
  if (!isMenu) {
    ...
    asITypeInfo *nodeArrayType =
        engine->GetTypeInfoById(engine->GetTypeIdByDecl("array<Node@>"));
    engine->SetUserData(nodeArrayType, EDID_nodeArray);

    bind("void nodeConeSelect(const line3dd &in line, double slope, "
         "array<Node@>&)",
         asFUNCTION(nodeConeSelect));
    bind("void nodeSyncObject(Node& node, Object& obj)",
         asFUNCTION(nodeSyncObject));
    ...
    node.addExternMethod(
        "BillboardNode@ opCast() const",
        asFUNCTION((grabbedCast<scene::Node, scene::SpriteNode>)));
    bb.addExternMethod(
        "Node@ opImplCast() const",
        asFUNCTION((grabbedCast<scene::SpriteNode, scene::Node>)));

    ClassBind beam("BeamNode", asOBJ_REF);
    beam.addFactory(
        "BeamNode@ f(const Material& mat, float width, const vec3d&in "
        "startPoint, const vec3d&in endPoint, bool staticSize = false)",
        asFUNCTION(makeBeam));
    ...
    node.addExternMethod(
        "BeamNode@ opCast() const",
        asFUNCTION((grabbedCast<scene::Node, scene::BeamNode>)));
    beam.addExternMethod(
        "Node@ opImplCast() const",
        asFUNCTION((grabbedCast<scene::BeamNode, scene::Node>)));

    ClassBind plane("PlaneNode", asOBJ_REF);
    plane.addMember("const Material@ material",
                    offsetof(scene::PlaneNode, material));
    ...
    node.addExternMethod(
        "PlaneNode@ opCast() const",
        asFUNCTION((grabbedCast<scene::Node, scene::PlaneNode>)));
    plane.addExternMethod(
        "Node@ opImplCast() const",
        asFUNCTION((grabbedCast<scene::PlaneNode, scene::Node>)));

    ClassBind mesh("MeshNode", asOBJ_REF);
    mesh.addMember("const Model@ model", offsetof(scene::MeshNode, mesh));
    mesh.addMember("const Material@ material",
                   offsetof(scene::MeshNode, material));

    node.addExternMethod(
        "MeshNode@ opCast() const",
        asFUNCTION((grabbedCast<scene::Node, scene::MeshNode>)));
    mesh.addExternMethod(
        "Node@ opImplCast() const",
        asFUNCTION((grabbedCast<scene::MeshNode, scene::Node>)));
  ...
  if (server) {
    bind("void playParticleSystem(const string& name, const vec3d& position, "
         "const quaterniond& rot, float scale = 1, uint mask = 4294967295, "
         "bool networked = true)",
         asFUNCTION(playParticleSys_server));
    bind("void playParticleSystem(const string& name, const vec3d& position, "
         "const quaterniond& rot, float scale, Object& parent, bool networked "
         "= true)",
         asFUNCTION(playParticleSys_server_o));
    return;
  }

  {
    ...
    devices.library.iterateShaderGlobals(
        [&](std::string &name, resource::ShaderGlobal *shader) {
          if (shader->arraySize == 1) {
            ...
            switch (shader->type) {
            case render::Shader::VT_int:
              fid = engine->RegisterGlobalFunction(
                  format("void set_$1(uint,const int &in)", name).c_str(),
                  asFUNCTION(setShaderValue<int>), asCALL_GENERIC);
              break;
            case render::Shader::VT_int2:
              fid = engine->RegisterGlobalFunction(
                  format("void set_$1(uint,const ::vec2i &in)", name).c_str(),
                  asFUNCTION(setShaderValue<vec2i>), asCALL_GENERIC);
              break;
            case render::Shader::VT_int3:
              fid = engine->RegisterGlobalFunction(
                  format("void set_$1(uint,const ::vec3i &in)", name).c_str(),
                  asFUNCTION(setShaderValue<vec3i>), asCALL_GENERIC);
              break;
            case render::Shader::VT_int4:
              fid = engine->RegisterGlobalFunction(
                  format("void set_$1(uint,const ::vec4i &in)", name).c_str(),
                  asFUNCTION(setShaderValue<vec4i>), asCALL_GENERIC);
              break;
            case render::Shader::VT_float:
              fid = engine->RegisterGlobalFunction(
                  format("void set_$1(uint,const float &in)", name).c_str(),
                  asFUNCTION(setShaderValue<float>), asCALL_GENERIC);
              break;
            case render::Shader::VT_float2:
              fid = engine->RegisterGlobalFunction(
                  format("void set_$1(uint,const ::vec2f &in)", name).c_str(),
                  asFUNCTION(setShaderValue<vec2f>), asCALL_GENERIC);
              break;
            case render::Shader::VT_float3:
              fid = engine->RegisterGlobalFunction(
                  format("void set_$1(uint,const ::vec3f &in)", name).c_str(),
                  asFUNCTION(setShaderValue<vec3f>), asCALL_GENERIC);
              break;
            case render::Shader::VT_float4:
              fid = engine->RegisterGlobalFunction(
                  format("void set_$1(uint,const ::vec4f &in)", name).c_str(),
  ...
  sprt.addExternMethod("void draw(const recti &in pos) const",
                       asFUNCTION(draw_sprt));
  sprt.addExternMethod(
      "void draw(const recti &in pos, const Color&in color) const",
      asFUNCTION(draw_sprt_c));
  sprt.addExternMethod("void draw(const recti &in pos, const Color&in color, "
                       "const Shader@ shader) const",
                       asFUNCTION(draw_sprt_csh));
  sprt.addExternMethod("void draw(const recti &in pos, const Color&in color, "
                       "double rotation) const",
                       asFUNCTION(draw_sprt_cr));
  ...
  mat.addExternMethod("bool isPixelActive(const vec2i&in px) const",
                      asFUNCTION(mat_pxactive));
  mat.addExternMethod("void draw(const recti &in pos) const",
                      asFUNCTION(draw_mat));
  mat.addExternMethod(
      "void draw(const recti &in pos, const Color&in color) const",
      asFUNCTION(draw_mat_c));
  mat.addExternMethod("void draw(const recti &in pos, const Color&in color, "
                      "const Shader@ shader) const",
                      asFUNCTION(draw_mat_csh));
  mat.addExternMethod(
      "void draw(const recti &in pos, const recti &in src) const",
      asFUNCTION(draw_mat_s));
  mat.addExternMethod("void draw(const recti &in pos, const recti &in src, "
                      "const Color&in color) const",
                      asFUNCTION(draw_mat_sc));
  mat.addExternMethod("void draw(const recti &in pos, const recti &in src, "
                      "const Color&in color, double rotation) const",
                      asFUNCTION(draw_mat_scr));
  mat.addExternMethod("void draw(const recti &in pos, const recti &in src, "
                      "const Color&in topleft, const Color&in topright, const "
                      "Color&in botright, const Color&in botleft) const",
                      asFUNCTION(draw_mat_scc));

  // Spritesheet definitions
  sheet.addMember("const Material material",
                  offsetof(render::SpriteSheet, material));
  sheet.addMember("int width", offsetof(render::SpriteSheet, width));
  sheet.addMember("int height", offsetof(render::SpriteSheet, height));
  sheet.addMethod("uint get_count() const",
                  asMETHOD(render::SpriteSheet, getCount));
  sheet.addExternMethod("void getSourceUV(uint index, vec4f&out value) const",
                        asFUNCTION(sprite_sourceUV));
  sheet.addExternMethod("vec2i get_size() const", asFUNCTION(sprite_size));
  sheet.addExternMethod("Sprite opAdd(uint num) const",
                        asFUNCTION(sprite_offset));

  sheet.addMethod("bool isPixelActive(uint index, const vec2i&in px) const",
                  asMETHOD(render::SpriteSheet, isPixelActive));
  sheet.addMethod(
      "recti getSource(uint index) const",
      asMETHODPR(render::SpriteSheet, getSource, (unsigned) const, recti));
  sheet.addExternMethod("void draw(uint index, const recti &in pos) const",
                        asFUNCTION(draw_sprite));
  sheet.addExternMethod(
      "void draw(uint index, const recti &in pos, const Shader@ shader) const",
      asFUNCTION(draw_sprite_s));
  sheet.addExternMethod(
      "void draw(uint index, const recti &in pos, const Color&in color) const",
      asFUNCTION(draw_sprite_c));
  sheet.addExternMethod("void draw(uint index, const recti &in pos, const "
                        "Color&in color, double rotation) const",
                        asFUNCTION(draw_sprite_cr));
  sheet.addExternMethod("void draw(uint index, const recti &in pos, const "
                        "Color&in topleft, const Color&in topright, const "
                        "Color&in botright, const Color&in botleft) const",
                        asFUNCTION(draw_sprite_cc));

  // Model definitions
  mod.addExternMethod(
      "void draw(const Material& mat, const recti&in pos, quaterniond rotation "
      "= quaterniond(), double scale = 1.0) const",
      asFUNCTION(draw_model));
  mod.addMethod("void draw() const", asMETHOD(render::RenderMesh, render));
  mod.addExternMethod("void draw(double lodDist) const",
                      asFUNCTION(meshRenderLOD));
  ...
  bind("void applyTransform(const vec3d &in pos, const vec3d &in scale, const "
       "quaterniond &in rot)",
       asFUNCTION(applyTransform))
      doc("Applies a transformation to the modelview matrix. Also applies the "
          "camera's position as an offset. Call undoTransform() afterwards.",
          "", "", "");
  bind("void applyAbsTransform(const vec3d &in pos, const vec3d &in scale, "
       "const quaterniond &in rot)",
       asFUNCTION(applyAbsTransform))
      doc("Applies a transformation to the modelview matrix. Call "
          "undoTransform() afterwards.",
          "", "", "");
  bind("void applyBBTransform(const vec3d &in pos, double width, double rot)",
       asFUNCTION(applyBBTransform))
      doc("Applies a transformation that rotates a model in the same way a "
          "billboard would rotate. Call undoTransform() afterwards.",
          "", "", "");
  bind("void undoTransform()", asFUNCTION(popTransform))
      doc("Undoes a modelview transform.");
  bind("void getBillboardVecs(vec3d &out upLeft, vec3d &out upRight, double "
       "rot = 0)",
       asFUNCTION(getBBVecs))
      doc("Fetches the two cached vectors necessary to build a billboard.",
          "Top left corner offset.", "Top right corner offset.",
          "Optional rotation to apply to the billboard.");
  bind("void getBillboardVecs(const vec3d &in from, vec3d &out upLeft, vec3d "
       "&out upRight, double rot = 0)",
       asFUNCTION(getBBVecsFacing))
      doc("Fetches the two cached vectors necessary to build a billboard that "
          "faces the camera.",
          "Position to face the billboard from.", "Top left corner offset.",
          "Top right corner offset.",
          "Optional rotation to apply to the billboard.");
  bind("const ParticleSystem@ getParticleSystem(const string& name)",
       asFUNCTION(getParticleSystem));
  if (!server) {
    ...
    node.addExternMethod(
        "ParticleSystemNode@ opCast() const",
        asFUNCTION((grabbedCast<scene::Node, scene::ParticleSystem>)));
    psn.addExternMethod(
        "Node@ opImplCast() const",
        asFUNCTION((grabbedCast<scene::ParticleSystem, scene::Node>)));

    bind("ParticleSystemNode@ playParticleSystem(const string& name, const "
         "vec3d& position, float scale = 1, Node@ parent = null)",
         asFUNCTION(playParticleSys));
    bind("ParticleSystemNode@ playParticleSystem(const ParticleSystem& system, "
         "const vec3d& position, float scale = 1, Node@ parent = null)",
         asFUNCTION(playParticleSys_ps));
  }

  // Camera
  ClassBind cam("Camera", asOBJ_REF);
  cam.addFactory("Camera@ f()", asFUNCTION(cameraFactory));
  cam.setReferenceFuncs(asMETHOD(render::Camera, grab),
                        asMETHOD(render::Camera, drop));
  cam.addMethod("void animate(double time)", asMETHOD(render::Camera, animate));
  cam.addExternMethod("Camera& opAssign(const Camera&in other)",
                      asFUNCTION(copyCamera));

  cam.addMethod("void yaw(double rad, bool snap = false)",
                asMETHOD(render::Camera, yaw));
  cam.addMethod("void pitch(double rad, bool snap = false)",
                asMETHOD(render::Camera, pitch));
  cam.addMethod("void roll(double rad, bool snap = false)",
                asMETHOD(render::Camera, roll));
  cam.addMethod("void zoom(double)", asMETHOD(render::Camera, zoom));
  cam.addMethod(
      "void zoomTo(double, const vec3d &in, double minDistance = 0.0)",
      asMETHOD(render::Camera, zoomTo));
  cam.addMethod("void zoomAlong(double, const vec3d &in)",
                asMETHOD(render::Camera, zoomAlong));
  cam.addMethod("void resetRotation()",
                asMETHOD(render::Camera, resetRotation));
  cam.addMethod("void resetZoom()", asMETHOD(render::Camera, resetZoom));
  cam.addMethod("void snap()", asMETHOD(render::Camera, snap));
  cam.addMethod("void snapTranslation()",
                asMETHOD(render::Camera, snapTranslation));

  cam.addMethod("double get_radius()", asMETHOD(render::Camera, getRadius));
  cam.addMethod("void set_radius(double value)",
                asMETHOD(render::Camera, setRadius));

  cam.addMethod("void set_linearZoom(bool) const",
                asMETHOD(render::Camera, setLinearZoom));
  cam.addMethod("void set_lockedRotation(bool) const",
                asMETHOD(render::Camera, setLockedRotation))
      doc("Sets whether the camera is allowed to rotate to be upside down.",
          "");

  cam.addMethod(
      "void setPositionBound(const vec3d &in minimum, const vec3d &in maximum)",
      asMETHOD(render::Camera, setPositionBound));
  cam.addMethod("void set_maxDistance(double dist)",
                asMETHOD(render::Camera, setMaxDistance));

  cam.addMethod("void abs_yaw(double rad, bool snap = false)",
                asMETHOD(render::Camera, abs_yaw));
  cam.addMethod("void abs_pitch(double rad, bool snap = false)",
                asMETHOD(render::Camera, abs_pitch));

  cam.addMethod("void abs_yaw_to(double rad, bool snap = false)",
                asMETHOD(render::Camera, abs_yaw_to));
  cam.addMethod("void abs_pitch_to(double rad, bool snap = false)",
                asMETHOD(render::Camera, abs_pitch_to));

  cam.addMethod("void move_abs(const vec3d &in)",
                asMETHOD(render::Camera, move_abs));
  cam.addMethod("void move_world_abs(const vec3d &in)",
                asMETHOD(render::Camera, move_world_abs));
  cam.addMethod("void move_world(const vec3d &in)",
                asMETHOD(render::Camera, move_world));
  cam.addMethod("void move_cam(const vec3d &in)",
                asMETHOD(render::Camera, move_cam));
  cam.addMethod("void move_cam_abs(const vec3d &in)",
                asMETHOD(render::Camera, move_cam_abs));

  cam.addMethod("vec3d get_position() const",
                asMETHOD(render::Camera, getPosition));
  cam.addMethod("vec3d get_finalPosition() const",
                asMETHOD(render::Camera, getFinalPosition));
  cam.addMethod("vec3d get_facing() const",
                asMETHOD(render::Camera, getFacing));
  cam.addMethod("vec3d get_right() const", asMETHOD(render::Camera, getRight));
  cam.addMethod("vec3d get_lookAt() const",
                asMETHOD(render::Camera, getLookAt));
  cam.addMethod("vec3d get_finalLookAt() const",
                asMETHOD(render::Camera, getFinalLookAt));
  cam.addMethod("vec3d get_up() const", asMETHOD(render::Camera, getUp));
  cam.addMethod("double get_distance() const",
                asMETHOD(render::Camera, getDistance));
  ...
  cam.addMethod("double screenAngle(const vec3d&in pos) const",
                asMETHOD(render::Camera, screenAngle));
  cam.addMethod("vec2i screenPos(const vec3d&in pos) const",
                asMETHOD(render::Camera, screenPos));
  cam.addMethod("line3dd screenToRay(double, double) const",
                asMETHOD(render::Camera, screenToRay));

  cam.addMethod("void toLookAt(vec3d &out, vec3d &out, vec3d &out) const",
                asMETHOD(render::Camera, toLookAt));
  cam.addMethod("void setRenderConstraints(double, double, double, double, "
                "double, double)",
                asMETHOD(render::Camera, setRenderConstraints));

  if (!isMenu) {
    cam.addExternMethod("Object@ getObject(const vec2i& line) const",
                        asFUNCTION(objectFromPixel));
    cam.addExternMethod("array<Object@>@ boxSelect(const recti& box)",
                        asFUNCTION(boxSelect));
  }

  bind("double getCameraDistance(const vec3d&in pos, double scale = 0)",
       asFUNCTION(cam_dist));
  ...
  bind("bool isSphereVisible(const vec3d& center, double radius)",
       asFUNCTION(isInView));
  ...
  rt.addFactory("RenderTarget@ f(const vec2i &in size)",
                asFUNCTION(makeRenderTarget));
  rt.setReferenceFuncs(asMETHOD(ScriptRenderTarget, grab),
                       asMETHOD(ScriptRenderTarget, drop));
  ...
  rt.addMethod("void animate(double time)",
               asMETHOD(ScriptRenderTarget, animate));
  rt.addMethod("void draw(const recti& in)",
               asMETHOD(ScriptRenderTarget, draw));
  rt.addMethod("void set_size(const vec2i& in)",
               asMETHOD(ScriptRenderTarget, resize));
  ...
  dtex.setReferenceFuncs(asMETHOD(DynamicTexture, grab),
                         asMETHOD(DynamicTexture, drop));

  dtex.addMember("Material@ material", offsetof(DynamicTexture, state));
  dtex.addMethod("bool isLoaded(uint index = 0)",
                 asMETHOD(DynamicTexture, isLoaded));
  dtex.addMethod("vec2i get_size(uint index = 0)",
                 asMETHOD(DynamicTexture, getSize));
  dtex.addMethod("void load(const string&in filename, uint index = 0)",
                 asMETHOD(DynamicTexture, load));
  dtex.addMethod("void set_image(uint index, const Image@+ img)",
                 asMETHOD(DynamicTexture, set));
  dtex.addMethod("void draw(const recti&in pos, const Color& color)",
                 asMETHOD(DynamicTexture, draw));
  dtex.addMethod("bool stream()", asMETHOD(DynamicTexture, stream));

  //-- Render functions
  bind("void drawRectangle(const recti &in pos, const Color&in color)",
       asFUNCTION(draw_rect));
  bind("void drawRectangle(const recti &in pos, const Material@ mat, const "
       "Color&in color)",
       asFUNCTION(draw_rect_m));
  bind("void drawRectangle(const recti &in pos, const Color&in topleft, const "
       "Color&in topright, const Color&in botright, const Color&in botleft)",
       asFUNCTION(draw_rect_g));

  // TODO: Figure out why the JIT doesn't supoprt asCALL_THISCALL_ASGLOBAL
  // bind("void updateRenderCamera(Camera& cam)",
  // asMETHOD(render::RenderDriver,setCameraData), devices.render); bind("void
  // prepareRender(Camera& cam)",
  // asMETHOD(render::RenderDriver,prepareRender3D), devices.render);
  ...
  bind("void prepareRender(Camera& cam, const recti&)",
       asFUNCTION(prepareRenderClip));
  bind("void renderWorld()", asFUNCTION(renderWorld));
  bind("void renderBillboard(const Material& mat, const vec3d &in pos, double "
       "width, double rotation = 0)",
       asFUNCTION(renderBillboard));
  bind("void renderBillboard(const SpriteSheet& sheet, uint index, const vec3d "
       "&in pos, double width, double rotation = 0)",
       asFUNCTION(renderBillboard_sheet));
  bind("void renderBillboard(const Sprite& spr, const vec3d &in pos, double "
       "width, double rotation = 0)",
       asFUNCTION(renderBillboard_sprite));
  bind("void renderBillboard(const Material& mat, const vec3d &in pos, double "
       "width, double rotation, const Color&in)",
       asFUNCTION(renderBillboard_c));
  bind("void renderBillboard(const SpriteSheet& sheet, uint index, const vec3d "
       "&in pos, double width, double rotation, const Color&in)",
       asFUNCTION(renderBillboard_sheet_c));
  bind("void renderBillboard(const Sprite& spr, const vec3d &in pos, double "
       "width, double rotation, const Color&in)",
       asFUNCTION(renderBillboard_sprite_c));
  bind("void renderPlane(const Material& mat, const vec3d &in pos, double "
       "width, const Color&in, double rotation = 0)",
       asFUNCTION(renderPlane));
  ...
  bind("void drawPolygonStart(uint triangles, const Material@ mat = null)",
       asFUNCTION(poly_start));
  bind("void drawPolygonStart(uint triangles, const Material@ mat, const "
       "Color&in)",
       asFUNCTION(poly_start_c));
  bind("void drawPolygonStart(PolygonType type, uint polyCount, const "
       "Material@ mat = null)",
       asFUNCTION(poly_start_t));
  bind("void drawPolygonEnd()", asFUNCTION(poly_end));

  bind("void drawPolygonPoint(const vec2i &in)", asFUNCTION(poly_coord));
  bind("void drawPolygonPoint(const vec2i &in, const vec2f& in)",
       asFUNCTION(poly_coord_uv));
  bind("void drawPolygonPoint(const vec2i &in, const vec2f& in, const Color& "
       "in)",
       asFUNCTION(poly_coord_uvc));
  bind("void drawPolygonPoint(const vec2i &in, const Color&in)",
       asFUNCTION(poly_coord_c));

  bind("void drawPolygonPoint(const vec2f &in)", asFUNCTION(poly_fcoord));
  bind("void drawPolygonPoint(const vec2f &in, const vec2f& in)",
       asFUNCTION(poly_fcoord_uv));
  bind("void drawPolygonPoint(const vec2f &in, const Color&in)",
       asFUNCTION(poly_fcoord_c));

  bind("void drawPolygonPoint(const vec3d &in)", asFUNCTION(poly3_coord));
  bind("void drawPolygonPoint(const vec3d &in, const vec2f& in)",
       asFUNCTION(poly3_coord_uv));
  bind("void drawPolygonPoint(const vec3d &in, const Color&in)",
       asFUNCTION(poly3_coord_c));
  bind(
      "void drawPolygonPoint(const vec3d &in, const vec2f& in, const Color&in)",
      asFUNCTION(poly3_coord_uvc));

  bind("void drawLine(const vec2i&in from, const vec2i& to, int size = 1, "
       "const Material@ mat = null)",
       asFUNCTION(draw_line));
  bind("void drawLine(const vec2i&in from, const vec2i& to, const Color&in "
       "color, int size = 1, const Material@ mat = null)",
       asFUNCTION(draw_line_c));

  //-- Video modes
  ClassBind vm("VideoMode", asOBJ_POD | asOBJ_VALUE,
               sizeof(os::OSDriver::VideoMode));
  ...
  bind("void getVideoModes(array<VideoMode>& output)",
       asFUNCTION(getVideoModes));
  bind("void getMonitorNames(array<string>& output)",
       asFUNCTION(getMonitorNames));

  bind("void set_vsync(int blanks)", asFUNCTION(setVsync)) doc(
      "Sets vsync mode (applied immediately if possible)",
      "Number of frames to wait for. -1 for adaptive vsync where available.");
  bindGlobal("double scale_3d", &scale_3d)
      doc("Linear factor by which to scale the 3D render for supersampling.");

  //-- Sound
  bind("void getAudioDeviceNames(array<string>& output)",
       asFUNCTION(getAudioNames));
}
