...
struct WindowData {
  ...
  WindowData()
      : width(1024), height(768), redbits(8), greenbits(8), bluebits(8),
        alphabits(8), depthbits(8), stencilbits(8), aa_samples(4),
        refreshRate(0), resizable(false), verticalSync(1), mode(WM_Window),
...
template <class Arg0, class Arg1> struct DriverCallbacks_2 {
  ...
  /*void remove(cb func) {
          callbacks.remove_if([func](cb f) {
                  return func.template target<bool(Arg0,Arg1)>() == f.template
  target<bool(Arg0,Arg1)>();
          });
  }*/
...
template <class Arg0> struct DriverCallbacks_1 {
  ...
  /*void remove(cb func) {
          callbacks.remove_if([func](cb f) {
                  return func.template target<bool(Arg0)>() == f.template
  target<bool(Arg0)>();
          });
  }*/
  // Calls each callback in order, stopping if true is returned
...
struct DriverCallbacks {
  ...
  /*void remove(cb func) {
          callbacks.remove_if([func](cb f) {
                  return func.template target<bool()>() == f.template
  target<bool()>();
          });
  }*/
...
public:
  ...
  // NOTE: If you need to keep the program awake but can't render, use this
  // instead of swapBuffers 		Skips the elapsed time with minimal impact on timers
  ...
  OSDriver()
      : shiftKey(false), altKey(false), ctrlKey(false), win_width(0),
        win_height(0), mouse_x(0), mouse_y(0), leftButton(false),
        rightButton(false), middleButton(false) {}
  virtual ~OSDriver() {}
};
