...
LineTrailNode::LineTrailNode(const render::RenderState &material)
    : storedPositions(0), firstIndex(1), stored_s(0), lineLen_s(1),
      qualitySteps(1), mat(material) {
...
void ProjectileBatch::registerProj(const render::RenderState &mat,
                                   ProjEffect &eff) {
    ...
    if (projs->begin()->line) {
      ...
      while (start < count) {
        ...
        for (unsigned p = start; p < start + drawCount; ++p, verts += 2) {
          ...
          verts[1].pos =
              vec3f(proj.pos - vec3d(proj.dir * proj.length) - camPos);
      ...
      while (start < count) {
        ...
        for (unsigned p = start; p < start + drawCount; ++p, verts += 4) {
          ...
          vec3d offset =
              vec3d(proj.pos - end).cross(driver.cam_facing).normalized(size);
...
void MissileBatch::registerProj(const render::RenderState &mat,
    ...
    for (unsigned ind = 0, cnt = msls->size(); ind != cnt; ++ind) {
      ...
      if (track.aliveUntil >= 0 && gameTime > track.aliveUntil) {
        if ((int)missile.lineCount <= (int)missile.startProgress) {
          ...
          missile.startProgress +=
              frameLen_s * float(LINE_POS_COUNT) / missile.length;
        ...
        missile.pos =
            (missile.pos)
                .interpolate(track.pos + vec3d(track.vel) *
                                             (gameTime - track.lastUpdate),
                             std::min(frameLen_s / 0.25, 1.0));
        missile.lastUpdate = gameTime;

        missile.lineProgress +=
            frameLen_s * float(LINE_POS_COUNT) / missile.length;
        while (missile.lineProgress >= 1.f) {
          missile.lineProgress -= 1.f;
          missile.lineCount += 1;
          missile.lineStart =
              (missile.lineStart + LINE_POS_COUNT - 1) % LINE_POS_COUNT;
  ...
  for (auto i = missiles.begin(), end = missiles.end(); i != end; ++i) {
    ...
    for (unsigned ind = 0, cnt = msls->size(); ind != cnt; ++ind) {
      ...
      if (until < 0 || until > gameTime)
        devices.render->drawBillboard(missile.pos, missile.size,
                                      *i->first.sprite, 0.0, &missile.color);
    }
    ...
    for (unsigned ind = 0, cnt = msls->size(); ind != cnt; ++ind) {
      ...
      auto *v =
          vertBuffer->request(missile.lineCount + 1, render::PT_LineStrip);
      float startPct = (missile.startProgress + missile.lineProgress) *
                       missile.length / float(LINE_POS_COUNT);

      v[0].pos = vec3f(missile.pos - cam_pos);
      v[0].col = missile.start;
      v[0].uv = vec2f(
          missile.startProgress * missile.length / float(LINE_POS_COUNT), 0);
