...
void PointLight::enable(unsigned &lightIndex, const vec3f &offset) {
  ...
  float pos[4] = {position.x + offset.x, position.y + offset.y,
                  position.z + offset.z, 1.f};
  glLightfv(LIGHT, GL_POSITION, (GLfloat *)&pos);
}

vec3f PointLight::getPosition() const { return position; }

float PointLight::getRadius() const { return radius; }
...
void registerLight(LightSource *light) { lights.insert(light); }

void unregisterLight(LightSource *light) { lights.erase(light); }
...
unsigned findNearestLights(const vec3f &from, LightSource **pLights,
                           unsigned maximum) {
  unsigned found = 0;
