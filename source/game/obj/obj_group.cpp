...
ObjectGroup::ObjectGroup(unsigned count, int id)
    : objectCount(count), origObjectCount(count), objects(new Object *[count]),
      physItem(new PhysicsItem), owner(0) {
...
unsigned ObjectGroup::getObjectCount() const { return objectCount; }

unsigned ObjectGroup::getMaxObjectCount() const { return origObjectCount; }

Object *ObjectGroup::getObject(unsigned index) { return objects[index]; }
...
Object *ObjectGroup::getOwner() { return owner; }

void ObjectGroup::setOwner(Object *obj) { owner = obj; }

vec3d ObjectGroup::getCenter() const { return physItem->bound.getCenter(); }

PhysicsItem *ObjectGroup::getPhysicsItem() { return physItem; }
...
bool ObjectGroup::removeOwner() {
  ...
  // Move objects down to the lower indices, and find the nearest object to the
  // previous owner to be the new owner
  unsigned j = 0;
  for (unsigned i = 0; i < objectCount; ++i) {
    Object *&obj = objects[i];
