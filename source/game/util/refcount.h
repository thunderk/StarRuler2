...
template <class T> class _RefCounted {
public:
  mutable T refs;
  _RefCounted() : refs(1) {}
...
template <class _T> struct heldPointer {
  // Actual Pointer - do not directly alter without handling the grab/drop
  // yourself
  _T *ptr;
  ...
  inline void operator=(_T *newPtr) {
    ...
    // We don't explicitly check, because that's a relatively unlikely case, and
    // only adds overhead where not necessary
    if (newPtr)
      newPtr->grab();
  ...
  inline const _T *operator->() const { return ptr; }

  inline operator _T *() { return ptr; }

  inline operator const _T *() const { return ptr; }
  ...
  heldPointer(_T &start) : ptr(&start) { ptr->grab(); }
  ...
  heldPointer(heldPointer<_T> &&move) : ptr(move.ptr) { move.ptr = 0; }
