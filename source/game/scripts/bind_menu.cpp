...
static void toMenu() { game_state = GS_Menu; }

static void toGame() { game_state = GS_Game; }

static void quitGame() { game_state = GS_Quit; }
...
static void makeSaveFileInfo(SaveFileInfo *dat) { new (dat) SaveFileInfo(); }

static void dtorSaveFileInfo(SaveFileInfo *dat) { dat->~SaveFileInfo(); }
...
static unsigned getModCount() { return devices.mods.mods.size(); }

static void saveModState() { devices.mods.saveState(); }
...
static bool createNewMod(const std::string &ident) {
  ...
  devices.mods.registerMod(dirname,
                           ident); // Nothing could possibly go wrong here
  return true;
}
...
void RegisterMenuBinds(bool ingame) {
  // Start a game
  bind("void startNewGame(Message&)",
       asFUNCTIONPR(startNewGame, (net::Message &), void));
  ...
  bind("void getSaveFileInfo(const string& fname, SaveFileInfo& dat)",
       asFUNCTION(saveInfo));
  ...
  mod.addMember("bool forCurrentVersion",
                offsetof(mods::Mod, forCurrentVersion));
  mod.addMember("Mod@ parent", offsetof(mods::Mod, parent));
  mod.addMethod("bool isCompatible(Mod& other)",
                asMETHOD(mods::Mod, isCompatible));
  mod.addExternMethod("void getConflicts(Mod& other, array<string>& files)",
                      asFUNCTION(modConflicts));
  ...
  bind("void switchToMods(const array<string>& mods)",
       asFUNCTION(switchToMods));
}
