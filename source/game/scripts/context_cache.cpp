...
namespace std {
template <> struct hash<ScriptStep> {
...
void logScriptProfile(asIScriptEngine *engine) {
  ...
  if (iSet != scriptSets.end()) {
    ...
    for (auto i = iSet->second->begin(), end = iSet->second->end(); i != end;
         ++i)
    ...
    for (auto i = orderedSteps.begin(), end = orderedSteps.end();
         i != end && i->count > 10; ++i)
      print("\t%8lld: %s line %d in %s", i->count,
            i->func->GetScriptSectionName(), i->line, i->func->GetName());
  } else {
    profile_lock.release();
  }
}
...
std::string getStackVariables(asIScriptContext *context, unsigned frame) {
  ...
  for (unsigned n = 0, ncnt = context->GetVarCount(frame); n < ncnt; ++n) {
    ...
    line += " = ";
    line += getScriptVariable(context->GetAddressOfVar(n, frame),
                              context->GetVarTypeId(n, frame), true,
                              context->GetEngine());
    line += "; ";
    if (line.size() > 100) {
      trace += line + "\n";
...
std::string getStackTrace(asIScriptContext *context, bool verbose) {
  ...
  int cnt =
      std::min((int)context->GetCallstackSize(), MAX_AS_CALLSTACK_PRINT_DEPTH);
  for (int i = 0; i < cnt; ++i) {
    ...
    if (func) {
      ...
      trace += format("  $1::$2 | Line $3 | Col $4\n", func->GetModuleName(),
                      func->GetDeclaration(), line, column);
    } else {
...
asIScriptContext *makeContext(asIScriptEngine *engine) {
  ...
  ctx->SetExceptionCallback(
      asFUNCTIONPR(excCallback, (asIScriptContext *), void), 0, asCALL_CDECL);
#ifdef DO_LINE_CALLBACK
...
std::string getScriptVariable(void *value, asUINT typeId, bool expandMembers,
  ...
  if (typeId == asTYPEID_VOID)
    ...
    {
            int enumVal;
            const char *enumName = engine->GetEnumValueByIndex(typeId, n,
    &enumVal); if( enumVal == *(int*)value )
            {
                    s << ", " << enumName;
                    break;
            }
    ...
    if (obj && expandMembers) {
      ...
      for (unsigned n = 0; n < obj->GetPropertyCount(); n++) {
        s << std::endl
          << "  " << type->GetPropertyDeclaration(n) << " = "
          << getScriptVariable(obj->GetAddressOfProperty(n),
                               obj->GetPropertyTypeId(n), false, engine);
      }
      s << "\n}";
    } else {
      s << " }";
