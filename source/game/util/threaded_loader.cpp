...
struct Task {
  ...
  Task(const std::string &Name, const char *depends)
      : name(Name), finished(false), threadRestriction(-1) {
  ...
  bool mayExecute() {
    if (threadRestriction != threads::invalidThreadID &&
        threadRestriction != threads::getThreadID())
      return false;
  ...
  bool isMyJob() { return threadRestriction == threads::getThreadID(); }

  void execute() {
    if (isFinished())
      return;
    double start = devices.driver->getAccurateTime();
    info("%s started at %.1fs on thread %i", name.c_str(), start - startTime,
         threads::getThreadID());
    ...
    info("%s took %.1fms on thread %i", name.c_str(), executionTime * 1000.0,
         threads::getThreadID());
  }
...
void addTask(const std::string &name, const char *depends,
             std::function<void(void)> execute, int threadRestriction) {
...
bool finished() { return tasksFinished; }
...
threads::threadreturn threadcall processLoad(void *arg) {
  ...
  if (workers == 0) {
    ...
    info("Tasks used a total of %.1f seconds (%d%% faster)", totalTime,
         (int)(100.0 * totalTime / loadTime) - 100);
    tasksFinished = true;
  }
