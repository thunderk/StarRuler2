...
void asGlobal(const std::string &strEngine, const std::string &strModule,
...
void asExec(const std::string &strEngine, const std::string &strModule,
...
Console::Console()
    : open(false), permaStats(false), bg(new render::RenderState),
      compact(false), historyItem(history.cbegin()), asMode(false), keybind(0),
      liveStats(~0) {
  ...
  addCommand("clear", [this](argList &) { clear(); });
  ...
  addCommand("resize", [this](argList &args) {
    ...
    devices.driver->setWindowSize(toNumber<int>(args[0]),
                                  toNumber<int>(args[1]));
  });
  ...
  addCommand("show", [this](argList &args) {
    if (args.empty()) {
      ...
      printLn("Choose which piece of data to toggle: all, fps, menu, server, "
              "client, tick, buffers, bandwidth");
      return;
    }
    ...
    if (args.empty()) {
      ...
      if (args.size() == 1) {
        ...
        if (devices.scripts.menu &&
            devices.scripts.menu->getModule(asModule.c_str())) {
          asEngine = "menu";
        } else if (devices.scripts.client &&
                   devices.scripts.client->getModule(asModule.c_str())) {
          asEngine = "client";
        } else if (devices.scripts.server &&
                   devices.scripts.server->getModule(asModule.c_str())) {
          asEngine = "server";
        } else {
          std::string temp;
...
Console::~Console() { delete bg; }

void Console::addCommand(
    std::string name, std::function<void(std::vector<std::string> &)> function,
    bool replace) {
  toLowercase(name);
...
void Console::addCommand(std::string name, ConsoleFunction &func,
                         bool replace) {
  toLowercase(name);
  if (replace || functions.find(name) == functions.end())
    functions[name] = func;
...
bool Console::character(int code) {
  ...
  undo.push_back(
      std::tuple<std::string, size_t, int>(currentLine, caret, length));
...
bool Console::globalKey(int code, bool pressed) {
  ...
  auto bind = binds.find(profile::getModifiedKey(code, devices.driver->ctrlKey,
                                                 devices.driver->altKey,
                                                 devices.driver->shiftKey));
  if (bind != binds.end()) {
    if (pressed)
...
bool Console::key(int code, bool pressed) {
  ...
  case 'X':
    if (devices.driver->ctrlKey) {
      if (length > 0)
        ...
        devices.driver->setClipboard(
            currentLine.substr(caret + length, -length));
      if (code == 'X') {
        eraseSelection();
        baseTimer = devices.driver->getTime();
      }
  ...
  case 'Y':
    if (devices.driver->ctrlKey) {
      ...
      baseTimer = devices.driver->getTime();
      return true; // Return here to prevent handling the line change by the
                   // undo system
    }
    break;
  case 'Z':
  ...
  case os::KEY_TAB:
    if (!currentLine.empty()) {
      ...
      if (space == std::string::npos) {
        ...
        for (auto i = functions.cbegin(), end = functions.cend(); i != end;
             ++i) {
          size_t p = i->first.find(currentLine);
          if (p == std::string::npos)
            continue;
          unsigned score =
              (unsigned)((p * 4) + (i->first.size() - currentLine.size()));
          if (bestScore < score)
            continue;
          bestScore = score;
          bestMatch = &i->first;
  ...
  // If the line has changed, store the previous state as an undo entry, and
  // clear the redo list
...
void Console::show() { open = true; }

void Console::toggle() { open = !open; }

Color bgCols[4] = {Color(0, 0, 0, 230), Color(0, 0, 0, 230),
                   Color(0, 0, 0, 230), Color(0, 0, 0, 230)};
Color selCols[4] = {Color(0, 0, 255, 80), Color(0, 0, 255, 80),
                    Color(0, 0, 255, 80), Color(0, 0, 255, 80)};
...
void Console::render(render::RenderDriver &driver) {
  ...
  if (!compact && (open || permaStats)) {
    ...
    if (liveStats & LS_FPS) {
      if (realFrameLen > 0.00001) {
        fps = (1.f - average_pct) * fps +
              (average_pct * (float)(1.0 / realFrameLen));

        if (prevTick_s > 0.00001) {
          double gs = devices.driver->getGameSpeed();
          if (gs > 0)
            tps = (1.f - average_pct) * tps +
                  (average_pct * (float)(1.0 / (prevTick_s / gs)));
        }

        char buffer[256];
        sprintf(buffer, "%.1ftps %.1ffps (a=%4.1fms r=%4.1fms p=%4.1fms)", tps,
                fps, animation_s * 1000.0, render_s * 1000.0,
                present_s * 1000.0);
        font.draw(devices.render, buffer,
                  screenSize.width - font.getDimension(buffer).width, 1);
      } else {
        font.draw(devices.render, "inf fps",
                  screenSize.width - font.getDimension("inf fps").width, 1);
      }
    ...
    if (devices.scripts.menu && (liveStats & LS_Menu)) {
      devices.scripts.menu->engine->GetGCStatistics(
          &gc_size, &gc_destroyed, &gc_detected, &gc_new, &gc_newDestroyed);
      std::string gc_state =
          format("Menu GC Entities: $1\n  Destroyed: $2\n  Detected: $3\n  "
                 "New: $4\n  New Destroyed: $5",
                 gc_size, gc_destroyed, gc_detected, gc_new, gc_newDestroyed);
    ...
    if (devices.scripts.client && (liveStats & LS_Client)) {
      devices.scripts.client->engine->GetGCStatistics(
          &gc_size, &gc_destroyed, &gc_detected, &gc_new, &gc_newDestroyed);
      std::string gc_state =
          format("Client GC Entities: $1\n  Destroyed: $2\n  Detected: $3\n  "
                 "New: $4\n  New Destroyed: $5",
                 gc_size, gc_destroyed, gc_detected, gc_new, gc_newDestroyed);
      font.draw(devices.render, gc_state.c_str(), x, y);
      y += lineHeight * 6;
    ...
    if (devices.scripts.server && (liveStats & LS_Server)) {
      devices.scripts.server->engine->GetGCStatistics(
          &gc_size, &gc_destroyed, &gc_detected, &gc_new, &gc_newDestroyed);
      std::string gc_state =
          format("Server GC Entities: $1\n  Destroyed: $2\n  Detected: $3\n  "
                 "New: $4\n  New Destroyed: $5",
                 gc_size, gc_destroyed, gc_detected, gc_new, gc_newDestroyed);
      font.draw(devices.render, gc_state.c_str(), x, y);
      y += lineHeight * 6;
    ...
    if (liveStats & LS_Tick) {
      ...
      std::string tickTimes =
          format("Server Script Tick: $1ms\nClient Script Tick: $2ms",
                 toString(lastServerTick * 1000.0, 2),
                 toString(lastClientTick * 1000.0, 2));
    ...
    if (liveStats & LS_Buffers) {
      ...
      std::string vbData = format(
          "VB Steps: $1\nVB Flushes: $2\n Verts= $3\n Steps= $4\n Shader= $5",
          toString(drawnSteps), toString(bufferFlushes),
          toString(render::vbFlushCounts[render::FC_VertexLimit]),
          toString(render::vbFlushCounts[render::FC_StepLimit]),
          toString(render::vbFlushCounts[render::FC_ShaderLimit]));
    ...
    if (liveStats & LS_Bandwidth && devices.network->monitorBandwidth) {
      std::string bwData = format("Incoming: $1/s\nOutgoing: $2/s\nQueued: $3",
                                  toSize(devices.network->currentIncoming),
                                  toSize(devices.network->currentOutgoing),
                                  toString(devices.network->queuedPackets));
  ...
  if (length != 0) {
    ...
    if (length > 0) {
      ...
      corner.x +=
          font.getDimension(currentLine.substr(0, caret + length).c_str())
              .width;
      size = font.getDimension(
          currentLine.substr(caret + length, -length).c_str());
    }
  ...
  if ((devices.driver->getTime() - baseTimer) % 2000 < 1000) {
    int caret_x =
        font.getDimension(currentLine.substr(0, caret).c_str()).width +
        lineStart.width;
    font.drawChar(&driver, '|', '\0', caret_x - 4, y + lineHeight);
  }

  if (!compact)
    for (auto line = lines.rbegin(), end = lines.rend();
         line != end && (y + lineHeight) > 0; ++line, y -= lineHeight)
