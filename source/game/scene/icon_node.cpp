...
IconNode::IconNode(const render::SpriteSheet *spriteSheet, unsigned spriteIndex)
    : sheet(spriteSheet), index(spriteIndex) {
...
bool IconNode::preRender(render::RenderDriver &driver) {
  ...
  if (sortDistance < MinRenderDistanceFactor * abs_scale ||
...
void IconNode::render(render::RenderDriver &driver) {
  ...
  // Calculate best-fit facing of the 2D sprite (Assuming the sprite is facing
  // +x)
  ...
  driver.drawBillboard(abs_position, iconScale, sheet->material,
                       sheet->getSource(index), rot);
