...
public:
  ...
  struct AppliedStage {
    ...
    AppliedStage() { memset(this, 0, sizeof(AppliedStage)); }
  };
  ...
  struct ModuleDesc {
    ...
    ModuleDesc()
        : index(-1), scr_onEnable(0), scr_onDisable(0), drawMode(0),
          required(false), unique(false), vital(false), defaultUnlock(false),
          scr_onCheckErrors(nullptr) {}
  };
...
Formula *parseFormula(const std::string &str, const SubsystemDef *def = 0,
                      const SubsystemDef::ModifyStage *modifier = 0);
