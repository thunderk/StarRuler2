...
static HullDef *makeHull() { return new HullDef(); }

static Shipset *makeShipset() { return new Shipset(); }
...
static void descDestroy(Design::Descriptor *desc) { desc->~Descriptor(); }

static Design::Descriptor *descCopy(Design::Descriptor *desc,
                                    const Design::Descriptor *other) {
  if (other->settings)
...
static void descAddHex_m(Design::Descriptor &desc, unsigned index, vec2u hex,
                         const SubsystemDef::ModuleDesc *mod) {
  if (index >= desc.systems.size()) {
    scripts::throwException("Subsystem index out of bounds.");
    return;
  }
...
static void descAddHex_lm(Design::Descriptor &desc, vec2u hex,
                          const SubsystemDef::ModuleDesc *mod) {
  if (desc.systems.empty()) {
    scripts::throwException("No subsystem exists.");
    return;
...
static const SubsystemDef::ModuleDesc *getModule(const SubsystemDef *def,
                                                 unsigned i) {
...
static void triggerEffector(const Effector *efft, Object *obj, Object *target,
                            float efficiency = 1.f, double tOffset = 0.0) {
  EffectorTarget targ;
  targ.target = target;
...
static void triggerEffector_t(const Effector *efft, Object *obj, Object *target,
                              const vec3d &tracking, float efficiency = 1.f,
                              double tOffset = 0.0) {
  EffectorTarget targ;
  targ.target = target;
...
static const SubsystemDef::ModuleDesc *getModule_n(const SubsystemDef *def,
                                                   std::string &name) {
...
static void sysvar_aGet(asIScriptGeneric *f) {
  ...
  if (index >= sys->type->variableIndices.size() ||
      sys->type->variableIndices[index] < 0)
    scripts::throwException(
        "Subsystem variable does not apply to this subsystem.");
...
static void subSysVar(const std::string &name, int index) {
  ...
  cls.addGenericMethod(format("float get_$1() const", name).c_str(),
                       asFUNCTION(sysvar_aGet), (void *)(size_t)index);
  cls.addGenericMethod(format("void set_$1(float)", name).c_str(),
                       asFUNCTION(sysvar_aSet), (void *)(size_t)index);
}
...
static float *getSysVar(Subsystem *sys, int index) {
  ...
  if (index >= (int)sys->type->variableIndices.size() ||
      sys->type->variableIndices[index] < 0) {
    scripts::throwException(
        "Subsystem variable does not apply to this subsystem.");
    return 0;
...
static float getSysHexVar(Subsystem *sys, int index, unsigned hexIndex) {
  ...
  if (index >= (int)sys->type->hexVariableIndices.size() ||
      sys->type->hexVariableIndices[index] < 0) {
...
static float *dsgHexVarPtr(Design *dsg, const vec2u &v, int index) {
  ...
  if (!ptr) {
    scripts::throwException(
        "Hexagon variable does not apply to this subsystem.");
    return 0;
  }
...
static int dsgGetBuilt(Design *dsg) { return dsg->built.get(); }

static void dsgDecBuilt(Design *dsg) { --dsg->built; }

static void dsgIncBuilt(Design *dsg) { ++dsg->built; }

static int dsgGetActive(Design *dsg) { return dsg->active.get(); }
...
static void makeEvt(void *mem) { new (mem) EffectEvent(); }

static void delEvt(EffectEvent *evt) { evt->~EffectEvent(); }
...
static Blueprint *dmgBlueprint(DamageEvent &evt) {
  ...
  if (evt.target->type->blueprintOffset == 0)
    return nullptr;
  return (Blueprint *)(((size_t)evt.target.ptr) +
                       evt.target->type->blueprintOffset);
}
...
template <class T> static Subsystem *evtSource(T &evt) {
  ...
  Blueprint *bp =
      (Blueprint *)(((size_t)evt.obj.ptr) + evt.obj->type->blueprintOffset);
...
template <class T> static Subsystem *evtDest(T &evt) {
  if (evt.destination < 0 || !evt.target)
    return 0;
  if (evt.target->type->blueprintOffset == 0)
    return 0;

  Blueprint *bp = (Blueprint *)(((size_t)evt.target.ptr) +
                                evt.target->type->blueprintOffset);
...
template <class T> static Blueprint::SysStatus *evtSourceStatus(T &evt) {
  if (evt.source < 0 || !evt.obj)
    return 0;
  if (evt.obj->type->blueprintOffset == 0)
    return 0;
...
template <class T> static Blueprint::SysStatus *evtDestStatus(T &evt) {
  if (evt.destination < 0 || !evt.target)
    return 0;
  if (evt.target->type->blueprintOffset == 0)
    return 0;
...
static void makeDamageEvt(void *mem) { new (mem) DamageEvent(); }

static void delDamageEvt(DamageEvent *evt) { evt->~DamageEvent(); }

static void emptyEff(void *mem) { new (mem) Effect(); }

static void makeEff(void *mem, unsigned effType) {
  new (mem) Effect(getEffectDefinition(effType));
}

static void emptyTimed(void *mem) { new (mem) TimedEffect(); }

static void makeTimed(void *mem, unsigned effType, double time) {
  new (mem) TimedEffect(getEffectDefinition(effType), time);
}

static void delTimed(TimedEffect *evt) { evt->~TimedEffect(); }
...
static const Subsystem *getHexSys(const Design *design, unsigned x,
                                  unsigned y) {
...
static const SubsystemDef::ModuleDesc *getHexModule(const Design *design,
...
static const SubsystemDef::ModuleDesc *getHexModule_v(const Design *design,
                                                      const vec2u &v) {
  if (v.x >= design->grid.width || v.y >= design->grid.height)
    return 0;
...
static void setDesignData(Design *design, asIScriptObject *obj) {
  ...
  if (design->data != nullptr) {
    scripts::throwException(
        "Setting design data on design that already has data.");
    return;
  }

  design->data = new net::Message();

  auto *writeFunc = (asIScriptFunction *)man->engine->GetUserData(
      scripts::EDID_SerializableWrite);
  if (writeFunc) {
...
static unsigned hexCount(const Subsystem *sys) { return sys->hexes.size(); }
...
static const SubsystemDef::ModuleDesc *getSysModule(const Subsystem *sys,
                                                    unsigned i) {
...
static unsigned dsgErrorCount(const Design *dsg) { return dsg->errors.size(); }
...
static void dsgAddError(Design *dsg, bool fatal, const std::string &text,
                        const Subsystem *sys,
                        const SubsystemDef::ModuleDesc *module, vec2u hex) {
...
public:
  ...
  void trigger(Object *obj, Object *targObj, float efficiency,
               double tOffset = 0) {
...
static ScriptTurret *loadScriptTurret(SaveMessage &file) {
  ...
  if (!file.readBit()) {
    ...
    if (subsysIndex >= dsg->subsystems.size() ||
        effectorIndex >= dsg->subsystems[subsysIndex].type->effectors.size())
...
static const std::string &strvec_get(std::vector<std::string> &vec,
                                     unsigned index) {
  if (index >= vec.size())
    return ERRSTR;
  return vec[index];
...
static unsigned dblvec_length(std::vector<double> &vec) { return vec.size(); }
...
static unsigned getSSTag(const std::string &tag) { return getSysTagIndex(tag); }
...
void RegisterDesignBinds(bool server, bool declarations) {
  if (declarations) {
    ...
    ClassBind evt("Event", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CD,
                  sizeof(EffectEvent));
    ClassBind dev("DamageEvent", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CD,
                  sizeof(DamageEvent));
    ClassBind def("EffectDef", asOBJ_REF | asOBJ_NOCOUNT);
    ClassBind deftr("EffectorDef", asOBJ_REF | asOBJ_NOCOUNT);
    ClassBind eff("Effect", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_C,
                  sizeof(Effect));
    ClassBind efftr("Effector", asOBJ_REF);
    ClassBind turr("Turret", asOBJ_REF);
    ClassBind timed("TimedEffect", asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CD,
                    sizeof(TimedEffect));
    ...
    ClassBind desc("DesignDescriptor", asOBJ_VALUE | asOBJ_APP_CLASS_CDA,
                   sizeof(Design::Descriptor));
    return;
  }
  ...
  shipset.addMethod("uint get_hullCount() const",
                    asMETHOD(Shipset, getHullCount));
  shipset.addMethod("bool hasHull(const Hull& hull) const",
                    asMETHOD(Shipset, hasHull));
  shipset.addMethod(
      "const Hull@+ get_hulls(uint index) const",
      asMETHODPR(Shipset, getHull, (unsigned) const, const HullDef *));
  shipset.addMethod("const Hull@+ getHull(const string& ident) const",
                    asMETHODPR(Shipset, getHull, (const std::string &) const,
                               const HullDef *));

  bind("uint getShipsetCount()", asFUNCTION(getShipsetCount));
  bind("const Shipset& getShipset(uint id)",
       asFUNCTIONPR(getShipset, (unsigned), const Shipset *));
  bind("const Shipset& getShipset(const string& ident)",
       asFUNCTIONPR(getShipset, (const std::string &), const Shipset *));
  ...
  shipset.addMethod("const ShipSkin& getSkin(const string& name) const",
                    asMETHOD(Shipset, getSkin));
  ...
  hull.addMethod("Hull& opAssign(const Hull&in other)",
                 asMETHOD(HullDef, operator=));
  hull.addMethod("bool hasTag(const string&in tag) const",
                 asMETHOD(HullDef, hasTag));
  hull.addMethod("bool isExterior(const vec2u& hex) const",
                 asMETHOD(HullDef, isExterior));
  hull.addMethod("bool isExteriorInDirection(const vec2u& hex, "
                 "HexGridAdjacency adj) const",
                 asMETHOD(HullDef, isExteriorInDirection));
  hull.addMethod(
  ...
  bind(
      "void readHullDefinitions(const string&in filename, array<Hull@>& hulls)",
      asFUNCTION(readHulls));
  bind("void writeHullDefinitions(const string&in filename, array<Hull@>& "
       "hulls)",
       asFUNCTION(writeHulls));
  ...
  bind("const EffectDef@ getEffectDefinition(EffectType type)",
       asFUNCTIONPR(getEffectDefinition, (int), const EffectDef *));
  bind("const EffectDef@ getEffectDefinition(const string &in name)",
       asFUNCTIONPR(getEffectDefinition, (const std::string &),
                    const EffectDef *));
  ...
  evt.addExternMethod("const Subsystem@ get_source()",
                      asFUNCTION(evtSource<EffectEvent>));
  evt.addExternMethod("const Subsystem@ get_destination()",
                      asFUNCTION(evtDest<EffectEvent>));
  evt.addExternMethod("SysStatus@ get_source_status()",
                      asFUNCTION(evtSourceStatus<EffectEvent>));
  evt.addExternMethod("SysStatus@ get_destination_status()",
                      asFUNCTION(evtDestStatus<EffectEvent>));
  ...
  dev.addExternBehaviour(asBEHAVE_DESTRUCT, "void f()",
                         asFUNCTION(delDamageEvt));
  ...
  dev.addExternMethod("const Subsystem@ get_source()",
                      asFUNCTION(evtSource<DamageEvent>));
  dev.addExternMethod("const Subsystem@ get_destination()",
                      asFUNCTION(evtDest<DamageEvent>));
  dev.addExternMethod("SysStatus@ get_source_status()",
                      asFUNCTION(evtSourceStatus<DamageEvent>));
  dev.addExternMethod("SysStatus@ get_destination_status()",
                      asFUNCTION(evtDestStatus<DamageEvent>));
  ...
  timed.addConstructor("void f(EffectType type, double time)",
                       asFUNCTION(makeTimed));
  ...
  effd.addExternMethod("Color get_trailStart() const",
                       asFUNCTION(effTrailStart));
  effd.addExternMethod("Color get_trailEnd() const", asFUNCTION(effTrailEnd));
  effd.addExternMethod("int getArgumentIndex(const string& name)",
                       asFUNCTION(effArg));

  {
    ...
    for (int i = 0, cnt = getEffectorDefinitionCount(); i < cnt; ++i) {
      ...
      bindGlobal(format("const ::EffectorDef $1", def->name).c_str(),
                 (void *)def);
    }
  }

  bind("EffectorDef@ getEffectorDef(uint id)",
       asFUNCTIONPR(getEffectorDefinition, (unsigned), const EffectorDef *));
  bind("EffectorDef@ getEffectorDef(const string& ident)",
       asFUNCTIONPR(getEffectorDefinition, (const std::string &),
                    const EffectorDef *));

  // Script effectors
  ClassBind efft("Effector");
  efft.setReferenceFuncs(asMETHOD(ScriptEffector, grab),
                         asMETHOD(ScriptEffector, drop));
  efft.addMember("vec3d turretAngle", offsetof(Effector, turretAngle));
  efft.addMember("vec3d relativePosition",
                 offsetof(Effector, relativePosition));
  efft.addMember("const double fireArc", offsetof(Effector, fireArc));
  efft.addMember("const double targetTolerance",
                 offsetof(Effector, targetTolerance));
  efft.addMember("const double fireTolerance",
                 offsetof(Effector, fireTolerance));
  ...
  if (server) {
    efft.addFactory("Effector@ f(const EffectorDef& def)",
                    asFUNCTION(makeScriptEff));
    efft.addExternMethod("const EffectorDef@ get_type() const",
                         asFUNCTION(getEffDefType));
    ...
    efft.addMethod("double& opIndex(uint index)",
                   asMETHOD(ScriptEffector, getValueByIndex));
    efft.addMethod("double& opIndex(const string& name)",
                   asMETHOD(ScriptEffector, getValue));
    efft.addMethod("void evaluate()", asMETHOD(ScriptEffector, evaluate));

    efft.addExternMethod("void trigger(Object& obj, Object& target, float "
                         "efficiency = 1.f, double tOffset = 0.0) const",
                         asFUNCTION(triggerEffector));
    efft.addExternMethod(
        "void trigger(Object& obj, Object& target, const vec3d& tracking, "
        "float efficiency = 1.f, double tOffset = 0.0) const",
        asFUNCTION(triggerEffector_t));
  }
  ...
  turr.setReferenceFuncs(asMETHOD(ScriptTurret, grab),
                         asMETHOD(ScriptTurret, drop));
  if (server) {
    turr.addFactory("Turret@ f(const Effector& efftr)",
                    asFUNCTION(makeScriptTurret));
    turr.addFactory("Turret@ f(SaveFile& file)", asFUNCTION(loadScriptTurret));
    turr.addMember("Object@ target", offsetof(ScriptTurret, target) +
                                         offsetof(EffectorTarget, target));
    turr.addMember("vec3d tracking", offsetof(ScriptTurret, target) +
                                         offsetof(EffectorTarget, tracking));
    turr.addMember("uint8 hits", offsetof(ScriptTurret, target) +
                                     offsetof(EffectorTarget, hits));
    turr.addMember("uint flags", offsetof(ScriptTurret, target) +
                                     offsetof(EffectorTarget, flags));

    turr.addMethod(
        "void update(Object& obj, double time, float efficiency = 1.f)",
        asMETHOD(ScriptTurret, update));
    turr.addMethod("void trigger(Object& obj, Object& target, float efficiency "
                   "= 1.f, double tOffset = 0.0)",
                   asMETHOD(ScriptTurret, trigger));
  ...
  sysdef.addMember("const ModuleDef@ coreModule",
                   offsetof(SubsystemDef, coreModule));
  sysdef.addMember("const ModuleDef@ defaultModule",
                   offsetof(SubsystemDef, defaultModule));
  sysdef.addExternMethod("uint get_effectCount() const",
                         asFUNCTION(effectCount));
  sysdef.addMethod(
      "bool hasTag(const string &in tag) const",
      asMETHODPR(SubsystemDef, hasTag, (const std::string &) const, bool));
  sysdef.addMethod("bool hasTag(SubsystemTag tag) const",
                   asMETHODPR(SubsystemDef, hasTag, (int) const, bool));
  sysdef.addMethod(
      "const string& getTagValue(SubsystemTag tag, uint index = 0) const",
      asMETHOD(SubsystemDef, getTagValue));
  sysdef.addMethod("uint getTagValueCount(SubsystemTag tag) const",
                   asMETHOD(SubsystemDef, getTagValueCount));
  sysdef.addMethod(
      "bool hasTagValue(SubsystemTag tag, const string& value) const",
      asMETHOD(SubsystemDef, hasTagValue));
  sysdef.addMethod("bool hasHullTag(const string &in tag) const",
                   asMETHOD(SubsystemDef, hasHullTag));
  sysdef.addMethod("bool canUseOn(const Hull@ hull) const",
                   asMETHOD(SubsystemDef, canUseOn));
  sysdef.addExternMethod("bool hasModifier(const string&in mod) const",
                         asFUNCTION(sysDefHasMod));

  sysdef.addExternMethod("uint get_moduleCount() const",
                         asFUNCTION(moduleCount));
  sysdef.addExternMethod("const ModuleDef@ get_modules(uint index) const",
                         asFUNCTION(getModule));
  sysdef.addExternMethod("const ModuleDef@ module(const string&in) const",
                         asFUNCTION(getModule_n));

  bind("int getSubsystemDefCount()", asFUNCTION(getSubsystemDefCount));
  bind("const SubsystemDef@ getSubsystemDef(int index)",
       asFUNCTIONPR(getSubsystemDef, (int), const SubsystemDef *));
  bind("const SubsystemDef@ getSubsystemDef(const string& id)",
       asFUNCTIONPR(getSubsystemDef, (const std::string &),
                    const SubsystemDef *));

  {
    ...
    for (int i = 0, cnt = getSubsystemDefCount(); i < cnt; ++i) {
      ...
      bindGlobal(format("const ::SubsystemDef $1", def->id).c_str(),
                 (void *)def);
    }
  }
  ...
  mod.addMember("string description",
                offsetof(SubsystemDef::ModuleDesc, description));
  ...
  mod.addMember("bool defaultUnlock",
                offsetof(SubsystemDef::ModuleDesc, defaultUnlock));
  mod.addMember("const Sprite sprite",
                offsetof(SubsystemDef::ModuleDesc, sprite));
  mod.addMember("int drawMode", offsetof(SubsystemDef::ModuleDesc, drawMode));

  mod.addMethod("bool hasTag(const string &in tag) const",
                asMETHODPR(SubsystemDef::ModuleDesc, hasTag,
                           (const std::string &) const, bool));
  mod.addMethod(
      "bool hasTag(SubsystemTag tag) const",
      asMETHODPR(SubsystemDef::ModuleDesc, hasTag, (int) const, bool));
  mod.addMethod(
      "const string& getTagValue(SubsystemTag tag, uint index = 0) const",
      asMETHOD(SubsystemDef::ModuleDesc, getTagValue));
  mod.addMethod("uint getTagValueCount(SubsystemTag tag) const",
                asMETHOD(SubsystemDef::ModuleDesc, getTagValueCount));
  mod.addMethod("bool hasTagValue(SubsystemTag tag, const string& value) const",
                asMETHOD(SubsystemDef::ModuleDesc, hasTagValue));

  EnumBind vars("SubsystemVariable");
  EnumBind hexvars("HexVariable");
  EnumBind shipvars("ShipVariable");

  bind("SubsystemVariable getSubsystemVariable(const string&in name)",
       asFUNCTION(getVariableIndex));
  bind("HexVariable getHexVariable(const string&in name)",
       asFUNCTION(getHexVariableIndex));
  bind("ShipVariable getShipVariable(const string&in name)",
       asFUNCTION(getShipVariableIndex));
  ...
  sys.addExternMethod("const float& opIndex(SubsystemVariable var) const",
                      asFUNCTION(getSysVar));
  sys.addExternMethod("float& opIndex(SubsystemVariable var)",
                      asFUNCTION(getSysVar));
  sys.addExternMethod("bool has(SubsystemVariable var) const",
                      asFUNCTION(hasSysVar));
  sys.addExternMethod("bool has(HexVariable var) const",
                      asFUNCTION(hasSysHexVar));
  sys.addExternMethod("const Effect& opIndex(uint ind) const",
                      asFUNCTION(getSysEff));
  sys.addExternMethod("Effect& opIndex(uint ind)", asFUNCTION(getSysEff));
  sys.addExternMethod("uint get_effectorCount() const",
                      asFUNCTION(effectorCount));
  sys.addExternMethod("const Effector@ get_effectors(uint index) const",
                      asFUNCTION(sysGetEffector));

  sys.addExternMethod("float total(HexVariable var) const",
                      asFUNCTION(sysHexTotal));
  ...
  sys.addExternMethod("const ModuleDef@ module(uint i) const",
                      asFUNCTION(getSysModule));

  sys.addExternMethod("const float hexVariable(HexVariable, uint) const",
                      asFUNCTION(getSysHexVar));
  sys.addExternMethod("float& hexVariable(HexVariable, uint)",
                      asFUNCTION(getSysHexVarRef));

  sys.addExternMethod("const float& variable(SubsystemVariable) const",
                      asFUNCTION(getSysVar));
  sys.addExternMethod("float& variable(SubsystemVariable)",
                      asFUNCTION(getSysVar));
  ...
  ds.addExternMethod("double get_quadrantTotalHP(uint index) const",
                     asFUNCTION(quadrantTotalHP));
  ds.addMethod("uint getQuadrant(const vec2u& pos) const",
               asMETHOD(Design, getQuadrant));

  ds.addMethod("const Design& newest() const", asMETHOD(Design, newest));
  ds.addMethod("const Design& next() const", asMETHOD(Design, next));
  ds.addMethod("const Design& mostUpdated() const",
               asMETHOD(Design, mostUpdated));
  ds.addMethod("const Design& base() const", asMETHOD(Design, base));
  ds.addMethod("bool hasTag(const string &in tag) const",
               asMETHODPR(Design, hasTag, (const std::string &) const, bool));
  ds.addMethod("bool hasTag(SubsystemTag tag) const",
               asMETHODPR(Design, hasTag, (int) const, bool));
  ds.addExternMethod("bool hasSubsystem(const SubsystemDef&) const",
                     asFUNCTION(hasSubsys));
  ds.addMethod("void toDescriptor(DesignDescriptor& desc) const",
               asMETHOD(Design, toDescriptor));

  ds.addExternMethod("void rename(const string &in name) const",
                     asFUNCTION(dsgRename))
  ...
  ds.addExternMethod("float variable(ShipVariable var) const",
                     asFUNCTION(dsgGetShipVar));
  ds.addExternMethod("float& variable(ShipVariable var)",
                     asFUNCTION(dsgShipVarPtr));

  ds.addExternMethod(
      "float variable(const Subsystem& sys, SubsystemVariable var) const",
      asFUNCTION(dsgGetVar));
  ds.addExternMethod("float& variable(Subsystem& sys, SubsystemVariable var)",
                     asFUNCTION(dsgVarPtr));

  ds.addExternMethod("bool has(const vec2u& hex, HexVariable var) const",
                     asFUNCTION(dsgHasHexVar));
  ds.addExternMethod("float variable(const vec2u& hex, HexVariable var) const",
                     asFUNCTION(dsgGetHexVar));
  ds.addExternMethod("float& variable(const vec2u& hex, HexVariable var)",
                     asFUNCTION(dsgHexVarPtr));

  ds.addExternMethod("float total(SubsystemVariable var) const",
                     asFUNCTION(dsgSysTotal));
  ds.addExternMethod("float total(HexVariable var) const",
                     asFUNCTION(dsgHexTotal));

  ds.addExternMethod("float average(SubsystemVariable var) const",
                     asFUNCTION(dsgSysAvg));
  ds.addExternMethod("float average(HexVariable var) const",
                     asFUNCTION(dsgHexAvg));

  ds.addExternMethod("uint get_subsystemCount() const", asFUNCTION(sysCount));
  ds.addExternMethod("const Subsystem@ get_subsystems(uint i) const",
                     asFUNCTION(getSys));
  ds.addExternMethod("const Subsystem@ subsystem(uint i) const",
                     asFUNCTION(getSys));
  ds.addExternMethod("const Subsystem@ subsystem(uint x, uint y) const",
                     asFUNCTION(getHexSys));
  ds.addExternMethod("const Subsystem@ subsystem(const vec2u& hex) const",
                     asFUNCTION(getHexSys_v));
  ds.addExternMethod("const ModuleDef@ module(uint x, uint y) const",
                     asFUNCTION(getHexModule));
  ds.addExternMethod("const ModuleDef@ module(const vec2u& hex) const",
                     asFUNCTION(getHexModule_v));
  ds.addExternMethod("const int hexIndex(const vec2u& hex) const",
                     asFUNCTION(getHexIndex));
  ds.addExternMethod("const int hexStatusIndex(const vec2u& hex) const",
                     asFUNCTION(getHexStatusIndex));
  ds.addExternMethod("bool validHex(const vec2u& hex) const",
                     asFUNCTION(isValidhex));
  ds.addExternMethod("void setObsolete(bool obsolete) const",
                     asFUNCTION(setDesignObsolete));
  ds.addExternMethod("void setSettings(const Serializable& ser) const",
                     asFUNCTION(setDesignData));
  ...
  ds.addExternMethod("DesignError@ get_errors(uint) const",
                     asFUNCTION(dsgError));
  ds.addExternMethod("void addError(bool fatal, const string&in text, const "
                     "Subsystem@ subsys, const ModuleDef@ module, vec2u hex)",
                     asFUNCTION(dsgAddError));
  ds.addExternMethod("void addErrorHex(const vec2u& hex)",
                     asFUNCTION(dsgAddErrorHex));
  ds.addExternMethod("bool isErrorHex(const vec2u& hex) const",
                     asFUNCTION(dsgIsErrorHex));
  ...
  dc.addExternMethod("const Design@ get_designs(uint i) const",
                     asFUNCTION(getDesign));
  ...
  desc.addExternMethod(
      "DesignDescriptor& opAssign(const DesignDescriptor&in other)",
      asFUNCTION(descCopy));
  ...
  desc.addMember("Serializable@ settings",
                 offsetof(Design::Descriptor, settings));
  ...
  desc.addExternMethod("uint addSystem(const SubsystemDef& type)",
                       asFUNCTION(descAddSys));
  desc.addExternMethod("void setDirection(const vec3d& dir)",
                       asFUNCTION(descSetDirection));
  desc.addExternMethod("void addHex(uint num, vec2u pos)",
                       asFUNCTION(descAddHex));
  desc.addExternMethod("void addHex(vec2u pos)", asFUNCTION(descAddHex_l));
  desc.addExternMethod("void addHex(uint num, vec2u pos, const ModuleDef& mod)",
                       asFUNCTION(descAddHex_m));
  desc.addExternMethod("void addHex(vec2u pos, const ModuleDef& mod)",
                       asFUNCTION(descAddHex_lm));
  desc.addExternMethod("void applySubsystem(const SubsystemDef& type)",
                       asFUNCTION(descApply));

  bind("const Design@ makeDesign(const DesignDescriptor &in desc)",
       asFUNCTION(makeDesign));
  ...
  se.addMember("const Subsystem@ subsystem",
               offsetof(SubsystemEvent, subsystem));
  ...
  if (server) {
    ...
    strlist.addExternMethod("const string& opIndex(uint num)",
                            asFUNCTION(strvec_get));
    ...
    sh.addMethod("bool init(Design& design, Subsystem& subsystem, StringList& "
                 "arguments, DoubleList& values) const",
                 &Subsystem::ScriptInitFunction);
    sh.addMethod("void start(SubsystemEvent& event) const",
                 &Subsystem::ScriptHookFunctions[EH_Start]);
    sh.addMethod("void tick(SubsystemEvent& event, double time) const",
                 &Subsystem::ScriptHookFunctions[EH_Tick]);
    sh.addMethod("void suspend(SubsystemEvent& event) const",
                 &Subsystem::ScriptHookFunctions[EH_Suspend]);
    sh.addMethod("void resume(SubsystemEvent& event) const",
                 &Subsystem::ScriptHookFunctions[EH_Continue]);
    sh.addMethod("void destroy(SubsystemEvent& event) const",
                 &Subsystem::ScriptHookFunctions[EH_Destroy]);
    sh.addMethod("void end(SubsystemEvent& event) const",
                 &Subsystem::ScriptHookFunctions[EH_End]);
    sh.addMethod("void change(SubsystemEvent& event) const",
                 &Subsystem::ScriptHookFunctions[EH_Change]);
    sh.addMethod("void save(SubsystemEvent& event, SaveFile& file) const",
                 &Subsystem::ScriptHookFunctions[EH_Save]);
    sh.addMethod("void load(SubsystemEvent& event, SaveFile& file) const",
                 &Subsystem::ScriptHookFunctions[EH_Load]);
    sh.addMethod("void ownerChange(SubsystemEvent& event, Empire@ prevEmpire, "
                 "Empire@ newEmpire) const",
                 &Subsystem::ScriptHookFunctions[EH_Owner_Change]);
    sh.addMethod("DamageEventStatus damage(SubsystemEvent& event, DamageEvent& "
                 "damage, const vec2u& position) const",
                 &Subsystem::ScriptHookFunctions[EH_Damage]);
    sh.addMethod(
        "DamageEventStatus globalDamage(SubsystemEvent& event, DamageEvent& "
        "damage, const vec2u& position, vec2d& endPoint) const",
        &Subsystem::ScriptHookFunctions[EH_GlobalDamage]);
    sh.addMethod("void preRetrofit(SubsystemEvent& event) const",
                 &Subsystem::ScriptHookFunctions[EH_Retrofit_Pre]);
    sh.addMethod("void postRetrofit(SubsystemEvent& event) const",
                 &Subsystem::ScriptHookFunctions[EH_Retrofit_Post]);

    sys.addExternMethod("uint get_hookCount() const", asFUNCTION(sysHookCount));
    sys.addExternMethod("SubsystemHook@ get_hooks(uint index) const",
                        asFUNCTION(sysHookGet));
  }
}
