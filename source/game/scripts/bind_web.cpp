...
static const std::string WikiURI(
    "http://wiki.starruler2.com/"
    "api.php?format=json&action=query&titles=$1&prop=revisions&rvprop=content");
...
struct WebData : AtomicRefCounted {
  ...
  WebData() : completed(true) { curl = curl_easy_init(); }
  ...
  void request(const std::string &URI,
               std::function<void(WebData &)> perf = nullptr) {
    if (!completed) {
      scripts::throwException(
          "Attempting to request WebData result before request completed.");
      return;
    }
  ...
  bool wasError() { return errorStr.size() != 0; }
  ...
  const std::string &getResult() {
    if (!completed) {
      scripts::throwException(
          "Attempting to get WebData result before request completed.");
      return EMPTY_STRING;
    }
  ...
  const std::string &getError() {
    if (!completed) {
      scripts::throwException(
          "Attempting to get WebData error before request completed.");
      return EMPTY_STRING;
    }
...
WebData *makeWebData() { return new WebData(); }
  ...
  if (!dat.postData.empty()) {
    for (size_t i = 0, cnt = dat.postData.size(); i < cnt; ++i) {
      curl_formadd(&formpost, &lastptr, CURLFORM_COPYNAME,
                   dat.postData[i].first.c_str(), CURLFORM_COPYCONTENTS,
                   dat.postData[i].second.c_str(), CURLFORM_END);
    }
    dat.postData.clear();
...
bool getIsUpdating() { return !isUpdating.check(0); }
...
struct MD5 {
  ...
  bool zero() { return !bytes[0] && !bytes[1] && !bytes[2] && !bytes[3]; }
...
MD5 hash(const std::string &filename) {
  ...
  // calculating
  const unsigned s[64] = {7,  12, 17, 22, 7,  12, 17, 22, 7,  12, 17, 22, 7,
...
static int GameUpdate() {
  ...
  auto checkDownloads = [&](bool beginDownloads) {
    for (auto dl = activeDownloads.begin(), dlend = activeDownloads.end();
         dl != dlend; ++dl) {
      auto *web = dl->second;
      if (web->completed) {
        if (web->wasError()) {
          ...
          if (FILE *file = fopen(patchName.c_str(), "wb")) {
            fwrite(dl->second->result.c_str(), 1, dl->second->result.size(),
                   file);
            fclose(file);
          }
          downloaded += 1;
    ...
    if (beginDownloads && activeDownloads.size() < 3 &&
        !queuedDownloads.empty()) {
  ...
  auto queueDownload = [&](const std::string &relpath, MD5 current,
                           MD5 updated) {
    if (updated.zero()) {
    ...
    totalFiles += 1;
    auto uri = format("http://api.starruler2.com/updates/$1/$2",
                      current.toString(), updated.toString());
  ...
  updateProgress = 5.0;
  // Calculate the hashes of all listed files, and queue the download if they
  // need to be updated
  for (auto f = files.begin(), fend = files.end(); f != fend; ++f) {
    ...
    updateProgress = 5.0 + 45.0 * std::min(1.0, (double)hashedFiles / 2000.0) +
                     45.0 * (double)downloaded / (double)totalFiles;
    if (md5 == f->second)
      continue;
    queueDownload(f->first, md5, f->second);
  }

  while ((!queuedDownloads.empty() || !activeDownloads.empty()) &&
         failures < 25) {
    checkDownloads(true);
    if (totalFiles > 0)
  ...
  if (!queuedDeletions.empty()) {
    ...
    for (auto f = queuedDeletions.begin(), fend = queuedDeletions.end();
         f != fend; ++f)
...
std::string formatURI(const std::string &uri, std::string param) {
  return replaced(format(uri.c_str(), replaced(param, "&", "&amp;")), " ",
                  "%20");
}
...
bool intoMember(WebData &dat, rapidjson::Value *&node, const char *name) {
  if (!node->IsObject()) {
    dat.errorStr =
        "Invalid json output. Not an object. " + toString(node->GetType());
    return false;
  }
...
void RegisterWebBinds() {
  ...
  wd.addMethod("void addPost(const string& name, const string& data)",
               asMETHOD(WebData, addPost))
      doc("Add a post parameter to the request.", "Parameter name.",
          "Parameter data.");

  bind("void getWikiPage(const string&in title, WebData& dat)",
       asFUNCTION(getWikiPage))
      doc("Load the contents of a page from the wiki.",
          "Wiki page title to load.", "WebData to use for the loading.");

  bind("void webAPICall(const string& page, WebData& dat)",
       asFUNCTION(webAPICall))
      doc("Make a call to the web API to get data.", "API page to call.",
