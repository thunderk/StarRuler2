...
public:
  ...
  void render(RenderDriver &driver, unsigned index, const vec3d *vertices,
              const Color *color = 0) const;
  void render(RenderDriver &driver, unsigned index, recti rectangle,
              const Color *color = 0, const recti *clip = 0,
              const render::Shader *shader = 0, double rotation = 0.0) const;
};
...
public:
  ...
  Sprite(const RenderState *Material) : sheet(0), mat(Material), index(0) {}
};
