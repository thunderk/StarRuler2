...
ifneq (,$(findstring clang, $(shell $(CXX) --version)))
	CXXFLAGS += -flto=thin
else ifneq (,$(findstring gcc, $(shell $(CXX) --version)))
	CXXFLAGS += -flto=6
endif
else
	CXXFLAGS += -fno-lto
...
WARNFLAGS += -Wall -Wno-invalid-offsetof -Wno-switch -Wno-reorder
WARNFLAGS += -Wno-unused-local-typedefs
WARNFLAGS += -Wuninitialized -Wmaybe-uninitialized
...
#Libraries
LDFLAGS += -lstdc++ -lGLEW -lpng -lz -pthread
...
LIB_NAMES = \
	...
	network \
	util
LIB_TARGETS = $(addsuffix .a, $(addprefix $(OBJDIR)/lib, $(LIB_NAMES)))

ifdef USE_SDL
	CXXFLAGS += -DUSE_SDL
	SOURCES += os/sdl_driver.cpp
else
	CXXFLAGS += -DUSE_GLFW -isystem./source/glfw/include
	SOURCES += os/glfw_driver.cpp
...
# Included libs
$(OBJDIR)/libircclient.a: source/libircclient/src/*.c source/libircclient/include/*.h
	@mkdir -p $(BINDIR)
	@$(MAKE) -s -f source/libircclient/gcc/Makefile compile

$(OBJDIR)/lib%.a: source/%/source/*.cpp source/%/include/*.h
	@mkdir -p $(BINDIR)
	@$(MAKE) -s -f source/$*/gcc/Makefile compile

#Complete binary compile
$(BINDIR)/$(BIN): $(LIB_TARGETS) $(OBJ_FILES) $(LIB_FILES) $(COPY_LIBS)
	@mkdir -p $(BINDIR)
	@echo $@
	@$(CXX) $(CXXFLAGS) $(OBJ_FILES) $(LIB_FILES) $(LIB_TARGETS) $(LDFLAGS) -o $@
...
$(BINDIR)/$(MS_BIN): $(OBJDIR)/master_server.o $(LIBDIR)/libnetwork.a $(LIBDIR)/libos.a
	@mkdir -p $(BINDIR)
	@$(CXX) $(CXXFLAGS) -lstdc++ -pthread -lm $^ -o $@

patcher: $(LIB_FILES)
	@$(CXX) $(CXXFLAGS) $(LDFLAGS) source/patcher/patcher/patcher.cpp $(LIB_FILES) -o $(BINDIR)/Patcher.bin
