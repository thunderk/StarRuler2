...
bool Mutex::try_lock() {
  ...
  if (owningThread != id) {
    for (unsigned i = 0; i < spinCount; ++i) {
      if (owningThread.compare_exchange_strong(id, invalidThreadID) ==
          invalidThreadID) {
        lockCount = 1;
#ifdef PROFILE_LOCKS
        ++profileCount;
...
bool Mutex::hasLock() { return owningThread == getThreadID(); }

void atomic_int::wait_compare_exchange(int xchg, int compareTo,
                                       const int spinCount) {
  int spins = 0;
...
bool ReadWriteMutex::hasWriteLock() { return owningThread == getThreadID(); }
...
void Signal::signal(int value) { flag.set_basic(value); }

void Signal::signalDown() { --flag; }

void Signal::signalUp() { ++flag; }

void Signal::signalDown(int value) { flag -= value; }

void Signal::signalUp(int value) { flag += value; }

bool Signal::check(int checkFor) const { return flag == checkFor; }
