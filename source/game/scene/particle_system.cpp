...
const uint32_t fileIdentifier =
    (uint32_t)(('S' << 24) | ('R' << 16) | ('2' << 8) | 'P');
const uint8_t currentVersion = 3;

struct BinaryFile {
  FILE *file;

  BinaryFile(const char *filename, const char *mode)
  ...
  template <class T> BinaryFile &operator<<(const T &data) {
    fwrite(&data, sizeof(T), 1, file);
    return *this;
  }

  template <class T> BinaryFile &operator>>(T &data) {
    fread(&data, sizeof(T), 1, file);
    return *this;
  }

  void read(char *buffer, unsigned bytes) { fread(buffer, bytes, 1, file); }
...
struct RandRange {
  ...
  float get() const { return randomf(min, max); }
...
template <class T> T interp(const T &a, const T &b, float percent) {
...
template <class T> struct BezierCurve {
  std::vector<T> values;
  T def;
  ...
  T interp(float percent) const {
    ...
    // Each iteration, swap dest and source, reduce the next count by one, and
    // interp between previous results When count reaches 0, we have the final
    // value
    T *source = dest + count;
...
struct ParticleFlowDesc {
  ...
  ParticleFlowDesc()
      : start(0.f), end(0.f), rate(1.f), sound_start(nullptr), flat(false) {
    size.def = 1.f;
    cone.set(0.f, pi);
    life.set(1.f, 1.f);
    scale.set(1.f, 1.f);
  ...
  unsigned getColorCount() const { return color.values.size(); }
  ...
  unsigned getSizeCount() const { return size.values.size(); }
  ...
  unsigned getMatCount() const { return matNames.size(); }
...
struct Particle {
  ...
  Particle(const ParticleFlowDesc *flow, const vec3d &position,
           const vec3d &velocity, const quaterniond &rot, float Scale,
           float Life, float timeAdvance)
  ...
  static Particle *create(const ParticleFlowDesc *flow, const vec3d &position,
                          const vec3d &velocity, const quaterniond &rot,
                          float Scale, float timeAdvance) {
    float life = flow->life.get();
    if (timeAdvance >= life)
      return nullptr;
    return new Particle(flow, position, velocity, rot, Scale, life,
                        timeAdvance);
  }
};
...
struct ParticleSystemDesc {
  ...
  unsigned getFlowCount() const { return (unsigned)flows.size(); }
...
ParticleSystem *playParticleSystem(const ParticleSystemDesc *desc, Node *parent,
                                   const vec3d &pos, const quaterniond &rot,
                                   const vec3d &vel, float scale, float delay) {
  if (desc == 0)
    return 0;
...
ParticleSystem::ParticleSystem(const ParticleSystemDesc *system)
    : age(0.f), delay(0.f), scale(1.f), lastUpdate(frameTime_s) {
  ...
  if (time > 0) {
    ...
    for (size_t i = 0, cnt = flows.size(); i != cnt; ++i) {
      ...
      if (age >= flow->start) {
        if (!flowData.started) {
          if (flow->sound_start) {
            // At the start of a flow, play its start sound (if the flow isn't
            // already over due)
            if (auto *sound = flow->sound_start->play3D(abs_position, false,
                                                        true, false)) {
              int msOffset = (int)(1000.0 * (age - flow->start));
              // Avoid offsetting the sound unless it's at least a few frames
              // off
              if (msOffset > 64)
                sound->setPlayPosition(msOffset);
              double base_dist = abs_scale * (flow->life.max * flow->speed.max +
                                              flow->scale.max);
        ...
        if (overtime < 0.f) {
          ...
          float iPart;
          flowData.progress =
              std::modf(flowData.progress + (flow->rate * time), &iPart);
          make = (unsigned)iPart;
        } else if (flowData.progress > 0.f) {
          // If we had enough time in our dying moment to create a particle, do
          // so Handles cases of very short lived flows that only generate one
          // particle or very few
          float iPart;
        ...
        if (make > 0) {
          ...
          if (time > flow->life.max) {
            ...
            //  Special case for long-duration particle systems that may spend
            //  long periods invisible
          ...
          while (make--) {
            Particle *particle =
                Particle::create(flow, abs_position, vel, totRot, scale, tOff);
            tOff += tStep;
...
void ParticleSystem::render(render::RenderDriver &driver) {
  for (size_t i = 0, cnt = flows.size(); i != cnt; ++i) {
    ...
    if (!flowData.flow->flat) {
      while (particle) {
        devices.render->drawBillboard(
            particle->pos, particle->frame_scale * 2.f, *particle->mat,
            particle->rotation, &particle->frame_color);
...
void saveParticleSystem(scene::ParticleSystemDesc *system,
                        const std::string &filename) {
  // TODO: Check that they aren't hacking
  system->save(filename.c_str());
}

scene::ParticleSystemDesc *
copyParticleSystem(scene::ParticleSystemDesc *system) {
...
void RegisterParticleSystemBinds() {
  ...
  ps.addMethod("ParticleFlow@ createFlow()",
               asMETHOD(scene::ParticleSystemDesc, addFlow));
  ps.addMethod("ParticleFlow@ duplicateFlow(ParticleFlow@ flow)",
               asMETHOD(scene::ParticleSystemDesc, copyFlow));
  ps.addMethod("void removeFlow(uint index)",
               asMETHOD(scene::ParticleSystemDesc, removeFlow));
  ps.addMethod("ParticleFlow@ get_flows(uint index)",
               asMETHOD(scene::ParticleSystemDesc, getFlow));
  ps.addMethod("uint get_flowCount() const",
               asMETHOD(scene::ParticleSystemDesc, getFlowCount));
  ps.addExternMethod("void save(const string& in filename) const",
                     asFUNCTION(saveParticleSystem));
  ps.addExternMethod("ParticleSystem@ duplicate() const",
                     asFUNCTION(copyParticleSystem));

  flow.addExternMethod("string get_soundStart() const",
                       asFUNCTION(flowGetStartSound));
  flow.addExternMethod("void set_soundStart(const string& sfx)",
                       asFUNCTION(flowSetStartSound));

  flow.addMethod("string get_materials(uint index) const",
                 asMETHOD(scene::ParticleFlowDesc, getMatName));
  flow.addMethod("void set_materials(uint index, const string& id)",
                 asMETHOD(scene::ParticleFlowDesc, setMatName));
  flow.addMethod("uint get_materialCount() const",
                 asMETHOD(scene::ParticleFlowDesc, getMatCount));
  flow.addMethod("void addMaterial(const string& id)",
                 asMETHOD(scene::ParticleFlowDesc, addMat));
  flow.addMethod("void removeMaterial(uint index)",
                 asMETHOD(scene::ParticleFlowDesc, removeMat));

  flow.addMethod("uint get_colorCount() const",
                 asMETHOD(scene::ParticleFlowDesc, getColorCount));
  flow.addMethod("Color get_colors(int index) const",
                 asMETHOD(scene::ParticleFlowDesc, getColor));
  flow.addMethod("void set_colors(int index, const Color& col)",
                 asMETHOD(scene::ParticleFlowDesc, setColor));
  flow.addMethod("void removeColor(uint index)",
                 asMETHOD(scene::ParticleFlowDesc, removeColor));
  flow.addMethod("void addColor(uint index, const Color& col) const",
                 asMETHOD(scene::ParticleFlowDesc, addColor));

  flow.addMethod("uint get_sizeCount() const",
                 asMETHOD(scene::ParticleFlowDesc, getSizeCount));
  flow.addMethod("float get_sizes(int i) const",
                 asMETHOD(scene::ParticleFlowDesc, getSize));
  flow.addMethod("void set_sizes(int i, float size)",
                 asMETHOD(scene::ParticleFlowDesc, setSize));
  flow.addMethod("void removeSize(uint index)",
                 asMETHOD(scene::ParticleFlowDesc, removeSize));
  flow.addMethod("void addSize(uint index, float size) const",
                 asMETHOD(scene::ParticleFlowDesc, addSize));

  flow.addMember("float start", offsetof(scene::ParticleFlowDesc, start)) doc(
      "Second offset from when the particle system starts to begin this flow.");
  flow.addMember("float end", offsetof(scene::ParticleFlowDesc, end)) doc(
  ...
  flow.addMember("Range spawnDist",
                 offsetof(scene::ParticleFlowDesc, spawnDist))
  ...
  flow.addMember("bool flat", offsetof(scene::ParticleFlowDesc, flat))
      doc("Whether particles should face the direction they are moving, rather "
          "than the camera.");
}
}; // namespace scripts
