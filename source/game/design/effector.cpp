...
void loadEffectorDefinitions(const std::string &filename) {
  ...
  handler("TargetAlgorithm",
          [&](std::string &value) { eff->def_algorithm = value; });

  handler("Activation",
          [&](std::string &value) { eff->def_activation = value; });

  handler("OnTrigger", [&](std::string &value) { eff->def_onTrigger = value; });

  handler("States", [&](std::string &value) {
    eff->stateCount = toNumber<unsigned>(value);
  });

  handler("CanTarget", [&](std::string &value) { eff->def_canTarget = value; });

  handler("AutoTarget",
          [&](std::string &value) { eff->def_autoTarget = value; });
  ...
  handler("Pierces",
          [&](std::string &value) { eff->pierces = toBool(value, true); });
  ...
  handler("Skin", [&](std::string &value) {
    if (eff->skinNames.find(value) != eff->skinNames.end())
      error("Duplicate skin '%s' in effector '%s'", value.c_str(),
            eff->name.c_str());
  ...
  handler("Inherit", [&](std::string &value) {
    ...
    if (it == eff->skinNames.end()) {
      error("Cannot find skin '%s' in effector '%s' to inherit from.",
            value.c_str(), eff->name.c_str());
      return;
    }
  ...
  handler("GfxLength",
          [&](std::string &value) { skin->length = toNumber<double>(value); });

  handler("Trail", [&](std::string &value) { skin->trailMatID = value; });
  ...
  handler("Color", [&](std::string &value) { skin->color = toColor(value); });

  handler("ImpactGfx", [&](std::string &value) { skin->def_impact = value; });

  handler("Material", [&](std::string &value) { skin->def_material = value; });

  handler("ImpactSfx",
          [&](std::string &value) { skin->def_impact_sound = value; });

  handler("FireSfx",
          [&](std::string &value) { skin->fire_sound_names.push_back(value); });
...
unsigned getEffectorDefinitionCount() { return (unsigned)effIds.size(); }
...
void bindEffectorHooks(bool shadow) {
  foreach (it, effDefs) {
    ...
    if (!def.def_activation.empty()) {
      ...
      if (f != EffectorActivation.end()) {
        ...
        if (arguments.size() != f->second.argCount) {
          ...
          error("Incorrect argument count for '%s' (got %i, expected %u)",
                func.c_str(), arguments.size(), f->second.argCount);
        }
      } else if (!shadow) {
        std::string decl;
        ...
        def.activation.script =
            devices.scripts.server->getFunction(func, decl.c_str(), "bool");
      }
    ...
    if (!def.def_canTarget.empty())
      def.canTarget =
          Formula::fromInfix(def.def_canTarget.c_str(), &targetFormulaVar);

    weightList = &def.autoTargetWeighters;
    if (!def.def_autoTarget.empty())
      def.autoTarget =
          Formula::fromInfix(def.def_autoTarget.c_str(), &targetFormulaVar);
  }
}
...
EffectorSkin::EffectorSkin()
    : graphicType(EGT_Line), graphicSize(1.0), length(1.0), material(0),
      impact(0), impact_sound(0), fire_pitch_variance(0.f), trailMat(0),
      trailStart((unsigned)0xffff0000), trailEnd((unsigned)0) {}

EffectorDef::EffectorDef()
    : range(0), lifetime(0), tracking(0), speed(0), spread(0), capTarget(0),
      fireArc(0), targetTolerance(0), fireTolerance(0), valueCount(0),
      stateCount(0), physicalType(EPT_Instant), physicalSize(1.0),
      efficiencyMode(EEM_Normal), onTrigger(0), canTarget(0), autoTarget(0),
      physicalImpact(true), passthroughInvalid(false), skins(1), pierces(false),
      recoverTime(1.f), effect(0) {
  skinNames["Default"] = 0;
}

Effector::Effector(const EffectorDef &def)
    : inDesign(0), subsysIndex(0), effectorIndex(0), effectorId(0),
      skinIndex(0), type(def), effect(def.effect), range(1000.0), lifetime(6.0),
      tracking(0.5), speed(50.0), spread(0.0), capTarget(1), fireArc(twopi),
      fireTolerance(twopi), targetTolerance(0.0), relativeSize(1.0),
      enabled(true), refs(1) {
  values = new double[def.valueCount + def.arguments.size() +
                      def.triggerArguments.size()];
}
...
void Effector::initValues() {
  ...
  for (unsigned i = 0; i < type.arguments.size(); ++i)
    values[type.valueCount + i] =
        type.arguments[i]->evaluate(effVariable, this);
  for (unsigned i = 0; i < type.triggerArguments.size(); ++i)
    values[type.valueCount + type.arguments.size() + i] =
        type.triggerArguments[i]->evaluate(effVariable, this);
  ...
  for (unsigned i = 0; i < type.effectValues.size(); ++i) {
    if (type.effectValues[i])
      ...
      effect.values[i] =
          type.effect->values[i].defaultValue->evaluate(effVariable, this);
  }
}
...
double Effector::getTargetWeight(Object *obj, Object *target) const {
  ...
  if (type.canTarget) {
    double c = type.canTarget->evaluate(&targetFormulaName, &dat,
                                        &targetFormulaWeight);
    if (c <= 0.0)
      return 0.0;
  }
  ...
  return type.autoTarget->evaluate(&targetFormulaName, &dat,
                                   &targetFormulaWeight);
}
...
bool Effector::canTarget(Object *obj, Object *target) const {
  ...
  double w =
      type.canTarget->evaluate(&targetFormulaName, &dat, &targetFormulaWeight);
  return w > 0.0;
}
...
bool Effector::autoTarget(Object *obj, Object *target) const {
  ...
  double w =
      type.autoTarget->evaluate(&targetFormulaName, &dat, &targetFormulaWeight);
  return w > 0.0;
}
...
struct BatchedMissile : public scene::NodeEvent {
  ...
  BatchedMissile(const render::RenderState &Mat,
                 const render::RenderState &Trail)
      : NodeEvent(0), mat(Mat), trail(Trail) {}
...
vec3d coneSpread(const vec3d &from, double angle) {
  ...
  vec3d perp =
      (perpRight * cos(perpAngle) + perpUp * sin(perpAngle)).normalized();
...
void EffectorDef::triggerGraphics(Object *obj, EffectorTarget &targ,
                                  const Effector *effector, double *pTime,
                                  vec2d *pDirection, float efficiency,
  ...
  bool eventVisible =
      player && (obj->isVisibleTo(player) || target->isVisibleTo(player));
  double size = 1.0;
  ...
  case EPT_Missile: {
    vec3d turretOffset =
        obj->rotation * (effector->relativePosition * obj->radius);
    ...
    if (physicalType == EPT_AimedMissile)
      proj->velocity =
          (target->position - proj->position).normalized(effector->speed);
    else if (effector->spread > 0)
      proj->velocity =
          coneSpread(trackPos.normalized(), effector->spread) * effector->speed;
    else
  ...
  case EPT_Projectile: {
    ...
    double coeffs[5] = {
        p.dot(p), 2.0 * p.dot(v),
        (v.dot(v) + p.dot(a) - effector->speed * effector->speed), v.dot(a),
        0.25 * a.dot(a)};
    double roots[4];
    // TODO: If we solve for t > effector->lifetime, we know that we can't
    // really hit the target 		Should we do something about that?
    double t = effector->lifetime;
    int rootCount = 0;

    // Very small coefficients on the upper values yield incorrect results, and
    // are unnecessary to evaluate
    ...
    vec3d velocity =
        coneSpread(offset.normalize(), effector->spread) * effector->speed;
    ...
    velocity =
        rotate * (obj->rotation * targ.tracking.normalized(effector->speed));
    ...
    if (eventVisible) {
      if (skin.graphicType != EGT_Sprite) {
        ...
        batched =
            new BatchedProjectile(isLine ? *skin.trailMat : *skin.material);
        ...
        proj.length = (float)(effector->speed / 60.0) * skin.graphicSize *
                      size * skin.length;
        proj.life = (float)effector->lifetime;
        proj.fadeStart =
            (float)(effector->lifetime - std::min(effector->lifetime * 0.8, t));
  ...
  case EPT_Beam: {
    ...
    scene::Node *node = 0;
    if (eventVisible) {
      node = new scene::BeamNode(
          skin.material, (float)(skin.graphicSize * size), start, vec3d());
    ...
    if (effector->spread > 0)
      proj->velocity =
          coneSpread(proj->velocity.normalized(), effector->spread) *
          proj->velocity.getLength();
    proj->source = obj;
    obj->grab();
    ...
    if (node) {
      ...
      node->position = vec3d();
      node->animator =
          new scene::BeamAnim(obj->node, start, (float)effector->range);
      node->queueReparent(devices.scene);
    }
  ...
  if (devices.network->isServer)
    devices.network->sendEffectorTrigger(
        this, effector, obj, targ, devices.driver->getGameTime() + tOffset);

  size *= skin.graphicSize;

  if (eventVisible && !skin.fire_sounds.empty()) {
    auto *fire_sound =
        skin.fire_sounds[randomi(0, (int)skin.fire_sounds.size() - 1)];
    if (fire_sound && fire_sound->loaded && !audio::disableSFX) {
      auto *sound = devices.sound->play3D(fire_sound->source,
                                          snd_vec(obj->position), false, true);
      if (sound) {
        sound->setVolume((float)size);
        if (skin.fire_pitch_variance > 0)
          sound->setPitch((float)randomd(1.0 - skin.fire_pitch_variance,
...
void Effector::trigger(Object *obj, EffectorTarget &target, float efficiency,
                       double tOffset) const {
  if (type.onTrigger && !devices.network->isClient) {
    ...
    for (unsigned i = 0, cnt = (unsigned)type.triggerArguments.size(); i < cnt;
         ++i)
  ...
  type.triggerGraphics(obj, target, this, &time, &direction, efficiency,
                       tOffset);
}

void Effector::triggerEffect(Object *obj, Object *target,
                             const vec3d &impactOffset, float efficiency,
                             float partiality, double delay) const {
  if (target->isValid()) {
    ...
    // vec3d localOffset = (target->rotation.inverted() * (target->position -
    // obj->position));
...
bool trackTo(Object *obj, const Effector &eff, double time,
             EffectorTarget &targ, vec3d &absTargDir, bool isAbs) {
  vec3d targDir = absTargDir;
  vec3d curDir = targ.tracking.normalized();

  // Put the target direction into object rotated space so we can do proper
  // relative tracking
  if (isAbs)
...
void Effector::update(Object *obj, double time, double *states,
                      EffectorTarget &targ, float efficiency,
                      bool holdFire) const {
  ...
  Object *&target = targ.target;
  if (target &&
      (!target->isValid() || !target->owner || !target->owner->valid())) {
  ...
  do {
    ...
    if (type.activation.isNative) {
      if (type.activation.native)
        activate = type.activation.native(this, obj, targ, took,
                                          values + type.valueCount, states);
    } else {
...
void Effector::setRelativePosition(vec2u hex, const HullDef *hull,
                                   vec3d direction) {
  // Set the relative source position from a hex position
  vec2d center =
      vec2d((double)hull->gridSize.x * 0.75, (double)hull->gridSize.y) / 2.0;
...
void Effector::load(SaveFile &file) {
  ...
  file.read(values, sizeof(double) *
                        (type.valueCount + (unsigned)type.arguments.size() +
                         (unsigned)type.triggerArguments.size()));
  if (effect.type)
...
void Effector::save(SaveFile &file) const {
  ...
  file << targetTolerance << spread;
  file << relativeSize << enabled;
  file.write(values, sizeof(double) *
                         (type.valueCount + (unsigned)type.arguments.size() +
                          (unsigned)type.triggerArguments.size()));
  if (effect.type)
...
const Effector *Effector::receiveUpdate(net::Message &msg) {
  if (msg.readBit()) {
    ...
    msg.readSmallVec3(eff->relativePosition.x, eff->relativePosition.y,
                      eff->relativePosition.z);
    msg.readDirection(eff->turretAngle.x, eff->turretAngle.y,
                      eff->turretAngle.z);
    ...
    {
      unsigned valueCount = eff->type.valueCount +
                            (unsigned)eff->type.arguments.size() +
                            (unsigned)eff->type.triggerArguments.size();
      unsigned effValueCount = eff->effect.type->valueCount;
      float *floatValues =
          (float *)alloca(sizeof(float) * (valueCount + effValueCount));
...
void Effector::sendUpdate(net::Message &msg) const {
  ...
  msg.writeSmallVec3(relativePosition.x, relativePosition.y,
                     relativePosition.z);
  ...
  {
    unsigned valueCount = type.valueCount + (unsigned)type.arguments.size() +
                          (unsigned)type.triggerArguments.size();
...
Effector::Effector(net::Message &msg)
    : inDesign(0), subsysIndex(0), effectorIndex(0), effectorId(0),
      type(*getEffectorDefinition(msg.readSmall())), effect(), range(1000.0),
      lifetime(6.0), tracking(0.5), speed(50.0), spread(0.03), capTarget(1),
      fireArc(twopi), fireTolerance(twopi), targetTolerance(0.0),
      relativeSize(1.0), enabled(true), refs(1) {
  values = new double[type.valueCount + (unsigned)type.arguments.size() +
                      (unsigned)type.triggerArguments.size()];
