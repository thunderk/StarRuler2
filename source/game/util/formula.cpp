...
// Function callable by formulas, receives a pointer to the stack and the number
...
typedef double compiledCFormula(double *, double *, const std::string *,
                                varInterpreter, void *, varIndexInterpreter);

const unsigned FormulaStackSize = 64;

double nullConverter(void *, const std::string *) { return 0.0; }
...
class CFormula : public Formula {
  ...
  static void convertInfix(const char *expression, std::list<Token> &rpn,
                           varIndexConverter conv);

public:
  double evaluate(varInterpreter VarConverter, void *user,
                  varIndexInterpreter conv) {
#ifdef COMPILE_FORMULAS
    if (formula) {
  ...
  static void buildCaches(const std::list<Token> &tokens, double **constants,
                          const std::string **names);

#ifdef COMPILE_FORMULAS
  void buildExecutable(std::list<Token> &tokens);

  CFormula(const char *expression, varIndexConverter conv)
      : page(0), formula(0), varIndex(-1) {
    std::list<Token> rpn;
#else
  double evaluateRPN(double *stack, varInterpreter VarConverter, void *user,
                     varIndexInterpreter conv);
...
Formula *Formula::fromInfix(const char *expression,
                            varIndexConverter VarConverter, bool catchErrors) {
  if (expression[0] == '\0')
    return 0;
  if (catchErrors) {
    try {
...
void CFormula::buildExecutable(std::list<Token> &rpn) {
  ...
  Register eax(cpu, EAX), ebx(cpu, EBX), ecx(cpu, ECX), edx(cpu, EDX),
      esp(cpu, ESP);
  ...
  while (!rpn.empty()) {
    ...
    case TT_Operator:
      ...
      case OT_Pow: {
        ...
        cpu.call_cdecl((void *)std::get<0>(formulaFunctions["pow"]), "rc",
                       &arg0, 2);
#else
        eax.copy_address(*stack + off * 8);
        cpu.call_cdecl((void *)std::get<0>(formulaFunctions["pow"]), "rc", &eax,
                       2);
#endif
      ...
      case OT_Func:
        ...
        Register arg0 = as<void *>(cpu.intArg64(0, 0));
        arg0.copy_address(*stack + off * 8);
        cpu.call_cdecl((void *)std::get<0>(formulaFunctions[token.name]), "rc",
                       &arg0, token.args);
#else
        eax.copy_address(*stack + off * 8);
        cpu.call_cdecl((void *)std::get<0>(formulaFunctions[token.name]), "rc",
...
#else
double CFormula::evaluateRPN(double *stack, varInterpreter VarConverter,
                             void *user, varIndexInterpreter conv) {
  double *nextConstant = constants;
  auto nextName = names;
...
void CFormula::buildCaches(const std::list<Token> &tokens, double **constants,
                           const std::string **names) {
...
void CFormula::optimizeRPN(std::list<Token> &rpn) {
  ...
  while (index + 2 < cnt) {
    ...
    switch (o->op) {
    case OT_Or:
      c1->value = c1->value > c2->value ? c1->value : c2->value;
      break;
    case OT_And:
      c1->value = c1->value < c2->value ? c1->value : c2->value;
      break;
    case OT_Less:
      c1->value = c1->value < c2->value ? 1.0 : 0.0;
      break;
    case OT_LessEquals:
      c1->value = c1->value <= c2->value ? 1.0 : 0.0;
      break;
    ...
    case OT_Div:
      ...
      break;
    case OT_Pow:
      c1->value = pow(c1->value, c2->value);
      break;
    default:
  ...
  while (index + 1 < cnt) {
    ...
    auto o = c1;
    ++o;
    if (c1->type != TT_Constant || o->type != TT_Operator || o->op != OT_Div) {
      ++i;
      ++index;
      continue;
    }
...
void CFormula::convertInfix(const char *expression, std::list<Token> &rpn,
                            varIndexConverter conv) {
...

  {
    ...
    std::list<Token> tokens;
    {
      ...
      auto push = [&](TokenType newType) {
        ...
        case TT_Operator:
          ...
          while (!tokenText.empty()) {
            ...
            // Parse progressively smaller chunks to find operators, error when
        ...
        case TT_Bracket:
          ...
          if (token.open && !tokens.empty() && tokens.back() == TT_Variable) {
            ...
            if (formulaFunctions.find(func.name) == formulaFunctions.end())
              throw FormulaError(std::string("Invalid function name: ") +
                                 func.name);
      ...
      while (char c = *expression) {
        ...
        // Push tokens when the type has changed, or for each character for
        // specific types
    ...
    while (!tokens.empty()) {
      ...
      case TT_Bracket:
        if (token.open) {
          // Setup the function's arg count based on the presence/abscense of an
          // immediate ')'
          if (!opStack.empty()) {
            auto &func = opStack.back();
            if (func == TT_Operator && func.op == OT_Func)
              func.args =
                  !tokens.empty() && (tokens.front().type != TT_Bracket ||
                                      tokens.front().open)
                      ? 1
                      : 0;
          }
          ...
          while (!opStack.empty()) {
            ...
            if (op == TT_Bracket) {
              ...
              if (!opStack.empty() && opStack.back() == TT_Operator &&
                  opStack.back().op == OT_Func) {
                Token func = opStack.back();
                opStack.pop_back();
                rpn.push_back(func);

                unsigned expectedArgs =
                    std::get<1>(formulaFunctions[func.name]);
                if (func.args < expectedArgs)
                  throw FormulaError(
                      std::string("Too few arguments to " + func.name));
                else if (func.args > expectedArgs)
                  throw FormulaError(
                      std::string("Too many arguments to " + func.name));
              }
              break;
            }
            rpn.push_back(op);
...
double f_abs(double *stack, unsigned args) { return fabs(stack[-1]); }

double f_ceil(double *stack, unsigned args) { return ceil(stack[-1]); }

double f_floor(double *stack, unsigned args) { return floor(stack[-1]); }

double f_round(double *stack, unsigned args) { return floor(stack[-1] + 0.5); }

double f_sqrt(double *stack, unsigned args) { return sqrt(stack[-1]); }

double f_log10(double *stack, unsigned args) { return log10(stack[-1]); }

double f_log(double *stack, unsigned args) { return log(stack[-1]); }

double f_exp(double *stack, unsigned args) { return exp(stack[-1]); }
...
double f_pow(double *stack, unsigned args) { return pow(stack[-2], stack[-1]); }
