...
void bindInputScripts(GameState state, scripts::Manager *Manager) {
  ...
  for (unsigned i = 0; i < IC_COUNT; ++i)
    inputCalls[state][i] =
        manager[state]->getFunction("input", InputCallDecls[i]);
}
...
void clearPressedKeys() { pressedKeys.clear(); }
...
bool onMouseButton(int button, int pressed) {
  // When the mouse is being dragged at the system level, don't pass it to the
  // GUI
  if (dragging) {
    if (pressed) {
      dragging |= (0x1 << button);
    ...
    if (!dragging) {
      ...
      if (tot_drag > 4) {
        ...
        if (manager[game_state] && inputCalls[game_state][IC_MouseDragEnd]) {
          auto call = manager[game_state]->call(
              inputCalls[game_state][IC_MouseDragEnd]);
          call.push(button);
  ...
  if (manager[game_state]) {
    ...
    if (inputCalls[game_state][IC_MouseButton]) {
      auto call =
          manager[game_state]->call(inputCalls[game_state][IC_MouseButton]);
      call.push(button);
      call.push(pressed == 1);
    ...
    if (!pressed) {
      ...
      if (mouseButtons[button].pressed_time > time - dblClickTimeout) {
        ...
        if (inputCalls[game_state][IC_DoubleClick]) {
          auto call =
              manager[game_state]->call(inputCalls[game_state][IC_DoubleClick]);
          call.push(button);
          call.call();
        }
      } else {
        ...
        if (inputCalls[game_state][IC_MouseClick]) {
          auto call =
              manager[game_state]->call(inputCalls[game_state][IC_MouseClick]);
          call.push(button);
          call.call();
        }
      }
...
bool onMouseMoved(int x, int y) {
  ...
  if (dragging) {
    ...
    if (manager[game_state] && inputCalls[game_state][IC_MouseDrag]) {
      ...
      if (dx || dy) {
        auto call =
            manager[game_state]->call(inputCalls[game_state][IC_MouseDrag]);
        call.push(dragging);
...
bool onMouseWheel(double x, double y) {
  if (manager[game_state] && inputCalls[game_state][IC_MouseWheel]) {
    auto call =
        manager[game_state]->call(inputCalls[game_state][IC_MouseWheel]);
    call.push(x);
    call.push(y);
    call.call();
  }
