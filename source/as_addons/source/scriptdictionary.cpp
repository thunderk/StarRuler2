...
CScriptDictionary::CScriptDictionary(asIScriptEngine *engine) {
  ...
  engine->NotifyGarbageCollectorOfNewObject(
      this, engine->GetTypeInfoByName("dictionary"));
}
...
int CScriptDictionary::GetRefCount() { return refCount.get(); }
...
bool CScriptDictionary::Iterator_GetValue(
    CScriptDictionary::mapType::const_iterator &it, void *value,
    int typeId) const {
  // Return the value
  if (typeId & asTYPEID_OBJHANDLE) {
...
CScriptArray *CScriptDictionary::GetKeys() const {
  ...
  //                 It should be recomputed when registering the dictionary
  //                 class. Only problem is if multiple engines are used, as
  //                 they may not share the same type id. Alternatively it can
  //                 be stored in the user data for the dictionary type.
...
CScriptDictionary::Iterator::Iterator() { container = 0; }
...
static void ConstructIterator(void *memory,
...
bool CScriptDictionary::Iterator::Iterate(string *key, void *value,
                                          int typeId) {
  if (!container) {
    asIScriptContext *ctx = asGetActiveContext();
    if (ctx)
...
bool CScriptDictionary::Iterator::IsValid() {
  return container && modCount == container->modCount &&
         it != container->dict.end();
}

CScriptDictionary::Iterator &CScriptDictionary::Iterator::operator=(
    const CScriptDictionary::Iterator &other) {
  if (other.container)
...
void ScriptDictionaryFactory_Generic(asIScriptGeneric *gen) {
  *(CScriptDictionary **)gen->GetAddressOfReturnLocation() =
      new CScriptDictionary(gen->GetEngine());
}
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine) {
  ...
  r = engine->RegisterObjectType("dictionary_iterator",
                                 sizeof(CScriptDictionary::Iterator),
  ...
  r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ADDREF, "void f()",
                                      asMETHOD(CScriptDictionary, AddRef),
                                      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
  ...
  r = engine->RegisterObjectMethod(
      ...
      asMETHODPR(CScriptDictionary, Set, (const string &, void *, int), void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary", "bool get(const string &in, ?&out) const",
      asMETHODPR(CScriptDictionary, Get, (const string &, void *, int) const,
                 bool),
      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod(
      "dictionary", "void set(const string &in, int64&in)",
      asMETHODPR(CScriptDictionary, Set, (const string &, asINT64 &), void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary", "bool get(const string &in, int64&out) const",
      asMETHODPR(CScriptDictionary, Get, (const string &, asINT64 &) const,
                 bool),
      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod(
      "dictionary", "void set(const string &in, double&in)",
      asMETHODPR(CScriptDictionary, Set, (const string &, double &), void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary", "bool get(const string &in, double&out) const",
      asMETHODPR(CScriptDictionary, Get, (const string &, double &) const,
                 bool),
      asCALL_THISCALL);
  assert(r >= 0);
  ...
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary", "void delete(const string &in)",
      asMETHODPR(CScriptDictionary, Delete, (const string &), void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary", "void delete(dictionary_iterator&)",
      asMETHODPR(CScriptDictionary, Delete, (CScriptDictionary::Iterator &),
                 void),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("dictionary", "void deleteAll()",
                                   ...
                                   asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod("dictionary", "void deleteNulls()",
                                   asMETHOD(CScriptDictionary, DeleteNulls),
                                   asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod(
      "dictionary", "dictionary_iterator iterator()",
      asMETHOD(CScriptDictionary, GetIterator), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
  ...
  r = engine->RegisterObjectMethod("dictionary", "void erase(const string &in)",
                                   asMETHOD(CScriptDictionary, Delete),
                                   asCALL_THISCALL);
  assert(r >= 0);
  // Same as deleteAll
  ...
  r = engine->RegisterObjectBehaviour(
      "dictionary_iterator", asBEHAVE_CONSTRUCT, "void f()",
      asFUNCTION(DefaultConstructIterator), asCALL_CDECL_OBJFIRST);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour(
      "dictionary_iterator", asBEHAVE_CONSTRUCT, "void f(const dictionary &in)",
      asFUNCTION(ConstructIterator), asCALL_CDECL_OBJFIRST);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("dictionary_iterator", asBEHAVE_DESTRUCT,
                                      "void f()", asFUNCTION(DestructIterator),
                                      asCALL_CDECL_OBJFIRST);
  assert(r >= 0);

  r = engine->RegisterObjectMethod(
      "dictionary_iterator", "const string& get_key()",
      asMETHOD(CScriptDictionary::Iterator, GetKey), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary_iterator", "bool iterate(?&out)",
      asMETHODPR(CScriptDictionary::Iterator, Iterate, (void *, int), bool),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary_iterator", "bool iterate(string &out, ?&out)",
      asMETHODPR(CScriptDictionary::Iterator, Iterate, (string *, void *, int),
                 bool),
      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary_iterator", "bool get_valid()",
      asMETHOD(CScriptDictionary::Iterator, IsValid), asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectMethod(
      "dictionary_iterator",
      "dictionary_iterator& opAssign(const dictionary_iterator &in)",
      asMETHOD(CScriptDictionary::Iterator, operator=), asCALL_THISCALL);
  assert(r >= 0);
}
...
void RegisterScriptDictionary_Generic(asIScriptEngine *engine) {
  ...
  r = engine->RegisterObjectMethod(
      "dictionary", "void set(const string &in, ?&in)",
