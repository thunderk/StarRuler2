...
void preProcessShader(std::string &shader) {
  ...
  while (start < shader.size()) {
    ...
    if (shader.size() - pos > 3 && shader[pos + 1] == '{' &&
        shader[pos + 2] == '{') {
      ...
      start = endpos + 2;
    } else if (shader.size() - pos > 11 &&
               shader.compare(pos, 10, "#include \"") == 0) {
      ...
      if (!fileExists(fname)) {
        error("ERROR COMPILING SHADER: Could not find include file %s. "
              "(Resolved to %s)",
              value.c_str(), fname.c_str());
      } else {
...
public:
  ...
  GLuint compileShader(GLenum type, const std::string &fname,
    ...
    if (!shader_ok) {
      ...
      // We're always provided at least an empty string, just ignore very small
      // logs
      if (log_length > 8) {
        char *log;
        log = (char *)malloc(log_length);
  ...
  int compile() override {
    ...
    // Compile fragment shader
    {
      ...
      preProcessShader(contents);
      fragment_shader =
          compileShader(GL_FRAGMENT_SHADER, fragment_file, contents);
      if (!fragment_shader) {
        if (contents.empty())
...
public:
  ...
  virtual int compile() {
    ...
    for (unsigned i = 0; i < uniform_names.size(); ++i) {
      GLuint location =
          glGetUniformLocation(glProgram->program, uniform_names[i].c_str());

      if (location == (GLuint)-1) {
        // NOTE: This is no longer considered an error, because dynamic shader
        // levels are going to be causing uniforms to be missing all the time.
        //
    ...
    for (unsigned i = 0; i < uniform_locations.size(); ++i) {
      ...
      size *= v.count;
      uniform_cache.push_back(
          glProgram->cacheUniform(uniform_locations[i], size));
    }
  ...
  virtual void bind(float *dynamic) const {
    ...
    const bool bindConst =
        lastBoundFrame != frameNumber || glProgram->lastShader != this;
  ...
  void updateDynamicVars() const {
    for (size_t i = 0, cnt = uniform_locations.size(); i < cnt; ++i) {
      ...
      GLuint uniform = uniform_locations[i];
      void *uniformMem =
          (void *)&((GLShaderProgram *)program)->mem_buffer[uniform_cache[i]];
...
Shader *createGLShader() { return new GLShader(); }
