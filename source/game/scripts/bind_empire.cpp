...
static unsigned getDesignCount(Empire *emp) { return emp->designIds.size(); }

static unsigned getDesignClassCount(Empire *emp) {
  return emp->designClassIds.size();
}

static Empire *empFactory() { return new Empire(); }

static Empire *empFactory_b(bool unlisted) {
  return new Empire(unlisted ? INVALID_EMPIRE : UNLISTED_EMPIRE);
}

static void writeDelta(Empire *emp, net::Message &msg) { emp->writeDelta(msg); }
...
static bool ssModUnlocked(Empire *emp, const SubsystemDef *def,
...
static void ssSetModUnlock(Empire *emp, const SubsystemDef *def,
                           const SubsystemDef::ModuleDesc *mod, bool val) {
...
static void addModifier_d(asIScriptGeneric *f) {
  ...
  if ((int)stage->argumentNames.size() != f->GetArgCount() - 2) {
    scripts::throwException(format("Error: Subsystem modifier '$1.$2'"
...
struct StatReader {
  ...
  StatReader(const StatHistory *History, Empire *Emp, unsigned ID)
      : history(History), entry(0), evt(0), emp(Emp), id(ID) {}
  ~StatReader() {
    if (emp)
  ...
  static void destroy(StatReader *reader) { delete reader; }
};
...
void RegisterEmpireBinds(bool declarations, bool server) {
  ...
  if (server) {
    ...
    emp.addExternMethod("bool isHostile(Empire@ emp)",
                        asFUNCTION(empIsHostile));
    emp.addExternMethod("void setHostile(Empire& emp, bool value)",
                        asFUNCTION(empSetHostile));
    emp.addMethod("void cacheVision()", asMETHOD(Empire, cacheVision));
  } else {
    emp.addExternMethod("bool isHostile(Empire@ emp)",
                        asFUNCTION(empIsHostile_restricted));
    emp.addExternMethod("bool get_controlled() const",
                        asFUNCTION(empControlled));
  ...
  classdoc(stats, "Accesses the history of an empire's stat. Locks the stat, "
                  "so limit access to these histories.");

  stats.addFactory("StatHistory@ f(Empire&, stat::EmpireStat)",
                   asFUNCTION(getStatHistory));
  stats.addExternBehaviour(asBEHAVE_RELEASE, "void f()",
                           asFUNCTION(StatReader::destroy));

  stats.addMethod("bool toTail()", asMETHOD(StatReader, toTail))
      doc("Advances to the most recent stat entry.",
          "True if there was a tail to move to.");
  stats.addMethod("bool toHead()", asMETHOD(StatReader, toHead))
      doc("Advances to the earliest stat entry.",
          "True if there was a head to move to.");

  stats.addMethod("bool advance(int offset = 1)", asMETHOD(StatReader, advance))
      doc("Advances to another stat entry.",
          "Number of entries to move. Positive is later in time, negative is "
          "earlier. Advancing 0 resets the event reader. If not on a valid "
          "entry, advances to a valid entry at the correspending head or "
          "tail. ",
  ...
  stats.addMethod("bool advanceEvent()", asMETHOD(StatReader, advanceEvent))
      doc("Advances to the next event for the current stat.",
          "True if there was an event to advance to.");
  stats.addMethod("uint get_eventType() const",
                  asMETHOD(StatReader, get_evt_type))
      doc("Returns the current event's type identifier.", "");
  stats.addMethod("string get_eventName() const",
                  asMETHOD(StatReader, get_evt_name))
      doc("Returns the current event's name.", "");

  if (server) {
    emp.addMethod("void recordStat(stat::EmpireStat, int)",
                  asMETHODPR(Empire, recordStat, (unsigned, int), void))
        doc("Records an int value for the specified empire stat at the current "
            "game time",
            "Stat index.", "Stat value.");
    emp.addMethod("void recordStat(stat::EmpireStat, float)",
                  asMETHODPR(Empire, recordStat, (unsigned, float), void))
        doc("Records a float value for the specified empire stat at the "
            "current game time",
            "Stat index.", "Stat value.");
    emp.addMethod("void recordStatDelta(stat::EmpireStat, int)",
                  asMETHODPR(Empire, recordStatDelta, (unsigned, int), void))
        doc("Records an int delta for the specified empire stat at the current "
            "game time",
            "Stat index.", "Stat delta.");
    emp.addMethod("void recordStatDelta(stat::EmpireStat, float)",
                  asMETHODPR(Empire, recordStatDelta, (unsigned, float), void))
        doc("Records a float delta for the specified empire stat at the "
            "current game time",
            "Stat index.", "Stat delta.");
    emp.addMethod(
        "void recordEvent(stat::EmpireStat, uint16, const string &in)",
        asMETHOD(Empire, recordEvent))
        doc("Records an event for the specified empire stat at the current "
            "game time",
            "Stat index.", "Event type.", "Event name.");
  }

  // Designs
  emp.addMember("ReadWriteMutex designMutex", offsetof(Empire, designMutex));
  emp.addExternMethod("uint get_designCount() const",
                      asFUNCTION(getDesignCount));
  emp.addExternMethod("const Design@ getDesign(const string &in)",
                      asFUNCTION(getDesignByName));
  emp.addExternMethod("const Design@ getDesign(int id)",
                      asFUNCTION(getDesignById));
  emp.addExternMethod("const Design@ get_designs(int)",
                      asFUNCTION(getDesignById));

  emp.addExternMethod("uint get_designClassCount() const",
                      asFUNCTION(getDesignClassCount));
  emp.addMethod("const DesignClass@ getDesignClass(int id)",
                asMETHODPR(Empire, getDesignClass, (int), DesignClass *));
  emp.addMethod(
      "const DesignClass@ getDesignClass(const string &in, bool add = true)",
      asMETHODPR(Empire, getDesignClass, (const std::string &, bool),
                 DesignClass *));
  emp.addMethod("bool addDesign(const DesignClass& cls, const Design&)",
                asMETHOD(Empire, addDesign));
  emp.addMethod("bool changeDesign(const Design&, const Design&, const "
                "DesignClass@ cls = null)",
                asMETHODPR(Empire, changeDesign,
                           (const Design *, const Design *, DesignClass *),
                           bool));
  emp.addMethod(
      "const Design@+ updateDesign(const Design&, bool onlyOutdated = false)",
      asMETHODPR(Empire, updateDesign, (const Design *, bool), const Design *));
  emp.addMethod("void flagDesignOld(const Design&)",
                asMETHODPR(Empire, flagDesignOld, (const Design *), void))
      doc("Marks a design as out of date, to be updated whenever it is "
  ...
  // Subsystem modifiers
  emp.addMember("ReadWriteMutex subsystemDataMutex",
                offsetof(Empire, subsystemDataMutex));
  emp.addExternMethod(
      "bool isUnlocked(const SubsystemDef& sys, const ModuleDef& mod)",
      asFUNCTION(ssModUnlocked));
  emp.addExternMethod("bool isUnlocked(const SubsystemDef& sys)",
                      asFUNCTION(ssUnlocked));
  emp.addExternMethod("void setUnlocked(const SubsystemDef& sys, const "
                      "ModuleDef& mod, bool unlocked)",
                      asFUNCTION(ssSetModUnlock));
  emp.addExternMethod(
      "void setUnlocked(const SubsystemDef& sys, bool unlocked)",
      asFUNCTION(ssSetUnlock));
  emp.addExternMethod("void removeModifier(const SubsystemDef& sys, uint id)",
                      asFUNCTION(removeModifier));
  ...
  for (unsigned i = 0; i <= MODIFY_STAGE_MAXARGS; ++i) {
    emp.addGenericMethod(
        format("uint addModifier(const SubsystemDef& def, const string&in$1)",
               dargs)
            .c_str(),
