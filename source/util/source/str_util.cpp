...
// Returns 0 if the strings are equal, the the difference of the first unequal
// character otherwise
...
bool streq_nocase(const std::string &str, const std::string &substr,
...
int strfind_nocase(const std::string &str, const std::string &substr,
                   unsigned start) {
...
bool splitKeyValue(const std::string &input, std::string &key,
                   std::string &value, const char *split) {
...
DataReader::DataReader()
    : allowLines(true), fullLine(false), allowMultiline(true), indent(0),
      inMultiline(false), squash(false), skipComments(true), skipEmpty(true) {}

DataReader::DataReader(const std::string &filename, bool AllowLines)
    : allowLines(AllowLines), fullLine(false), allowMultiline(true), indent(0),
      inMultiline(false), squash(false), skipComments(true), skipEmpty(true) {
...
DataHandler::BlockHandler &
DataHandler::BlockHandler::block(const std::string &name) {
...
void DataHandler::BlockHandler::openBlock(
...
void DataHandler::BlockHandler::lineHandler(
    std::function<void(std::string &)> cb) {
  lineHandlerCB = cb;
}

void DataHandler::BlockHandler::defaultHandler(
    std::function<void(std::string &, std::string &)> cb) {
  defaultHandlerCB = cb;
}

void DataHandler::BlockHandler::operator()(
...
void DataHandler::defaultHandler(
    std::function<void(std::string &, std::string &)> cb) {
  defaultBlock.defaultHandler(cb);
}

void DataHandler::operator()(const std::string &name,
                             std::function<void(std::string &)> cb) {
      ...
      if (bit != curHandler->blocks.end()) {
        ...
        if (handler->openHandler)
          if (!handler->openHandler(datafile.value))
            return;
        blockStack.push(
            std::pair<BlockHandler *, int>(handler, datafile.indent));
...
void split(const std::string &input, std::vector<std::string> &out,
           char delimit, bool doTrim, bool listEmpty) {
  size_t start = 0;
  size_t size = 0;
...
// Generic string split
void split(const std::string &input, std::vector<std::string> &out,
           const char *delimit, bool doTrim, bool listEmpty) {
  size_t start = 0;
  size_t size = 0;
...
bool split(const std::string &input, std::string &front, char delim_front,
           std::string &inner, char delim_back, std::string *back) {
...
bool funcSplit(const std::string &input, std::string &name,
               std::vector<std::string> &arguments, bool strip) {
...
std::string &replace(std::string &str, const std::string &replace,
                     const std::string &with) {
  str = std::move(replaced(str, replace, with));
  return str;
}

std::string replaced(const std::string &input, const std::string &replace,
                     const std::string &with) {
  std::string out;
  size_t at = 0, found = 0;
  while (at < input.size()) {
...
std::string &paragraphize(std::string &input, const std::string &parSep,
                          const std::string &lineSep, bool startsParagraph) {
  std::string out;
  size_t at = 0, found = 0;
  if (startsParagraph) {
...
template <> std::string toString<Color>(Color color, unsigned precision) {
  std::string out = "#";
...
// Read color from string in the following formats: (Alpha is optional and
// assumed to 255 if missing) RGBA RRGGBBAA RR GG BB AA
#define hexNibble(x)                                                           \
  ((x - '0') < 10 ? (x - '0')                                                  \
...
bool toBool(const std::string &str, bool def) {
  static const std::string strYes("yes"), strNo("no"), strTrue("true"),
      strFalse("false"), strOn("on"), strOff("off");

  if (streq_nocase(str, strYes) || streq_nocase(str, strTrue) ||
      streq_nocase(str, strOn))
    return true;
  else if (streq_nocase(str, strFalse) || streq_nocase(str, strNo) ||
           streq_nocase(str, strOff))
    return false;
...
void romanNumerals(unsigned int numIn, char *romanOut) {
  ...
  for (int i = 2; i >= 0; --i) {
    if (digits[i]) {
      char One = (romanEquiv[i])[0], Five = (romanEquiv[i])[1],
           Ten = (romanEquiv[i])[2];
      switch (digits[i]) {
      case 1:
        *(romanOut++) = One;
