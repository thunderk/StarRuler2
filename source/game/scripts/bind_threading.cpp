...
public:
  ...
  int postInc() { return (*this)++; }

  int postDec() { return (*this)--; }
};

refAtomic *makeAtomic(int val) { return new refAtomic(val); }
...
refMutex *makeMutex() { return new refMutex(); }
...
void unlockMutex(refMutex *mutex) { mutex->release(); }
...
refRWMutex *makeRWMutex() { return new refRWMutex(); }
...
void unlockRWMutex(refRWMutex *mutex) { mutex->release(); }
...
refSignal *makeSignal() { return new refSignal(); }
...
public:
  refThreadLocal(asITypeInfo *ot) : objType(ot) {}
    ...
    if (typeId & asTYPEID_OBJHANDLE) {
      ...
      *(void **)p = next;
      ...
      return p;
               ...
               typeId == asTYPEID_FLOAT || typeId > asTYPEID_DOUBLE) /*enums*/ {
      if (!p) {
        p = malloc(sizeof(int));
        *(int *)p = 0;
        ptr.set(p);
...
bool IsHandleCompatibleWithObject(asCScriptEngine *engine, void *obj,
  ...
  // A handle to const cannot be passed to a handle that is not referencing a
  // const object
  ...
  if (objDt.GetTypeInfo() == hdlDt.GetTypeInfo()) {
    ...
    // Check if the object implements the interface, or derives from the base
    // class This will also return true, if the requested handle type is an
    // exact match for the object type
...
struct ScriptThread {
  ...
  //  When the script counter reaches 0, one system reference is lost, and the
  //  thread is shut down at the next opportunity When the system counter
  //  reaches 0, this struct is deleted
  ...
  bool getScriptObject(void *pointer, int typeID) {
    ...
    if (typeID & asTYPEID_OBJHANDLE) {
      if (typeID & asTYPEID_MASK_OBJECT &&
          IsHandleCompatibleWithObject((asCScriptEngine *)engine, object,
                                       objType->GetTypeId(), typeID)) {
        engine->AddRefScriptObject(object, objType);
        *(void **)pointer = object;
        return true;
      }
  ...
  void stop() { runThread.compare_exchange_strong(1, 2); }
  ...
  bool start(const std::string &func) {
    ...
    asIScriptFunction *localFunc =
        localMan->getFunction(func, "(double, ScriptThread&)", "double");
  ...
  bool isRunning() const { return runThread != 0; }

  ScriptThread()
      : runThread(false), systemRefs(1), scriptRefs(1), gcFlag(false),
        engine(asGetActiveContext()->GetEngine()), function(0), object(0),
        wasError(false) {
    engine->NotifyGarbageCollectorOfNewObject(
        this, engine->GetTypeInfoByName("ScriptThread"));
  }

  ScriptThread(const std::string &func)
      : runThread(false), systemRefs(1), scriptRefs(1), gcFlag(false),
        engine(asGetActiveContext()->GetEngine()), function(0), object(0),
        wasError(false) {
    engine->NotifyGarbageCollectorOfNewObject(
  ...
  ScriptThread(const std::string &func, void *pointer, int typeID)
      : runThread(false), systemRefs(1), scriptRefs(1), gcFlag(false),
        engine(asGetActiveContext()->GetEngine()), function(0), object(0),
        wasError(false) {
    engine->NotifyGarbageCollectorOfNewObject(
  ...
  static ScriptThread *create() { return new ScriptThread(); }

  static ScriptThread *create_f(const std::string &func) {
    return new ScriptThread(func);
  }

  static ScriptThread *create_fo(const std::string &func, void *pointer,
                                 int typeID) {
  ...
  bool getGCFlag() const { return gcFlag; }

  int getRefcount() const { return scriptRefs; }
  ...
  void releaseRefs(asIScriptEngine *eng) { setScriptObject(0, 0); }
};
...
threads::threadreturn threadcall runScriptThread(void *arg) {
  ...
  bool useGameTime = thread.engine != devices.scripts.client->engine &&
                     thread.engine != devices.scripts.menu->engine;

  double curTime = useGameTime ? devices.driver->getGameTime()
                               : devices.driver->getFrameTime();
  ...
  while (thread.runThread == 2) {
    if (!thread.manager->scriptThreadStart())
      break;
    curTime = useGameTime ? devices.driver->getGameTime()
                          : devices.driver->getFrameTime();
...
void RegisterThreadingBinds() {
  ...
  aint.addMethod("atomic_int& opAssign(int)",
                 asMETHODPR(refAtomic, operator=, (int), refAtomic &));
  aint.addMethod("void opAddAssign(int)",
                 asMETHODPR(refAtomic, operator+=, (int), int));
  aint.addMethod("void opDecAssign(int)",
                 asMETHODPR(refAtomic, operator-=, (int), int));
  aint.addMethod("atomic_int& set(int)",
                 asMETHODPR(refAtomic, operator=, (int), refAtomic &));
  aint.addMethod("int get()", asMETHODPR(refAtomic, get, () const, int));
  aint.addMethod("int get_value()", asMETHODPR(refAtomic, get_basic, (), int));
  aint.addMethod("void set_value(int)",
                 asMETHODPR(refAtomic, set_basic, (int), void));
  ...
  lock.addExternBehaviour(asBEHAVE_RELEASE, "void f()",
                          asFUNCTION(unlockMutex));
  ...
  rw_mutex.setReferenceFuncs(asMETHOD(refRWMutex, grab),
                             asMETHOD(refRWMutex, drop));

  ClassBind readLock("ReadLock", asOBJ_REF | asOBJ_SCOPED, 0);
  readLock.addFactory("ReadLock@ f(ReadWriteMutex&)",
                      asFUNCTION(lockReadMutex));
  readLock.addExternBehaviour(asBEHAVE_RELEASE, "void f()",
                              asFUNCTION(unlockRWMutex));

  ClassBind writeLock("WriteLock", asOBJ_REF | asOBJ_SCOPED, 0);
  writeLock.addFactory("WriteLock@ f(ReadWriteMutex&)",
                       asFUNCTION(lockWriteMutex));
  writeLock.addExternBehaviour(asBEHAVE_RELEASE, "void f()",
                               asFUNCTION(unlockRWMutex));

  // Register signal
  ClassBind signal("Signal", asOBJ_REF, 0);
  signal.addFactory("Signal@ f()", asFUNCTION(makeSignal));
  signal.setReferenceFuncs(asMETHOD(refSignal, grab),
                           asMETHOD(refSignal, drop));
  ...
  signal.addMethod("void waitAndSignal(int, int)",
                   asMETHOD(refSignal, waitAndSignal));
  ...
  tl.addLooseBehaviour(asBEHAVE_TEMPLATE_CALLBACK, "bool f(int&in,bool&out)",
                       asFUNCTION(validateThreadLocal));
  tl.setReferenceFuncs(asMETHOD(refThreadLocal, grab),
                       asMETHOD(refThreadLocal, drop));
  ...
  thread.setReferenceFuncs(asMETHOD(ScriptThread, scriptGrab),
                           asMETHOD(ScriptThread, scriptDrop));
  thread.addGarbageCollection(
      asMETHOD(ScriptThread, setGCFlag), asMETHOD(ScriptThread, getGCFlag),
      asMETHOD(ScriptThread, getRefcount), asMETHOD(ScriptThread, enumRefs),
      asMETHOD(ScriptThread, releaseRefs));

  thread.addFactory("ScriptThread@ f()", asFUNCTION(ScriptThread::create));
  thread.addFactory("ScriptThread@ f(const string &in entry)",
                    asFUNCTION(ScriptThread::create_f))
      doc("Starts a thread using the specified function.",
          "Entry point of type 'double f(double, ScriptThread&)'. Receives "
          "time since last call and a reference to the associated ScripThread. "
          "Returns delay till next call.",
          "");
  thread.addFactory("ScriptThread@ f(const string &in entry, ?&in)",
                    asFUNCTION(ScriptThread::create_fo))
      doc("Starts a thread using the specified function and script object.",
          "Entry point of type 'double f(double, ScriptThread&)'. Receives "
          "time since last call and a reference to the associated ScripThread. "
          "Returns delay till next call.",
          "Script object to assign to thread.", "");
  thread.addMethod("bool start(const string &in entry)",
                   asMETHOD(ScriptThread, start))
      doc("Starts a thread using the specified function.",
          "Entry point of type 'double f(double, ScriptThread&)'. Receives "
          "time since last call and a reference to the associated ScripThread. "
          "Returns delay till next call.",
  ...
  thread.addMethod("bool setObject(?&)",
                   asMETHOD(ScriptThread, setScriptObject))
      doc("Sets the thread's object. Can only receive script object handles.",
          "", "True if the reference could be taken");
  thread.addMethod("bool getObject(?&out)",
                   asMETHOD(ScriptThread, getScriptObject))
      doc("Gets the thread's object. Can only receive script object handles.",
