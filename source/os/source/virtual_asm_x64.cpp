...
struct Argument {
  ...
  Argument(unsigned Constant32)
      : reg(0), mem(0), constant(Constant32), is32(true) {}
  Argument(uint64_t Constant)
      : reg(0), mem(0), constant(Constant), is32(false) {}
};
...
unsigned Processor::call_thiscall_args(Register *obj, const char *args,
  ...
  while (!arg_stack.empty()) {
    ...
    if (arg.reg) {
      ...
      if (addr.code == R15 || addr.other == R15 ||
          (addr.scaleFactor != 0 && addr.scaleReg == R15))
        throw "Cannot use this register for cdecl call wrapper address.";
      if (addr.other == ESP)
        throw "Cannot use this register for cdecl call wrapper address.";
      if (addr.code == ESP) {
...
template <> Processor &Processor::operator<<(MemAddress addr) {
  ...
  if (addr.scaleFactor != 0) {
    ...
    if (addr.offset == 0 && addr.code % 8 != EBP)
      return *this << mod_rm(addr.other % 8, ADR, SIB)
                   << sib(addr.scaleReg % 8, scale, addr.code % 8);
    else if (addr.offset >= CHAR_MIN && addr.offset <= CHAR_MAX)
      return *this << mod_rm(addr.other % 8, ADR8, SIB)
                   << sib(addr.scaleReg % 8, scale, addr.code % 8)
                   << (char)addr.offset;
    else
      return *this << mod_rm(addr.other % 8, ADR32, SIB)
                   << sib(addr.scaleReg % 8, scale, addr.code % 8)
                   << addr.offset;
  }
...
void Processor::end_long_jump(void *p) {
  volatile byte *jumpFrom = (volatile byte *)p;
  bool isSamePage =
      (size_t)jumpFrom >= (size_t)pageStart && (size_t)jumpFrom < (size_t)op;
  int64_t offset = ((size_t)op - (size_t)jumpFrom) - 4;
  if (offset < (int64_t)INT_MIN || offset > (int64_t)INT_MAX) {
    if (isSamePage)
      throw "Inside-page long jump too long. This should never ever happen, "
...
void Processor::jump(JumpType type, volatile byte *dest) {
  ...
  if (offset >= CHAR_MIN && offset <= CHAR_MAX)
    ...
    *this << longJumpCodes[Jump]
          << (int)(offset - 3); // Long jump is 3 bytes larger, jump is from the
                                // end of the full opcode
  else
    *this << '\x0F' << longJumpCodes[type]
          << (int)(offset - 4); // Conditional long jump is 4 bytes larger, jump
                                // is from the end of the full opcode
}
...
MemAddress::MemAddress(Processor &CPU, void *address)
    : cpu(CPU), code(ESP), absolute_address(address), other(NONE), offset(0),
      bitMode(cpu.bitMode), Float(false), Signed(false), scaleFactor(0),
      scaleReg(NONE) {}
...
bool Register::xmm() { return code >= 16; }

bool Register::extended() { return (code % 16) >= 8; }

RegCode Register::index() { return (RegCode)(code % 8); }
...
void Register::operator-() { cpu << prefix(EX_3) << '\xF7' << modrm(EX_3); }
...
void Register::operator==(unsigned int test) {
  ...
  case 8:
    ...
    break;
  case 16:
    cpu << '\x66' << prefix(EX_7) << '\x80' << modrm(EX_7)
        << (unsigned short)test;
    break;
  case 32:
  case 64:
...
void Register::divide() { cpu << prefix() << '\xF7' << modrm(EX_6); }

void Register::divide_signed() { cpu << prefix() << '\xF7' << modrm(EX_7); }
