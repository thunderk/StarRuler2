...
void prepareShaderStateVars() {
  ...
  for (auto i = vars.begin(), end = vars.end(); i != end; ++i) {
    ...
    if (pos == std::string::npos) {
      error("Shader State Variable: '%s' is not a state (should be "
            "ObjectType::MemberName)",
            i->first.c_str());
      continue;
    }

    std::string objName = i->first.substr(0, pos),
                memberName = i->first.substr(pos + 2);

    ScriptObjectType *type = getScriptObjectType(objName);
    if (type == 0) {
      error("Shader State Variable: '%s' is not an object type (from '%s')",
            objName.c_str(), i->first.c_str());
      continue;
    }
    ...
    if (!memberFound) {
      error("Shader State Variable: %s has no member of type '%s'",
            objName.c_str(), memberName.c_str());
    }
  }
}
