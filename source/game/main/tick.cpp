...
public:
  RunScriptTick(scripts::Manager *Manager, double Time, bool Gc,
                double &LogTime)
  ...
  bool run() {
    ...
    if (end - start > 0.016) {
      ...
      if (manager == devices.scripts.client)
        ...
        name = "Menu";
      error("%s GC took %dms (Mode %d)", name, (int)((end - gcStart) * 1.0e3),
            gcMode);
      if (gcStart - start > 0.01)
        error("%s script pausing took %dms", name,
              (int)((gcStart - start) * 1.0e3));
    }
#endif
...
threads::threadreturn threadcall animateNodes(void *arg) {
  ...
  while (true) {
    ...
    if (!devices.scene->children.empty()) {
      auto *nodes = &devices.scene->children.front();
      int animCount =
          (int)devices.scene->children.size() / (animationThreadCount * 16);
      if (animCount == 0)
        animCount = 1;
...
void shader_gameTime_cycle(float *pFloats, unsigned short n, void *pArgs) {
  ...
  do {
    *pFloats =
        (float)(std::fmod(frameTime_s, double(*period)) / double(*period));
    ++period;
    ++pFloats;
  } while (--n);
}
...
void shader_frameTime_cycle(float *pFloats, unsigned short n, void *pArgs) {
  ...
  do {
    *pFloats =
        (float)(std::fmod(realFrameTime, double(*period)) / double(*period));
    ++period;
    ++pFloats;
  } while (--n);
}
...
double menuTickTime = 0, serverTickTime = 0, clientTickTime = 0,
       animationTime = 0;
...
void tickGlobal(bool hasScripts) {
  ...
  if (lockProfileTimer >= 1.0) {
    ...
    if (printLockProfile) {
      ...
      threads::profileReadWriteMutexCycle([](threads::ReadWriteMutex *mtx) {
        if ((mtx->observed || !requireObserved) &&
            (mtx->profileReadCount > 0 || mtx->profileWriteCount > 0)) {
          print("%s: %d read, %d write", mtx->name.c_str(),
                mtx->profileReadCount, mtx->profileWriteCount);
        }
      });
  ...
  if (hasScripts) {
    scriptTickSignal.signal(1);
    processing::queueAction(new RunScriptTick(
        devices.scripts.menu, realFrameLen, false, menuTickTime));

    if (game_running) {
      scriptTickSignal.signalUp(2);
      processing::queueAction(new RunScriptTick(
          devices.scripts.server, frameLen_s, true, serverTickTime));
      processing::queueAction(new RunScriptTick(
          devices.scripts.client, realFrameLen, false, clientTickTime));
    }
  }
...
void tickGuiReload() {
  if (!devices.scripts.client)
    return;
  for (auto it = devices.scripts.client->modules.begin();
       it != devices.scripts.client->modules.end(); ++it) {
...
void renderFrame(scripts::Manager *uiScript, scripts::Manager *renderScript) {
  ...
  devices.sound->setListenerData(
      vec3f(devices.render->cam_pos), vec3f(),
      vec3f(devices.render->cam_pos + devices.render->cam_facing),
      vec3f(devices.render->cam_up));
  ...
  if (!animTimes) {
    animationThreadCount =
        std::max(1u, devices.driver->getProcessorCount() - 1);
    ...
    console.addCommand(
        "anim_times", [](argList &args) { dumpAnimTimes = true; }, true);
#endif
  }
  ...
  if (render) {
    ...
    // Animation system tells us the near and far distances of everything that
    // gets rendered
    ...
    if (rt) {
      ...
      if (devices.render->isRenderPrepared()) {
        ...
        rs.textures[0] = rt;
        rs.textures[7] =
            ((render::glFrameBuffer *)rt)
                ->depthTexture; // fuck you framebuffers aren't textures
  ...
  if (render && !hide_ui) {
    ...
    vec2i uiArea;
    if (ui_scale != 1.0) {
      uiArea = vec2d(devices.driver->win_width, devices.driver->win_height) /
               ui_scale;
...
void tickGame() {
  ...
  if (devices.driver->getGameSpeed() > 0 && !devices.network->isClient) {
    ...
    if (interval && interval->getDouble() >= 1.0 &&
        autosaveTimer > interval->getDouble() * 60.0) {
      int maxAutosaves = 1;
      ...
      processing::pause();
      saveGame(
          path_join(devices.mods.getGlobalProfile("saves"), "autosave.sr2"));
      processing::resume();
