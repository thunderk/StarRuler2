...
Empire *Empire::getDefaultEmpire() { return defaultEmpire; }

Empire *Empire::getSpectatorEmpire() { return spectatorEmpire; }

Empire *Empire::getPlayerEmpire() { return playerEmpire; }

unsigned Empire::getEmpireCount() { return empires.size(); }
...
void Empire::setPlayerEmpire(Empire *emp) { playerEmpire = emp; }
...
unsigned Empire::objectCount() { return objects.size(); }
...
const StateDefinition *Empire::getEmpireStates() { return empStates; }

Empire::Empire(unsigned char ID)
    : index(-1), background(0), portrait(0), flag(0), flagID(0), player(0),
      hullIconIndex(0), validEmpIndex(0) {
  if (ID == INVALID_EMPIRE) {
...
bool Empire::valid() { return id < UNLISTED_EMPIRE; }
...
void Empire::recordStat(unsigned id, int value) {
  if (id < statHistories.size()) {
    ...
    StatEntry *entry =
        history->addStatEntry(unsigned(devices.driver->getGameTime()));
    entry->asInt = value;
  }
}
...
void Empire::recordStatDelta(unsigned id, int delta) {
  if (id < statHistories.size()) {
    ...
    if (tail && tail->time == time) {
      ...
      entry->asInt =
          entry->prev ? entry->prev->asInt + delta : entry->asInt + delta;
    }
  }
}
...
void Empire::recordStatDelta(unsigned id, float delta) {
  if (id < statHistories.size()) {
    ...
    if (tail && tail->time == time) {
      ...
      StatEntry *entry = history->addStatEntry(time);
      entry->asFloat =
          entry->prev ? entry->prev->asFloat + delta : entry->asFloat + delta;
    }
  }
}

void Empire::recordEvent(unsigned id, unsigned short type,
...
bool Empire::addDesign(DesignClass *cls, const Design *design) {
  if (design->hasFatalErrors()) {
    error("Could not add design '%s', design has fatal errors:",
          design->name.c_str());
    for (auto i = design->errors.begin(), end = design->errors.end(); i != end;
         ++i)
  ...
  design->revision = 1;
  designs.insert(it,
                 std::pair<std::string, const Design *>(design->name, design));
...
bool Empire::changeDesign(const Design *older, const Design *newer,
                          DesignClass *cls) {
...
void Empire::setDesign(const Design *older, const Design *newer,
                       DesignClass *cls) {
  if (older->owner != this)
    return;
  threads::WriteLock lock(designMutex);
...
void Empire::flagDesignOld(const Design *design) {
  if (devices.network->isClient)
    return;
  // NOTE: This can use a readlock as outdated is only used by updateDesign
  // which can't be writing at the same time
...
void Empire::makeDesignIcon(const Design *design) {
  ...
  unsigned perLine =
      (SHEET_PAGE_SIZE.x - sheet->spacing) / (sheet->width + sheet->spacing);
  ...
  resource::queueTextureUpdate(sheet->material.textures[0], new Image(*img),
                               prior, sheet->material.mipmap);
...
Empire *recvEmpireInitial(net::Message &msg) {
  ...
  info("Received empire '%s' (%d).", emp->name.c_str(), emp->id);

  return emp;
}

Empire::Empire(SaveFile &file)
    : background(0), portrait(0), flag(0), flagID(0), player(0) {
  ...
  if (file >= SFV_0014) {
    ...
    while (count--) {
      ...
      auto addEvent = [&](StatEntry *entry, const std::string &text,
                          unsigned short id) {
        if (stat)
...
void Empire::save(SaveFile &file) {
  ...
  unsigned statID = 0;
  for (auto i = statHistories.begin(), end = statHistories.end(); i != end;
       ++i, ++statID) {
    StatHistory &stat = **i;
