...
uint8_t *Message::Buffer::data() { return (uint8_t *)this; }
...
Message::Message()
    : buffer(0), allocated(0), used_bytes(0), used_bits(0), read_bytes(0),
      read_bits(0), HasID(false), readError(false) {}
...
Message::Message(uint8_t type, uint8_t flags)
    : buffer(Buffer::create(64)), allocated(64), used_bytes(0), used_bits(0),
      read_bytes(0), read_bits(0), HasID(false), readError(false) {
  buffer->type = type;
...
bool Message::hasError() const { return readError; }

void Message::setType(uint8_t type) { buffer->type = type; }
...
bool Message::hasID() const { return HasID; }
...
void Message::finalize() { writeAlign(); }
...
void Message::copyTo(Message &to, msize_t fromPos, msize_t toPos) const {
  ...
  if (toPos == 0) {
    if (used_bits == 0)
      ...
      to.writeBits(buffer->data() + fromPos,
                   (used_bytes - fromPos) * 8 + used_bits);
  } else {
...
void Message::readBits(uint8_t *ptr, unsigned bits) {
  ...
  if (read_bits == 0) {
    ...
    for (unsigned i = 0; i < bytes; ++i) {
      ptr[i] = (data[read_bytes] << read_bits) |
               (data[read_bytes + 1] >> (8 - read_bits));
      read_bytes += 1;
    }
