...
public:
  ...
  bool systemRandom(unsigned char *buffer, unsigned bytes) {
    ...
    BOOL ctx =
        CryptAcquireContext(&provider, "SR2SysRand", NULL, PROV_RSA_AES, NULL);
    if (!ctx)
      ctx = CryptAcquireContext(&provider, "SR2SysRand", NULL, PROV_RSA_AES,
                                CRYPT_NEWKEYSET);
  ...
  void setVerticalSync(int waitFrames) { glfwSwapInterval(waitFrames); }
  ...
  void createWindow(WindowData &data) {
    ...
    if (data.mode == WM_Fullscreen) {
      ...
      if (monitor && !data.overrideMonitor) {
        ...
        for (int i = 0; i < count; ++i) {
          auto &mode = modes[i];
          if (mode.width == data.width && mode.height == data.height &&
              (data.refreshRate == 0 || data.refreshRate == mode.refreshRate)) {
            match = true;
            break;
          }
        }
    ...
    window = glfwCreateWindow(data.width, data.height, "Star Ruler 2", monitor,
                              nullptr);

    if (window == 0)
      window = glfwCreateWindow(
          1024, 768, "Star Ruler 2 - Error Creating Window", nullptr, nullptr);
...
#ifdef WIN_MODE
    HICON hSmallIcon = (HICON)LoadImage(0, "sr2.ico", IMAGE_ICON, 32, 32,
  ...
  void getVideoModes(std::vector<OSDriver::VideoMode> &output) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      uint64_t size = (uint64_t)m.width << 16 | (uint64_t)m.height |
                      ((uint64_t)mode.refreshRate << 32);
  ...
  void flashWindow() override { glfwFlashWindow(window); }
  ...
  int getCharForKey(int key) { return glfwGetCharForKey(key); }

  int getKeyForChar(unsigned char chr) { return glfwGetKeyForChar(chr); }
  ...
  void setMousePos(int x, int y) { glfwSetCursorPos(window, x, y); }
  ...
  void sleep(int milliseconds) { threads::sleep(milliseconds); }
  ...
  int getTime() const {
    ...
    return (int)(1000 * (cur_time.tv_sec - start_time.tv_sec) +
    ...
    auto timeInSeconds =
        (double)(cur_time.QuadPart - start_time.QuadPart) / ticksPerSecond;
  ...
  double getAccurateTime() const {
    ...
    return (double)cur_time.QuadPart / (double)cur_freq.QuadPart -
           (double)start_time_hq.QuadPart / (double)start_time_hq_freq.QuadPart;
#endif
  }
  ...
  double getGameSpeed() const { return gameSpeed; }

  void setGameSpeed(double speed) { gameSpeed = speed; }

  unsigned getProcessorCount() {
    return std::max(threads::getNumberOfProcessors(), 1u);
  }

  void setWindowTitle(const char *str) { glfwSetWindowTitle(window, str); }

  void setWindowSize(int width, int height) {
    glfwSetWindowSize(window, width, height);
  }

  void closeWindow() { glfwDestroyWindow(window); }
};

void redirectWindowClose(GLFWwindow *) { driver->onWindowClose(); }
...
void redirectCharKey(GLFWwindow *, unsigned key) { driver->onCharTyped(key); }
