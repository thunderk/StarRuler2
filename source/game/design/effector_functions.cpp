...
static Object *SingleTarget(const Effector *eff, Object *obj,
                            EffectorTarget *efftarg) {
  ...
  obj->findTargets(finder, TARGET_SINGLE_TARGET_DEPTH,
                   Object::RANDOMIZE_TARGETS);
  return finder.best;
}

//* Weighters
static double NotOurs(const Effector *eff, Object *obj, Object *targ,
                      void *arg) {
...
static double isEnemy(const Effector *eff, Object *obj, Object *targ,
                      void *arg) {
...
static double isDamageable(const Effector *eff, Object *obj, Object *targ,
                           void *arg) {
...
static double isAttackable(const Effector *eff, Object *obj, Object *targ,
                           void *arg) {
  if (obj->owner == targ->owner || targ->owner == nullptr ||
      !targ->owner->valid())
    return 1.0;
  return isEnemy(eff, obj, targ, arg);
}

static double hasDamagedBlueprint(const Effector *eff, Object *obj,
                                  Object *targ, void *arg) {
  if (obj != nullptr && obj->type->blueprintOffset != 0) {
    auto *blueprint = (Blueprint *)(((size_t)obj) + obj->type->blueprintOffset);
    return blueprint->currentHP < blueprint->design->totalHP - 0.0001 ? 1.0
                                                                      : 0.0;
  }
  return 0.0;
}
...
static double Distance(const Effector *eff, Object *obj, Object *targ,
                       void *arg) {
...
double hasTag(const Effector *eff, Object *obj, Object *targ, void *arg) {
  if (targ != nullptr && targ->isValid() && targ->type->blueprintOffset != 0) {
    auto *blueprint =
        (Blueprint *)(((size_t)targ) + targ->type->blueprintOffset);
...
static double targRadius(const Effector *eff, Object *obj, Object *targ,
                         void *arg) {
  if (targ == nullptr)
    return 1.0;
  return targ->radius;
...
static double originRadius(const Effector *eff, Object *obj, Object *targ,
                           void *arg) {
  if (obj == nullptr)
    return 1.0;
  return obj->radius;
...
static double sizeDifference(const Effector *eff, Object *obj, Object *targ,
                             void *arg) {
...
//* Activation
static EffectorActivationType Always(const Effector *eff, Object *obj,
                                     EffectorTarget &targ, double &time,
                                     double *args, double *states) {
  return EAT_Activate;
}

static EffectorActivationType Timed(const Effector *eff, Object *obj,
                                    EffectorTarget &targ, double &time,
                                    double *args, double *states) {
...
static EffectorActivationType VariableTimed(const Effector *eff, Object *obj,
                                            EffectorTarget &targ, double &time,
                                            double *args, double *states) {
  if (!(targ.flags & TF_WithinFireTolerance)) {
    if (states[0] > 0.0)
...
static EffectorActivationType StaggeredTimed(const Effector *eff, Object *obj,
                                             EffectorTarget &targ, double &time,
                                             double *args, double *states) {
  if (!(targ.flags & TF_WithinFireTolerance)) {
...
// Fires arg[1] shots at arg[0] second intervals, then reloads over arg[2]
// seconds
static EffectorActivationType Magazine(const Effector *eff, Object *obj,
                                       EffectorTarget &targ, double &time,
                                       double *args, double *states) {
...
decltype(EffectorActivation) makeActivationList() {
  ...
  auto addFunc = [&](std::string name, nativeEffectorActivation f,
                     unsigned stateCount, unsigned argCount) {
    auto &cb = list[name];
    cb.func = f;
