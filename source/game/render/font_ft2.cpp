...
namespace resource {
extern render::Texture *queueImage(Image *img, int priority, bool mipmap,
                                   bool cachePixels);
};

namespace render {

Font *Font::createDummyFont() { return new Font(); }

bool clipQuad(rectf pos, rectf source, const rectf *clip, vec2f *verts,
              vec2f *texcoords) {
  if (clip) {
    ...
    if (!clip->isRectInside(pos)) {
      // TODThere seem to be some oddities with the texture coordinates if a
      // clip occurs
...
public:
  ...
  void draw(render::RenderDriver *driver, const char *text, int X, int Y,
            const Color *color, const recti *clip = 0) const {
    float x = (float)X, y = (float)Y;
    Color col = color ? *color : Color();

    float xFactor = 1.f / float(textureWidth),
          yFactor = 1.f / float(textureHeight);
    ...
    while (fontChar c = it++) {
      if (c == L'\n') {
        ...
        if (c >= '0' && c <= '9') {
          ...
          if (lastC) {
            FT_Vector kerning;
            FT_Get_Kerning(*face, lastC, glyph.glyph_index,
                           0 /*FT_KERNING_UNFITTED*/, &kerning);
        ...
        if (glyph.draw) {
          rectf pos =
              rectf::area(vec2f(x + (float)glyph.wOff, y + (float)glyph.hOff),
                          vec2f(glyph.w, glyph.h));
          rectf source = rectf::area(
              vec2f((float)glyph.x * xFactor, (float)glyph.y * yFactor),
  ...
  vec2i drawChar(render::RenderDriver *driver, int c, int lastC, int X, int Y,
                 const Color *color, const recti *clip = 0) const {
    float x = (float)X, y = (float)Y;
    Color colors[4];
    if (color)
    ...
    if (glyph.draw) {
      rectf pos = rectf::area(vec2f(x + glyph.wOff, y + glyph.hOff),
                              vec2f(glyph.w, glyph.h));
      rectf source = rectf::area(
          vec2f((float)glyph.x * xFactor, (float)glyph.y * yFactor),
          vec2f((float)glyph.w * xFactor, (float)glyph.h * yFactor));
      ...
      if (clipQuad(pos, source, clip ? &fclip : 0, font_verts, font_tc))
        driver->drawQuad(textures[glyph.page], font_verts, font_tc,
                         color ? colors : 0);
    }
  ...
  unsigned getBaseline() const { return baseLine; }

  unsigned getLineHeight() const { return glyphHeight; }

  FontFT2()
      : lowChars(new Glyph[256]), glyphHeight(0), baseLine(0), textureHeight(1),
        textureWidth(1) {
...
Font *loadFontFT2(render::RenderDriver &driver, const char *filename,
  ...
  font->baseLine =
      font->glyphHeight + (unsigned)((*face)->size->metrics.descender >> 6);
  int lineHeight = 0;
