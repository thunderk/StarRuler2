...
public:
  ...
  double getEfficiencySum(int variable, int tag = -1,
                          bool ignoreInactive = true);
  double getEfficiencyFactor(int variable, int tag = -1,
                             bool ignoreInactive = true);
  ...
  void target(Object *obj, unsigned efftrIndex, Object *target,
              TargetFlags flags = TF_Target);
  void target(Object *obj, const Subsystem *sys, Object *target,
              TargetFlags flags = TF_Target);
  void clearTracking(Object *obj);
  Object *getCombatTarget();

  vec3d getOptimalFacing(int sysVariable, int tag = -1,
                         bool ignoreInactive = true);
  ...
  void damage(Object *obj, DamageEvent &evt, double position,
              const vec2d &direction);
  void damage(Object *obj, DamageEvent &evt, const vec2u &position,
