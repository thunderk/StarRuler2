...
template <class T = double> struct vec3 {
  ...
  template <class O>
  explicit vec3(const vec3<O> &other)
  ...
  double distanceTo(const vec3 &other) const {
    double tx = x - other.x, ty = y - other.y, tz = z - other.z;
  ...
  vec3 operator*(double scalar) const {
    return vec3(T((double)x * scalar), T((double)y * scalar),
                T((double)z * scalar));
  }
  ...
  vec3 slerp(const vec3 &other, double pct) const {
    ...
    if (_dot <= -1.0) {
      // Normally this would reduce to lerp, but we know we want to rotate in
      // some direction instead
      //  We find a vector perpendicular to our endpoints and interpolate based
      //  on our progress
    ...
    return ((*this * sin((1.0 - pct) * omega) / sinOmg) +
            (other * sin(pct * omega) / sinOmg))
        .normalized();
  }

  vec3 &normalize(T length = (T)1.0) {
    double X = x, Y = y, Z = z, L = (X * X) + (Y * Y) + (Z * Z);
    if (L == 0.0)
      return *this;
  ...
  vec3<T> elementMax(const vec3<T> &other) const {
    return vec3<T>(x > other.x ? x : other.x, y > other.y ? y : other.y,
                   z > other.z ? z : other.z);
  }
  ...
  bool zero() { return x == 0.0 && y == 0.0 && z == 0.0; }
};
