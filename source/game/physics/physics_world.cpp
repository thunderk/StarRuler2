...
PhysicsWorld::PhysicsWorld(double GridSize, double GridFuzz, unsigned GridCount)
    : gridSize(GridSize), halfGridSize(GridSize * 0.5), gridFuzz(GridFuzz),
      gridCount(GridCount) {
  while (gridCount > 200) {
...
PhysicsWorld::~PhysicsWorld() { delete[] grid; }
...
void PhysicsWorld::findInBoxInZone(
    const PhysicsGrid &zone, const AABBoxd &box,
    const std::function<void(const PhysicsItem &)> &callback,
    unsigned ownerMask) {
...
void PhysicsWorld::findInBox(
    const AABBoxd &box,
    const std::function<void(const PhysicsItem &)> &callback,
    unsigned ownerMask) {
  // double start = devices.driver->getAccurateTime();
  unsigned fromX = getLowerBound(box.minimum.x),
           toX = getUpperBound(box.maximum.x);
  unsigned fromY = getLowerBound(box.minimum.z),
           toY = getUpperBound(box.maximum.z);
...
bool PhysicsGrid::empty() const { return itemCount == 0; }
...
void PhysicsGrid::removeItem(PhysicsItem *item) {
  ...
  for (auto i = bisect->items.begin(), end = bisect->items.end(); i != end;
       ++i) {
    if ((*i) == item) {
      itemRemoved = true;
...
void PhysicsGrid::addItem(PhysicsItem *item) {
  ...
  if (bisect->a) {
    PhysBisect *refuse = 0;
    while (bisect->childrenA > REBALANCE_RATIO * bisect->childrenB ||
           bisect->childrenB > REBALANCE_RATIO * bisect->childrenA) {
      refuse = bisect;
...
void PhysicsWorld::updateItem(PhysicsItem &item, const AABBoxd &newBox) {
  if (newBox.isWithin(item.gridLocation->fuzzBound)) {
    // TODO: Handle transition back from 'outside' if it is now able to be
...
PhysicsItem *PhysicsWorld::registerItem(const AABBoxd &bounds,
                                        scene::Node *node) {
  PhysicsItem *item = new PhysicsItem;
  item->bound = bounds;
  item->node = node;
