...
public:
  ...
  void parse(const std::string &str) { doc.Parse<0>(str.c_str()); }
  ...
  rapidjson::Value *root() { return &doc; }
};

static JSONTree *makeJSONTree() { return new JSONTree(); }
...
static rapidjson::Value *findMember(rapidjson::Value *node,
...
static rapidjson::Value *getMember(rapidjson::Value *node,
                                   const std::string &name) {
  if (!node->IsObject()) {
    scripts::throwException("Node is not an object.");
    return 0;
...
static rapidjson::Value *setString(rapidjson::Value *node,
...
void RegisterJSONBinds() {
  ...
  classdoc(node,
           "A node within a json tree. Careful when holding references, as"
  ...
  classdoc(
      tree,
      "Represents a tree of json nodes that can be altered."
  ...
  tree.addMethod("void readFile(const string& filename)",
                 asMETHOD(JSONTree, readFile))
      doc("Read a json tree from a file.",
          "Filename of the file to read from.");

  tree.addMethod("string toString(bool pretty = false)",
                 asMETHOD(JSONTree, toString))
  ...
  node.addMethod("JSONNode& setBool(bool value)",
      ...
      doc("Set this node to a boolean value.", "Boolean value to set to.",
          "This node.");

  node.addMethod("JSONNode& makeArray()", asMETHOD(rapidjson::Value, SetArray))
      doc("Set this node to be an array.", "This node.");

  node.addMethod("JSONNode& makeObject()",
                 asMETHOD(rapidjson::Value, SetObject))
      doc("Set this node to be an object.", "This node.");

  node.addMethod("JSONNode& setInt(int value)",
                 asMETHOD(rapidjson::Value, SetInt))
      doc("Set this node to an int value.", "Integer value to set to.",
          "This node.");

  node.addMethod("JSONNode& setUint(uint value)",
                 asMETHOD(rapidjson::Value, SetUint))
      doc("Set this node to an unsigned int value.",
          "Unsigned integer value to set to.", "This node.");

  node.addMethod("JSONNode& setInt64(int64 value)",
                 asMETHOD(rapidjson::Value, SetInt64))
      doc("Set this node to a long int value.", "Long integer value to set to.",
          "This node.");
  ...
  node.addMethod("JSONNode& setUint64(uint64 value)",
      ...
      doc("Set this node to a long unsigned int value.",
          "Long unsigned integer value to set to.", "This node.");

  node.addMethod("JSONNode& setDouble(double value)",
                 asMETHOD(rapidjson::Value, SetDouble))
      doc("Set this node to a double value.", "Double value to set to.",
          "This node.");

  node.addExternMethod("JSONNode& setString(const string&in value)",
                       asFUNCTION(setString))
      doc("Set this node to a string value.", "String value to set to.",
          "This node.");
  ...
  node.addExternMethod("JSONNode@ findMember(const string& name)",
      ...
      doc("Retrieve a member node from an object.", "The name of the member.",
  ...
  node.addExternMethod("JSONNode@ getMember(const string& name)",
      ...
      doc("Retrieve a member node from an object, creating it if it does not "
          "exist.",
  ...
  node.addExternMethod("void removeMember(const string& name)",
                       asFUNCTION(removeMember))
