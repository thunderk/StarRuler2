  ...
  if (group && group->getObjectCount() != 0) {
    ...
    if (obj) {
      ...
      if (obj->lastTick > node->lastUpdate + tickTime) {
        node->position = node->position.interpolate(
            group->getCenter(),
            frameLen_s / (obj->lastTick - node->lastUpdate));
        node->rotation = node->rotation.slerp(
            group->formationFacing,
            frameLen_s / (obj->lastTick - node->lastUpdate));
...
GroupAnim::GroupAnim(ObjectGroup *Group) : group(Group) { group->grab(); }
}; // namespace scene
