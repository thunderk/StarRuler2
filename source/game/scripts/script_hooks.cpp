...
static void emptyHook(asIScriptFunction **f) { *f = 0; }
...
static bool validHook(asIScriptFunction **f) { return *f != 0; }

static asIScriptFunction **assignHook(asIScriptFunction **dest,
                                      asIScriptFunction **src) {
  *dest = *src;
  return dest;
}
...
static asITypeInfo *getClass(const std::string &func) {
  ...
  if (parts.size() == 1) {
    ...
    if (ctx) {
      ...
      type =
          getActiveManager()->getClass(func->GetModuleName(), parts[0].c_str());
    }
...
static std::string modName(asIScriptModule *mod) { return mod->GetName(); }

static std::string className(asITypeInfo *mod) { return mod->GetName(); }
...
static asITypeInfo *modClassName(asIScriptModule *mod,
...
static int clsId(asCObjectType *type) { return type->GetTypeId(); }
...
static bool objGC(asCObjectType *type) { return type->GetFlags() & asOBJ_GC; }

static asIScriptObject *objMember(asCObjectType *type, void *ptr, int typeId,
                                  unsigned index) {
  if (ptr != nullptr && (typeId & asTYPEID_SCRIPTOBJECT)) {
...
static render::Sprite getGlobalSprite(const std::string &module,
                                      const std::string &name) {
  render::Sprite sprt;
  auto *man = getActiveManager();
  if (man != nullptr) {
    ...
    if (mod != nullptr && type != nullptr) {
      int index = mod->module->GetGlobalVarIndexByDecl(
          (std::string("const Sprite ") + name).c_str());
      if (index < 0)
        index = mod->module->GetGlobalVarIndexByDecl(
            (std::string("Sprite ") + name).c_str());
      if (index >= 0) {
        int typeId = 0;
        if (mod->module->GetGlobalVar(index, nullptr, nullptr, &typeId,
                                      nullptr) == asSUCCESS) {
...
static Color getGlobalColor(const std::string &module,
                            const std::string &name) {
  Color col;
  auto *man = getActiveManager();
  if (man != nullptr) {
    ...
    if (mod != nullptr && type != nullptr) {
      int index = mod->module->GetGlobalVarIndexByDecl(
          (std::string("const Color ") + name).c_str());
      if (index < 0)
        index = mod->module->GetGlobalVarIndexByDecl(
            (std::string("Color ") + name).c_str());
      if (index >= 0) {
        int typeId = 0;
        if (mod->module->GetGlobalVar(index, nullptr, nullptr, &typeId,
                                      nullptr) == asSUCCESS) {
...
void RegisterScriptHooks() {
  foreach (it, hooks) {
    ...
    ClassBind cls(name.c_str(), asOBJ_VALUE | asOBJ_APP_CLASS_CDAK,
                  sizeof(void *));
    cls.addConstructor("void f()", asFUNCTION(emptyHook));
    cls.addConstructor(format("void f(const $1&)", name).c_str(),
                       asFUNCTION(assignHook));
    cls.addDestructor("void f()", asFUNCTION(dummy));

    cls.addExternMethod("bool get_valid() const", asFUNCTION(validHook));
    cls.addExternMethod(format("$1& opAssign(const $1&in)", name, name).c_str(),
                        asFUNCTION(assignHook));
    ...
    fid = getEngine()->RegisterObjectMethod(
        cls.name.c_str(), "void bind(const string &in)", asFUNCTION(getHook),
        asCALL_GENERIC);
  ...
  classptr.addExternMethod("IAnyObject@ create() const",
                           asFUNCTION(createClass));
  classptr.addExternMethod("bool implements(const AnyClass@ check) const",
                           asFUNCTION(implClass));
  classptr.addExternMethod("uint get_memberCount() const",
                           asFUNCTION(clsMemberCount));
  classptr.addExternMethod("AnyClass@ get_members(uint index) const",
                           asFUNCTION(clsMember));
  classptr.addExternMethod("string get_memberName(uint index) const",
                           asFUNCTION(clsMemberName));
  classptr.addExternMethod("IAnyObject@ getMember(const ?&, uint index) const",
                           asFUNCTION(objMember));
  ...
  modptr.addExternMethod("uint get_classCount() const",
                         asFUNCTION(modClassCount));
  modptr.addExternMethod("AnyClass& get_classes(uint index) const",
                         asFUNCTION(modClass));
  modptr.addExternMethod("AnyClass& getClass(const string& name) const",
                         asFUNCTION(modClassName));

  classptr.addExternMethod("ScriptModule@ get_module() const",
                           asFUNCTION(clsModule));
  bind("ScriptModule@ getScriptModule(const string& name)",
       asFUNCTION(getModule));
  bind("ScriptModule@ get_THIS_MODULE()", asFUNCTION(thisModule));
  bind("void getClassesImplementing(array<AnyClass@>& results, AnyClass& base)",
       asFUNCTION(getImplementing));

  bind("Sprite getGlobalSprite(const string& module, const string& name)",
       asFUNCTION(getGlobalSprite));
  bind("Color getGlobalColor(const string& module, const string& name)",
       asFUNCTION(getGlobalColor));
}
