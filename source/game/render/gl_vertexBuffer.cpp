...
inline bool getRequestSize(unsigned &indexCount, unsigned &vertCount,
                           unsigned polygons, unsigned polyType) {
  switch (polyType) {
  case PT_Lines:
...
void buildIndices(std::vector<unsigned short> &indices, unsigned indexCount,
                  unsigned prevVerts, unsigned polygons, unsigned polyType) {
...
void renderVertexBuffers() { tcv.draw(); }

VertexBufferTCV::VertexBufferTCV() : last(0) {
...
void VertexBufferTCV::flushRetainingLast(FlushCause reason) {
  ...
  // We may have added a step that we decided we can't render, we just pop the
  // empty step
  if (step.vertEnd == 0)
    steps.pop_back();
  ...
  if (step.shaderCache) {
    memcpy(pNextShaderBuffer, step.shaderCache,
           (prevBuffer - step.shaderCache) * sizeof(float));
...
void VertexBufferTCV::draw(FlushCause reason) {
...

  {
    ...
    glTexCoordPointer(2, GL_FLOAT, sizeof(VertexTCV),
                      (void *)offsetof(VertexTCV, uv));
    glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(VertexTCV),
                   (void *)offsetof(VertexTCV, col));
    glVertexPointer(3, GL_FLOAT, sizeof(VertexTCV),
                    (void *)offsetof(VertexTCV, pos));
  }

  glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(unsigned short),
               &indices[0], GL_STREAM_DRAW);

  glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(VertexTCV),
  ...
  for (unsigned i = 0, cnt = (unsigned)steps.size(); i < cnt; ++i) {
    ...
    unsigned count = ((i + 1) < cnt ? pSteps[i + 1].indexOffset
                                    : (unsigned short)indices.size()) -
                     step.indexOffset;

    glDrawRangeElements(step.lines ? GL_LINES : GL_TRIANGLES, step.vertStart,
                        step.vertEnd, count, GL_UNSIGNED_SHORT,
                        (void *)(step.indexOffset * sizeof(unsigned short)));
  }
...
VertexBufferTCV *VertexBufferTCV::fetch(const RenderState *mat) {
  ...
  if (vbuff.last == 0 || vbuff.last->material != mat) {
    ...
    if (buffer + floats >= vbuff.shaderBuffer + vbuff.shaderBufferSize) {
      ...
      // When a shader uses a small enough amount of data, check to see if it's
      // duplicated If it is, we can batch it into one step
      if (floats > vbFloatLimit ||
          memcmp(lastStep->shaderCache, buffer, floats * sizeof(float)) != 0) {
