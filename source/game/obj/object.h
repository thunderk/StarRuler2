...
enum ObjectFlag : unsigned {
  ...
  objWakeUp = 0x4,
  // Object has been allocated as a reference-only object, and does not yet have
  // data
...
public:
  ...
  static Object *create(ScriptObjectType *type, LockGroup *lock = 0,
                        int id = 0);
  void grab() const;
  void drop() const;
  ...
  template <class T> bool findTargets(T &cb, int depth) {
  ...
  bool findTargets(T &cb, int depth, unsigned char randomizer) {
    ...
    for (unsigned i = 0; i < OBJ_TARGETS;
         ++i, index = (index + 1) % OBJ_TARGETS) {
